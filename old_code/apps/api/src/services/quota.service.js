...
export class QuotaService {
    ...
    constructor(monitoring, shardRepository, shardTypeRepository, revenueAtRiskService) {
     ...
     * Create a new quota
     */
    async createQuota(tenantId, userId, input) {
        const startTime = Date.now();
        try {
            // Validate input
            this.validateQuotaInput(input);
            // Get shard type
            const shardType = await this.shardTypeRepository.findByName(CORE_SHARD_TYPE_NAMES.QUOTA, 'system');
            if (!shardType) {
     ...
     * Get a quota by ID
     */
    async getQuota(quotaId, tenantId) {
        try {
     ...
     */
    async listQuotas(tenantId, filters) {
        try {
            // Get shard type
            const shardType = await this.shardTypeRepository.findByName(CORE_SHARD_TYPE_NAMES.QUOTA, 'system');
            if (!shardType) {
     ...
     * Update a quota
     */
    async updateQuota(quotaId, tenantId, userId, updates) {
        const startTime = Date.now();
        try {
            // Get existing quota
            const quotaShard = await this.shardRepository.findById(quotaId, tenantId);
            ...
            const currentData = quotaShard.structuredData;
            // Validate updates
            if (updates.period) {
     ...
     * Delete a quota
     */
    async deleteQuota(quotaId, tenantId, userId) {
        const startTime = Date.now();
        try {
     ...
     */
    async calculatePerformance(quotaId, tenantId, userId) {
        const startTime = Date.now();
        try {
            // Get quota
            const quotaShard = await this.shardRepository.findById(quotaId, tenantId);
            ...
            const quotaData = quotaShard.structuredData;
            ...
            try {
                if (quota.quotaType === 'individual' && quota.targetUserId) {
                    const portfolio = await this.revenueAtRiskService.calculateForPortfolio(quota.targetUserId, tenantId);
                    // Filter to period
                    const periodOpportunities = portfolio.opportunities.filter(opp => {
                        ...
                        const oppData = oppShard.structuredData;
     ...
     */
    async rollupQuotas(parentQuotaId, tenantId, userId) {
        const startTime = Date.now();
        try {
            // Get parent quota
            ...
            // Calculate attainment percentages
            const parentData = parentQuotaShard.structuredData;
     ...
     */
    async getForecast(quotaId, tenantId, userId) {
        const startTime = Date.now();
        try {
            // Get quota
            const quotaShard = await this.shardRepository.findById(quotaId, tenantId);
     ...
     */
    async getQuotaOpportunities(quota, tenantId) {
        ...
        filtered = filtered.filter(shard => {
            ...
            if (!closeDate) {
                return true;
            } // Include if no date (assume in period)
            const date = new Date(closeDate);
            return date >= quota.period.startDate && date <= quota.period.endDate;
        });
        return filtered;
     ...
     */
    calculateActual(opportunities, period) {
     ...
     */
    calculateForecasted(opportunities, period) {
        return opportunities.reduce((sum, opp) => {
            const data = opp.structuredData;
