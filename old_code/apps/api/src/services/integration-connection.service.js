...
export class IntegrationConnectionService {
    ...
    async storeSecret(name, value, tags) {
    ...
    // OAuth Flow
    // =====================
    async startOAuthFlow(integrationId, tenantId, userId, returnUrl) {
        ...
        if (this.redis) {
            await this.redis.setex(`oauth:state:${state}`, this.OAUTH_STATE_TTL, JSON.stringify(oauthState));
        }
    ...
    async startOAuthFlowForIntegration(integration, userId, returnUrl) {
        const provider = await this.providerRepo.findByIdAcrossCategories(integration.integrationId);
        if (!provider) {
            throw new Error('Integration provider not found');
        }
    ...
    async handleOAuthCallback(code, state) {
        ...
        try {
            ...
            const integration = await this.integrationRepo.findById(oauthState.integrationId, oauthState.tenantId);
            if (!integration) {
                throw new Error('Integration not found');
            }
            const provider = await this.providerRepo.findByIdAcrossCategories(integration.integrationId);
            ...
            const tokens = await this.exchangeCodeForTokens(provider.oauthConfig, code, oauthState.codeVerifier);
            const connection = await this.createOrUpdateConnectionForIntegration(integration, oauthState.userId, {
                type: 'oauth2',
            ...
            await this.integrationRepo.update(integration.id, integration.tenantId, {
            ...
            if (this.externalUserIdService && integration.userScoped && oauthState.userId) {
                try {
                    ...
                    if (this.adapterManager) {
                        try {
                            const adapter = await this.adapterManager.getAdapter(integration.providerName, integration, oauthState.userId);
                            if (adapter && adapter.getUserProfile) {
                                const userProfile = await adapter.getUserProfile();
                                if (userProfile && userProfile.id) {
                                    await this.externalUserIdService.storeExternalUserId(oauthState.userId, oauthState.tenantId, {
                                        integrationId: integration.id,
                            ...
                            this.monitoring.trackException(adapterError instanceof Error ? adapterError : new Error(String(adapterError)), {
                    ...
                    if (provider.oauthConfig?.userInfoUrl && tokens.accessToken) {
                        try {
                            ...
                            // Log but don't fail - external user ID retrieval is optional
                            this.monitoring.trackException(userInfoError instanceof Error ? userInfoError : new Error(String(userInfoError)), {
    ...
    async exchangeCodeForTokens(oauthConfig, code, codeVerifier) {
    ...
    async refreshTokens(connectionId, integrationId) {
        ...
        try {
            ...
            await this.connectionRepo.updateOAuthTokens(connectionId, integrationId, {
                ...
                expiresAt: data.expires_in
                    ...
                    : undefined,
                scope: data.scope?.split(' ') || connection.oauth.scope,
            });
     ...
     */
    async connectWithApiKey(integrationId, tenantId, userId, apiKey, displayName) {
        ...
        return this.createOrUpdateConnectionForIntegration(integrationDoc, userId, { type: 'api_key', apiKey }, displayName);
    }
    /**
     * Connect with basic auth
     */
    async connectWithBasicAuth(integrationId, tenantId, userId, username, password, displayName) {
        const integrationDoc = await this.integrationRepo.findById(integrationId, tenantId);
        if (!integrationDoc) {
        ...
        return this.createOrUpdateConnectionForIntegration(integrationDoc, userId, { type: 'basic', username, password }, displayName);
    }
    /**
     * Connect with custom credentials
     */
    async connectWithCustomCredentials(integrationId, tenantId, userId, credentials, displayName) {
        const integrationDoc = await this.integrationRepo.findById(integrationId, tenantId);
        if (!integrationDoc) {
            throw new Error('Integration instance not found for custom credentials connection');
        }
        return this.createOrUpdateConnectionForIntegration(integrationDoc, userId, { type: 'custom', data: credentials }, displayName);
    }
    // =====================
     ...
     */
    async createOrUpdateConnectionForIntegration(integration, userId, credentials, displayName) {
        ...
        let connectionId;
        if (scope === 'user') {
            const userConnections = await this.connectionRepo.findUserScoped(integration.id, integration.tenantId, userId);
            ...
            if (!existing) {
                if (userConnections.length >= MAX_USER_CONNECTIONS_PER_INTEGRATION) {
                    throw new Error(`Maximum number of connections (${MAX_USER_CONNECTIONS_PER_INTEGRATION}) reached for this integration. Please delete an existing connection before creating a new one.`);
                }
            }
        }
        else {
            const tenantConnections = await this.connectionRepo.findByIntegration(integration.id, 'tenant', integration.tenantId);
        ...
        if (existing) {
            return await this.connectionRepo.update(existing.id, integration.id, {
    ...
    async createOrUpdateConnection(provider, tenantId, userId, credentials, displayName) {
        const scope = provider.audience === 'system' ? 'system' : 'tenant';
    ...
    async getConnection(integrationId, tenantId) {
     ...
     */
    async getUserConnections(integrationId, tenantId, userId) {
        return this.connectionRepo.findUserScoped(integrationId, tenantId, userId);
    }
    /**
     * Get a specific user connection by ID
     */
    async getUserConnection(integrationId, connectionId, tenantId, userId) {
     ...
     */
    async getUserOAuthToken(integrationId, tenantId, userId) {
        const integration = await this.integrationRepo.findById(integrationId, tenantId);
        if (!integration) {
        ...
        let connection = null;
        if (userId) {
            connection = await this.connectionRepo.findByIntegration(integration.id, 'user', tenantId, userId);
        }
        // Fall back to tenant-level connection
        if (!connection) {
            connection = await this.connectionRepo.findByIntegration(integration.id, 'tenant', tenantId);
        }
        if (!connection) {
    ...
    async getDecryptedCredentials(connectionId, integrationId) {
        const connection = await this.connectionRepo.findById(connectionId, integrationId);
        if (!connection) {
            return null;
        }
        try {
            if (connection.authType === 'oauth2' && connection.oauth) {
                ...
                if (!accessToken) {
                    throw new Error('Access token missing in Key Vault');
                }
                let refreshToken;
    ...
    async testConnection(integrationId, tenantId, userId) {
        const integration = await this.integrationRepo.findById(integrationId, tenantId);
        if (!integration) {
        ...
        if (!creds) {
            return { success: false, error: 'Could not retrieve credentials from Vault' };
        }
        return { success: true };
    }
    /**
     ...
     */
    async testSpecificConnection(integrationId, connectionId, tenantId, userId) {
        const integration = await this.integrationRepo.findById(integrationId, tenantId);
        if (!integration) {
            return { success: false, error: 'Integration not found' };
        }
     ...
     */
    async recordConnectionUsage(connectionId, integrationId) {
        try {
            const connection = await this.connectionRepo.findById(connectionId, integrationId);
            if (!connection) {
            ...
            if (connection.oauth?.expiresAt && connection.status === 'active') {
                ...
                if (expiresAt <= fiveMinutesFromNow) {
                    try {
                        ...
                        this.monitoring.trackException(refreshError, {
     ...
     */
    async bulkDeleteUserConnections(integrationId, connectionIds, tenantId, userId) {
        const results = [];
        let successCount = 0;
        let failureCount = 0;
     ...
     */
    async bulkTestUserConnections(integrationId, connectionIds, tenantId, userId) {
        const results = [];
        let successCount = 0;
        let failureCount = 0;
        for (const connectionId of connectionIds) {
     ...
     */
    async getConnectionUsageStats(tenantId, userId, integrationId) {
        try {
            let allConnections = [];
            if (integrationId) {
                // Get connections for a specific integration
                allConnections = await this.connectionRepo.findUserScoped(integrationId, tenantId, userId);
            }
            else {
            ...
            const activeConnections = allConnections.filter((c) => c.status === 'active' && (!c.lastUsedAt || new Date(c.lastUsedAt) >= thirtyDaysAgo));
            const inactiveConnections = allConnections.filter((c) => c.status !== 'active' || (c.lastUsedAt && new Date(c.lastUsedAt) < thirtyDaysAgo));
            ...
            const updatedActiveConnections = allConnections.filter((c) => c.status === 'active' && (!c.lastUsedAt || new Date(c.lastUsedAt) >= thirtyDaysAgo));
            const updatedInactiveConnections = allConnections.filter((c) => c.status !== 'active' || (c.lastUsedAt && new Date(c.lastUsedAt) < thirtyDaysAgo));
...
//# sourceMappingURL=integration-connection.service.js.map