...
export class DashboardController {
     ...
     */
    createDashboard = async (req, reply) => {
     ...
     * List dashboards
     */
    listDashboards = async (req, reply) => {
        const startTime = Date.now();
        try {
            const { tenantId, id: userId } = req.auth || {};
     ...
     */
    getMergedDashboard = async (req, reply) => {
        const startTime = Date.now();
        try {
            const { tenantId, id: userId } = req.auth || {};
            if (!tenantId || !userId) {
            ...
            const filters = datePreset
                ...
                : undefined;
            const merged = await this.service.getMergedDashboard(userId, tenantId, dashboardId, context, filters);
     ...
     */
    getDashboardWithWidgets = async (req, reply) => {
        const startTime = Date.now();
        try {
            const { tenantId, id: userId } = req.auth || {};
            if (!tenantId || !userId) {
     ...
     * Update dashboard
     */
    updateDashboard = async (req, reply) => {
        const startTime = Date.now();
        try {
            const { tenantId, id: userId } = req.auth || {};
            if (!tenantId || !userId) {
     ...
     * Delete dashboard
     */
    deleteDashboard = async (req, reply) => {
        const startTime = Date.now();
        try {
            const { tenantId, id: userId } = req.auth || {};
            if (!tenantId || !userId) {
     ...
     * Duplicate dashboard
     */
    duplicateDashboard = async (req, reply) => {
        const startTime = Date.now();
        try {
            const { tenantId, id: userId } = req.auth || {};
            if (!tenantId || !userId) {
    ...
    createWidget = async (req, reply) => {
        ...
        try {
            ...
            const { name, description, icon, widgetType, config, dataSource, position, size, refreshInterval, } = req.body;
     ...
     */
    reorderWidgets = async (req, reply) => {
        const startTime = Date.now();
        try {
            const { tenantId, id: userId } = req.auth || {};
            if (!tenantId || !userId) {
            ...
            const widgets = await this.service.batchUpdatePositions(dashboardId, tenantId, userId, { positions });
     ...
     * Get recent shards
     */
    getRecentShards = async (req, reply) => {
        try {
            const { tenantId } = req.auth || {};
            if (!tenantId) {
                reply.status(401).send({ error: 'Unauthorized' });
     ...
     */
    rollbackToVersion = async (req, reply) => {
        try {
            const { tenantId, id: userId } = req.auth || {};
            if (!tenantId || !userId) {
                reply.status(401).send({ error: 'Unauthorized' });
     ...
     */
    getTenantDashboardConfig = async (req, reply) => {
        try {
            const { tenantId } = req.auth || {};
            if (!tenantId) {
                reply.status(401).send({ error: 'Unauthorized' });
     ...
     */
    getFiscalYearConfig = async (req, reply) => {
        try {
            const { tenantId } = req.auth || {};
            if (!tenantId) {
                reply.status(401).send({ error: 'Unauthorized' });
     ...
     */
    updateFiscalYearConfig = async (req, reply) => {
        try {
            const { tenantId, id: userId } = req.auth || {};
            if (!tenantId || !userId) {
                reply.status(401).send({ error: 'Unauthorized' });
