...
import { VectorSearchService } from '@castiel/api-core';
...
export class VectorSearchController {
  constructor(
    ...
    private readonly uiService?: import('../services/vector-search-ui.service.js').VectorSearchUIService,
    private readonly analyticsService?: import('../services/search-analytics.service.js').SearchAnalyticsService
  ) { }
  ...
  async semanticSearch(
    request: FastifyRequest<{ Body: VectorSearchRequest }>,
    reply: FastifyReply
  ): Promise<void> {
    ...
    try {
      ...
      const response: VectorSearchResponse = await this.vectorSearchService.semanticSearch(
        searchRequest,
        req.user.id
      );
      ...
      if (this.uiService) {
        this.uiService.recordSearchHistory(
          req.user.id,
          req.user.tenantId,
          searchRequest.query,
          {
        ...
        this.uiService.trackPopularSearch(
          req.user.tenantId,
          searchRequest.query,
          {
            shardTypeId: searchRequest.filter?.shardTypeId,
   ...
   */
  async hybridSearch(
    request: FastifyRequest<{ Body: HybridSearchRequest }>,
    reply: FastifyReply
  ): Promise<void> {
    const startTime = Date.now();
    const req = request as AuthenticatedRequest;
    ...
    try {
      ...
      const response: VectorSearchResponse = await this.vectorSearchService.hybridSearch(
        searchRequest,
        req.user.id
      );
   ...
   */
  async globalSearch(
    request: FastifyRequest<{ Body: Omit<VectorSearchRequest, 'filter'> & { filter?: Omit<VectorSearchRequest['filter'], 'tenantId'> } }>,
    reply: FastifyReply
  ): Promise<void> {
    const startTime = Date.now();
    const req = request as AuthenticatedRequest;
    ...
    try {
      ...
      const response: VectorSearchResponse = await this.vectorSearchService.globalSearch(
        searchRequest,
        req.user.id
      );
      ...
      this.monitoring.trackMetric('vsearch-api-global', 1, {
        userId: req.user.id,
        resultCount: response.results.length,
        tenantCount: (response.metadata as any)?.tenantsSearched?.length || 0,
        duration,
      });
