...
import type { RichFieldDefinition, RichFieldType } from '@castiel/shared-types';
...
export class SchemaMigrationService {
  private repository: SchemaMigrationRepository;
  ...
  diffSchemas(
    ...
    const requiresDataMigration = changes.some(c => 
      c.changeType !== FieldChangeType.ADDED || c.isBreaking
    );

    // Determine recommended strategy
    let recommendedStrategy: MigrationStrategy;
  ...
  private diffRichSchemaFields(
    oldFields: RichFieldDefinition[],
    newFields: RichFieldDefinition[]
  ): FieldChange[] {
    const changes: FieldChange[] = [];
    ...
    for (const [name, newField] of newFieldMap) {
      if (!oldFieldMap.has(name)) {
        const fieldConfig = newField.config as any;
        const isBreaking = newField.required === true && fieldConfig?.default === undefined;
        changes.push({
    ...
    for (const [name, newField] of newFieldMap) {
      ...
      if (!oldField.required && newField.required) {
        changes.push({
          ...
          oldDefinition: oldField,
          newDefinition: newField,
          defaultValue: (newField.config as any)?.default,
          isBreaking: (newField.config as any)?.default === undefined,
  ...
  private checkBreakingConfigChange(
    oldField: RichFieldDefinition,
    newField: RichFieldDefinition
  ): boolean {
    const oldConfig = oldField.config as any;
    const newConfig = newField.config as any;

    // Check for restrictive changes
    if (newConfig?.minLength > (oldConfig?.minLength || 0)) {return true;}
    if (newConfig?.maxLength && newConfig.maxLength < (oldConfig?.maxLength || Infinity)) {return true;}
    if (newConfig?.min > (oldConfig?.min ?? -Infinity)) {return true;}
    if (newConfig?.max < (oldConfig?.max ?? Infinity)) {return true;}
    if (newConfig?.minSelection > (oldConfig?.minSelection || 0)) {return true;}
    if (newConfig?.maxSelection && newConfig.maxSelection < (oldConfig?.maxSelection || Infinity)) {return true;}

    // Check for option list changes (removed options)
    if (oldConfig?.options && newConfig?.options) {
      const oldValues = new Set(oldConfig.options.map((o: any) => o.value));
      const newValues = new Set(newConfig.options.map((o: any) => o.value));
      for (const v of oldValues) {
        if (!newValues.has(v)) {return true;} // Option removed
      }
    }
   ...
   */
  private suggestTransformation(oldType: string, newType: string): string | undefined {
  ...
  private generateDiffSummary(
    ...
    const requiredChanged = changes.filter(c => 
      c.changeType === FieldChangeType.MADE_REQUIRED || 
      c.changeType === FieldChangeType.MADE_OPTIONAL
    ).length;

    if (added > 0) {parts.push(`${added} field(s) added`);}
    if (removed > 0) {parts.push(`${removed} field(s) removed`);}
    if (typeChanged > 0) {parts.push(`${typeChanged} field type(s) changed`);}
    if (requiredChanged > 0) {parts.push(`${requiredChanged} required status change(s)`);}
   ...
   */
  private getResolution(change: FieldChange): string {
  ...
  async createMigration(
    ...
    if (options?.fieldMappings) {
      for (const [oldName, newName] of Object.entries(options.fieldMappings)) {
        const removeChange = diff.changes.find(
          c => c.changeType === FieldChangeType.REMOVED && c.fieldName === oldName
        );
        const addChange = diff.changes.find(
          c => c.changeType === FieldChangeType.ADDED && c.fieldName === newName
        );
   ...
   */
  private applyTransformation(value: any, transformFn: string): any {
    switch (transformFn) {
  ...
  async runEagerMigration(
    ...
    const results: ShardMigrationResult[] = [];
    let succeeded = 0;
    let failed = 0;
    ...
    try {
      ...
      for (const shard of shardResult.shards) {
        ...
        if (result.success && result.migratedData) {
          ...
          try {
            ...
            result.error = (updateError as Error).message;
            failed++;
          }
      ...
      if (!hasMore) {
        await this.repository.updateStatus(
          migrationId, 
          tenantId, 
          failed > 0 ? MigrationStatus.FAILED : MigrationStatus.COMPLETED,
          newProgress
        );
      } else {
   ...
   */
  async getMigrationsForShardType(
    tenantId: string,
    shardTypeId: string
  ): Promise<SchemaMigration[]> {
   ...
   * Get migration by ID
   */
  async getMigration(id: string, tenantId: string): Promise<SchemaMigration | undefined> {
   ...
   */
  async cancelMigration(id: string, tenantId: string): Promise<SchemaMigration> {
