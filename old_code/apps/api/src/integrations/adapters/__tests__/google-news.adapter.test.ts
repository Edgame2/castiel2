...
describe('GoogleNewsAdapter', () => {
  ...
  describe('testConnection', () => {
    it('should test connection successfully', async () => {
      ...
      vi.spyOn(adapter as any, 'fetchFromNewsAPI').mockResolvedValue({
    ...
    it('should handle connection test failure when API key is missing', async () => {
      ...
      (mockConnectionService.getDecryptedCredentials as any).mockResolvedValueOnce(null);
  ...
  describe('fetch', () => {
    ...
    it('should handle fetch errors gracefully', async () => {
      ...
      if (definition.availableEntities.length > 0) {
        ...
        vi.spyOn(adapter as any, 'fetchFromNewsAPI').mockRejectedValue(new Error('API Error'));
