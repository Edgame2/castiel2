...
import { ShardTypeCategory, RichSchema } from '../types/shard-type.types.js';
...
const eventFields: RichFieldDefinition[] = [
  ...
  // ============================================
  {
    name: 'calendarId',
    label: 'Calendar',
    description: 'Reference to the calendar (Google Calendar, Outlook Calendar, etc.) that contains this event',
  ...
  },
  {
    name: 'organizerId',
    label: 'Organizer',
    description: 'User who organized this event. Used for calendar integration and attendee management',
    type: RichFieldType.USER,
    design: { columns: 6 },
  },
  {
  ...
  // ============================================
  {
    ...
    description: 'Recurrence rule in RRULE format (RFC 5545). Examples: "FREQ=DAILY", "FREQ=WEEKLY;BYDAY=MO,WE,FR", "FREQ=MONTHLY;BYMONTHDAY=1"',
    type: RichFieldType.TEXT,
    config: { 
      maxLength: 500,
      placeholder: 'FREQ=DAILY;COUNT=10 or FREQ=WEEKLY;BYDAY=MO,WE,FR',
    },
    design: { columns: 6 },
  },
  {
  ...
  // ============================================
  {
    ...
    description: 'Virtual meeting/conference information including platform, meeting ID, join URL, and dial-in details. Used for Zoom, Teams, Google Meet, Webex, and other video conferencing platforms',
    ...
    config: {
      schema: {
        ...
        properties: {
          ...
          dialInNumber: { 
            type: 'string',
            description: 'Phone number for dial-in access',
          },
          passcode: { 
            type: 'string',
            description: 'Meeting passcode or PIN',
          },
        },
        required: ['platform'],
      },
...
const emailFields: RichFieldDefinition[] = [
  ...
  // ============================================
  {
    name: 'messageId',
    label: 'Message ID',
    description: 'Unique message identifier from the RFC 5322 Message-ID header. Used for email threading and deduplication across email providers (Gmail, Outlook, etc.)',
    type: RichFieldType.TEXT,
  ...
  },
  {
    name: 'inReplyTo',
    label: 'In Reply To',
    description: 'Message-ID of the message this email is replying to (RFC 5322 In-Reply-To header). Used to establish email thread relationships',
    type: RichFieldType.TEXT,
    config: { maxLength: 500 },
    design: { columns: 6 },
  },
  {
    name: 'references',
    label: 'References',
    description: 'Space-separated list of Message-IDs from the RFC 5322 References header. Contains the full thread history for proper email threading',
    type: RichFieldType.TEXT,
  ...
  },
  {
    ...
    description: 'Zero-based index indicating the position of this message within the email thread. 0 = first message (thread starter), 1 = first reply, etc.',
    type: RichFieldType.INTEGER,
  ...
  },
  {
    name: 'importance',
    label: 'Importance',
    description: 'Email importance level (Outlook-style). Used to prioritize emails and indicate urgency. High importance emails are typically marked with a red exclamation mark',
    type: RichFieldType.SELECT,
    config: {
      options: [
        { value: 'low', label: 'Low' },
  ...
  },
  {
    name: 'sensitivity',
    label: 'Sensitivity',
    description: 'Email sensitivity classification (Outlook-style). Used to indicate the confidentiality level of the email content. Confidential emails may have restricted forwarding/printing',
    type: RichFieldType.SELECT,
    config: {
      options: [
        { value: 'normal', label: 'Normal' },
...
const productFields: RichFieldDefinition[] = [
  ...
  // ============================================
  {
    ...
    description: 'Current quantity of this product available in inventory. Used for stock management and availability tracking',
    type: RichFieldType.INTEGER,
    config: { min: 0 },
    design: { columns: 6 },
  },
  {
    ...
    description: 'Quantity available for sale (on hand minus reserved/allocated). Used for real-time availability checks',
    type: RichFieldType.INTEGER,
    config: { min: 0, readOnly: true },
    design: { columns: 6 },
  },
  {
    name: 'reorderLevel',
    label: 'Reorder Level',
    description: 'Minimum quantity threshold that triggers a reorder alert. When quantity on hand falls below this level, reordering is recommended',
    type: RichFieldType.INTEGER,
    config: { min: 0 },
    design: { columns: 6 },
  },
  {
    ...
    description: 'Standard quantity to reorder when reorder level is reached. Used for automated purchase order generation',
    type: RichFieldType.INTEGER,
    config: { min: 0 },
    design: { columns: 6 },
  },
  {
    name: 'stockStatus',
    label: 'Stock Status',
    description: 'Current stock availability status. Automatically calculated based on quantity on hand and reorder level',
    type: RichFieldType.SELECT,
    config: {
      options: [
  ...
  // ============================================
  {
    name: 'vendorId',
    label: 'Vendor',
    description: 'Reference to the vendor/supplier account that provides this product. Used for purchase orders and vendor management',
    type: RichFieldType.REFERENCE,
    config: {
  ...
  },
  {
    ...
    description: 'Vendor-specific part number or SKU for this product. Used for purchase orders and vendor catalogs',
    type: RichFieldType.TEXT,
    config: { maxLength: 100 },
    design: { columns: 6 },
  },
  {
    name: 'weight',
    label: 'Weight',
    description: 'Product weight for shipping calculations and logistics',
    type: RichFieldType.FLOAT,
  ...
  },
  {
    name: 'weightUnit',
    label: 'Weight Unit',
    description: 'Unit of measurement for weight (kg, lb, oz, g)',
    type: RichFieldType.SELECT,
    config: {
      options: [
  ...
  },
  {
    name: 'dimensions',
    label: 'Dimensions',
    description: 'Product dimensions (length, width, height) for shipping calculations, storage planning, and logistics. Supports multiple units (cm, in, m, ft)',
    type: RichFieldType.JSON,
    config: {
      schema: {
        type: 'object',
  ...
  // ============================================
  {
    ...
    description: 'Reference to the parent product if this is a variant (e.g., size, color, configuration variant). Used for product hierarchies and variant management',
    type: RichFieldType.REFERENCE,
    config: {
  ...
  },
  {
    name: 'hasVariants',
    label: 'Has Variants',
    description: 'Indicates if this product has child variant products (e.g., different sizes, colors, configurations). Automatically set when variants exist',
    type: RichFieldType.BOOLEAN,
    config: { readOnly: true, default: false },
    design: { columns: 6 },
  },
  {
    ...
    description: 'Variant-specific attributes (e.g., size, color, material, configuration). Used to distinguish this variant from other variants of the same parent product',
    type: RichFieldType.JSON,
    config: {
      schema: {
        type: 'object',
        properties: {
          size: { type: 'string', description: 'Product size (e.g., "Small", "Medium", "Large", "XL")' },
          color: { type: 'string', description: 'Product color (e.g., "Red", "Blue", "Black")' },
          material: { type: 'string', description: 'Product material (e.g., "Cotton", "Leather", "Plastic")' },
          configuration: { type: 'string', description: 'Product configuration or model variant' },
        },
      },
    },
    design: { columns: 12 },
  },
  {
    name: 'isBundle',
    label: 'Is Bundle',
    description: 'Indicates if this product is a bundle containing multiple products sold together. Bundle pricing may differ from sum of individual products',
    type: RichFieldType.BOOLEAN,
  ...
  },
  {
    name: 'bundleItems',
    label: 'Bundle Items',
    description: 'List of products included in this bundle with quantities and optional pricing. Used for bundle product configuration and pricing calculations',
    type: RichFieldType.JSON,
    config: {
      schema: {
        type: 'array',
        items: {
          ...
          properties: {
            ...
            productName: { 
              type: 'string',
              description: 'Name of the bundled product (for display)',
            },
            quantity: { 
              type: 'number', 
              minimum: 1,
            ...
            unitPrice: { 
              type: 'number', 
              minimum: 0,
              description: 'Optional unit price override for this bundle item',
            },
          },
  ...
  schema: {
    ...
    formLayout: {
      ...
      groups: [
        ...
        { id: 'inventory', label: 'Inventory', fields: ['quantityOnHand', 'quantityAvailable', 'reorderLevel', 'reorderQuantity', 'stockStatus'] },
        { id: 'vendor', label: 'Vendor & Physical', fields: ['vendorId', 'vendorName', 'vendorPartNumber', 'weight', 'weightUnit', 'dimensions'] },
        { id: 'variants', label: 'Variants & Bundles', fields: ['parentProductId', 'parentProductName', 'hasVariants', 'variantAttributes', 'isBundle', 'bundleItems'] },
...
const folderFields: RichFieldDefinition[] = [
  ...
  },
  {
    name: 'provider',
    label: 'Provider',
    description: 'Cloud storage provider for this folder. Supports Google Drive, SharePoint, OneDrive (personal and business), and Dropbox',
    type: RichFieldType.SELECT,
    required: true,
    config: {
      options: [
  ...
  // ============================================
  {
    name: 'driveId',
    label: 'Drive ID',
    description: 'Drive identifier for the storage location. For OneDrive: the drive ID (e.g., "b!abc123..."). For SharePoint: the site drive ID. For Google Drive: the drive ID or "myDrive" for personal drive',
    type: RichFieldType.TEXT,
  ...
  },
  {
    name: 'webUrl',
    label: 'Web URL',
    description: 'Direct web URL to access this folder in the provider\'s web interface. For OneDrive: the sharing link or direct folder URL. For SharePoint: the folder URL within the site',
    type: RichFieldType.URL,
    config: { readOnly: true },
    design: { columns: 6 },
  },
...
const fileFields: RichFieldDefinition[] = [
  ...
  },
  {
    name: 'provider',
    label: 'Provider',
    description: 'Cloud storage provider for this file. Supports Google Drive, SharePoint, OneDrive (personal and business), and Dropbox',
    type: RichFieldType.SELECT,
    required: true,
    config: {
      options: [
  ...
  },
  {
    name: 'externalId',
    label: 'External ID',
    description: 'Provider-specific file identifier. For OneDrive: the item ID from Microsoft Graph API (e.g., "01ABC123..."). For Google Drive: the file ID. For SharePoint: the list item ID',
    type: RichFieldType.TEXT,
    required: true,
    config: { readOnly: true },
    design: { columns: 6 },
  ...
  },
  {
    name: 'webUrl',
    label: 'Web URL',
    description: 'Direct web URL to view this file in the provider\'s web interface. For OneDrive: the sharing link or direct file URL. For SharePoint: the file URL within the site. For Google Drive: the file viewer URL',
    type: RichFieldType.URL,
    config: { readOnly: true },
    design: { columns: 6 },
  },
  {
    name: 'downloadUrl',
    label: 'Download URL',
    description: 'Direct download URL for the file. For OneDrive: the download URL from Microsoft Graph API. For Google Drive: the export/download URL. Note: URLs may be temporary and require authentication',
    type: RichFieldType.URL,
    config: { readOnly: true },
    design: { columns: 6 },
  },
...
const messageFields: RichFieldDefinition[] = [
  ...
  },
  {
    name: 'channelId',
    label: 'Channel',
    description: 'Reference to the channel (Slack channel, Teams channel, Discord channel) where this message was posted. Required for organizing messages by channel and enabling channel-based filtering',
    type: RichFieldType.REFERENCE,
    required: true,
    config: {
  ...
  // ============================================
  {
    name: 'threadId',
    label: 'Thread ID',
    description: 'Provider-specific thread identifier. For Slack: the thread_ts timestamp. For Teams: the thread ID. For Discord: the thread channel ID. Used to group related messages in a conversation thread',
    type: RichFieldType.TEXT,
    config: { maxLength: 200 },
    design: { columns: 6 },
  },
  {
    ...
    description: 'Reference to the parent message if this is a reply or thread reply. Used to establish message threading relationships and build conversation trees. Null for thread starter messages',
    type: RichFieldType.REFERENCE,
    config: {
