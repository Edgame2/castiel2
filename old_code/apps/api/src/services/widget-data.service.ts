...
import { hashQuery } from '../utils/cache-keys.js';
...
export class WidgetDataService {
  ...
  private tenantIntegrationRepo?: IntegrationRepository;
  ...
  constructor(
    ...
    tenantIntegrationRepo?: IntegrationRepository
  ) {
  ...
  async getWidgetData(
    ...
    try {
      ...
      if (this.cacheService) {
        const cached = await this.cacheService.getCachedWidgetData(
          context.tenantId,
          widget.id,
          isSharedCacheable ? 'shared' : context.userId,
          queryParams
        );
      ...
      if (this.cacheService) {
        ...
        await this.cacheService.cacheWidgetData(
          context.tenantId,
          widget.id,
          context.userId,
          queryParams,
  ...
  private buildQueryParams(
    ...
    if (dataSource.customQuery?.applyDashboardContext && dashboardContext?.shardContext?.shardId) {
      (params.filters as QueryFilter[]).push({
    ...
    if (dataSource.customQuery?.applyDashboardFilters && dashboardFilters?.dateRange) {
      const dateRange = this.resolveDateFilter(
        dashboardFilters.dateRange,
        context.fiscalYearConfig
      );
      if (dateRange) {
        (params.filters as QueryFilter[]).push({
          field: 'createdAt',
    ...
    if (request.dashboardFilters?.dateRange) {
      const dateRange = this.resolveDateFilter(
        request.dashboardFilters.dateRange,
        context.fiscalYearConfig
      );
      if (dateRange) {
        (params.filters as QueryFilter[]).push({
  ...
  private isQueryShareable(
    widget: Widget,
    context: { userRoles: string[]; userGroups: string[] }
  ): boolean {
  ...
  private async applyUserFilter(
    data: unknown,
    context: { userId: string; userRoles: string[]; userGroups: string[] }
  ): Promise<unknown> {
    ...
    return data.filter((item: Record<string, unknown>) => {
      ...
      const acl = item.acl as Array<{ userId?: string; groupId?: string }> | undefined;
      if (!acl) {
        return true;
      }
  ...
  private async executePredefinedQuery(
    ...
    switch (queryType) {
      case PredefinedQuery.RECENT_SHARDS:
        return this.repository.queryRecentShards(
          context.tenantId,
          page,
          pageSize
        );
      ...
      case PredefinedQuery.USER_ACTIVITY_TIMELINE:
        return this.repository.queryUserActivityTimeline(
          context.tenantId,
          context.userId,
          page,
          pageSize
      ...
      case PredefinedQuery.MY_TASKS:
        return this.repository.queryMyTasks(
          context.tenantId,
          context.userId,
          page,
          pageSize
      ...
      case PredefinedQuery.UPCOMING_EVENTS:
        return this.repository.queryUpcomingEvents(
          context.tenantId,
          context.userId,
          page,
          pageSize
  ...
  private async executeCustomQuery(
    ...
    const aggregationConfig = params.aggregation as AggregationConfig | undefined;
    ...
    let aggregation: { type: string; field?: string; groupBy?: string } | undefined = undefined;
    ...
    const sort = params.sort as { field: string; direction: 'asc' | 'desc' } | undefined;
   ...
   */
  private async executeIntegrationQuery(
    source: {
   ...
   */
  private async executeGoogleWorkspaceQuery(
    source: {
      integrationId: string;
      endpoint: string;
      parameters: Record<string, unknown>;
    ...
    try {
      ...
      const tenantIntegrationId = source.parameters.tenantIntegrationId as string;
      ...
      if (endpoint.includes('gmail') && endpoint.includes('inbox')) {
        ...
        const days = (source.parameters.days as number) || 7;
        ...
        entity = 'task';
        filters = {
          tasklist: (source.parameters.tasklistId as string) || '@default',
          showCompleted: false,
        };
      } else {
        return { results: [], totalCount: 0, hasMore: false };
      ...
      const limit = (source.parameters.limit as number) || (params.limit as number) || 10;
  ...
  private resolveDateFilter(
    ...
    fiscalConfig?: TenantFiscalYearConfig
  ...
  private getFiscalQuarterRange(
    ...
    const end = new Date(
      quarterStartMonth + 3 >= 12 ? targetYear + 1 : targetYear,
      (quarterStartMonth + 3) % 12,
      fiscalStart.day - 1,
      23, 59, 59, 999
    );
   ...
   */
  async invalidateOnShardChange(tenantId: string, shardTypeId?: string): Promise<void> {
    if (this.cacheService) {
