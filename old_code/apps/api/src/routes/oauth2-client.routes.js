 ...
 */
export async function registerOAuth2ClientRoutes(server) {
    const oauth2ClientService = server.oauth2ClientService;
    ...
    // List OAuth2 clients
    server.get('/api/tenants/:tenantId/oauth2/clients', {
        onRequest: tenantAdminGuards,
    }, (request, reply) => controller.listClients(request, reply));
    // Create OAuth2 client
    server.post('/api/tenants/:tenantId/oauth2/clients', {
        onRequest: tenantAdminGuards,
    }, (request, reply) => controller.createClient(request, reply));
    // Get OAuth2 client
    server.get('/api/tenants/:tenantId/oauth2/clients/:clientId', {
        onRequest: tenantAdminGuards,
    }, (request, reply) => controller.getClient(request, reply));
    // Update OAuth2 client
    server.patch('/api/tenants/:tenantId/oauth2/clients/:clientId', {
        onRequest: tenantAdminGuards,
    }, (request, reply) => controller.updateClient(request, reply));
    // Delete OAuth2 client
    server.delete('/api/tenants/:tenantId/oauth2/clients/:clientId', {
        onRequest: tenantAdminGuards,
    }, (request, reply) => controller.deleteClient(request, reply));
    // Rotate client secret
    server.post('/api/tenants/:tenantId/oauth2/clients/:clientId/rotate-secret', {
        onRequest: tenantAdminGuards,
...
//# sourceMappingURL=oauth2-client.routes.js.map