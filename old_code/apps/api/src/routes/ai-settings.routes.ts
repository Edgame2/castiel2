...
import type { UpdateSystemAIConfigInput, AIProviderName } from '../types/ai-provider.types.js';
...
interface AIModelUpdate extends Partial<AIModelCreate> {
  isActive?: boolean;
...
let systemAIConfig: SystemAIConfig = {
...
const tenantConfigs = new Map<string, TenantAIConfig & { customCredentials: { provider: string; hasCredentials: boolean }[] }>();

/**
 * Register AI Settings routes
 */
export async function aiSettingsRoutes(
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if ((fastify as any).aiConfigService) {
        const config = await (fastify as any).aiConfigService.getSystemConfig();
   ...
   */
  fastify.patch<{ Body: Partial<SystemAIConfig & { modelSelection?: any }> }>(
    '/admin/ai/config',
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      if ((fastify as any).aiConfigService) {
        ...
        const backendUpdates: UpdateSystemAIConfigInput = {
          defaultModel: updates.defaultLLMModelId,
          defaultEmbeddingModel: updates.defaultEmbeddingModelId,
          allowedProviders: updates.enabledProviders as AIProviderName[],
        ...
        const updated = await (fastify as any).aiConfigService.updateSystemConfig(
          backendUpdates,
          userId
        );
   ...
   */
  fastify.patch<{ Body: Partial<TenantAIConfig> }>(
    '/tenant/ai/config',
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (fastify.aiConfigService) {
          ...
          existing.customCredentials = existing.customCredentials.filter(
  ...
  fastify.get<{ Querystring: { type?: string } }>(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (type) {
          models = models.filter((m: any) => m.modelType === type);
        }
