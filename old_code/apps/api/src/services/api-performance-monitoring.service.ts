...
export interface PerformanceBaseline {
  endpoint: string;
  method: string;
  targetP95: number; // Target p95 in ms
  targetP99: number; // Target p99 in ms
  currentP95: number;
  currentP99: number;
...
export class APIPerformanceMonitoringService {
  ...
  private endpointResponseTimes: Map<string, number[]> = new Map();
  private endpointErrors: Map<string, number> = new Map();
  private endpointSuccesses: Map<string, number> = new Map();
  private readonly MAX_SAMPLES_PER_ENDPOINT = 1000; // Keep last 1000 samples per endpoint

  // Performance targets
  private readonly TARGET_P95 = 500; // 500ms
  private readonly TARGET_P99 = 1000; // 1000ms
  ...
  getAllEndpointStats(): EndpointPerformanceStats[] {
    const stats: EndpointPerformanceStats[] = [];
   ...
   */
  private trackPercentiles(endpoint: string, method: string, times: number[]): void {
    if (times.length < 10) {return;} // Need at least 10 samples
   ...
   */
  private parseEndpointKey(key: string): [string, string] {
