...
export class GoogleDocsRewriter extends BaseDocumentRewriter {
      ...
      try {
        ...
        if (statusCode === 429) {
          if (attempt >= maxRetries) {
            ...
            throw new Error(
              `Google API rate limit exceeded after ${maxRetries} attempts. Please try again later. ` +
        ...
        if (statusCode === 401) {
          ...
          try {
            ...
            this.monitoring.trackException(retryError as Error, {
              operation: `rewriter.${operationName}`,
              format: 'google_docs',
              errorType: 'token_refresh_failed',
            });
            throw new Error(
              `OAuth token expired and refresh failed. Please reconnect your Google Workspace integration. ` +
              `Original error: ${(error as Error).message}`
            );
          }
        }
  ...
  async duplicateDocument(
    ...
    return this.executeWithTokenRefresh(async () => {
      ...
      throw new Error(`Failed to duplicate Google Docs document: ${(error as Error).message}`);
    });
  }
  ...
  async replacePlaceholders(
    ...
    return this.executeWithTokenRefresh(async () => {
      ...
      throw new Error(`Failed to replace placeholders in Google Docs: ${(error as Error).message}`);
    });
  }
  ...
  async insertCharts(
    ...
    return this.executeWithTokenRefresh(async () => {
      ...
      for (const placeholder of chartPlaceholders) {
        ...
        const imageFile = await this.uploadChartImageToDrive(
          chartImage,
          `chart-${placeholder.name}-${Date.now()}.png`,
          auth
        );
      ...
      const findAndReplacePlaceholders = (content: any[], baseIndex: number = 1) => {
      ...
      throw new Error(`Failed to insert charts in Google Docs: ${(error as Error).message}`);
    });
  }
   ...
   */
  private async uploadChartImageToDrive(
    imageBuffer: Buffer,
    fileName: string,
    auth: DocumentAuthToken
  ): Promise<{ id: string }> {
   ...
   * Make image publicly accessible
   */
  private async makeImagePublic(fileId: string, _auth: DocumentAuthToken): Promise<void> {
  ...
  async getDocumentUrl(
    ...
    try {
      ...
      throw new Error(`Failed to get Google Docs document URL: ${(error as Error).message}`);
    }
  }
  ...
  async getFolderPath(
    ...
    try {
      ...
      throw new Error(`Failed to get Google Drive folder path: ${(error as Error).message}`);
    }
  }
