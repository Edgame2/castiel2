 ...
 */
import { ChunkingStrategy, estimateTokenCount, } from '../types/vectorization.types.js';
import { v4 as uuidv4 } from 'uuid';
/**
 * Extract text from a Shard based on text sources configuration
 */
export function extractTextFromShard(shard, textSources) {
 ...
 */
function extractFieldText(shard, fieldPath) {
 ...
 */
export function chunkText(text, strategy, chunkSize = 512, overlap = 50, source = 'text', weight = 1.0) {
 ...
 */
function chunkByFixedSize(text, chunkSize, overlap, source, weight) {
    const chunks = [];
 ...
 * Chunk by sentences
 */
function chunkBySentence(text, maxChunkSize, source, weight) {
    const chunks = [];
 ...
 * Chunk by paragraphs
 */
function chunkByParagraph(text, maxChunkSize, source, weight) {
    const chunks = [];
 ...
 * Create a text chunk
 */
function createChunk(text, startIndex, endIndex, source, weight) {
    return {
        id: uuidv4(),
        text: text.trim(),
        source,
 ...
 */
export function validateText(text, maxTokens) {
    if (!text || text.trim().length === 0) {
 ...
 */
export function calculateChunkingStats(chunks) {
    if (chunks.length === 0) {
        return {
            totalChunks: 0,
            totalTokens: 0,
...
//# sourceMappingURL=text-processing.utils.js.map