...
export class PlaceholderExtractionService {
   ...
   */
  private async parseDocument(request: ExtractionRequest): Promise<DocumentParseResult> {
    ...
    for (const element of elements) {
      ...
      while ((match = regex.exec(element.content)) !== null) {
        ...
        const existingMatch = matches.find(
          m => m.placeholderName === placeholderName && 
        ...
        if (!existingMatch) {
          matches.push({
            ...
            endIndex: -1,
            context: element.content.substring(
              Math.max(0, match.index - this.config.extractionContextRadius),
              Math.min(element.content.length, match.index + match[0].length + this.config.extractionContextRadius)
            ),
          });
        }
      }
  ...
  deduplicatePlaceholders(
    ...
    const locationMap = new Map<string, PlaceholderLocation[]>();
   ...
   */
  private inferPlaceholderType(name: string): PlaceholderType {
   ...
   */
  private async extractColors(request: ExtractionRequest): Promise<string[]> {
