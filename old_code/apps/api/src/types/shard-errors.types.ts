 ...
 */
export function errorCodeToHttpStatus(code: ShardErrorCode): number {
...
export class ShardError extends Error {
   ...
   */
  static fromError(error: unknown, defaultCode: ShardErrorCode = ShardErrorCode.SHARD_VALIDATION_FAILED): ShardError {
...
export const ShardErrors = {
  ...
  accessDenied(shardId: string, userId?: string): ShardError {
    return new ShardError(
      ...
      'Access denied to shard',
      { shardId, details: userId ? { userId } : undefined }
    );
  },
  ...
  insufficientPermissions(shardId: string, requiredPermission: string): ShardError {
    return new ShardError(
      ...
      { shardId, details: { requiredPermission } }
    );
  },
  ...
  requiredFieldMissing(field: string): ShardError {
    return new ShardError(
      ShardErrorCode.REQUIRED_FIELD_MISSING,
  ...
  invalidFieldValue(field: string, reason?: string): ShardError {
    return new ShardError(
      ShardErrorCode.INVALID_FIELD_VALUE,
      reason ? `Invalid value for field '${field}': ${reason}` : `Invalid value for field: ${field}`,
      { field, details: reason ? { reason } : undefined }
    );
  },

  typeNotFound(shardTypeId: string): ShardError {
    return new ShardError(
      ShardErrorCode.SHARD_TYPE_NOT_FOUND,
      `Shard type not found: ${shardTypeId}`,
      { details: { shardTypeId } }
    );
  },
  ...
  typeInactive(shardTypeId: string): ShardError {
    return new ShardError(
      ShardErrorCode.SHARD_TYPE_INACTIVE,
      `Shard type is inactive: ${shardTypeId}`,
      { details: { shardTypeId } }
    );
  },
  ...
  alreadyDeleted(shardId: string): ShardError {
    return new ShardError(
      ShardErrorCode.SHARD_ALREADY_DELETED,
      'Shard is already deleted',
      { shardId }
    );
  },
  ...
  alreadyArchived(shardId: string): ShardError {
    return new ShardError(
      ShardErrorCode.SHARD_ALREADY_ARCHIVED,
      'Shard is already archived',
      { shardId }
    );
  },
  ...
  notArchived(shardId: string): ShardError {
    return new ShardError(
      ShardErrorCode.SHARD_NOT_ARCHIVED,
      'Shard is not archived',
      { shardId }
    );
  },
  ...
  crossTenantAccess(shardId: string, targetTenantId: string): ShardError {
    return new ShardError(
      ShardErrorCode.CROSS_TENANT_ACCESS,
      'Cross-tenant access is not allowed',
      { shardId, details: { targetTenantId } }
    );
  },

  relationshipTargetNotFound(targetId: string): ShardError {
    return new ShardError(
      ShardErrorCode.RELATIONSHIP_TARGET_NOT_FOUND,
      `Relationship target not found: ${targetId}`,
      { details: { targetId } }
    );
  },
  ...
  relationshipCycleDetected(shardId: string, targetId: string): ShardError {
    return new ShardError(
      ShardErrorCode.RELATIONSHIP_CYCLE_DETECTED,
      'Relationship would create a cycle',
      { shardId, details: { targetId } }
    );
  },

  invalidRelationshipType(relationshipType: string): ShardError {
    return new ShardError(
      ShardErrorCode.INVALID_RELATIONSHIP_TYPE,
  ...
  schemaMigrationFailed(shardId: string, reason: string): ShardError {
    return new ShardError(
      ShardErrorCode.SCHEMA_MIGRATION_FAILED,
      `Schema migration failed: ${reason}`,
      { shardId, details: { reason } }
    );
  },

  schemaVersionMismatch(shardId: string, expected: number, actual: number): ShardError {
    return new ShardError(
      ShardErrorCode.SCHEMA_VERSION_MISMATCH,
      `Schema version mismatch: expected ${expected}, got ${actual}`,
      { shardId, details: { expected, actual } }
    );
  },

  bulkOperationPartialFailure(
    summary: { total: number; succeeded: number; failed: number },
    errors: Array<{ index: number; error: ShardError }>
  ): ShardError {
    return new ShardError(
      ShardErrorCode.BULK_OPERATION_PARTIAL_FAILURE,
      `Bulk operation partially failed: ${summary.failed}/${summary.total} failed`,
      {
        details: {
          summary,
          errors: errors.map((e) => ({
  ...
  rateLimitExceeded(retryAfter?: number): ShardError {
    return new ShardError(
      ShardErrorCode.RATE_LIMIT_EXCEEDED,
      'Rate limit exceeded',
