...
import { UserService, UserCacheService, CosmosDbClient, OAuthService, OAuth2ClientService, OAuth2AuthService, } from './services/auth/index.js';
...
let cacheSubscriberInstance = null;
let cacheManagerInstance = null;
let invitationLifecycleScheduler = null;
let shardEventServiceInstance = null;
let webhookDeliveryServiceInstance = null;
let auditWebhookEmitterInstance = null;
/**
 * Start the server
 */
const start = async () => {
    try {
        ...
        let userCacheService = null;
        let userService = null;
        let cacheManager = null;
        let mfaController = null;
        let magicLinkController = null;
        let ssoController = null;
        let userManagementController = null;
        let roleManagementController = null;
        let roleManagementServiceInstance = null;
        let sessionManagementController = null;
        let tenantController = null;
        let tenantMembershipController = null;
        let tenantServiceInstance = null;
        let auditLogService;
        let tenantJoinRequestService = null;
        let tenantInvitationService = null;
        ...
        try {
            ...
            try {
                ...
                const webhookDeliveryService = new WebhookDeliveryService(redisClient, monitoring, shardEventService);
        ...
        try {
            ...
            const connectionPolicy = {
                connectionMode: 'Direct', // Best performance (ConnectionMode enum not available in this version)
            ...
            try {
                ...
                const phase2ShardRepository = new ShardRepository(monitoring, tempCacheService, undefined // serviceBusService - will be set up in routes
                );
                // Initialize RedactionService (needs ShardRepository)
                const redactionService = new RedactionService(monitoring, phase2ShardRepository);
                ...
                const auditTrailService = new AuditTrailService(monitoring, phase2ShardRepository);
                ...
                const insightComputationService = new InsightComputationService(monitoring, phase2ShardRepository, shardsContainer, {
                ...
                const metricsShardService = new MetricsShardService(monitoring, phase2ShardRepository, process.env.ENABLE_METRICS_SHARDS !== 'false');
        ...
        if (cosmosClient && redisClient && keyVaultService) {
            try {
                aiConfigService = new AIConfigService(cosmosClient.getClient(), redisClient, keyVaultService);
        ...
        let aiConnectionService = null;
        if (redisClient) {
            try {
                aiConnectionService = new AIConnectionService(monitoring, redisClient, keyVaultService || undefined);
        ...
        if (cosmosClient) {
            try {
                userService = new UserService(cosmosClient.getUsersContainer(), userCacheService || undefined, monitoring);
        ...
        if (redisClient && cosmosClient) {
            try {
                const mfaService = new MFAService(server, redisClient, cosmosClient.getUsersContainer(), cosmosClient.getTenantsContainer(), emailService);
        ...
        if (redisClient && userService && cacheManager) {
            try {
                const magicLinkService = new MagicLinkService(server, redisClient, userService, emailService, process.env.FRONTEND_URL ||
                    (process.env.NODE_ENV === 'production'
                        ? (() => { throw new Error('FRONTEND_URL is required in production'); })()
                        : 'http://localhost:3000'));
                magicLinkController = new MagicLinkController(magicLinkService, cacheManager, process.env.JWT_ACCESS_TOKEN_EXPIRY || '9h');
        ...
        if (redisClient && cosmosClient && userService && cacheManager) {
            try {
                ...
                ssoController = new SSOController(samlService, ssoConfigService, userService, cacheManager, process.env.JWT_ACCESS_TOKEN_EXPIRY || '9h', process.env.FRONTEND_URL ||
                    (process.env.NODE_ENV === 'production'
                        ? (() => { throw new Error('FRONTEND_URL is required in production'); })()
                        : 'http://localhost:3000'));
        ...
        if (redisClient && userService && cacheManager) {
            try {
                ...
                if (azureB2CConfig.tenantName && azureB2CConfig.clientId) {
                    ...
                    const azureB2CController = new AzureADB2CController(azureB2CService, userService, cacheManager, process.env.JWT_ACCESS_TOKEN_EXPIRY || '9h', process.env.FRONTEND_URL ||
                        (process.env.NODE_ENV === 'production'
                            ? (() => { throw new Error('FRONTEND_URL is required in production'); })()
                            : 'http://localhost:3000'));
        ...
        if (cosmosClient && cacheManager && userService) {
            const userManagementService = new UserManagementService(cosmosClient.getUsersContainer(), userCacheService || undefined, emailService);
            userManagementController = new UserManagementController(userManagementService, cacheManager, userService);
        ...
        if (userService && mfaController && cacheManager) {
            try {
                const mfaService = new MFAService(server, redisClient, cosmosClient.getUsersContainer(), cosmosClient.getTenantsContainer(), emailService);
                const userSecurityController = new UserSecurityController(userService, mfaService, cacheManager, emailService, process.env.FRONTEND_URL ||
                    (process.env.NODE_ENV === 'production'
                        ? (() => { throw new Error('FRONTEND_URL is required in production'); })()
                        : 'http://localhost:3000'));
        ...
        if (cosmosClient) {
            try {
                const roleService = new RoleManagementService(cosmosClient.getDatabase(), cosmosClient.getRolesContainer(), cosmosClient.getUsersContainer());
        ...
        if (cosmosClient && auditLogService) {
            try {
                ...
                const documentController = new DocumentController(monitoring, auditLogService, config.azureStorage, // Pass Azure Storage config if available
                ...
                server.log.info(config.azureStorage
        ...
        if (server.cosmosDatabase && server.documentController && monitoring) {
            try {
                ...
                const shardsDatabase = server.cosmosDatabase;
                ...
                const bulkDocumentService = new BulkDocumentService(bulkJobRepository, server.shardRepository, null, auditWebhookEmitterInstance || undefined, monitoring);
                const documentBulkController = new DocumentBulkController(bulkDocumentService, monitoring);
                ...
                const bulkJobWorker = new BulkJobWorker(bulkJobRepository, monitoring, {
        ...
        if (cosmosClient) {
            try {
                tenantJoinRequestService = new TenantJoinRequestService(cosmosClient.getTenantJoinRequestsContainer());
            ...
            try {
                tenantInvitationService = new TenantInvitationService(cosmosClient.getTenantInvitationsContainer(), config.membership.invitations, redisClient);
        ...
        if (tenantServiceInstance &&
            ...
            userService) {
            tenantMembershipController = new TenantMembershipController(tenantJoinRequestService, tenantInvitationService, tenantServiceInstance, userService, emailService, config.frontend.baseUrl, config.membership.invitations);
        ...
        if (tenantInvitationService &&
            tenantServiceInstance &&
            userService &&
            !invitationLifecycleScheduler) {
            invitationLifecycleScheduler = new TenantInvitationLifecycleScheduler(tenantInvitationService, tenantServiceInstance, userService, emailService, config.frontend.baseUrl, config.membership.invitations, server.log);
            invitationLifecycleScheduler.start();
        }
        // Initialize device security service for new device detection
        let deviceSecurityService;
        if (emailService) {
            deviceSecurityService = new DeviceSecurityService(redisClient, emailService, config.api.publicUrl, monitoring);
        ...
        if (cacheManager && userService && tenantServiceInstance && tenantJoinRequestService) {
            const authController = new AuthController(userService, emailService, cacheManager, config.api.publicUrl, config.jwt.accessTokenExpiry, tenantServiceInstance, tenantJoinRequestService, mfaController ?? undefined, auditLogService, deviceSecurityService, roleManagementServiceInstance ?? undefined);
        ...
        if (redisClient && cacheManager && userService) {
            try {
                const oauthService = new OAuthService(redisClient, {
                ...
                const oauthControllerInstance = new OAuthController(oauthService, userService, cacheManager, config.jwt.accessTokenExpiry);
        ...
        if (redisClient && cosmosClient) {
            try {
                const oauth2ClientService = new OAuth2ClientService(cosmosClient.getOAuth2ClientsContainer());
                const oauth2AuthService = new OAuth2AuthService(redisClient);
                const oauth2Controller = new OAuth2Controller(oauth2ClientService, oauth2AuthService);
        ...
        if (cosmosClient && userService && tenantServiceInstance) {
            try {
                ...
                try {
                    ...
                    rawCosmosClient = server.cosmos;
                    database = server.cosmosDatabase;
                ...
                if (!rawCosmosClient || !database) {
                    ...
                    server.log.error(`   server.cosmos exists: ${!!server.cosmos}`);
                    server.log.error(`   server.cosmosDatabase exists: ${!!server.cosmosDatabase}`);
                }
                else {
                    // Ensure notifications container exists
                    try {
                        await NotificationRepository.ensureContainer(rawCosmosClient, database.id, 'notifications');
                    ...
                    const notificationRepository = new NotificationRepository(rawCosmosClient, database.id, 'notifications');
                    ...
                    try {
                        ...
                        await NotificationPreferenceRepository.ensureContainer(rawCosmosClient, database.id, 'notification-preferences');
                        notificationPreferenceRepository = new NotificationPreferenceRepository(rawCosmosClient, database.id, 'notification-preferences');
                    ...
                    let notificationDigestRepository;
                    try {
                    ...
                    try {
                        const { DeliveryTrackingService } = await import('./services/notifications/delivery-tracking.service.js');
                        deliveryTrackingService = new DeliveryTrackingService(notificationRepository, monitoring, {
                            maxAttempts: 3,
                    ...
                    if (emailService && emailService.isReady()) {
                        try {
                            const { EmailNotificationService } = await import('./services/notifications/email-notification.service.js');
                            ...
                            }, monitoring, deliveryTrackingService // Pass delivery tracking service
                            );
                    ...
                    let webhookNotificationService;
                    try {
                        const { WebhookNotificationService } = await import('./services/notifications/webhook-notification.service.js');
                        webhookNotificationService = new WebhookNotificationService({
                            ...
                            maxRetries: 3,
                        }, monitoring, deliveryTrackingService);
                    ...
                    let slackNotificationService;
                    try {
                        const { SlackNotificationService } = await import('./services/notifications/slack-notification.service.js');
                    ...
                    let teamsNotificationService;
                    try {
                        const { TeamsNotificationService } = await import('./services/notifications/teams-notification.service.js');
                    ...
                    const notificationService = new NotificationService(notificationRepository, userService, tenantServiceInstance, emailNotificationService, notificationPreferenceRepository, deliveryTrackingService, webhookNotificationService, slackNotificationService, teamsNotificationService, notificationDigestRepository, pushNotificationService, monitoring);
                    let notificationRealtimeService;
                    if (redisClient) {
                        notificationRealtimeService = new NotificationRealtimeService(redisClient, monitoring);
                    }
                    const notificationController = new NotificationController(notificationService, notificationRealtimeService, pushNotificationService);
                ...
                server.log.error(`Error stack: ${err?.stack}`);
        ...
        const rawCosmosClient = server.cosmos;
        const database = server.cosmosDatabase;
        if (rawCosmosClient && database && userService && tenantServiceInstance && keyVaultService) {
            try {
                ...
                const { AdapterManagerService } = await import('./services/adapter-manager.service.js');
                ...
                const auditLogService = server.auditLogService;
                const providerService = new IntegrationProviderService(providerRepository, integrationRepository, notificationService, auditLogService, monitoring);
                const integrationService = new IntegrationService(integrationRepository, providerRepository, notificationService, auditLogService, userService, monitoring);
                // Initialize IntegrationConnectionService
                const { IntegrationConnectionService } = await import('./services/integration-connection.service.js');
                ...
                if (!cosmosClient) {
                    ...
                    const adapterManager = new AdapterManagerService(connectionService, monitoring, connectionRepository);
                    // Update externalUserIdService and connectionService with adapterManager
                    externalUserIdService.adapterManager = adapterManager;
                    connectionService.adapterManager = adapterManager;
                    const searchService = new IntegrationSearchService(integrationRepository, providerRepository, adapterManager, externalUserIdService);
                    ...
                    let syncTaskService;
                    try {
                        const { SyncTaskService } = await import('./services/sync-task.service.js');
                        const { SyncTaskRepository, SyncExecutionRepository, SyncConflictRepository } = await import('./repositories/sync-task.repository.js');
                        const { ConversionSchemaRepository } = await import('./repositories/conversion-schema.repository.js');
                        const { ConversionSchemaService } = await import('./services/conversion-schema.service.js');
                        const { IntegrationShardService } = await import('./services/integration-shard.service.js');
                        const { IntegrationDeduplicationService } = await import('./services/integration-deduplication.service.js');
                        const { BidirectionalSyncEngine } = await import('./services/bidirectional-sync.service.js');
                        ...
                        if (!shardRepository) {
                            ...
                            const secureCredentialService = server.secureCredentialService;
                            if (!secureCredentialService) {
                                ...
                                const relationshipService = server.shardRelationshipService;
                                if (!relationshipService) {
                                    ...
                                    // Create SyncTaskService with all dependencies including connectionRepository
                                    syncTaskService = new SyncTaskService({
                                        ...
                                        connectionRepository, // Pass connectionRepository to fix the critical blocker
                                    });
                    ...
                    try {
                        const { WebhookManagementService } = await import('./services/webhook-management.service.js');
            ...
            if (!database)
                missingDeps.push('Database');
            if (!userService)
                missingDeps.push('User service');
            if (!tenantServiceInstance)
                missingDeps.push('Tenant service');
            if (!keyVaultService)
                missingDeps.push('Key Vault service');
            ...
            if (database)
                server.log.debug('  ✅ Database available');
            if (userService)
                server.log.debug('  ✅ User service available');
            if (tenantServiceInstance)
                server.log.debug('  ✅ Tenant service available');
            if (keyVaultService)
                server.log.debug('  ✅ Key Vault service available');
        }
        ...
        try {
            ...
            if (rawCosmosClient && databaseId) {
                ...
                const connectionService = server.integrationConnectionService;
                const emailTemplateService = new EmailTemplateService(rawCosmosClient, databaseId, integrationService, connectionService, monitoring);
...
// Graceful shutdown
const gracefulShutdown = async (signal, cacheSubscriberInstance, lifecycleScheduler) => {
    server.log.info(`${signal} received, shutting down gracefully...`);
    try {
        // Shutdown proactive insights event subscriber
        if (server.proactiveInsightsEventSubscriber) {
            await server.proactiveInsightsEventSubscriber.shutdown();
            server.log.info('Proactive insights event subscriber stopped');
        }
        // Shutdown proactive insights worker
        if (server.proactiveInsightsWorker) {
            server.proactiveInsightsWorker.stop();
            server.log.info('Proactive insights worker stopped');
        }
        // Shutdown proactive insights digest worker
        if (server.proactiveInsightsDigestWorker) {
            server.proactiveInsightsDigestWorker.stop();
            server.log.info('Proactive insights digest worker stopped');
        }
        // Shutdown bulk job worker
        if (server.bulkJobWorker) {
            await server.bulkJobWorker.stop();
            server.log.info('Bulk job worker stopped');
        }
        // Shutdown shard embedding change feed processor
        if (server.shardEmbeddingChangeFeedService) {
            await server.shardEmbeddingChangeFeedService.stop();
            server.log.info('Shard embedding change feed processor stopped');
        }
        // Shutdown embedding worker
        if (server.embeddingWorker) {
            await server.embeddingWorker.stop();
