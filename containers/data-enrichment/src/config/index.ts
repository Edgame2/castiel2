/**
 * Configuration loader for data-enrichment module
 */
...
let cachedConfig: DataEnrichmentConfig | null = null;
...
export function loadConfig(): DataEnrichmentConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as DataEnrichmentConfig;
  let envConfig: Partial<DataEnrichmentConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<DataEnrichmentConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as DataEnrichmentConfig;
  
  if (typeof resolved.server.port === 'string') {
    resolved.server.port = parseInt(resolved.server.port, 10);
  }
  const s = resolved.reembedding_scheduler;
  if (s) {
    if (s.enabled !== undefined && typeof s.enabled !== 'boolean') s.enabled = s.enabled === true || s.enabled === 'true' || s.enabled === '1';
    if (s.interval_ms !== undefined && typeof s.interval_ms !== 'number') s.interval_ms = parseInt(String(s.interval_ms), 10) || 86400000;
  }
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'data-enrichment' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'data-enrichment' });
  }
...
export function getConfig(): DataEnrichmentConfig {
