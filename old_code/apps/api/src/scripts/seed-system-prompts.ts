...
import { Prompt, PromptScope, PromptStatus, InsightType } from '../types/ai-insights/prompt.types.js';
...
function validatePromptDefinition(def: any): def is PromptDefinition {
...
async function verifyContainer(cosmos: AIInsightsCosmosService): Promise<boolean> {
  try {
    ...
    try {
      ...
      if (existing) {
        ...
        const needsUpdate = 
          ...
          existing.status !== (def.status as PromptStatus) ||
        ...
        if (needsUpdate) {
          ...
          await repo.update('SYSTEM', existing.id, {
            ...
            insightType: def.insightType as InsightType | undefined,
            tags: def.tags,
            status: def.status as PromptStatus,
            version: nextVersion,
            updatedAt: new Date(),
        ...
        const prompt: Prompt = {
          ...
          template: def.template,
          ragConfig: def.ragConfig,
          status: (def.status as PromptStatus) || PromptStatus.Active,
