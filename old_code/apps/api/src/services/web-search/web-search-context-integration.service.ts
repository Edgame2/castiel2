...
import { WebPageDocument, SemanticChunk } from './types.js';
...
export class WebSearchContextIntegrationService {
    ...
    private readonly AUTO_TRIGGER_CONFIDENCE = 0.7; // Intent confidence threshold
    private readonly MIN_RELEVANCE_SCORE = 0.65; // Minimum cosine similarity
    private readonly MAX_CACHE_AGE_HOURS = 24; // Use pages scraped within 24 hours
    private readonly MAX_CHUNKS_DEFAULT = 10; // Default max chunks to include

    // Redis cache settings
    private readonly REDIS_CACHE_TTL = 3600; // 1 hour in seconds
    private readonly REDIS_CACHE_PREFIX = 'websearch:';
     ...
     */
    async integrateWebSearchContext(
        tenantId: string,
        projectId: string,
        intent: IntentAnalysisResult,
        userQuery: string,
        baseContext: AssembledContext,
        ...
        try {
            ...
            let cachedPages: WebPageDocument[] = [];
            let cacheHit = false;
            ...
            if (this.redis) {
                try {
                    ...
                    if (cached) {
                        cachedPages = JSON.parse(cached) as WebPageDocument[];
                        cacheHit = true;
                    }
                } catch (error) {
                    this.monitoring?.trackException(error as Error, { operation: 'web-search-context-integration.redis-cache-read' });
                }
            }

            // Step 2: If Redis cache miss, check Cosmos DB for recent pages
            if (!cacheHit) {
                cachedPages = await this.getCachedPages(
                    tenantId,
                    projectId,
                    searchQuery,
                    options.maxCacheAge || this.MAX_CACHE_AGE_HOURS
                );
                ...
                if (this.redis && cachedPages.length > 0) {
                    try {
                        ...
                        this.monitoring?.trackException(error as Error, { operation: 'web-search-context-integration.redis-cache-write' });
                    }
                }
            }
            ...
            if (cachedPages.length === 0) {
                await this.triggerWebSearch(
                    tenantId,
                    projectId,
                    searchQuery,
                    options.enableDeepSearch || false,
                    options.deepSearchPages || 3
                );

                // Re-fetch cached pages after search
                const newPages = await this.getCachedPages(
                    tenantId,
                    projectId,
                    searchQuery,
                    1 // Just scraped, so use very recent
                );
                ...
                if (this.redis && newPages.length > 0) {
                    try {
                        ...
                        this.monitoring?.trackException(error as Error, { operation: 'web-search-context-integration.redis-cache-update' });
                    }
                }
            }

            // Step 3: Perform semantic retrieval on cached chunks
            const ragChunks = await this.performSemanticRetrieval(
                userQuery,
                cachedPages,
                options.minRelevanceScore || this.MIN_RELEVANCE_SCORE,
                options.maxChunks || this.MAX_CHUNKS_DEFAULT
            );

            // Step 4: Calculate metadata
            const totalChunks = cachedPages.reduce(
                (sum, page) => sum + (page.chunks?.length || 0),
                0
            );
            ...
            this.monitoring?.trackException(error as Error, { operation: 'web-search-context-integration.integrate' });
    ...
    private shouldTriggerWebSearch(
        ...
        const hasWebSearchKeywords = this.WEB_SEARCH_KEYWORDS.some(keyword =>
        ...
        const isExternalInfoQuery =
            queryLower.match(/\b(what|who|when|where|why|how)\b/) &&
     ...
     */
    private extractSearchQuery(userQuery: string, intent: IntentAnalysisResult): string {
    ...
    private async getCachedPages(
        ...
        try {
            ...
            this.monitoring?.trackException(error as Error, { operation: 'web-search-context-integration.get-cached-pages' });
            return [];
        }
    }
     ...
     */
    private async triggerWebSearch(
        tenantId: string,
        projectId: string,
        query: string,
        enableDeepSearch: boolean,
        ...
        try {
            ...
            this.monitoring?.trackException(error as Error, { operation: 'web-search-context-integration.trigger-web-search' });
            throw error;
        }
    }
    ...
    private async performSemanticRetrieval(
        ...
        try {
            ...
            // Step 2: Calculate similarity scores for all chunks
            ...
            const ragChunks: RAGChunk[] = topChunks.map((scored, index) => {
            ...
            this.monitoring?.trackException(error as Error, { operation: 'web-search-context-integration.semantic-retrieval' });
            return [];
        }
    }
     ...
     */
    private deduplicateChunks(chunks: RAGChunk[]): RAGChunk[] {
        if (chunks.length <= 1) {
            return chunks;
        }

        const deduplicated: RAGChunk[] = [];
     ...
     */
    private getCacheKey(tenantId: string, projectId: string, query: string): string {
     ...
     */
    async invalidateCache(tenantId: string, projectId: string, query: string): Promise<void> {
        if (!this.redis) {
            return;
        }
        ...
        try {
            ...
            this.monitoring?.trackException(error as Error, { operation: 'web-search-context-integration.cache-invalidation' });
        }
    }
    ...
    async invalidateProjectCache(tenantId: string, projectId: string): Promise<void> {
        ...
        try {
            ...
            this.monitoring?.trackException(error as Error, { operation: 'web-search-context-integration.project-cache-invalidation' });
        }
    }
     ...
     */
    async getCacheStats(tenantId: string, projectId?: string): Promise<{
        totalKeys: number;
        totalSize: number;
        hitRate?: number;
    }> {
        ...
        try {
            ...
            this.monitoring?.trackException(error as Error, { operation: 'web-search-context-integration.cache-stats' });
            return { totalKeys: 0, totalSize: 0 };
        }
    }
}
