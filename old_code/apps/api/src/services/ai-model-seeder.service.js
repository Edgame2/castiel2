...
export class AIModelSeederService {
    ...
    shardTypeContainer;
    monitoring;
    constructor(cosmosClient, monitoring) {
        const database = cosmosClient.database(env.cosmosDb.database);
        this.shardContainer = database.container(env.cosmosDb.containers.shards);
        this.shardTypeContainer = database.container(env.cosmosDb.containers.shardTypes);
     ...
     */
    async getActiveModels(modelType, provider) {
        let query = `
      SELECT * FROM c 
      WHERE c.shardTypeId = 'c_aimodel' 
     ...
     */
    async getModelByModelId(modelId) {
        const query = {
            query: `
        SELECT * FROM c 
        WHERE c.shardTypeId = 'c_aimodel' 
...
//# sourceMappingURL=ai-model-seeder.service.js.map