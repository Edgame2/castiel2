...
const createShardTypeContainerConfig = (containerName: string): ContainerDefinition => ({
  id: containerName,
  partitionKey: {
    paths: ['/tenantId'],
  },
...
export class ShardTypeRepository {
  ...
  private shardContainerName: string;
  ...
  constructor(
    ...
    this.shardContainerName = shardContainerName;
    const database = this.client.database(databaseId);
    this.container = database.container(containerName);
    this.shardContainer = database.container(shardContainerName);
  }
  ...
  async ensureContainer(): Promise<void> {
    try {
      ...
      await database.containers.createIfNotExists(createShardTypeContainerConfig(this.containerName));

      this.monitoring.trackEvent('cosmosdb.container.ensured', {
        container: this.containerName,
      });

      this.monitoring.trackEvent('cosmosdb.container.ensured', { containerId: this.containerName });
    } catch (error) {
