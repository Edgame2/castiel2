...
async function verifyAuditLogs() {
    ...
    `;
        ...
        resources.forEach((log, i) => {
            console.log(`${i + 1}. ${log.action}`);
            console.log(`   Resource: ${log.resourceType}/${log.resourceId}`);
            console.log(`   Timestamp: ${log.timestamp}`);
            if (log.metadata?.fileName) {
                console.log(`   File: ${log.metadata.fileName}`);
            }
            if (log.metadata?.fileSize) {
                console.log(`   Size: ${(log.metadata.fileSize / 1024).toFixed(2)} KB`);
            }
            if (log.metadata?.ipAddress) {
                console.log(`   IP: ${log.metadata.ipAddress}`);
            }
            console.log();
        });
