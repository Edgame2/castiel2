...
export class OpportunityAutoLinkingService {
    ...
    ACTIVITY_TIME_WINDOW_DAYS = 30;
    // Confidence thresholds
    STRONG_CONFIDENCE_THRESHOLD = 0.7;
    MEDIUM_CONFIDENCE_THRESHOLD = 0.4;
    constructor(monitoring, shardRepository, relationshipService, shardTypeRepository) {
     ...
     */
    async processShardCreated(shard) {
     ...
     */
    async findCandidateOpportunities(shard) {
     ...
     */
    async evaluateLinkingRules(shard, opportunity) {
        const rules = [];
     ...
     */
    checkContentOverlap(shard, opportunity) {
        const shardData = shard.structuredData;
        const oppData = opportunity.structuredData;
        let matched = false;
        let confidence = 0;
        const details = {};
        // Check account ID
     ...
     */
    checkMetadataOverlap(shard, opportunity) {
        const shardData = shard.structuredData;
        const oppData = opportunity.structuredData;
        let matched = false;
        let confidence = 0;
        ...
        if (Array.isArray(shardTeam) && Array.isArray(oppTeam) && shardTeam.length > 0 && oppTeam.length > 0) {
            const shardTeamIds = shardTeam.map((m) => (typeof m === 'string' ? m : m.id));
            const oppTeamIds = oppTeam.map((m) => (typeof m === 'string' ? m : m.id));
            const overlap = shardTeamIds.filter((id) => oppTeamIds.includes(id));
            if (overlap.length > 0) {
                matched = true;
     ...
     */
    checkTemporalOverlap(shard, opportunity) {
        const shardDate = shard.createdAt;
     ...
     */
    async checkAccountBased(shard, opportunity) {
        const oppData = opportunity.structuredData;
        ...
        try {
            const relatedShards = await this.relationshipService.getRelatedShards(shard.tenantId, shard.id, 'outgoing', { limit: 100 });
            const hasAccountLink = relatedShards.some((rel) => {
                const relShard = rel.shard;
                const relData = relShard.structuredData;
     ...
     */
    calculateTotalConfidence(rules) {
     ...
     */
    async linkShardToOpportunity(shard, opportunity, rules, confidence) {
        try {
     ...
     */
    getRelationshipType(shardTypeId) {
        const typeMap = {
            'c_task': 'has_task',
     ...
     */
    isAlreadyLinkedToOpportunity(shard) {
...
//# sourceMappingURL=opportunity-auto-linking.service.js.map