...
export class WebhookManagementService {
     ...
     */
    async registerWebhook(tenantId, integrationId, connectionId, events, options) {
        ...
        try {
            ...
            const providerResponse = await this.registerWithProvider(integration, connection, callbackUrl, events, webhookSecret);
            ...
            if (this.redis) {
                await this.redis.setex(`webhook:registration:${registrationId}`, 3600, // 1 hour
            ...
            throw new Error(`Failed to register webhook with ${integration.name}: ${error.message}`);
        }
    }
    /**
     * Unregister a webhook from provider
     */
    async unregisterWebhook(registrationId, tenantId) {
        ...
        try {
            // Get integration provider definition
            const integration = await this.integrationRepo.findByProviderName(registration.integrationId);
            if (!integration) {
                throw new Error(`Integration not found: ${registration.integrationId}`);
            }
            const connection = await this.connectionRepo.findById(registration.connectionId, registration.integrationId);
            if (!connection) {
            ...
            if (registration.providerWebhookId) {
                await this.unregisterWithProvider(integration, connection, registration.providerWebhookId, registration.metadata?.resourceId // Some adapters need resourceId (e.g., Google Workspace)
                );
            }
     ...
     */
    async processWebhookEvent(registrationId, headers, body) {
        ...
        try {
            ...
            const verified = await this.verifyWebhookSignature(registration, headers, body);
            if (!verified) {
                ...
                // Track failure
                await this.recordWebhookFailure(registrationId, 'Signature verification failed');
            ...
            const webhookEvent = this.parseProviderEvent(integration.name, eventData);
            // Trigger sync if applicable
            let syncTriggered = false;
            if (this.shouldTriggerSync(registration, webhookEvent)) {
                syncTriggered = await this.triggerSyncForEvent(registration, webhookEvent);
            }
     ...
     */
    async verifyWebhookSignature(registration, headers, body) {
        const integration = await this.integrationRepo.findByProviderName(registration.integrationId);
        if (!integration) {
            return false;
        }
        ...
        switch (config.algorithm) {
            case 'hmac-sha256':
                return this.verifyHmacSignature(registration.webhookSecret, config.headerName, headers, bodyStr);
            case 'hmac-sha1':
                return this.verifyHmacSignature(registration.webhookSecret, config.headerName, headers, bodyStr, 'sha1');
            case 'rsa-sha256':
                return this.verifyRsaSignature(config.headerName, headers, bodyStr);
            case 'request-signing':
                return this.verifyRequestSigning(registration.webhookSecret, headers, bodyStr);
            default:
                return false;
        }
    }
     ...
     */
    verifyHmacSignature(secret, headerName, headers, body, algorithm = 'sha256') {
     ...
     */
    async verifyRsaSignature(headerName, _headers, _body) {
     ...
     */
    verifyRequestSigning(secret, headers, body) {
     ...
     */
    async checkWebhookHealth(registrationId) {
        const registration = await this.getRegistration(registrationId);
        if (!registration) {
            throw new Error(`Webhook registration not found: ${registrationId}`);
        }
     ...
     */
    async recordWebhookFailure(registrationId, reason) {
        const registration = await this.getRegistration(registrationId);
        if (!registration) {
            return;
        }
        ...
        if (registration.failureReasons.length < 10) {
            registration.failureReasons.push(`${new Date().toISOString()}: ${reason}`);
        }
     ...
     */
    parseProviderEvent(provider, eventData) {
     ...
     */
    shouldTriggerSync(registration, event) {
     ...
     */
    async triggerSyncForEvent(registration, event) {
        try {
            ...
            try {
                await this.syncTaskService.triggerSync(task.id, registration.tenantId, 'system' // System user for webhook-triggered syncs
                );
     ...
     */
    async publishToEventGrid(registrationId, eventData) {
        if (!this.eventGridTopicEndpoint || !this.eventGridAccessKey) {
            return;
        }
        try {
     ...
     */
    async registerWithProvider(integration, connection, callbackUrl, events, _secret) {
        ...
        try {
            ...
            const adapter = await this.adapterManager.getAdapter(integration.provider, {
                id: integration.id,
            ...
            if (typeof adapter.registerWebhook !== 'function') {
            ...
            const result = await adapter.registerWebhook(events, callbackUrl, undefined // Some adapters need resource parameter
            );
     ...
     */
    async unregisterWithProvider(integration, connection, providerWebhookId, resourceId) {
        ...
        try {
            ...
            if (typeof adapter.unregisterWebhook !== 'function') {
            ...
            await adapter.unregisterWebhook(providerWebhookId, resourceId);
    ...
    async getRegistration(registrationId) {
        ...
        // Check Redis
        if (this.redis) {
            const cached = await this.redis.get(`webhook:registration:${registrationId}`);
            if (cached) {
                registration = JSON.parse(cached);
                this.webhookCache.set(registrationId, registration);
                return registration;
            }
        }
     ...
     */
    async updateRegistration(registrationId, registration) {
        // Update in-memory cache
        this.webhookCache.set(registrationId, registration);
        // Update Redis
        if (this.redis) {
            await this.redis.setex(`webhook:registration:${registrationId}`, 3600, JSON.stringify(registration));
        }
    }
    // =====================
    // Utilities
     ...
     */
    async listRegistrations(tenantId, integrationId, connectionId) {
        const registrations = [];
        for (const registration of this.webhookCache.values()) {
     ...
     */
    async getWebhookRegistration(registrationId) {
    ...
    async healthCheck() {
        ...
        const activeRegistrations = Array.from(this.webhookCache.values()).filter((w) => w.status === 'active').length;
