...
const DEFAULT_CONFIG: ChangeFeedProcessorConfig = {
  enabled: true,
...
export class ShardEmbeddingChangeFeedService {
  private processor: any = null; // ChangeFeedProcessor from @azure/cosmos
  private isRunning = false;
  private config: ChangeFeedProcessorConfig;
  private processingQueue: Map<string, Promise<void>> = new Map();
  private leaseContainer?: Container; // Optional - will be created if needed
  private serviceBusService?: any; // Optional - for enqueue mode

  constructor(
    private readonly shardContainer: Container,
    private readonly shardEmbeddingService: ShardEmbeddingService,
   ...
   */
  private startPolling(pollIntervalMs: number): void {
    const poll = async () => {
      if (!this.isRunning) {
        return;
      }
      ...
      try {
        ...
        if (shards.length > 0) {
          ...
          if (this.serviceBusService && this.config.mode === 'enqueue') {
            ...
            const jobMessages: EmbeddingJobMessage[] = batch
              .map((shard) => this.createEmbeddingJobMessage(shard as Shard))
              .filter((msg): msg is EmbeddingJobMessage => msg !== null);
              ...
              try {
                ...
                if (this.serviceBusService.sendEmbeddingJobBatch) {
                  ...
                  const enqueueResults = await Promise.allSettled(
                    jobMessages.map((msg) => this.serviceBusService!.sendEmbeddingJob(msg))
                  );
                ...
                await Promise.allSettled(
                  batch.map((shard) => this.processShard(shard as Shard))
                );
              }
            }
          } else {
   ...
   */
  private createEmbeddingJobMessage(shard: Shard): EmbeddingJobMessage | null {
    try {
   ...
   */
  private async processShard(shard: Shard): Promise<void> {
