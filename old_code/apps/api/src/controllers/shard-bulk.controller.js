...
import { ShardErrorCode, } from '../types/shard-errors.types.js';
...
export class ShardBulkController {
    ...
    bulkUpdate = async (req, reply) => {
        ...
        try {
            ...
            for (let i = 0; i < updates.length; i++) {
                ...
                try {
                    // Check permission
                    const hasAccess = await this.repository.checkPermission(update.id, tenantId, userId);
     ...
     */
    bulkStatusChange = async (req, reply) => {
        const startTime = Date.now();
        try {
            const auth = req.auth;
            if (!auth || !auth.tenantId || !(auth).userId) {
            ...
            for (let i = 0; i < shardIds.length; i++) {
                ...
                try {
                    ...
                    // Emit event if enabled
                    if (!options.skipEvents && this.eventService) {
                        await this.eventService.emit(status === ShardStatus.ARCHIVED ? ShardEventType.ARCHIVED : ShardEventType.STATUS_CHANGED, updated, {
                            triggeredBy: userId,
                            triggerSource: 'api',
     ...
     */
    bulkTagOperation = async (req, reply) => {
        const startTime = Date.now();
        try {
            const auth = req.auth;
            if (!auth || !auth.tenantId || !(auth).userId) {
    ...
    bulkExport = async (req, reply) => {
        ...
        try {
            ...
            const { shardIds, shardTypeId, status, limit = MAX_EXPORT_SIZE, format = 'json', includeMetadata = true, fields, } = req.body;
...
//# sourceMappingURL=shard-bulk.controller.js.map