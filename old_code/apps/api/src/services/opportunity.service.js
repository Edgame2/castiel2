...
export class OpportunityService {
    ...
    teamService;
    constructor(monitoring, shardRepository, shardTypeRepository, relationshipService, riskEvaluationService, teamService) {
     ...
     */
    async listOwnedOpportunities(userId, tenantId, filters, options) {
        ...
        try {
            ...
            try {
                ...
                if (error.message?.includes('input values is invalid')) {
            ...
            if (filters?.riskLevel || filters?.riskCategory || filters?.riskScoreMin || filters?.riskScoreMax) {
                filteredOpportunities = await this.applyRiskFilters(filteredOpportunities, filters, tenantId, userId);
            }
     ...
     */
    async getOpportunity(opportunityId, tenantId, includeRelated = true) {
        const startTime = Date.now();
        try {
            // Get opportunity
            ...
            if (includeRelated) {
                // Get related shards
                const relatedShardsResult = await this.relationshipService.getRelatedShards(tenantId, opportunityId, 'both', { limit: 100 });
     ...
     */
    async getOpportunitiesByAccount(accountId, tenantId, options) {
        try {
            const shardType = await this.shardTypeRepository.findByName(CORE_SHARD_TYPE_NAMES.OPPORTUNITY, 'system');
            if (!shardType) {
                return [];
     ...
     */
    async updateStage(opportunityId, newStage, tenantId, userId) {
        const startTime = Date.now();
        try {
            const opportunity = await this.shardRepository.findById(opportunityId, tenantId);
            if (!opportunity) {
            ...
            const currentData = opportunity.structuredData;
            ...
            if (this.riskEvaluationService && oldStage !== newStage) {
                try {
                    await this.riskEvaluationService.queueRiskEvaluation(opportunityId, tenantId, userId, 'opportunity_updated', 'normal', {
     ...
     */
    async applyRiskFilters(opportunities, filters, tenantId, userId) {
        if (!filters.riskLevel && !filters.riskCategory && !filters.riskScoreMin && !filters.riskScoreMax) {
        ...
        for (const opp of opportunities) {
            ...
            if (filters.riskCategory) {
                ...
                const riskCategories = riskEvaluation?.risks?.map((r) => r.category) || [];
     ...
     */
    async listTeamOpportunities(teamId, tenantId, userId, filters, options) {
        const startTime = Date.now();
        try {
            if (!this.teamService) {
     ...
     */
    async listManagerOpportunities(managerId, tenantId, userId, filters, options) {
        const startTime = Date.now();
        try {
            if (!this.teamService) {
