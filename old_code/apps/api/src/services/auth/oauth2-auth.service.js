...
export class OAuth2AuthService {
    redis;
    // Redis key prefixes
    AUTH_CODE_PREFIX = 'oauth2:auth_code:';
    ACCESS_TOKEN_PREFIX = 'oauth2:access_token:';
    REFRESH_TOKEN_PREFIX = 'oauth2:refresh_token:';
    USER_TOKENS_PREFIX = 'oauth2:user_tokens:'; // Track user's active tokens
    // TTLs
     ...
     */
    async generateAuthorizationCode(clientId, userId, tenantId, redirectUri, scope, codeChallenge, codeChallengeMethod) {
     ...
     */
    async validateAuthorizationCode(code, clientId, redirectUri, codeVerifier) {
        const key = this.AUTH_CODE_PREFIX + code;
        ...
        if (authCode.codeChallenge) {
            ...
            const isValid = this.validatePKCE(codeVerifier, authCode.codeChallenge, authCode.codeChallengeMethod || 'plain');
            if (!isValid) {
                return null;
            }
        }
     ...
     */
    async generateTokens(client, scope, userId // Optional for client_credentials flow
    ) {
     ...
     */
    async refreshAccessToken(refreshToken, clientId, requestedScope) {
     ...
     */
    async revokeUserTokens(userId, tenantId) {
     ...
     */
    async revokeClientTokens(clientId, tenantId) {
     ...
     */
    async addUserToken(userId, tenantId, accessToken, refreshToken) {
     ...
     */
    async getUserActiveTokensCount(userId, tenantId) {
        const key = this.USER_TOKENS_PREFIX + `${tenantId}:${userId}`;
        const tokensData = await this.redis.get(key);
        if (!tokensData) {
            return 0;
