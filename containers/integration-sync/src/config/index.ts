/**
 * Configuration loader for integration-sync module
 */
...
let cachedConfig: IntegrationSyncConfig | null = null;
...
export function loadConfig(): IntegrationSyncConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as IntegrationSyncConfig;
  let envConfig: Partial<IntegrationSyncConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<IntegrationSyncConfig>;
  }
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'integration-sync' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'integration-sync' });
  }
...
export function getConfig(): IntegrationSyncConfig {
