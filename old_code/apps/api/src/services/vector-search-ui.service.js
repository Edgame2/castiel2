...
export class VectorSearchUIService {
     ...
     */
    async recordSearchHistory(userId, tenantId, query, options) {
        const entry = {
            id: uuidv4(),
            userId,
            tenantId,
     ...
     */
    async getSearchHistory(userId, tenantId, options) {
        ...
        try {
            ...
            const { resources } = await this.historyContainer.items.query(query).fetchAll();
            return {
                entries: resources,
     ...
     */
    async createSavedSearch(userId, tenantId, input) {
        const savedSearch = {
            id: uuidv4(),
            userId,
            tenantId,
     ...
     */
    async getSavedSearches(userId, tenantId, options) {
        try {
            const query = {
            ...
            if (options?.visibility === 'private') {
                ...
                query.parameters.push({ name: '@userId', value: userId }, { name: '@visibility', value: 'private' });
            }
            else if (options?.includeShared) {
                query.query += ' AND (c.userId = @userId OR c.visibility IN (@team, @tenant) OR @userId = ANY(c.sharedWith))';
                query.parameters.push({ name: '@userId', value: userId }, { name: '@team', value: 'team' }, { name: '@tenant', value: 'tenant' });
            }
            else {
            ...
            const { resources } = await this.savedSearchesContainer.items.query(query).fetchAll();
            return resources;
        }
        catch (error) {
            this.monitoring.trackException(error, {
     ...
     */
    async getSavedSearch(id, userId, tenantId) {
        try {
     ...
     */
    async updateSavedSearch(id, userId, tenantId, input) {
     ...
     */
    async recordSavedSearchUsage(id, userId, tenantId) {
        try {
     ...
     */
    async getAutocompleteSuggestions(userId, tenantId, request) {
        const suggestions = [];
     ...
     */
    async trackPopularSearch(tenantId, query, options) {
        if (!this.redis) {
            return;
        }
        try {
...
//# sourceMappingURL=vector-search-ui.service.js.map