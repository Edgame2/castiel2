...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  await fastify.register(swagger, {
    openapi: {
      openapi: '3.0.3',
      info: {
        title: 'Integration Manager Service API',
        description: 'Third-party integrations and webhooks service',
        version: '1.0.0',
      },
      ...
      tags: [
        ...
        { name: 'SyncTasks', description: 'Sync task management' },
...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Integration Manager service listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
    console.error('Failed to start Integration Manager service:', error);
    process.exit(1);
  }
}
