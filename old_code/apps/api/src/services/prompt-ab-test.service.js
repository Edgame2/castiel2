...
export class PromptABTestService {
    cosmosService;
    monitoring;
    constructor(cosmosService, monitoring) {
     ...
     */
    async createExperiment(tenantId, input, userId) {
        ...
        const created = await this.cosmosService.create(container, experiment);
     ...
     */
    async getExperiment(tenantId, experimentId) {
        const container = this.cosmosService.getExperimentsContainer();
        const partitionKey = this.cosmosService.buildPartitionKey(tenantId, experimentId, 'system');
        return this.cosmosService.read(container, experimentId, partitionKey);
    }
    /**
     * List experiments
     */
    async listExperiments(tenantId, options = {}) {
        ...
        const result = await this.cosmosService.query(container, querySpec, {
            maxItems: limit,
            continuationToken,
        });
        return {
     ...
     */
    async updateExperiment(tenantId, experimentId, input, userId) {
        ...
        const container = this.cosmosService.getExperimentsContainer();
        const partitionKey = this.cosmosService.buildPartitionKey(tenantId, experimentId, 'system');
        const updated = await this.cosmosService.update(container, experimentId, partitionKey, updates);
     ...
     */
    async selectVariant(tenantId, userId, insightType, slug) {
     ...
     */
    async recordEvent(tenantId, experimentId, userId, event) {
        // Get assignment
        ...
        const container = this.cosmosService.getExperimentsContainer();
        await this.cosmosService.create(container, eventDoc);
        // Update assignment
        ...
        this.updateExperimentMetrics(tenantId, experimentId, assignment.variantId, event).catch((err) => {
     ...
     */
    async calculateResults(tenantId, experimentId) {
        const experiment = await this.getExperiment(tenantId, experimentId);
        if (!experiment) {
            throw new Error(`Experiment ${experimentId} not found`);
        }
        ...
        const statisticalSignificance = this.calculateStatisticalSignificance(controlMetrics, treatmentMetrics, experiment.primaryMetric);
        // Determine winner
        let winner;
        let improvement;
     ...
     */
    async getResults(tenantId, experimentId) {
        const experiment = await this.getExperiment(tenantId, experimentId);
        if (!experiment) {
            throw new Error(`Experiment ${experimentId} not found`);
        }
     ...
     */
    async findActiveExperiments(tenantId, insightType, slug) {
        const container = this.cosmosService.getExperimentsContainer();
        let query = `
      SELECT * FROM c 
    ...
    `;
        ...
        query += ' ORDER BY c.createdAt DESC';
        const querySpec = { query, parameters };
        const results = await this.cosmosService.queryAll(container, querySpec, 10);
        return results;
    }
    /**
     * Check if user matches targeting criteria
     */
    matchesTargeting(experiment, tenantId, userId) {
     ...
     */
    async getOrCreateAssignment(tenantId, experimentId, userId) {
        ...
        const container = this.cosmosService.getExperimentsContainer();
        return this.cosmosService.create(container, assignment);
    }
    /**
     * Get assignment
     */
    async getAssignment(tenantId, experimentId, userId) {
        const container = this.cosmosService.getExperimentsContainer();
      ...
      `,
        ...
        const results = await this.cosmosService.queryAll(container, query, 1);
        return results.length > 0 ? results[0] : null;
    }
    /**
     * Update assignment
     */
    async updateAssignment(tenantId, experimentId, userId, updates) {
        const assignment = await this.getAssignment(tenantId, experimentId, userId);
        if (!assignment) {
            return;
        }
        const container = this.cosmosService.getExperimentsContainer();
        const partitionKey = this.cosmosService.buildPartitionKey(tenantId, experimentId, userId);
        await this.cosmosService.update(container, assignment.id, partitionKey, {
            ...updates,
            updatedAt: new Date(),
        });
    }
     ...
     */
    async getExperimentEvents(tenantId, experimentId) {
        const container = this.cosmosService.getExperimentsContainer();
        const query = {
            query: `
        SELECT * FROM c 
      ...
      `,
        ...
        return this.cosmosService.queryAll(container, query, 10000);
    }
    /**
     * Update experiment metrics (async aggregation)
     */
    async updateExperimentMetrics(tenantId, experimentId, variantId, event) {
        const experiment = await this.getExperiment(tenantId, experimentId);
        if (!experiment) {
            return;
        }
        ...
        const container = this.cosmosService.getExperimentsContainer();
        const partitionKey = this.cosmosService.buildPartitionKey(tenantId, experimentId, 'system');
        await this.cosmosService.update(container, experimentId, partitionKey, {
     ...
     */
    calculateStatisticalSignificance(control, treatment, metric) {
     ...
     */
    getMetricValue(variantResults, metric) {
        switch (metric) {
            case 'quality':
...
//# sourceMappingURL=prompt-ab-test.service.js.map