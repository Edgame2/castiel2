/**
 * Configuration loader for recommendations module
 */
...
export function loadConfig(): RecommendationsConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as RecommendationsConfig;
  let envConfig: Partial<RecommendationsConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<RecommendationsConfig>;
  }
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'recommendations' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'recommendations' });
  }
...
export function getConfig(): RecommendationsConfig {
