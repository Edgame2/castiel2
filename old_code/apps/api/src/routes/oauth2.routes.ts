...
export async function registerOAuth2Routes(server: FastifyInstance): Promise<void> {
  const controller: OAuth2Controller | undefined = (server as any).oauth2Controller;
  if (!controller) {
    server.log.warn('OAuth2 controller not available - skipping OAuth2 routes');
    return;
  }

  const optionalAuthentication = (server as any).optionalAuthenticate;
  ...
  server.get(
    ...
    async (request, reply) => controller.authorize(request as any, reply)
  );
  ...
  server.post(
    ...
    async (request, reply) => controller.token(request as any, reply)
  );
  ...
  server.post(
    ...
    async (request, reply) => controller.revoke(request as any, reply)
  );
}
