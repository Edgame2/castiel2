...
describe('IntegrationRateLimiter', () => {
    ...
    beforeEach(() => {
        ...
        rateLimiter = new IntegrationRateLimiter(mockCache, mockMonitoring, mockServiceBus);
    });
    afterEach(() => {
        vi.clearAllMocks();
    });
    describe('checkRateLimit', () => {
        ...
        it('should reject request when rate limit exceeded', async () => {
            ...
            mockCache.get.mockResolvedValue(JSON.stringify({ count: 300, createdAt: Date.now() }));
            const result = await rateLimiter.checkRateLimit({
                integrationId: 'salesforce',
                tenantId: 'tenant-123',
            });
            ...
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('rate_limit_exceeded', expect.any(Object));
        });
        it('should enforce per-integration limits', async () => {
            // Salesforce: 300/min
            mockCache.get.mockResolvedValueOnce(JSON.stringify({ count: 299, createdAt: Date.now() }));
            ...
            mockCache.get.mockResolvedValueOnce(JSON.stringify({ count: 300, createdAt: Date.now() }));
        ...
        it('should enforce per-tenant limits', async () => {
            ...
            mockCache.get.mockResolvedValueOnce(JSON.stringify({ count: 5000, createdAt: Date.now() }));
            const result = await rateLimiter.checkRateLimit({
                integrationId: 'salesforce',
                tenantId: 'tenant-123',
            });
        ...
        it('should enforce per-operation limits', async () => {
            ...
            mockCache.get.mockResolvedValue(JSON.stringify({ count: 100, createdAt: Date.now() }));
            mockCache.get.mockResolvedValueOnce(null);
            const result = await rateLimiter.checkRateLimit({
                integrationId: 'salesforce',
                tenantId: 'tenant-123',
        ...
        it('should warn when approaching rate limit (80%)', async () => {
            ...
            mockCache.get.mockResolvedValueOnce(JSON.stringify({ count: 239, createdAt: Date.now() }));
            mockCache.get.mockResolvedValueOnce(null);
            const result = await rateLimiter.checkRateLimit({
                integrationId: 'salesforce',
                tenantId: 'tenant-123',
            ...
            expect(result.allowed).toBe(true);
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('rate_limit_warning', expect.any(Object));
        });
        it('should queue requests instead of rejecting for fetch operations', async () => {
            ...
            expect(result.allowed).toBe(true);
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('rate_limit_queued', expect.any(Object));
        });
    ...
    describe('updateAdaptiveLimit', () => {
        it('should reduce rate limit when provider near capacity', async () => {
            ...
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('adaptive_limit_reduced', expect.any(Object));
        });
        it('should restore rate limit when provider has capacity', async () => {
            ...
            await rateLimiter.updateAdaptiveLimit('salesforce', 'tenant-123', headers);
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('adaptive_limit_restored', expect.any(Object));
        });
        it('should parse Notion rate limit headers', async () => {
            ...
            expect(mockMonitoring.trackEvent).not.toHaveBeenCalledWith('adaptive_limit_reduced', expect.any(Object));
        });
    ...
    describe('getStatus', () => {
        ...
        it('should indicate when throttled (80% used)', async () => {
            mockCache.get.mockResolvedValue(JSON.stringify({ count: 240, createdAt: Date.now() }));
            const status = await rateLimiter.getStatus({
                integrationId: 'salesforce',
                tenantId: 'tenant-123',
            });
        ...
        it('should include queued requests count', async () => {
            ...
            rateLimiter.queuedRequests.set('rate_limit:tenant-123:salesforce:all', queue);
            mockCache.get.mockResolvedValue(null);
            const status = await rateLimiter.getStatus({
                integrationId: 'salesforce',
                tenantId: 'tenant-123',
        ...
        it('should reflect adaptive multiplier in rate limit', async () => {
            ...
            rateLimiter.adaptiveMultipliers.set('salesforce:tenant-123', 0.5);
            mockCache.get.mockResolvedValue(null);
            const status = await rateLimiter.getStatus({
                integrationId: 'salesforce',
                tenantId: 'tenant-123',
    ...
    describe('processQueue', () => {
        it('should process queued requests when rate limit allows', async () => {
            ...
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('queue_processed', expect.any(Object));
        });
        ...
        it('should clear queue when all requests processed', async () => {
            ...
            const remaining = rateLimiter.queuedRequests.get('rate_limit:tenant-123:salesforce:all');
    ...
    describe('setIntegrationLimit', () => {
        it('should set custom rate limit for integration', async () => {
            ...
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('rate_limit_updated', { integrationId: 'salesforce', requestsPerMinute: 500 });
        });
    });
    describe('setTenantLimit', () => {
        it('should set custom rate limit for tenant', async () => {
            rateLimiter.setTenantLimit('tenant-456', 10000);
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('tenant_rate_limit_updated', { tenantId: 'tenant-456', requestsPerMinute: 10000 });
        });
    });
    describe('resetIntegration', () => {
        it('should reset rate limit counters for integration', async () => {
            ...
            expect(mockCache.delete).toHaveBeenCalledWith('rate_limit:tenant-123:salesforce:all');
            expect(mockCache.delete).toHaveBeenCalledWith('rate_limit:tenant-123:salesforce:create');
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('integration_rate_limit_reset', expect.any(Object));
        });
    });
    describe('resetTenant', () => {
        it('should reset rate limit counters for tenant', async () => {
            ...
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('tenant_rate_limit_reset', { tenantId: 'tenant-123' });
        });
    });
    describe('onAlert', () => {
...
//# sourceMappingURL=integration-rate-limiter.service.test.js.map