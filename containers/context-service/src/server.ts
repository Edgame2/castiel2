...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  await fastify.register(swagger, {
    openapi: {
      ...
      info: {
        ...
        description: 'Centralized context management and assembly service',
        version: '1.0.0',
      },
      servers: [{ url: '/api/v1', description: 'API Version 1' }],
      tags: [
        { name: 'Contexts', description: 'Context management' },
        { name: 'Context Assembly', description: 'Dynamic context assembly' },
        { name: 'Dependencies', description: 'Dependency tracking' },
        { name: 'Call Graphs', description: 'Call graph analysis' },
  ...
  initializeDatabase({
    ...
    containers: {
      ...
      dependencyTrees: 'context_dependency_trees',
      callGraphs: 'context_call_graphs',
...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Context Service listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
