...
} from '../services/content-generation/types/template.types.js';
import {
  ExtractionRequest,
  PlaceholderPreviewRequest,
} from '../services/content-generation/types/placeholder.types.js';
import {
  GenerationRequest,
  GenerationJobStatus,
} from '../services/content-generation/types/generation.types.js';
...
export class DocumentTemplateController {
  ...
  private previewService?: PlaceholderPreviewService;

  constructor(
    service: DocumentTemplateService,
    generationService?: DocumentGenerationService,
    serviceBus?: QueueService,
    jobRepository?: GenerationJobRepository,
    previewService?: PlaceholderPreviewService
  ) {
   ...
   * Admin only
   */
  async extractPlaceholders(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      const result = await this.service.extractPlaceholders(
        body.templateId,
        user.tenantId,
  ...
  async list(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      if (query.documentFormat) {filters.documentFormat = query.documentFormat;}
   ...
   * Admin only
   */
  async updateColors(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      if (!isTenantAdmin(user)) {
        throw new AppError('Tenant administrator access required', 403);
   ...
   * Admin only
   */
  async updatePlaceholderConfigs(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      if (!isTenantAdmin(user)) {
        throw new AppError('Tenant administrator access required', 403);
      ...
      const params = request.params as { id: string };
      const body = request.body as { configs: PlaceholderConfiguration[] };

      if (!body.configs || !Array.isArray(body.configs)) {
        throw new AppError('configs array is required', 400);
      }

      const template = await this.service.updatePlaceholderConfigs(
        params.id,
        user.tenantId,
        body.configs
      );
  ...
  async createVersion(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      const params = request.params as { id: string };
      const body = request.body as { changes: string };
  ...
  async rollback(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      const params = request.params as { id: string };
      const body = request.body as { versionNumber: number };

      if (body.versionNumber === undefined) {
        throw new AppError('versionNumber is required', 400);
      }

      const template = await this.service.rollbackToVersion(
        params.id,
        user.tenantId,
        body.versionNumber
      );
   ...
   * Admin only
   */
  async testPlaceholder(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      if (!isTenantAdmin(user)) {
        throw new AppError('Tenant administrator access required', 403);
      ...
      const params = request.params as { id: string; placeholderName: string };
      const body = request.body as { context?: Record<string, any> };

      const previewRequest: PlaceholderPreviewRequest = {
      ...
      const result = await this.previewService.testPlaceholderGeneration(
        user.tenantId,
        user.id,
        previewRequest
      );
  ...
  async validate(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      const result = await this.previewService.validateAllPlaceholders(
        params.id,
        user.tenantId
      );
  ...
  async generate(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      const userToken = await this.getUserOAuthToken(
        user.tenantId,
        user.id,
        body.destinationFolder.provider,
        request
      );
   ...
   */
  async cancelJob(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      ...
      const result = await this.generationService.cancelJob(
        params.jobId,
        user.tenantId,
        user.id
      );
    ...
    try {
      ...
      if (query.createdAfter) {
        ...
        if (isNaN(createdAfter.getTime())) {
          throw new ValidationError(
            `Invalid date format for createdAfter: "${query.createdAfter}". Expected ISO 8601 format (e.g., 2024-01-01T00:00:00Z)`
          );
        }
      }
      ...
      if (query.createdBefore) {
        ...
        if (isNaN(createdBefore.getTime())) {
          throw new ValidationError(
            `Invalid date format for createdBefore: "${query.createdBefore}". Expected ISO 8601 format (e.g., 2024-01-01T00:00:00Z)`
          );
        }
      }
   ...
   */
  async getJobStats(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
  ...
  async retryJob(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      const _userToken = await this.getUserOAuthToken(
        user.tenantId,
        user.id,
        'google', // Default to google, could be enhanced to detect from job
        request
      ).catch((error) => {
      ...
      const result = await this.generationService.retryJob(
        params.jobId,
        user.tenantId,
        user.id
      );
  ...
  async getJobStatus(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    ...
    try {
      ...
      const result = await this.generationService.getJobStatus(
        params.jobId,
        user.tenantId,
        user.id
      );
   ...
   */
  async findStuckJobs(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
   ...
   */
  async markStuckJobsAsFailed(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
   ...
   */
  async cleanupOldJobs(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      ...
      const deletedCount = await this.generationService.cleanupOldJobs(
        user.tenantId,
        olderThanDays
      );
   ...
   */
  private async getUserOAuthToken(
    tenantId: string,
    userId: string,
    provider: 'google' | 'microsoft',
    ...
    const integrationService = (request.server as any).integrationService;
    const integrationConnectionService = (request.server as any).integrationConnectionService;

    if (!integrationService || !integrationConnectionService) {
      throw new AppError(
        'Integration services not available. Cannot retrieve OAuth token.',
        503
      );
    }

    // Map provider to integration provider name
    const providerMap: Record<'google' | 'microsoft', string> = {
    ...
    if (integrations.integrations.length === 0) {
      throw new ValidationError(
        `No connected ${providerName} integration found. Please connect the integration first.`
      );
    }
    ...
    try {
      const accessToken = await integrationConnectionService.getUserOAuthToken(
        integration.id,
        tenantId,
        userId
      );
      ...
      throw new AppError(
        `Failed to retrieve OAuth token for ${providerName}: ${(error as Error).message}`,
        400
      );
    }
  }
   ...
   */
  async listGoogleDriveFolders(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const query = request.query as {
        parentId?: string;
   ...
   */
  async listOneDriveFolders(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const query = request.query as {
        parentId?: string;
      ...
      const folders = (response.data.value || [])
        .filter((item: any) => item.folder) // Ensure it's a folder
        .map((item: any) => ({
          id: item.id,
          name: item.name,
      ...
      const filteredFolders = search
        ? folders.filter((f: any) => f.name.toLowerCase().includes(search.toLowerCase()))
        : folders;
   ...
   * Get folder details
   */
  async getFolderDetails(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const params = request.params as { provider: string; folderId: string };
      ...
      if (provider === 'google') {
        ...
        const response = await axios.get(
          `${graphApiBaseUrl}/me/drive/items/${params.folderId}`,
          {
            headers: {
              Authorization: `Bearer ${userToken}`,
            },
      ...
      const errorName = error instanceof Error ? error.name : (error && typeof error === 'object' && 'name' in error ? String((error as { name?: unknown }).name) : 'Internal Server Error');
