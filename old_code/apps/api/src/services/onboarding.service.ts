...
const DEFAULT_CHECKLIST: Omit<OnboardingChecklistItem, 'completed' | 'completedAt' | 'skipped' | 'skippedAt'>[] = [
  {
    id: 'verify-email',
...
export class OnboardingService {
  ...
  private defaultConfig: OnboardingConfig;
  ...
  async initializeOnboarding(
    ...
    const checklist: OnboardingChecklistItem[] = this.defaultConfig.checklistItems.map(item => ({
      ...item,
      completed: false,
      skipped: false,
    }));
  ...
  async updateOnboardingProgress(
    userId: string,
    tenantId: string,
    update: OnboardingUpdateRequest
  ): Promise<OnboardingProgress> {
   ...
   */
  async getOnboardingStats(tenantId: string): Promise<OnboardingStats> {
    const query = {
    ...
    const completedUsers = resources.filter((r: OnboardingProgress) => r.status === 'completed').length;
    const inProgressUsers = resources.filter((r: OnboardingProgress) => r.status === 'in_progress').length;
    const skippedUsers = resources.filter((r: OnboardingProgress) => r.status === 'skipped').length;

    // Calculate average completion time
    const completedProgresses = resources.filter(
      (r: OnboardingProgress) => r.status === 'completed' && r.startedAt && r.completedAt
    );
    
    const totalCompletionTime = completedProgresses.reduce((sum: number, p: OnboardingProgress) => {
    ...
    const skippedSteps = new Map<string, { title: string; count: number }>();
    resources.forEach((r: OnboardingProgress) => {
