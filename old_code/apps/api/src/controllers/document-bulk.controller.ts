...
import { BulkDocumentService, BulkUploadInput, BulkDeleteInput, BulkUpdateInput, BulkCollectionAssignInput } from '../services/bulk-document.service.js';
...
export class DocumentBulkController {
  constructor(
    private bulkService: BulkDocumentService,
  ...
  startBulkUpload = async (
    req: AuthenticatedRequest<{ Body: { items: BulkUploadInput['items'] } }>,
    reply: FastifyReply
  ) => {
    try {
  ...
  startBulkDelete = async (
    req: AuthenticatedRequest<{ Body: { documentIds: string[]; reason?: string; hardDelete?: boolean } }>,
    reply: FastifyReply
  ) => {
    try {
      const { tenantId, id: userId, email: userEmail } = req.auth || {};
  ...
  startBulkUpdate = async (
    req: AuthenticatedRequest<{ Body: { updates: BulkUpdateInput['updates'] } }>,
    reply: FastifyReply
  ) => {
    try {
      const { tenantId, id: userId, email: userEmail } = req.auth || {};
   ...
   */
  startBulkCollectionAssign = async (
    req: AuthenticatedRequest<{ Params: { collectionId: string }; Body: { documentIds: string[] } }>,
    reply: FastifyReply
  ) => {
    try {
      const { tenantId, id: userId, email: userEmail } = req.auth || {};
   ...
   */
  getJobStatus = async (
    req: AuthenticatedRequest<{ Params: { jobId: string } }>,
    reply: FastifyReply
  ) => {
    try {
   ...
   */
  cancelJob = async (
    req: AuthenticatedRequest<{ Params: { jobId: string }; Body: { reason?: string } }>,
    reply: FastifyReply
  ) => {
    try {
