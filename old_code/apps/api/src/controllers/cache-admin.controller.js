...
export class CacheAdminController {
    ...
    async getStats(request, reply) {
        ...
        try {
            ...
            this.deps.monitoring?.trackException(error, {
     ...
     * Get current warming status
     */
    async getWarmingStatus(request, reply) {
        try {
            if (!this.hasAdminAccess(request)) {
                return reply.status(403).send({
                    success: false,
    ...
    async generateReport(request, reply) {
        ...
        try {
            ...
            const report = await this.deps.cacheMonitor.generatePerformanceReport(startDate, endDate);
...
//# sourceMappingURL=cache-admin.controller.js.map