...
export class AdvancedSearchController {
     ...
     */
    async quickSearch(request, reply) {
        const auth = request.user || request.auth;
        if (!auth) {
            reply.code(401).send({ error: 'Unauthorized' });
            return;
    ...
    async listSavedSearches(request, reply) {
        ...
        const searches = await this.searchService.listSavedSearches(auth.tenantId, auth.id, { visibility, limit });
    ...
    async getSavedSearch(request, reply) {
        ...
        // Check access
        if (savedSearch.visibility === 'private' &&
    ...
    async updateSavedSearch(request, reply) {
        ...
        const updated = await this.searchService.updateSavedSearch(id, auth.tenantId, request.body);
        reply.send(updated);
    }
    /**
     * Delete saved search
     ...
     */
    async executeSavedSearch(request, reply) {
        const auth = request.user || request.auth;
        if (!auth) {
            reply.code(401).send({ error: 'Unauthorized' });
            return;
        ...
        const { id } = request.params;
        try {
            const result = await this.searchService.executeSavedSearch(id, auth.tenantId, request.body);
            reply.send(result);
        }
        catch (error) {
     ...
     */
    async getPopularTerms(request, reply) {
        const auth = request.user || request.auth;
        if (!auth) {
            reply.code(401).send({ error: 'Unauthorized' });
            return;
        ...
        const terms = await this.searchService.getPopularSearchTerms(auth.tenantId, limit || 10);
        reply.send({ terms });
    }
}
