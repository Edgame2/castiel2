...
export class AppError extends Error {
  ...
  constructor(
    ...
    // Keep isOperational for backward compatibility
...
export class UnauthorizedError extends AppError {
  constructor(message: string = 'Authentication required. Please log in to access this resource.') {
 ...
 */
export function errorHandler(monitoring: IMonitoringProvider) {
  return async (
    ...
    } else if (error && typeof error === 'object' && 'name' in error && typeof (error as { name?: unknown }).name === 'string') {
      errorMessage = (error as { name: string }).name;
    }
