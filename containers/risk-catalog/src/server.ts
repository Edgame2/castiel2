...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  await fastify.register(swagger, {
    openapi: {
      openapi: '3.0.3',
      info: {
        title: 'Risk Catalog Service API',
        description: 'Risk catalog management with global, industry, and tenant-specific risk definitions.',
        version: '1.0.0',
      },
      servers: [{ url: '/api/v1', description: 'API Version 1' }],
      tags: [
        { name: 'Risk Catalog', description: 'Risk catalog management' },
  ...
  try {
    ...
    log.info('Database connected successfully', { service: 'risk-catalog' });
  } catch (error) {
    log.error('Failed to connect to database', error, { service: 'risk-catalog' });
    throw error;
  }
  ...
  try {
    ...
    log.warn('Failed to initialize event publisher', { error, service: 'risk-catalog' });
  }
...
export async function start(): Promise<void> {
  try {
    ...
    log.info(`Risk Catalog service listening on http://${config.server.host}:${config.server.port}`, {
      port: config.server.port,
      host: config.server.host,
      service: 'risk-catalog',
    });
  } catch (error) {
    log.error('Failed to start risk catalog service', error, { service: 'risk-catalog' });
    process.exit(1);
  }
}
...
process.on('uncaughtException', (error: Error) => {
  log.error('Uncaught exception', error, { service: 'risk-catalog' });
