...
export class NotificationController {
    ...
    constructor(notificationService, realtimeService, pushNotificationService) {
    ...
    async listNotifications(request, reply) {
        ...
        try {
            const result = await this.notificationService.getUserNotifications(user.tenantId, user.id, options);
            reply.send(result);
        }
        catch (error) {
    ...
    async getNotification(request, reply) {
        ...
        try {
            const notification = await this.notificationService.getNotification(id, user.tenantId, user.id);
            if (!notification) {
    ...
    async updateNotification(request, reply) {
        ...
        try {
            const notification = await this.notificationService.updateNotificationStatus(id, user.tenantId, user.id, status);
            if (!notification) {
                throw new NotFoundError('Notification not found');
            }
            // Broadcast unread count update
            if (this.realtimeService) {
                const unreadCount = await this.notificationService.getUnreadCount(user.tenantId, user.id);
                await this.realtimeService.broadcastUnreadCount(user.tenantId, user.id, unreadCount);
            }
            reply.send(notification);
        }
        catch (error) {
    ...
    async deleteNotification(request, reply) {
        ...
        const { id } = request.params;
        try {
            const deleted = await this.notificationService.deleteNotification(id, user.tenantId, user.id);
            if (!deleted) {
                throw new NotFoundError('Notification not found');
            }
            // Broadcast unread count update
    ...
    async markAllAsRead(request, reply) {
        const user = getUser(request);
        try {
            const count = await this.notificationService.markAllAsRead(user.tenantId, user.id);
            // Broadcast unread count update
            if (this.realtimeService) {
                await this.realtimeService.broadcastUnreadCount(user.tenantId, user.id, 0);
            }
            reply.send({ count });
        }
        catch (error) {
     ...
     */
    async createAdminNotification(request, reply) {
        const user = getUser(request);
    ...
    async updatePreferences(request, reply) {
        ...
        try {
            const preferences = await this.notificationService.updatePreferences(user.tenantId, user.id, body);
            reply.send({ data: preferences });
        }
        catch (error) {
            // Re-throw AppError instances (will be handled by Fastify error handler)
     ...
     * This method handles business logic.
     */
    async getNotificationDeliveryStatus(request, reply) {
        const user = getUser(request);
        // Fastify schema validation ensures id is present and channel is valid enum (if provided)
        const params = request.params;
        const query = request.query;
        try {
            const records = await this.notificationService.getNotificationDeliveryStatus(params.id, user.tenantId, user.id, query.channel);
     ...
     * This method handles business logic.
     */
    async getDeliveryTracking(request, reply) {
        const user = getUser(request);
        ...
        try {
            const result = await this.notificationService.getDeliveryTracking(user.tenantId, user.id, options);
            reply.send(result);
        }
        catch (error) {
            // Re-throw AppError instances (will be handled by Fastify error handler)
     ...
     * This method handles business logic.
     */
    async getVapidPublicKey(request, reply) {
     ...
     */
    async subscribePushDevice(request, reply) {
        const user = getUser(request);
        ...
        try {
            ...
            const existingDeviceIndex = preferences.channels.push.devices?.findIndex((device) => {
                try {
     ...
     * This method handles business logic (device management).
     */
    async unsubscribePushDevice(request, reply) {
        const user = getUser(request);
        // Fastify schema validation ensures body structure is valid
        const body = request.body;
        ...
        const endpointToRemove = body.endpoint || body.subscription.endpoint;
        try {
            // Get current preferences
            const preferences = await this.notificationService.getPreferences(user.tenantId, user.id);
