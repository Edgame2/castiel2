...
| **dashboard** | Dashboard CRUD, widget management, organization-scoped dashboards; executive/manager/board dashboards, prioritized opportunities, portfolio drill-down (includes former dashboard-analytics). | **Storage:** Cosmos DB (dashboard_dashboards, dashboard_widgets, dashboard_admin_data, dashboard_widget_cache). **Downstream:** risk-analytics, forecasting, shard-manager when configured. **Gateway:** Routes `/api/dashboard/*`, `/api/v1/dashboards/*`, `/api/v1/portfolios/*`, etc. Used by UI for risk/pipeline/executive views. |
