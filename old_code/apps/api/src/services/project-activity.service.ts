...
import { CosmosClient, Container, Database, ConnectionPolicy, RetryOptions, SqlQuerySpec } from '@azure/cosmos';
import { IMonitoringProvider, SeverityLevel } from '@castiel/monitoring';
...
export class ProjectActivityService {
  private readonly ACTIVITY_CACHE_TTL = 300; // 5 minutes for recent activities
  private readonly STATS_CACHE_TTL = 3600; // 1 hour for statistics
  private client: CosmosClient;
  private database: Database;
  private activitiesContainer: Container;
  private monitoring: IMonitoringProvider;
  private shardRepository: ShardRepository;
  private redis?: Redis;
  private cleanupIntervalId?: NodeJS.Timeout; // Track cleanup interval for shutdown

  // Default retention policy: 90 days (7776000 seconds)
  private readonly DEFAULT_RETENTION_POLICY: ActivityRetentionPolicy = {
   ...
   */
  async logActivity(tenantId: string, input: CreateActivityInput): Promise<ProjectActivity> {
    try {
      // Validate input
      ...
      this.monitoring.trackException(error as Error, { operation: 'project-activity.logActivity' });
      throw error;
    }
  }
   ...
   */
  async logActivitiesBulk(tenantId: string, input: BulkActivityInput): Promise<ProjectActivity[]> {
    try {
      ...
      this.monitoring.trackException(error as Error, { operation: 'project-activity.logActivitiesBulk' });
      throw error;
    }
  }
  ...
  async getActivities(
    ...
    try {
      ...
      const { resources: totalActivities } = await this.activitiesContainer.items
        .query<ProjectActivity>(countQuery)
        .fetchAll();
      ...
      const { resources: activities } = await this.activitiesContainer.items
        .query<ProjectActivity>(paginatedQuery)
        .fetchAll();
      ...
      this.monitoring.trackException(error as Error, { operation: 'project-activity.getActivities' });
      throw error;
    }
  }
  ...
  async getRecentActivities(
    ...
    try {
      ...
      const summaries: ActivitySummary[] = page.items.map((activity) => ({
        id: activity.id,
        type: activity.type,
      ...
      this.monitoring.trackException(error as Error, { operation: 'project-activity.getRecentActivities' });
      return [];
    }
  }
  ...
  async getStatistics(
    ...
    try {
      ...
      if (this.redis) {
        const cached = await this.redis.get(cacheKey);
        if (cached) {
          return JSON.parse(cached) as ActivityStatistics;
        }
      }
      ...
      const byType: Record<ProjectActivityType, number> = {} as any;
      const bySeverity: Record<ActivitySeverity, number> = {} as any;
      const topActorsMap = new Map<string, { userId: string; displayName: string; count: number }>();
      const topAffectedMap = new Map<string, any>();
      const dailyTrend = new Map<string, number>();

      // Initialize counts
      Object.values(ProjectActivityType).forEach((type) => {
        byType[type as ProjectActivityType] = 0;
      });

      Object.values(ActivitySeverity).forEach((sev) => {
        bySeverity[sev as ActivitySeverity] = 0;
      });
      ...
      this.monitoring.trackException(error as Error, { operation: 'project-activity.getStatistics' });
      throw error;
    }
  }
  ...
  async exportActivities(
    ...
    try {
      ...
      this.monitoring.trackException(error as Error, { operation: 'project-activity.exportActivities' });
      throw error;
    }
  }
   ...
   */
  async cleanupOldActivities(tenantId?: string): Promise<number> {
    try {
      const now = Date.now();
      let deletedCount = 0;
      ...
      const { resources: expiredActivities } = await this.activitiesContainer.items
        .query<ProjectActivity>(querySpec)
        .fetchAll();
      ...
      for (const activity of expiredActivities) {
        try {
          ...
          this.monitoring.trackTrace(
            `Failed to delete activity ${activity.id}: ${(error as Error).message}`,
            SeverityLevel.Warning,
            { operation: 'project-activity.cleanupOldActivities', activityId: activity.id },
          );
        }
      }
      ...
      if (deletedCount > 0) {
        this.monitoring.trackTrace(
          `Cleaned up ${deletedCount} expired activities${tenantId ? ` for tenant ${tenantId}` : ''}`,
          SeverityLevel.Information,
          { operation: 'project-activity.cleanupOldActivities', deletedCount, tenantId },
        );
      }

      return deletedCount;
    } catch (error) {
      this.monitoring.trackException(error as Error, { operation: 'project-activity.cleanupOldActivities' });
      return 0;
    }
  }
   ...
   */
  private calculateTTL(type: ProjectActivityType, severity: ActivitySeverity): number {
   ...
   */
  private extractRelatedResource(activity: ProjectActivity): ActivitySummary['relatedResource'] | undefined {
    if (activity.details?.shardId) {
      return {
        type: 'shard',
   ...
   */
  private exportAsCSV(activities: ProjectActivity[]): string {
    const headers = [
      'ID',
      'Timestamp',
      'Type',
   ...
   */
  private exportAsPDF(activities: ProjectActivity[]): string {
  ...
  private scheduleCleanup(): void {
    ...
    this.cleanupIntervalId = setInterval(() => {
      this.cleanupOldActivities().catch((error) => {
        this.monitoring.trackException(error as Error, { operation: 'project-activity.scheduleCleanup' });
      });
    }, CLEANUP_INTERVAL);
