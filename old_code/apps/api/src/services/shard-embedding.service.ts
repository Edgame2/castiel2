...
import type { VectorEmbedding } from '../types/shard.types.js';
...
export class ShardEmbeddingService {
  constructor(
    ...
    private embeddingCache?: EmbeddingContentHashCacheService
  ) {}
  ...
  async generateEmbeddingsForShard(
    ...
    try {
      ...
      if (parentCfg && parentCfg.mode === 'always' && shard.parentShardId) {
        try {
          ...
          if (parent) {
            ...
            const parts: string[] = [];
            const sd: any = parent.structuredData || {};
            const name = sd.name || sd.title || parent.shardTypeName || '';
            if (fields.includes('name') && name) {parts.push(`Project: ${String(name)}`);}
      ...
      const contentHashes = limitedTexts.map(text => 
        this.embeddingCache?.calculateContentHash(text, template.id) || 
        this.calculateContentHash(text, template.id)
      );

      // 6.2. Check cache for existing embeddings
      let rawEmbeddings: number[][];
      const textsToGenerate: { text: string; index: number; hash: string }[] = [];
      ...
      if (this.embeddingCache) {
        ...
        rawEmbeddings = Array.from({ length: limitedTexts.length }, (_, i) => 
      ...
      const vectors: VectorEmbedding[] = normalizedEmbeddings.map((embedding, index) => ({
        id: uuidv4(),
   ...
   */
  async getEmbeddingStatus(shardId: string, tenantId: string): Promise<{
    hasEmbeddings: boolean;
    vectorCount: number;
    latestVectorDate?: Date;
    oldestVectorDate?: Date;
    isRecent: boolean; // Less than 7 days old
    model?: string;
    dimensions?: number;
  }> {
    try {
      ...
      const dimensions = firstVector.embedding?.length;
   ...
   */
  async getEmbeddingHistory(shardId: string, tenantId: string, limit: number = 20): Promise<Array<{
    jobId: string;
    status: string;
    createdAt: Date;
    completedAt?: Date;
   ...
   */
  async validateEmbeddingQuality(shardId: string, tenantId: string): Promise<{
    isValid: boolean;
    issues: string[];
    metrics: {
      vectorCount: number;
    ...
    try {
      ...
      const metrics = {
        ...
        isNormalized: false,
        hasValidModel: false,
        averageMagnitude: null as number | null,
      };
      ...
      const dimensions = shard.vectors![0].embedding?.length;
      ...
      if (!dimensions || dimensions === 0) {
        ...
        const inconsistentDimensions = shard.vectors!.some(v => v.embedding?.length !== dimensions);
   ...
   */
  async regenerateEmbeddingsForShard(
    shardId: string,
    tenantId: string
  ): Promise<EmbeddingGenerationResult> {
    try {
   ...
   */
  private calculateContentHash(extractedText: string, templateId: string): string {
   ...
   */
  private hasMatchingContentHash(shard: Shard, contentHash: string): boolean {
    // Check if metadata contains the content hash
    const storedHash = shard.metadata?.customFields?.embeddingContentHash as string | undefined;
      ...
      for (let i = 0; i < shardsToProcess.length; i += batchSize) {
        ...
        const results = await Promise.allSettled(
          batch.map(async (shard) => {
            const result = await this.generateEmbeddingsForShard(
              shard, 
              tenantId,
    ...
    try {
      ...
      for (let i = 0; i < shardIds.length; i += batchSize) {
        ...
        const shardResults = await Promise.allSettled(
          batch.map(id => this.shardRepository.findById(id, tenantId))
        );
   ...
   */
  async getEmbeddingStats(tenantId: string): Promise<{
    totalShards: number;
