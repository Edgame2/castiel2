...
export class SSOConfigController {
     ...
     */
    async activateConfig(request, reply) {
        try {
            const user = request.user;
            if (!user) {
                return reply.code(401).send({
     ...
     */
    async deactivateConfig(request, reply) {
        try {
            const user = request.user;
            if (!user) {
                return reply.code(401).send({
     ...
     */
    async validateConfig(request, reply) {
        try {
            const user = request.user;
            if (!user) {
                return reply.code(401).send({
     ...
     * Get test SSO URL
     */
    async testConfig(request, reply) {
        try {
            const user = request.user;
            if (!user) {
                return reply.code(401).send({
