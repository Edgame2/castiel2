...
export class RevisionsController {
  constructor(
    ...
    private readonly shardCacheService: ShardCacheService | null,
  ...
  async getRevision(
    request: FastifyRequest<{
      Params: { shardId: string; revisionNumber: string };
    }>,
    reply: FastifyReply
  ): Promise<void> {
    const startTime = Date.now();
    ...
    try {
      ...
      // Get revision
      const revision = await this.revisionRepository.findByRevisionNumber(
        shardId,
  ...
  async getLatestRevision(
    ...
    try {
      ...
      // Get latest revision
      const revision = await this.revisionRepository.getLatestRevision(
        shardId,
        authRequest.user.tenantId
      );
  ...
  async compareRevisions(
    ...
    try {
      ...
      const fromRevision = await this.revisionRepository.findByRevisionNumber(
        shardId,
        authRequest.user.tenantId,
        fromRevisionNumber
      );

      const toRevision = await this.revisionRepository.findByRevisionNumber(
        shardId,
        authRequest.user.tenantId,
        toRevisionNumber
      );
      ...
      const comparison = await this.revisionRepository.compareRevisions(
        fromRevision.id,
        toRevision.id,
        authRequest.user.tenantId
      );
  ...
  async revertToRevision(
    ...
    try {
      ...
      const targetRevision = await this.revisionRepository.findByRevisionNumber(
        shardId,
        authRequest.user.tenantId,
        parsedRevisionNumber
      );
      ...
      // Get current shard
      const currentShard = await this.shardRepository.findById(
        shardId,
        authRequest.user.tenantId
      );
