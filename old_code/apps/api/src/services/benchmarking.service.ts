...
export class BenchmarkingService {
  ...
  async calculateWinRates(
    ...
    try {
      ...
      const opportunities = await this.getClosedOpportunities(
        tenantId,
        shardType.id,
        startDate,
        endDate,
   ...
   */
  async calculateDealSizeDistribution(
    tenantId: string,
    options?: {
      industryId?: string;
      startDate?: Date;
   ...
   */
  async estimateRenewal(
    contractId: string,
    tenantId: string
  ): Promise<RenewalEstimate> {
    const startTime = Date.now();
    ...
    try {
      ...
      const contractData = contract.structuredData as any;

      // Get related opportunities (renewal opportunities)
      const relatedOpportunities = await this.relationshipService.getRelatedShards(
        tenantId,
        contractId,
        'both',
        {
      ...
      if (relatedOpportunities.length > 0) {
        const activeRenewalOpps = relatedOpportunities.filter(rel => {
          const oppData = rel.shard.structuredData as any;
   ...
   */
  private async getClosedOpportunities(
    tenantId: string,
    shardTypeId: string,
    startDate: Date,
    endDate: Date,
    industryId?: string,
    stageFilter?: 'closed_won' | 'closed_lost'
  ): Promise<Shard[]> {
