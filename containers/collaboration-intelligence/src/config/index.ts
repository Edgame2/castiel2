/**
 * Configuration loader for collaboration_intelligence module
 */
...
export interface CollaborationIntelligenceConfig {
...
let cachedConfig: CollaborationIntelligenceConfig | null = null;
...
export function loadConfig(): CollaborationIntelligenceConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as CollaborationIntelligenceConfig;
  let envConfig: Partial<CollaborationIntelligenceConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<CollaborationIntelligenceConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as CollaborationIntelligenceConfig;
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'collaboration_intelligence' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'collaboration_intelligence' });
  }
...
export function getConfig(): CollaborationIntelligenceConfig {
