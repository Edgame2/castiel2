...
import {
  CreateWidgetCatalogEntrySchema,
  UpdateWidgetCatalogEntrySchema,
  UpdateTenantWidgetAccessSchema,
  UpdateTenantWidgetConfigSchema,
  ListWidgetCatalogSchema,
} from '../schemas/widget-catalog.schemas.js';

export async function widgetCatalogRoutes(fastify: FastifyInstance): Promise<void> {
   ...
   */
    fastify.get(
      '/admin/tenants/:tenantId/widget-access',
    {
   ...
   */
    fastify.patch(
      '/admin/tenants/:tenantId/widget-access/:widgetId',
    {
      preHandler: [requireAuth, requireRole("admin", "owner", "global_admin"), requireSameTenant()],
      schema: {
        tags: ['admin', 'widget-access'],
    ...
    async (request, reply) => {
      try {
        const { tenantId, widgetId } = request.params as {
          tenantId: string;
          widgetId: string;
        };
    ...
    async (request, reply) => {
      try {
        ...
        const result = await service.getUserWidgetCatalog(tenantId, userRoles, {
          category: (request.query as any).category,
