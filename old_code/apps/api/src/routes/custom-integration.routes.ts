...
import { CustomIntegrationDefinition, CustomIntegrationStructuredData } from '../types/custom-integration.types.js';
...
interface CreateCustomIntegrationBody {
  name: string;
  displayName: string;
  description?: string;
...
interface UpdateCustomIntegrationBody extends Partial<CreateCustomIntegrationBody> {
...
export async function customIntegrationRoutes(
  ...
  const encryptionService = new CredentialEncryptionService(
    process.env.CREDENTIAL_ENCRYPTION_KEY || 'default-key-for-dev-only-change-in-prod'
  );
  
  const customIntegrationService = new CustomIntegrationService(
    shardRepository,
    shardTypeRepository,
    encryptionService,
    monitoring
  ...
  fastify.get<{
    ...
    async (request, reply) => {
      const { tenantId } = request.user as AuthUser;
      ...
      try {
        ...
        const customIntegrationType = shardTypes.shardTypes.find(
          (st) => st.name === 'c_customIntegration'
        );
        ...
        // Apply filters
        if (status) {
          integrations = integrations.filter((i: any) => i.status === status);
        }
        if (type) {
          integrations = integrations.filter((i: any) => i.integrationType === type);
        }
   ...
   */
  fastify.post<{ Body: CreateCustomIntegrationBody }>(
    '/custom-integrations',
    {
      schema: {
    ...
    async (request, reply) => {
      const { tenantId, id: userId } = request.user as AuthUser;
      ...
      try {
        ...
        const shardTypes = await shardTypeRepository.list({ filter: { tenantId } });
        let customIntegrationType = shardTypes.shardTypes.find(
          (st) => st.name === 'c_customIntegration'
        );
        ...
        const structuredData: CustomIntegrationStructuredData = {
          ...
          endpoints: (body.endpoints || []) as any,
        ...
        if (body.integrationType === 'webhook' && webhookConfig) {
          webhookUrl = customIntegrationService.generateWebhookUrl(
            shard.id,
            (webhookConfig as any).webhookSecret
          );
        }
   ...
   */
  fastify.patch<{ Params: { id: string }; Body: UpdateCustomIntegrationBody }>(
    '/custom-integrations/:id',
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const currentData = shard.structuredData as CustomIntegrationStructuredData;

        // Encrypt auth config if updated
        let authConfig = currentData.authConfig;
        if (updates.authConfig) {
          authConfig = (await encryptAuthConfig(updates.authConfig, encryptionService)) as any;
        }

        const updatedData: CustomIntegrationStructuredData = {
          ...currentData,
          ...
          authConfig,
          endpoints: (updates.endpoints ?? currentData.endpoints) as any,
          webhookConfig: (updates.webhookConfig ?? currentData.webhookConfig) as any,
  ...
  fastify.post<{
    ...
    '/custom-integrations/:id/test/:endpointId',
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const integration = shardToIntegration(shard);
        const result = await customIntegrationService.testEndpoint(
          integration,
          endpointId,
          params,
          body
  ...
  fastify.post<{
    ...
    '/custom-integrations/:id/execute/:endpointId',
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const result = await customIntegrationService.executeEndpoint(
          integration,
          endpointId,
          params,
          body
   ...
   */
  fastify.get<{ Params: { id: string } }>(
    '/custom-integrations/:id/webhook-url',
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const data = shard.structuredData as CustomIntegrationStructuredData;
        if (!data.webhookConfig) {
          return reply.status(400).send({ error: 'Integration does not have webhook configured' });
        }

        const webhookUrl = customIntegrationService.generateWebhookUrl(
          id,
          (data.webhookConfig as any).webhookSecret
        );
   ...
   */
  fastify.post<{ Params: { id: string } }>(
    '/custom-integrations/:id/regenerate-secret',
    {
      schema: {
...
export async function customIntegrationWebhookRoutes(
  fastify: FastifyInstance,
  options: {
    monitoring: IMonitoringProvider;
    shardRepository: ShardRepository;
   ...
   */
  fastify.post<{
    Params: { integrationId: string; secret: string };
    Body: unknown;
  }>(
    '/webhooks/custom/:integrationId/:secret',
    {
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        // Verify secret
        if ((data.webhookConfig as any).webhookSecret !== secret) {
        ...
        const signature = signatureHeader ? request.headers[signatureHeader.toLowerCase()] as string : undefined;

        const integration = shardToIntegration(shard);
        const result = await customIntegrationService.processWebhook(
          integration,
          request.body,
          signature
        );
        ...
        const responseStatus = (data.webhookConfig as any).responseStatus || 200;
        const responseBody = (data.webhookConfig as any).responseBody || { status: 'received' };
...
async function encryptAuthConfig(
  ...
  if (encrypted.headers && typeof encrypted.headers === 'object') {
    ...
    for (const [key, value] of Object.entries(encrypted.headers as Record<string, string>)) {
...
function shardToIntegration(shard: any): CustomIntegrationDefinition {
  const data = shard.structuredData as CustomIntegrationStructuredData;
  return {
    id: shard.id,
