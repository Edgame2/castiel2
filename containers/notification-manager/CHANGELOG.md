...
### Added
- **anomaly.detected consumer (Plan §7.2):** `eventMapper` maps `anomaly.detected` (from risk-analytics) to NotificationInput. Uses `tenantId` as organizationId, `ownerId` (or `userId`) as recipientId; skips when recipientId is missing. High severity → IN_APP+EMAIL; medium/low → IN_APP. Payload should include `ownerId` (opportunity OwnerId) for notifications to be created; risk-analytics to add when publishing. README: BI/risk event mapping.
