...
import { BaseDocumentRewriter, DocumentAuthToken } from '../rewriters/base-rewriter.js';
import {
  InsightService,
...
import type { NotificationService } from '@castiel/api-core';
...
import { DocumentTemplate, PlaceholderConfiguration } from '../types/template.types.js';
import { InsightRequest } from '../../../types/ai-insights.types.js';
import { CreateShardInput, ShardStatus, ShardSource, SyncStatus, SyncDirection } from '../../../types/shard.types.js';
import { DocumentStructuredData, StorageProvider, VisibilityLevel } from '../../../types/document.types.js';
...
export class GenerationProcessorService {
  ...
  constructor(
    ...
    private rewriterFactory: DocumentRewriterFactory,
   ...
   */
  async processJob(
    job: GenerationJob & { template?: DocumentTemplate; userToken?: string }
  ): Promise<GenerationResult> {
    const startTime = Date.now();
    let generatedDocumentId: string | undefined;
    let generatedDocumentUrl: string | undefined;
    let shardId: string | undefined;
    const errors: Array<{ placeholderName: string; error: string }> = [];
    ...
    try {
      ...
      const template = job.template || await this.templateService.getTemplate(
        job.templateId,
        job.tenantId
      );
      ...
      if (template.status !== 'active') {
        ...
        // Create error with proper code for classification
        const error = new Error(errorMessage);
        (error as any).code = 'TEMPLATE_NOT_ACTIVE';
        throw error;
      }
      ...
      if (!template.sourceDocumentId || template.sourceDocumentId.trim().length === 0) {
        ...
        const error = new Error(errorMessage);
        (error as any).code = 'TEMPLATE_INVALID';
        throw error;
      }
      ...
      const validFormats: string[] = ['google_docs', 'google_slides', 'word', 'powerpoint'];
      ...
      if (isGoogleTemplate && job.destinationProvider !== 'google') {
        ...
        const error = new Error(errorMessage);
        (error as any).code = 'PROVIDER_MISMATCH';
        throw error;
      }
      ...
      generatedValues = {};
      const generatedCharts: Record<string, Buffer> = {};
      ...
      for (const placeholder of template.placeholders || []) {
        ...
        try {
          ...
          if (placeholder.type === 'chart' || config.typeOverride === 'chart') {
            const chartImage = await this.chartGenerationService.generateChart(
              placeholder,
              config,
              template,
              job.context
          ...
          const generated = await this.generatePlaceholderContent(
            job.tenantId,
            job.userId,
            placeholder,
            config,
      ...
      if (successfulPlaceholders === 0 && attemptedPlaceholders > 0) {
        ...
        const error = new Error(errorMessage);
        (error as any).code = 'ALL_PLACEHOLDERS_FAILED';
        throw error;
      }
      ...
      let duplicateResult: { documentId: string; url: string } | undefined;
      
      try {
        duplicateResult = await rewriter.duplicateDocument(
        ...
        if (!duplicateResult) {
          ...
          const error = new Error(errorMessage);
          (error as any).code = 'DUPLICATION_FAILED';
          throw error;
        }
        ...
        if (duplicateError instanceof Error && (duplicateError as any).code) {
          throw duplicateError;
        }
        const error = new Error(`Failed to duplicate document: ${(duplicateError as Error).message}`);
        (error as any).code = 'DUPLICATION_FAILED';
        throw error;
      }
      ...
      const validGeneratedValues: Record<string, string> = {};
      ...
      if (Object.keys(validGeneratedValues).length > 0) {
        await rewriter.replacePlaceholders(
          generatedDocumentId,
          template,
      ...
      if (!generatedDocumentId || generatedDocumentId.trim().length === 0) {
        ...
        const error = new Error(errorMessage);
        (error as any).code = 'DOCUMENT_CREATION_FAILED';
        throw error;
      }
      ...
      try {
        folderPath = await rewriter.getFolderPath(
          job.destinationFolderId,
          authToken
        );
        ...
        this.monitoring.trackException(folderPathError as Error, {
      ...
      let shardCreationError: Error | null = null;
      try {
        ...
        shardCreationError = shardError as Error;
      ...
      if (job.options?.notifyOnComplete && this.notificationService) {
        try {
          ...
          this.monitoring.trackException(notificationError as Error, {
      ...
      try {
        ...
        await this.jobRepository.update(job.id, job.tenantId, {
          ...
          error: {
            ...
            code: (error as any)?.code || this.getErrorCode(error),
  ...
  private async generatePlaceholderContent(
    tenantId: string,
    userId: string,
    placeholder: { name: string; type: string },
    config: any,
    template: DocumentTemplate,
    context?: { projectId?: string; variables?: Record<string, string> }
  ): Promise<{ value: string; tokensUsed?: number; model?: string }> {
    ...
    let generatedValue = (insightResponse as any).content || (insightResponse as any).result || '';
    ...
    if (!generatedValue && config.isRequired) {
      throw new Error(
        `Required placeholder "${placeholder.name}" generated empty content. ` +
    ...
    if (constraints) {
      ...
      if (constraints.pattern && content.length > 0) {
        try {
          ...
          this.monitoring.trackException(regexError as Error, {
  ...
  private parseAuthToken(
    ...
    if (!userToken || userToken.trim().length === 0) {
      ...
      (error as any).code = 'OAUTH_TOKEN_MISSING';
    ...
    try {
      ...
      if (decryptError.code) {
        this.monitoring.trackException(decryptError as Error, {
      ...
      const error = new Error(`Failed to decrypt user token: ${(decryptError as Error).message}`);
      (error as any).code = 'OAUTH_TOKEN_DECRYPTION_FAILED';
      throw error;
    }
