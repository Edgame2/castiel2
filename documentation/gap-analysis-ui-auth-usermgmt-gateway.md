...
- **API base URL:** From `NEXT_PUBLIC_API_BASE_URL`; default in config `http://localhost:3001` is env override only (acceptable for dev).
...
- README says "API Gateway running on port 3001" — prefer "API Gateway URL configurable via NEXT_PUBLIC_API_BASE_URL".
