/**
 * Configuration loader for security-scanning module
 */
...
import { SecurityScanningConfig } from '../types/config.types.js';

let cachedConfig: SecurityScanningConfig | null = null;
...
export function loadConfig(): SecurityScanningConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as SecurityScanningConfig;
  let envConfig: Partial<SecurityScanningConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<SecurityScanningConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as SecurityScanningConfig;
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'security-scanning' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'security-scanning' });
  }
...
export function getConfig(): SecurityScanningConfig {
