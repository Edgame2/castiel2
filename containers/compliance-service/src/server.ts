...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  await fastify.register(swagger, {
    openapi: {
      ...
      tags: [
        { name: 'Compliance Checks', description: 'Compliance check management' },
        { name: 'Compliance Policies', description: 'Compliance policy management' },
...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Compliance Service listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
