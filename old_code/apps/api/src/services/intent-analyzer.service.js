...
export class IntentAnalyzerService {
    ...
    aiModelSelection;
    unifiedAIClient;
    constructor(monitoring, shardRepository, shardTypeRepository, redis, aiModelSelection, unifiedAIClient) {
     ...
     */
    async analyze(query, tenantId, context = {}) {
        ...
        try {
            ...
            const historyStrings = context.conversationHistory
                ? context.conversationHistory.map(h => typeof h === 'string' ? h : (h).content)
                : [];
     ...
     */
    async classifyIntentWithLLM(query, tenantId, conversationHistory) {
        ...
        try {
            ...
            const messages = [
                ...
                { role: 'user', content: `Classify this query:${historyText}\n\nQuery: "${query}"\n\nRespond with JSON only.` },
            ];
            // Use low temperature for consistent classification (zero-shot)
            // Request JSON format for structured output
            const response = await this.unifiedAIClient.chat(conn.connection, conn.apiKey, {
                messages,
     ...
     */
    async detectMultiIntent(query, tenantId, conversationHistory) {
        if (!this.aiModelSelection || !this.unifiedAIClient) {
    ...
    parseMultiIntentOutput(text) {
        try {
            ...
            if (!allowed.includes(rawPrimaryType)) {
                return null;
            }
     ...
     */
    classifyInsightType(query, conversationHistory) {
        const scores = {
            summary: 0,
            analysis: 0,
            comparison: 0,
                ...
                for (const [type, patterns] of Object.entries(INTENT_PATTERNS)) {
                    for (const pattern of patterns) {
                        if (pattern.test(message)) {
                            scores[type] += 0.3; // Smaller boost from history
                        }
                    }
                }
            }
     ...
     */
    async extractEntities(query, tenantId) {
        const entities = [];
     ...
     */
    async resolveEntityReferences(entities, tenantId) {
        const resolved = [];
        for (const entity of entities) {
            if (entity.type === 'shard' && !entity.shardId) {
                ...
                if (searchResult.shards.length > 0) {
                    ...
                    const exactMatch = shardsWithNames.find(s => s.name.toLowerCase() === entity.value.toLowerCase());
     ...
     */
    determineScope(entities, currentScope) {
        const scope = {
            ...currentScope,
        };
     ...
     */
    estimateComplexity(type, scope, entities) {
     ...
     */
    isFollowUp(query, conversationHistory) {
        if (!conversationHistory || conversationHistory.length === 0) {
            return false;
        }
     ...
     */
    async resolveFollowUpReferences(query, conversationMessages, tenantId) {
        if (conversationMessages.length === 0) {
            return query;
        }
...
Rewritten Query (standalone, with all references resolved):`;
                    const response = await this.unifiedAIClient.chat(selectionResult.connection.connection, selectionResult.connection.apiKey, {
                        messages: [
     ...
     */
    mergeFollowUpContext(currentQuery, previousScope, previousType) {
...
//# sourceMappingURL=intent-analyzer.service.js.map