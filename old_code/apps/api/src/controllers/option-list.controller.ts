...
import {
  ...
  OptionListError,
  OptionListErrorCode,
} from '../services/option-list.service.js';
...
export class OptionListController {
  ...
  createOptionList = async (
    req: FastifyRequest<{ Body: CreateOptionListBody }>,
    reply: FastifyReply
  ): Promise<void> => {
  ...
  listOptionLists = async (
    req: FastifyRequest<{ Querystring: ListQueryParams }>,
    reply: FastifyReply
  ): Promise<void> => {
    const startTime = Date.now();
    ...
    try {
      ...
      const result = await this.service.list({
        ...
        orderBy: orderBy as 'name' | 'displayName' | 'createdAt' | 'updatedAt' | undefined,
   ...
   */
  getOptions = async (
    req: FastifyRequest<{ Params: NameParams; Querystring: { scope?: string } }>,
    reply: FastifyReply
  ): Promise<void> => {
    const startTime = Date.now();
  ...
  updateOptionList = async (
    req: FastifyRequest<{ Params: IdParams; Body: UpdateOptionListBody }>,
    reply: FastifyReply
  ): Promise<void> => {
    const startTime = Date.now();
   ...
   */
  addOptions = async (
    req: FastifyRequest<{ Params: IdParams; Body: AddOptionsBody }>,
    reply: FastifyReply
  ): Promise<void> => {
    const startTime = Date.now();
   ...
   */
  removeOptions = async (
    req: FastifyRequest<{ Params: IdParams; Body: RemoveOptionsBody }>,
    reply: FastifyReply
  ): Promise<void> => {
    const startTime = Date.now();
   ...
   */
  createTenantOverride = async (
    req: FastifyRequest<{ Params: NameParams; Body: CreateOverrideBody }>,
    reply: FastifyReply
  ): Promise<void> => {
    const startTime = Date.now();
    ...
    try {
      ...
      const override = await this.service.createTenantOverride(
        name,
        tenantId,
        options,
        userId
   ...
   */
  private handleError(error: unknown, reply: FastifyReply, startTime: number): void {
    const duration = Date.now() - startTime;
