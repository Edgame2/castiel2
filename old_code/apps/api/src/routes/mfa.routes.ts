...
export async function registerMFARoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post('/auth/mfa/enroll/totp', { schema: enrollTOTPSchema }, (request, reply) =>
    mfaController.enrollTOTP(request, reply)
  );

  server.post('/auth/mfa/verify/totp', { schema: verifyTOTPSchema }, (request, reply) =>
    mfaController.verifyTOTP(request, reply)
  );

  server.post('/auth/mfa/enroll/sms', { schema: enrollSMSSchema }, (request, reply) =>
    mfaController.enrollSMS(request, reply)
  );

  server.post('/auth/mfa/verify/sms', { schema: verifySMSSchema }, (request, reply) =>
    mfaController.verifySMS(request, reply)
  );

  server.post('/auth/mfa/enroll/email', { schema: enrollEmailSchema }, (request, reply) =>
    mfaController.enrollEmail(request, reply)
  );

  server.post('/auth/mfa/verify/email', { schema: verifyEmailSchema }, (request, reply) =>
    mfaController.verifyEmail(request, reply)
  );

  server.post('/auth/mfa/challenge', { schema: mfaChallengeSchema }, (request, reply) =>
  ...
  server.post('/auth/mfa/send-code', { schema: sendMFACodeSchema }, (request, reply) =>
    mfaController.sendMFACode(request, reply)
  );

  server.get('/auth/mfa/methods', { schema: listMFAMethodsSchema }, (request, reply) =>
    mfaController.listMFAMethods(request, reply)
  );

  server.post('/auth/mfa/disable/:method', { schema: disableMFAMethodSchema }, (request, reply) =>
