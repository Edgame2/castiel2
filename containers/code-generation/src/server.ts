  ...
  await fastify.register(swagger, {
    openapi: {
      ...
      info: {
        ...
        description: 'Specialized code generation tasks',
        version: '1.0.0',
      },
      servers: [{ url: '/api/v1', description: 'API Version 1' }],
      tags: [
        { name: 'Code Generation', description: 'Code generation job management' },
        { name: 'Templates', description: 'Generation template management' },
...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Code Generation listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
