...
export class AIRecommendationService {
     ...
     */
    registerHandlers() {
        const schemaHandler = new SchemaRecommendationHandler(this.azureOpenAI, this.promptResolver, this.promptRenderer, this.monitoring);
        const embeddingTemplateHandler = new EmbeddingTemplateRecommendationHandler(this.azureOpenAI, this.promptResolver, this.promptRenderer, this.monitoring);
     ...
     */
    shouldAutoApply(response, optionIndex = 0) {
        ...
        return handler.shouldAutoApply(option, context);
    }
    /**
     ...
     */
    getRateLimitStatus(tenantId, userId) {
     ...
     */
    getCostTracking(tenantId, startDate, endDate) {
     ...
     * Check rate limits
     */
    async checkRateLimits(tenantId, userId) {
        ...
        if (userState) {
            if (userState.resetAt < new Date()) {
                ...
                throw new Error(`Rate limit exceeded for user. Resets at ${userState.resetAt.toISOString()}`);
            }
        }
        ...
        if (tenantState) {
            if (tenantState.resetAt < new Date()) {
                ...
                throw new Error(`Rate limit exceeded for tenant. Resets at ${tenantState.resetAt.toISOString()}`);
            }
        }
        // Increment counters
     ...
     */
    incrementRateLimit(key, tenantId, userId) {
