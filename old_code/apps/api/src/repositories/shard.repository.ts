...
import { trackQueryPerformance, trackQueryError } from '../utils/query-performance-tracker.js';
...
export class ShardRepository {
  ...
  constructor(
    ...
    const connectionPolicy: ConnectionPolicy = {
      connectionMode: 'Direct' as any, // Best performance - Direct mode uses TCP connections
    ...
    try {
      ...
      const { resources, continuationToken: newContinuationToken } = await this.container.items
        .query(querySpec, {
          maxItemCount: limit,
          continuationToken,
          populateQueryMetrics: true, // Enable query metrics for RU tracking
        })
        .fetchNext();

      // Extract request charge if available
      const requestCharge = (resources as any).requestCharge || undefined;
      ...
      trackQueryPerformance(
        this.monitoring,
        {
          ...
          query,
          parameters,
          partitionKey: options.filter?.tenantId,
      ...
      // Track query error with context
      trackQueryError(
        this.monitoring,
        {
          operation: 'shard.repository.list',
