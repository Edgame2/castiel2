/**
 * Unit tests for ForecastingEventPublisher
 */

import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import * as config from '../../../src/config';
import * as publisherModule from '../../../src/events/publishers/ForecastingEventPublisher';
...
describe('ForecastingEventPublisher', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
  ...
  describe('closeEventPublisher', () => {
    it('clears publisher so subsequent publish is skipped', async () => {
      ...
      await publisherModule.publishForecastEvent('forecast.generated', 'tenant-1', {});

      expect(log.warn).toHaveBeenCalledWith(
        'Event publisher not initialized, skipping event',
        expect.objectContaining({ eventType: 'forecast.generated', service: 'forecasting' })
      );
    });
  });

  describe('publishForecastEvent', () => {
    it('logs warn and returns when publisher is not initialized', async () => {
      await publisherModule.publishForecastEvent('forecast.generated', 'tenant-1', { opportunityId: 'o1' });
    ...
    it('calls publisher.publish when publisher is initialized', async () => {
      ...
      await publisherModule.publishForecastEvent(
        'forecast.generated',
        'tenant-1',
        { opportunityId: 'o1', forecastId: 'f1' },
