...
import { DocumentTemplate, DocumentFormat } from '../types/template.types.js';
...
export class DocumentGenerationService {
  ...
  private config: ReturnType<typeof getContentGenerationConfig>;

  constructor(
    private monitoring: IMonitoringProvider,
    private templateService: DocumentTemplateService,
    private serviceBus: QueueService,
    redis?: Redis | null
  ) {
  ...
  async generateDocument(
    ...
    userToken?: string // OAuth token for accessing user's Drive/OneDrive
    ...
    try {
      ...
      if (template.status !== 'active') {
        throw new AppError(
          'Template is not active. Only active templates can be used for generation.',
          400,
          'TEMPLATE_NOT_ACTIVE'
        );
      }
      ...
      if (isGoogleTemplate && request.destinationProvider !== 'google') {
        throw new AppError(
          `Template format "${template.documentFormat}" requires Google Drive as destination. ` +
          `Please specify a Google Drive folder.`,
          400,
          'PROVIDER_MISMATCH',
          {
      ...
      if (isMicrosoftTemplate && request.destinationProvider !== 'microsoft') {
        throw new AppError(
          `Template format "${template.documentFormat}" requires OneDrive as destination. ` +
          `Please specify a OneDrive folder.`,
          400,
          'PROVIDER_MISMATCH',
          {
            templateFormat: template.documentFormat,
      ...
      if (placeholderCount > this.config.maxPlaceholdersPerTemplate) {
        throw new AppError(
          `Template has ${placeholderCount} placeholders, which exceeds the maximum of ${this.config.maxPlaceholdersPerTemplate}`,
          400,
          'VALIDATION_ERROR',
          {
            placeholderCount,
      ...
      if (request.options?.skipPlaceholders) {
        const validPlaceholderNames = new Set(
        ...
        if (duplicates.length > 0) {
          throw new AppError(
            `Duplicate placeholder names in skipPlaceholders: ${duplicates.join(', ')}. ` +
            `Each placeholder can only be skipped once.`,
            400,
            'VALIDATION_ERROR',
            {
              duplicates,
        ...
        const invalidSkipPlaceholders = request.options.skipPlaceholders.filter(
          name => !validPlaceholderNames.has(name)
        );

        if (invalidSkipPlaceholders.length > 0) {
          throw new AppError(
            `Invalid placeholder names in skipPlaceholders: ${invalidSkipPlaceholders.join(', ')}. ` +
            `These placeholders do not exist in the template.`,
            400,
            'VALIDATION_ERROR',
            {
      ...
      if (request.context?.variables) {
        ...
        const invalidVariableNames = Object.keys(request.context.variables).filter(
          name => !validPlaceholderNames.has(name)
        );

        if (invalidVariableNames.length > 0) {
          throw new AppError(
            `Invalid placeholder names in context variables: ${invalidVariableNames.join(', ')}. ` +
            `These placeholders do not exist in the template.`,
            400,
            'VALIDATION_ERROR',
            {
      ...
      if (request.options?.skipPlaceholders && request.context?.variables) {
        ...
        if (conflicts.length > 0) {
          throw new AppError(
            `Conflicting placeholder configuration: ${conflicts.join(', ')}. ` +
            `These placeholders are both in skipPlaceholders and context variables. ` +
            `A placeholder cannot be both skipped and overridden. Please remove them from one of the lists.`,
            400,
            'VALIDATION_ERROR',
            {
      ...
      if (missingConfigs.length > 0) {
        throw new AppError(
          `Template has placeholders without configurations: ${missingConfigs.join(', ')}. Please configure all placeholders before generating documents.`,
          400,
      ...
      await this.validateDestinationFolder(
      ...
      try {
        ...
        if (jobCreated) {
          try {
            ...
            this.monitoring.trackException(cleanupError as Error, {
  ...
  private validateGenerationRequest(request: GenerationRequest): void {
    ...
    if (!['google', 'microsoft'].includes(request.destinationProvider)) {
      throw new AppError(
        'Destination provider must be "google" or "microsoft"',
        400,
        'VALIDATION_ERROR'
      );
    }
    ...
    if (request.context !== undefined && request.context !== null) {
      ...
      if (request.context.variables !== undefined && request.context.variables !== null) {
        ...
        if (variableCount > this.config.maxContextVariables) {
          throw new AppError(
            `Number of context variables (${variableCount}) exceeds maximum (${this.config.maxContextVariables})`,
            400,
            'VALIDATION_ERROR',
            {
              variableCount,
        ...
        for (const [key, value] of Object.entries(request.context.variables)) {
          ...
          if (value.length > this.config.maxVariableValueLength) {
            throw new AppError(
              `Variable "${key}" value length (${value.length}) exceeds maximum (${this.config.maxVariableValueLength} characters)`,
              400,
              'VALIDATION_ERROR',
              {
                variableName: key,
    ...
    if (request.options !== undefined && request.options !== null) {
      ...
      if (request.options.skipPlaceholders !== undefined && request.options.skipPlaceholders !== null) {
        ...
        if (request.options.skipPlaceholders.length > this.config.maxSkipPlaceholders) {
          throw new AppError(
            `skipPlaceholders array cannot exceed ${this.config.maxSkipPlaceholders} items`,
            400,
            'VALIDATION_ERROR'
          );
        }
        ...
        if (skipSet.size !== request.options.skipPlaceholders.length) {
          const skipPlaceholders = request.options.skipPlaceholders;
          const duplicates = skipPlaceholders.filter(
            (item, index) => skipPlaceholders.indexOf(item) !== index
          );
          throw new AppError(
            `skipPlaceholders array contains duplicate placeholder names: ${[...new Set(duplicates)].join(', ')}`,
            400,
            'VALIDATION_ERROR',
            {
   ...
   */
  private async validateDestinationFolder(
    ...
    if (!userToken) {
      throw new AppError(
        'User OAuth token is required for folder validation',
        400,
        'VALIDATION_ERROR'
      );
    }
    ...
    try {
      ...
      throw new AppError(
        `Failed to validate destination folder: ${(error as Error).message}`,
        400,
   ...
   */
  private async validateGoogleDriveFolder(
    folderId: string,
    userToken: string
  ): Promise<void> {
    try {
      ...
      const driveClient = drive({ version: 'v3', auth: oauth2Client as any });
      ...
      if (mimeType !== 'application/vnd.google-apps.folder') {
        throw new AppError(
          `The specified ID is not a folder (mimeType: ${mimeType})`,
          400,
          'NOT_A_FOLDER'
        );
      }
      ...
      if (capabilities && !capabilities.canAddChildren) {
        throw new AppError(
          'User does not have write access to the destination folder',
          403,
   ...
   */
  private async validateOneDriveFolder(
    folderId: string,
    userToken: string
  ): Promise<void> {
    try {
      ...
      const response = await axios.get(
        `${graphApiBaseUrl}/me/drive/items/${folderId}`,
        {
          headers: {
      ...
      if (item.folder === undefined) {
        throw new AppError(
          'The specified ID is not a folder',
          400,
          'NOT_A_FOLDER'
        );
      }
    ...
    try {
      ...
      if (userToken) {
        try {
          ...
          this.monitoring.trackException(encryptError as Error, {
            operation: 'generation.encrypt_token',
            jobId: job.id,
          });
          throw new AppError(
            'Failed to encrypt user token',
            500,
            'ENCRYPTION_ERROR',
      ...
      throw new AppError(
        `Failed to queue generation job: ${errorMessage}`,
        500,
        errorCode,
        error as Error
      );
   ...
   */
  private async checkQuotaLimits(tenantId: string, userId: string): Promise<void> {
    if (!this.redis) {
    ...
    try {
      ...
      if (dailyCount >= this.config.defaultDailyLimit) {
        throw new AppError(
          `Daily generation limit exceeded. Limit: ${this.config.defaultDailyLimit}, Used: ${dailyCount}. Please try again tomorrow.`,
          429,
          'QUOTA_EXCEEDED',
          {
            limitType: 'daily',
      ...
      if (monthlyCount >= this.config.defaultMonthlyLimit) {
        throw new AppError(
          `Monthly generation limit exceeded. Limit: ${this.config.defaultMonthlyLimit}, Used: ${monthlyCount}. Please try again next month.`,
          429,
          'QUOTA_EXCEEDED',
          {
            limitType: 'monthly',
   ...
   */
  private async rollbackQuota(tenantId: string, userId: string): Promise<void> {
    if (!this.redis) {
  ...
  async getJobStats(tenantId: string): Promise<{
    ...
    // Enhanced analytics
    analytics?: {
      averageDuration?: number; // Average duration in ms for completed jobs
      successRate?: number; // Percentage of completed vs total (excluding pending/processing)
      averagePlaceholdersFilled?: number; // Average placeholders filled per completed job
      averageTokensUsed?: number; // Average tokens used per completed job
      mostCommonErrors?: Array<{ errorCode: string; count: number }>; // Top error codes
      jobsByTemplate?: Array<{ templateId: string; count: number }>; // Jobs grouped by template
    };
  }> {
    const startTime = Date.now();
    try {
   ...
   */
  async findStuckJobs(tenantId: string): Promise<GenerationJob[]> {
    try {
   ...
   */
  async markStuckJobsAsFailed(tenantId: string): Promise<number> {
    try {
      // Validate inputs
      if (!tenantId || typeof tenantId !== 'string' || tenantId.trim().length === 0) {
        throw new AppError('Tenant ID is required and must be a non-empty string', 400, 'VALIDATION_ERROR');
