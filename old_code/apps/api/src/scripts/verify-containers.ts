...
function extractContainersFromInitScript(filePath: string): Set<string> {
...
function extractContainersFromConfig(filePath: string): Set<string> {
  const content = readFileSync(filePath, 'utf-8');
  const containers = new Set<string>();
...
function main() {
  ...
  try {
    ...
    const missingInInit = Array.from(configContainers).filter(
      (c) => !initContainers.has(c)
    );
    const extraInInit = Array.from(initContainers).filter(
      (c) => !configContainers.has(c)
    );
