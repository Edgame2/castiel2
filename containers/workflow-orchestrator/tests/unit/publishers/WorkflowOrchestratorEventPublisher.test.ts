/**
 * Unit tests for WorkflowOrchestratorEventPublisher
 */

import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import * as config from '../../../src/config';
import * as publisherModule from '../../../src/events/publishers/WorkflowOrchestratorEventPublisher';
...
describe('WorkflowOrchestratorEventPublisher', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
  ...
  describe('publishWorkflowEvent', () => {
    it('logs warn and returns when publisher is not initialized', async () => {
      await publisherModule.publishWorkflowEvent('workflow.step.completed', 'tenant-1', { workflowId: 'w1' });
    ...
    it('calls publisher.publish when publisher is initialized', async () => {
      ...
      await publisherModule.publishWorkflowEvent(
        'workflow.step.completed',
        'tenant-1',
        { workflowId: 'w1', step: 'risk_analysis' },
        { correlationId: 'c1', workflowId: 'w1' }
      );
