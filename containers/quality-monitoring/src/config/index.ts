/**
 * Configuration loader for quality_monitoring module
 */
...
let cachedConfig: QualityMonitoringConfig | null = null;
...
export function loadConfig(): QualityMonitoringConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as QualityMonitoringConfig;
  let envConfig: Partial<QualityMonitoringConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<QualityMonitoringConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as QualityMonitoringConfig;
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'quality_monitoring' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'quality_monitoring' });
  }
...
export function getConfig(): QualityMonitoringConfig {
