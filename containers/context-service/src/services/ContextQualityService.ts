 ...
 * Assesses context quality for AI responses
...
import {
  ...
  InsightType,
  AssembledContext,
} from '../types/context-quality.types';
...
export class ContextQualityService {
  ...
  assessContextQuality(
    ...
    const relatedTokens = (context.related || []).reduce(
      (sum, r) => sum + this.estimateTokens(r.content || {}),
      0
    );
    const ragTokens = (context.ragChunks || []).reduce(
      (sum, c) => sum + (c.tokenCount || this.estimateTokens(c.content || '')),
      0
    );
    ...
    const truncationInfo = context.metadata?.truncationInfo;
    ...
    if (truncationInfo && truncationInfo.truncated) {
      ...
      if (truncationInfo.summarizedSections.length > 0) {
        truncatedSections.push(
          ...truncationInfo.summarizedSections.map((s: string) => `Summarized: ${s}`)
        );
      }
      ...
      warnings.push({
        ...
        severity: 'medium',
        impact:
          'Some content was truncated or summarized to fit token limits. Key information was preserved, but some details may be missing.',
        remediation:
          truncationInfo.summarizedSections.length > 0
      ...
      warnings.push({
        ...
        remediation:
          'Consider expanding the token limit or refining your query to focus on specific information.',
      });
    }
    ...
    (context.sources || []).forEach((s) => {
      if (s.shardTypeId) sourceTypes.add(s.shardTypeId);
    });
    ...
    (context.ragChunks || []).forEach((c) => {
      if (c.score && c.score > 0) {
        relevanceScores.push(c.score);
      }
    });
    (context.sources || []).forEach((s) => {
      if (s.score && s.score > 0) {
        relevanceScores.push(s.score);
      }
    });
    ...
    const relevanceDistribution = [
      {
        range: '0.0-0.5',
        count: relevanceScores.filter((s) => s >= 0 && s < 0.5).length,
      },
      {
        range: '0.5-0.7',
        count: relevanceScores.filter((s) => s >= 0.5 && s < 0.7).length,
      },
      {
        range: '0.7-0.9',
        count: relevanceScores.filter((s) => s >= 0.7 && s < 0.9).length,
      },
      {
        range: '0.9-1.0',
        count: relevanceScores.filter((s) => s >= 0.9 && s <= 1.0).length,
      },
    ];
    ...
    const fallbackChunks = (context.ragChunks || []).filter(
      (chunk: any) => chunk._fallbackInfo
    );
    if (fallbackChunks.length > 0) {
      ...
      warnings.push({
        ...
        impact:
          'Results may be less relevant as they were retrieved using fallback methods (keyword search or cached queries)',
        remediation:
          'Vector search may be unavailable or returned insufficient results. Try refining your query or expanding the search scope.',
      });
    }

    // Check for stale cache usage
    const staleCacheChunks = (context.ragChunks || []).filter(
      (chunk: any) => chunk._fromStaleCache
    );
    const cacheStalenessInfo = context.metadata?.cacheStalenessInfo;
    if (staleCacheChunks.length > 0 || cacheStalenessInfo) {
      const stalenessInfo = cacheStalenessInfo || {
        ...
        cachedAt: (staleCacheChunks[0] as any)?._cachedAt
          ? new Date((staleCacheChunks[0] as any)._cachedAt)
          : new Date(),
      };
      ...
      warnings.push({
        type: 'stale_cache',
        message: `Context includes cached data from ${ageMinutes} minute(s) ago${
          stalenessInfo.isCriticallyStale
            ? ' (critically stale)'
            : stalenessInfo.isStale
            ? ' (stale)'
            : ''
        }`,
        severity: stalenessInfo.isCriticallyStale
          ? 'high'
          : stalenessInfo.isStale
          ? 'medium'
          : 'low',
    ...
    // Get minimum requirements based on insight type
    ...
    const meetsMinRelevance =
      averageRelevance >= minimumRequirements.minRelevanceScore || relevanceScores.length === 0;
    const meetsMinTokens =
      !minimumRequirements.minTokens || totalTokens >= minimumRequirements.minTokens;
    const meetsMinimumRequirements = meetsMinSourceCount && meetsMinRelevance && meetsMinTokens;

    // Check required source types
    if (
      minimumRequirements.requiredSourceTypes &&
      minimumRequirements.requiredSourceTypes.length > 0
    ) {
      const foundSourceTypes = Array.from(sourceTypes);
    ...
    // Enhanced empty context handling
    if (sourceCount === 0) {
      if (minimumRequirements.allowEmpty) {
        ...
        warnings.push({
          ...
          remediation:
            'Try refining your query, expanding the search scope, or adding relevant documents to the system',
        });
      }
    }

    // Enhanced low source count warning
    ...
    // Enhanced low relevance warning
  ...
  checkMinimumRequirements(
    ...
    if (sourceCount < requirements.minSourceCount) {
      if (sourceCount === 0 && !requirements.allowEmpty) {
        ...
        warnings.push(
          `Insufficient sources: ${sourceCount} < ${requirements.minSourceCount} required`
        );
      }
    }
    ...
    if (relevanceScores.length > 0) {
      ...
      if (avgRelevance < requirements.minRelevanceScore) {
        warnings.push(
          `Low relevance: ${(avgRelevance * 100).toFixed(0)}% < ${(requirements.minRelevanceScore * 100).toFixed(0)}% required`
        );
      }
    }
    ...
    // Check tokens
    const totalTokens =
      this.estimateTokens(context.primary?.content || {}) +
      (context.related || []).reduce(
        (sum, r) => sum + this.estimateTokens(r.content || {}),
        0
      ) +
      (context.ragChunks || []).reduce(
        (sum, c) => sum + (c.tokenCount || this.estimateTokens(c.content || '')),
        0
      );
    if (requirements.minTokens && totalTokens < requirements.minTokens) {
      warnings.push(
        `Insufficient tokens: ${totalTokens} < ${requirements.minTokens} required`
      );
    }
    ...
    if (requirements.requiredSourceTypes) {
      const missing = requirements.requiredSourceTypes.filter((req) => !sourceTypes.has(req));
      if (missing.length > 0) {
