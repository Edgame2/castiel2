/**
 * Unit tests for IntegrationSyncEventConsumer
 */

import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import * as config from '../../../src/config';
import * as consumerModule from '../../../src/events/consumers/IntegrationSyncEventConsumer';
...
vi.mock('../../../src/services/IntegrationSyncService', () => ({
  IntegrationSyncService: vi.fn().mockImplementation(function (this: unknown) {
    return { handleBidirectionalSync: vi.fn().mockResolvedValue(undefined) };
  }),
}));

const mockLoadConfig = vi.mocked(config).loadConfig;

describe('IntegrationSyncEventConsumer', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
  ...
  describe('initializeEventConsumer', () => {
    ...
    it('creates consumer, registers shard.updated handler, calls start and logs info when URL is configured', async () => {
      mockLoadConfig.mockReturnValue({
        rabbitmq: {
          url: 'amqp://localhost',
          exchange: 'coder_events',
          queue: 'integration_sync_queue',
          bindings: [],
        },
      } as any);
