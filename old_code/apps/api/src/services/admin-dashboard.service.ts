...
export class AdminDashboardService {
    ...
    private providers: Map<string, SearchProviderConfig> = new Map();
    private fallbackChains: Map<string, ProviderFallbackChain> = new Map();
    private providerHealth: Map<string, ProviderHealth> = new Map();
    private usageMetrics: Map<string, UsageAnalytics> = new Map();
    private quotaConfigs: Map<string, QuotaConfig> = new Map();
    private tenantConfigs: Map<string, TenantWebSearchConfig> = new Map();
    private rand: () => number;

    constructor(monitoring: IMonitoringProvider, options?: { deterministic?: boolean; seed?: number }) {
    ...
    private initializeDefaults(): void {
        ...
        const defaultFallbackChain: ProviderFallbackChain = {
            id: 'default-chain',
     ...
     */
    private createRandom(options?: { deterministic?: boolean; seed?: number }): () => number {
     ...
     * Get all providers
     */
    async getProviders(): Promise<SearchProviderConfig[]> {
        try {
     ...
     */
    async getProvider(providerId: string): Promise<SearchProviderConfig | null> {
        try {
    ...
    async testProvider(
        ...
        try {
            ...
            const baseLatency =
                provider.type === 'azure-ai-search' ? 120 : 240;
    ...
    async getFallbackChain(
        chainId: string = 'default-chain'
    ): Promise<ProviderFallbackChain | null> {
        try {
    ...
    async getProviderHealth(
        providerId?: string
    ): Promise<ProviderHealth | ProviderHealth[]> {
        try {
            if (providerId) {
     ...
     */
    private generateDailyBreakdown(startDate: Date, endDate: Date): Array<{
        date: Date;
        searches: number;
        cost: number;
        avgLatency: number;
     ...
     */
    async getQuotaConfig(tenantId: string): Promise<QuotaConfig | null> {
        try {
     ...
     */
    async getTenantWebSearchConfig(
        tenantId: string
     ...
     */
    async updateTenantWebSearchConfig(
        tenantId: string,
        updates: Partial<TenantWebSearchConfig>
    ): Promise<TenantWebSearchConfig> {
        try {
