...
export interface CreateUserRequest {
  ...
  roles?: string[];
  status?: UserStatus;
  sendInvite?: boolean; // If true, send invitation email
...
export interface AdminPasswordResetRequest {
  sendEmail?: boolean; // If false, return reset token directly
}
...
export interface AdminPasswordResetResponse {
  message: string;
  resetToken?: string; // Only if sendEmail=false
  expiresAt?: string;
}
...
export interface ImpersonateUserResponse {
  ...
  expiresIn: number;
  expiresAt: string;
  impersonationId: string; // For audit logging
  message: string;
  user: UserResponse;
}
...
export interface BulkUserOperationResponse {
  successful: string[]; // User IDs that succeeded
  failed: {
    userId: string;
    error: string;
  }[];
