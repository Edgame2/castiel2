...
export class MemoryController {
  constructor(
    private readonly memoryService: MemoryContextService
  ) {}
   ...
   */
  async remember(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      const fact = await this.memoryService.remember(
        user.id,
        user.tenantId,
        body.information.trim(),
        {
   ...
   */
  async forget(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const body = request.body as {
        query: string;
  ...
  async search(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      const facts = await this.memoryService.searchUserMemory(
        user.id,
        user.tenantId,
        query.q.trim(),
        {
   ...
   * List all user facts
   */
  async listFacts(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const query = request.query as {
        category?: string;
      ...
      const result = await this.memoryService.listUserFacts(
        user.id,
        user.tenantId,
        {
          category: query.category as any,
   ...
   */
  async removeFact(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const params = request.params as { factId: string };

      if (!params.factId) {
        throw new AppError('Fact ID is required', 400);
      }

      const removed = await this.memoryService.removeUserFact(
        user.id,
        user.tenantId,
        params.factId
      );
   ...
   */
  async clearMemory(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
