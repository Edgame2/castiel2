...
function buildContactData(projectName, projectIndex, contactIndex) {
...
function buildOpportunityData(projectName, projectIndex, oppIndex) {
...
function buildNoteData(projectName, projectIndex, noteIndex) {
...
async function createShard(input) {
    return apiPost('/shards', input);
}
async function seedProject(index) {
    ...
    for (let i = 0; i < contactCount; i++) {
        contacts.push(await createShard({
    ...
    for (let i = 0; i < oppCount; i++) {
        opportunities.push(await createShard({
    ...
    for (let i = 0; i < noteCount; i++) {
        notes.push(await createShard({
            ...buildNoteData(projectInput.structuredData.name, index, i),
            parentShardId: project.id,
        }));
    }
    console.log(`Seeded project ${index}/${PROJECT_COUNT}: ${project.id} with ${contacts.length} contacts, ${opportunities.length} opps, ${notes.length} notes`);
}
async function main() {
...
//# sourceMappingURL=seed-project-shards.js.map