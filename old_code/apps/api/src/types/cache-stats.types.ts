...
export interface CacheKeyInfo {
  key: string;
  pattern: string; // e.g., "shard", "acl", "vsearch"
  hits: number;
  size?: number;
  ttl?: number;
  lastAccessed?: Date;
...
export interface CacheWarmingConfig {
  ...
  topN: number; // Number of items to preload per tenant
  tenants?: string[]; // Specific tenants to warm, or all if undefined
  includeShards: boolean;
  includeACL: boolean;
  includeShardTypes?: boolean; // Optional: warm shard types (default: false)
  maxDurationMs: number; // Maximum time to spend warming
}
...
export interface CacheClearOptions {
  pattern?: string; // Redis key pattern (e.g., "tenant:*:shard:*")
  service?: 'shards' | 'acl' | 'vectorSearch' | 'tokenValidation' | 'all';
  tenantId?: string; // Clear specific tenant's cache
  force?: boolean; // Force clear even if warming is in progress
}
...
export interface CacheAlertConfig {
  enabled: boolean;
  thresholds: {
    lowHitRate: number; // Alert if hit rate falls below this (%)
    highMemoryUsage: number; // Alert if memory usage exceeds this (%)
    highLatency: number; // Alert if latency exceeds this (ms)
    highEvictionRate: number; // Alert if eviction rate exceeds this (per minute)
  };
...
export interface CachePerformanceReport {
  ...
  insights: {
    ...
    leastEffectiveService: string; // Service with lowest hit rate
