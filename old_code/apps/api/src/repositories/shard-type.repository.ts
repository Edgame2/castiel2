import { CosmosClient, Container, ContainerDefinition, SqlParameter, JSONValue } from '@azure/cosmos';
...
const SHARD_TYPE_CONTAINER_CONFIG: ContainerDefinition = {
 ...
 * Uses cache-aside pattern with optional caching for improved performance
 */
export class ShardTypeRepository {
  ...
  private shardTypeCacheService?: any; // ShardTypeCacheService (optional)

  constructor(
    monitoring: IMonitoringProvider,
    shardTypeCacheService?: any // Optional cache service
  ) {
    this.monitoring = monitoring;
    this.shardTypeCacheService = shardTypeCacheService;
  ...
  async create(input: CreateShardTypeInput): Promise<ShardType> {
    ...
    try {
      ...
      // Invalidate cache for the new shard type
      if (this.shardTypeCacheService) {
        await this.shardTypeCacheService.invalidateShardTypeCache(
          shardType.tenantId,
          shardType.id,
          true
        );
   ...
   */
  async findById(id: string, tenantId: string): Promise<ShardType | null> {
    const startTime = Date.now();

    // Try cache first if cache service is available
    if (this.shardTypeCacheService) {
      // Try tenant-specific cache first
      const cached = await this.shardTypeCacheService.getCachedShardType(tenantId, id);
      if (cached) {
        const duration = Date.now() - startTime;
        this.monitoring.trackDependency(
          'cosmosdb.shardType.findById',
          'CosmosDB',
          config.cosmosDb.endpoint,
          duration,
          true
      ...
      // Also try system tenant cache (for global types)
      if (tenantId !== SYSTEM_TENANT_ID) {
        const systemCached = await this.shardTypeCacheService.getCachedShardType(SYSTEM_TENANT_ID, id);
        if (systemCached) {
          const duration = Date.now() - startTime;
          this.monitoring.trackDependency(
            'cosmosdb.shardType.findById',
    ...
    const partitions =
      tenantId === SYSTEM_TENANT_ID ? [SYSTEM_TENANT_ID] : [tenantId, SYSTEM_TENANT_ID];
    let resource: ShardType | undefined;
    let foundTenantId: string | undefined;

    try {
      for (const partitionKey of partitions) {
        try {
          const { resource: item } = await this.container.item(id, partitionKey).read<ShardType>();
          if (item && item.status !== ShardTypeStatus.DELETED) {
            resource = item;
            foundTenantId = partitionKey;
            break;
          }
        } catch (error: unknown) {
      ...
      if (resource && this.shardTypeCacheService && foundTenantId) {
        await this.shardTypeCacheService.cacheShardType(foundTenantId, id, resource);
      }
   ...
   */
  async findByName(name: string, tenantId: string): Promise<ShardType | null> {
    const startTime = Date.now();
    ...
    try {
      ...
      return resources.length > 0 ? (resources[0] as ShardType) : null;
    } catch (error) {
      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.shardType.findByName',
   ...
   */
  async findByIdWithInheritance(id: string, tenantId: string): Promise<ResolvedShardType | null> {
    try {
   ...
   */
  private async resolveInheritance(shardType: ShardType): Promise<ResolvedShardType> {
    const inheritanceChain: string[] = [shardType.id];
   ...
   */
  async update(id: string, tenantId: string, input: UpdateShardTypeInput): Promise<ShardType | null> {
    const startTime = Date.now();
    ...
    try {
      ...
      // Invalidate cache for the updated shard type
      if (this.shardTypeCacheService) {
        await this.shardTypeCacheService.invalidateShardTypeCache(
          existing.tenantId,
          id,
  ...
  async delete(id: string, tenantId: string): Promise<boolean> {
    ...
    try {
      ...
      // Invalidate cache for the deleted shard type
      if (this.shardTypeCacheService) {
        await this.shardTypeCacheService.invalidateShardTypeCache(
          existing.tenantId,
          id,
   ...
   */
  async list(options: ShardTypeListOptions): Promise<ShardTypeListResult> {
    const startTime = Date.now();
    ...
    try {
      ...
      const parameters: SqlParameter[] = [
        { name: '@tenantId', value: tenantFilterId as JSONValue },
        { name: '@systemTenant', value: SYSTEM_TENANT_ID as JSONValue },
        { name: '@deletedStatus', value: ShardTypeStatus.DELETED as JSONValue },
      ];

      if (filter.name) {
        query += ' AND CONTAINS(LOWER(c.name), LOWER(@name))';
        parameters.push({ name: '@name', value: (filter.name) as JSONValue });
      }

      if (filter.category) {
        query += ' AND c.category = @category';
        parameters.push({ name: '@category', value: filter.category as JSONValue });
      }

      if (filter.isCustom !== undefined) {
        query += ' AND c.isCustom = @isCustom';
        parameters.push({ name: '@isCustom', value: filter.isCustom as JSONValue });
      }

      if (filter.isBuiltIn !== undefined) {
        query += ' AND c.isBuiltIn = @isBuiltIn';
        parameters.push({ name: '@isBuiltIn', value: filter.isBuiltIn as JSONValue });
      }

      if (filter.status) {
        query += ' AND c.status = @status';
        parameters.push({ name: '@status', value: filter.status as JSONValue });
      }

      if (filter.isGlobal !== undefined) {
        query += ' AND c.isGlobal = @isGlobal';
        parameters.push({ name: '@isGlobal', value: filter.isGlobal as JSONValue });
      }

      if (filter.tags?.length) {
        filter.tags.forEach((tag, index) => {
          query += ` AND ARRAY_CONTAINS(c.tags, @tag${index})`;
          parameters.push({ name: `@tag${index}`, value: tag as JSONValue });
        });
      }

      if (filter.search) {
        query += ' AND (CONTAINS(LOWER(c.name), LOWER(@search)) OR CONTAINS(LOWER(c.displayName), LOWER(@search)))';
        parameters.push({ name: '@search', value: (filter.search) as JSONValue });
      }

      if (filter.parentShardTypeId) {
        query += ' AND c.parentShardTypeId = @parentShardTypeId';
        parameters.push({ name: '@parentShardTypeId', value: filter.parentShardTypeId as JSONValue });
      }

      if (filter.createdBy) {
        query += ' AND c.createdBy = @createdBy';
        parameters.push({ name: '@createdBy', value: filter.createdBy as JSONValue });
      }

      if (filter.createdAfter) {
        query += ' AND c.createdAt >= @createdAfter';
        parameters.push({ name: '@createdAfter', value: filter.createdAfter.toISOString() as JSONValue });
      }

      if (filter.createdBefore) {
        query += ' AND c.createdAt <= @createdBefore';
        parameters.push({ name: '@createdBefore', value: filter.createdBefore.toISOString() as JSONValue });
      }
      ...
      const shardTypes = (resources || []) as ShardType[];
      const uniqueShardTypes = new Map<string, ShardType>();
  ...
  async getGlobalShardTypes(): Promise<ShardType[]> {
    try {
      const querySpec = {
  ...
  async checkCircularInheritance(shardTypeId: string, parentShardTypeId: string, tenantId: string): Promise<boolean> {
    if (!parentShardTypeId) {
      return false;
    }
    const visited = new Set<string>([shardTypeId]);
    let currentParentId: string | undefined = parentShardTypeId;
  ...
  async getShardTypeUsageCount(shardTypeId: string, tenantId?: string): Promise<number> {
    const parameters: SqlParameter[] = [
      { name: '@shardTypeId', value: shardTypeId as JSONValue },
      { name: '@deletedStatus', value: ShardStatus.DELETED as JSONValue },
    ];
    ...
    if (tenantId) {
      ...
      parameters.push({ name: '@tenantId', value: tenantId as JSONValue });
    }
  ...
  async checkShardTypeInUse(shardTypeId: string, tenantId?: string): Promise<boolean> {
    const count = await this.getShardTypeUsageCount(shardTypeId, tenantId);
    return count > 0;
  }

  async getChildTypes(shardTypeId: string, tenantId?: string): Promise<ShardType[]> {
    const parameters: SqlParameter[] = [
      { name: '@parentId', value: shardTypeId as JSONValue },
      { name: '@deletedStatus', value: ShardTypeStatus.DELETED as JSONValue },
    ];
   ...
   */
  async findChildren(parentId: string, tenantId: string): Promise<ShardType[]> {
    try {
   ...
   */
  async getTenantShardTypes(tenantId: string): Promise<ShardType[]> {
    try {
      const querySpec = {
        query: 'SELECT * FROM c WHERE c.tenantId = @tenantId AND c.status != @deletedStatus ORDER BY c.createdAt DESC',
        parameters: [
          { name: '@tenantId', value: tenantId as JSONValue },
          { name: '@deletedStatus', value: ShardTypeStatus.DELETED as JSONValue },
        ],
      };
      const { resources } = await this.container.items.query(querySpec).fetchAll();
   ...
   */
  async getCloneableShardTypes(includeGlobalOnly: boolean = true): Promise<ShardType[]> {
    try {
   ...
   */
  async getByIdWithRelationships(id: string, tenantId: string): Promise<ShardType & { resolvedRelationships?: any[] } | null> {
    try {
      const shardType = await this.findById(id, tenantId);
      ...
      const resolvedRelationships = await Promise.all(
        shardType.relationships.map(async (rel) => {
  ...
  async updateEmbeddingTemplate(
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.shardType.updateEmbeddingTemplate',
        'CosmosDB',
        config.cosmosDb.endpoint,
        duration,
        true
   ...
   */
  async getEmbeddingTemplate(shardTypeId: string, tenantId: string): Promise<any | null> {
    try {
      const shardType = await this.findById(shardTypeId, tenantId);
      if (!shardType) {
        throw new Error(`ShardType ${shardTypeId} not found`);
   ...
   */
  async listWithEmbeddingTemplates(tenantId: string): Promise<ShardType[]> {
    const startTime = Date.now();
    ...
    try {
      ...
      const { resources } = await this.container.items.query<ShardType>(querySpec).fetchAll();

      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.shardType.listWithEmbeddingTemplates',
        'CosmosDB',
        config.cosmosDb.endpoint,
        duration,
        true
