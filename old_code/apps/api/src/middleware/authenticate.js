...
export const mapPayloadToAuthUser = (payload) => {
    return {
        id: payload.sub,
        email: payload.email,
 ...
 */
export function authenticate(tokenCache) {
            ...
            if (!user) {
                ...
                try {
                    ...
                    const verifyPromise = (async () => {
                        ...
                        try {
                            const decoded = jwtPlugin.verify(token);
                            if (request.log) {
                                request.log.debug({
                                    type: decoded.type,
                                    sub: decoded.sub,
                            ...
                            const errorName = verifyError && typeof verifyError === 'object' && 'name' in verifyError ? verifyError.name : undefined;
                            if (request.log) {
                                request.log.error({
                                    error: errorMessage,
                                    name: errorName
                    ...
                    const errorName = jwtError && typeof jwtError === 'object' && 'name' in jwtError ? jwtError.name : undefined;
                    if (request.log) {
                        request.log.error({
                            error: errorMessage,
                            name: errorName,
 ...
 */
export function optionalAuthenticate(tokenCache) {
    return async (request, _reply) => {
        try {
            const authHeader = request.headers.authorization;
            if (!authHeader) {
