...
import { RolePermissions } from '@castiel/shared-types';
...
export class AuthController {
    ...
    mfaPolicyService;
    constructor(userService, emailService, cacheManager, publicApiUrl, accessTokenExpiry = '15m', tenantService, tenantJoinRequestService, mfaController, auditLogService, deviceSecurityService, roleManagementService) {
     ...
     */
    async logAuthEvent(eventType, outcome, tenantId, options) {
    ...
    async getUserPermissions(user) {
        try {
            ...
            for (const roleName of userRoles) {
                // Static
                const staticPerms = RolePermissions[roleName];
                if (staticPerms) {
    ...
    async register(request, reply) {
        const { email, password, firstName, lastName, tenantName, tenantDomain } = request.body;
        try {
            ...
            if (user.verificationToken && this.emailService.isReady()) {
                try {
                    await this.emailService.sendVerificationEmail(user.email, user.verificationToken, this.publicApiUrl);
    ...
    async login(request, reply) {
        const { email, password, tenantId, deviceFingerprint, rememberDevice } = request.body;
        try {
            ...
            if (rateLimiter && loginRateLimitEnabled) {
                ...
                if (!rateLimitResult.allowed) {
                    request.log.warn({ email, ip: request.ip, blockedUntil: rateLimitResult.resetAt }, 'Login attempt rate limited');
                    // Log rate limit event
                    await this.logAuthEvent(AuditEventType.LOGIN_FAILURE, AuditOutcome.FAILURE, tenantId || 'unknown', {
                        actorEmail: email,
            ...
            if (this.mfaPolicyService) {
                const policyEval = await this.mfaPolicyService.evaluatePolicyForUser(user.id, resolvedTenantId);
                // Block login if policy requires MFA but user hasn't set it up
                if (policyEval.shouldBlockLogin) {
                    request.log.warn({ userId: user.id, tenantId: resolvedTenantId, gracePeriod: policyEval.isInGracePeriod }, 'Login blocked: Tenant MFA required but user has no MFA methods');
                    return reply.status(403).send({
                        error: 'Forbidden',
                ...
                if (hasMFA || tenantRequiresMFA) {
                    ...
                    if (!isTrusted) {
                        ...
                        if (this.mfaPolicyService) {
                            ...
                            methods = methods.filter(method => allowedMethods.includes(method));
                        }
                        if (tenantRequiresMFA && (!methods || methods.length === 0)) {
                            // This case should be caught above by policy evaluation
                            // But keep as fallback
                            request.log.warn({ userId: user.id, tenantId: resolvedTenantId }, 'Tenant MFA required but user has no MFA methods');
                            return reply.status(403).send({
                                error: 'Forbidden',
                        ...
                        const challengeToken = request.server.jwt.sign({
                            sub: user.id,
                            email: user.email,
            ...
            if (this.deviceSecurityService) {
                await this.deviceSecurityService.checkAndNotifyNewDevice(user.id, user.email, `${user.firstName || ''} ${user.lastName || ''}`.trim() || user.email, user.tenantId, request, deviceFingerprint);
            }
            request.log.info({ userId: user.id, email: user.email }, 'User logged in successfully');
            // Log successful login
            await this.logAuthEvent(AuditEventType.LOGIN_SUCCESS, AuditOutcome.SUCCESS, user.tenantId, {
                actorId: user.id,
            ...
            await this.logAuthEvent(AuditEventType.LOGIN_FAILURE, AuditOutcome.FAILURE, 'default', {
                actorEmail: email,
                ipAddress: request.ip,
                userAgent: request.headers['user-agent'],
    ...
    async verifyEmail(request, reply) {
        ...
        const { tenantId = DEFAULT_TENANT_ID } = request.query;
        try {
    ...
    async resendVerificationEmail(request, reply) {
        ...
        try {
            ...
            if (verificationToken) {
                ...
                await this.emailService.sendVerificationEmail(email, verificationToken, this.publicApiUrl);
            }
            return reply.status(200).send({
                message: 'If your email is registered and unverified, you will receive a verification email shortly.',
            });
    ...
    async forgotPassword(request, reply) {
        const { email, tenantId = DEFAULT_TENANT_ID } = request.body;
        try {
            // Create reset token
            const resetToken = await this.userService.createPasswordResetToken(email, tenantId);
            if (resetToken) {
                // Send reset email
                await this.emailService.sendPasswordResetEmail(email, resetToken, this.publicApiUrl);
            }
    ...
    async resetPassword(request, reply) {
        const { token, password, tenantId = DEFAULT_TENANT_ID } = request.body;
        try {
    ...
    async refreshToken(request, reply) {
        ...
        try {
            ...
            const headerTenant = (request.headers['x-tenant-id'] || request.headers['tenant-id']);
            ...
            const user = await this.userService.findById(refreshResult.tokenData.userId, refreshResult.tokenData.tenantId);
            if (!user) {
                return reply.status(401).send({
                    error: 'Unauthorized',
    ...
    async logout(request, reply) {
        try {
            // Get user from JWT
            const user = request.server.jwt.verify(request.headers.authorization?.replace('Bearer ', ''));
            ...
            await this.logAuthEvent(AuditEventType.LOGOUT, AuditOutcome.SUCCESS, user.tenantId || 'default', {
                actorId: user.sub,
                actorEmail: user.email,
                ipAddress: request.ip,
                userAgent: request.headers['user-agent'],
     ...
     * POST /auth/revoke
     */
    async revokeToken(request, reply) {
        try {
            ...
            let user;
            try {
                user = request.server.jwt.verify(request.headers.authorization?.replace('Bearer ', ''));
            }
            catch {
            ...
            if (token_type_hint === 'access_token' || !token_type_hint) {
                ...
                try {
                    ...
                    if (decoded && decoded.exp) {
                        await this.cacheManager.blacklist.blacklistTokenString(token, decoded.exp * 1000);
     ...
     */
    async introspectToken(request, reply) {
        try {
            const { token, token_type_hint } = request.body;
            if (!token) {
                return reply.status(400).send({
     ...
     */
    async completeMFAChallenge(request, reply) {
        const { challengeToken, code, method } = request.body;
        try {
            // Verify challenge token
            let challengeData;
            try {
                challengeData = request.server.jwt.verify(challengeToken);
            ...
            const isValid = await this.verifyMFACode(challengeData.sub, challengeData.tenantId, method, code);
            if (!isValid) {
                return reply.status(401).send({
                    error: 'Unauthorized',
            ...
            if (challengeData.rememberDevice && challengeData.deviceFingerprint) {
                ...
                if (!tenantRequiresMFA) {
                    const metadata = SessionManagementService.extractSessionMetadata(request);
                    await this.mfaController.trustDevice(challengeData.sub, challengeData.tenantId, challengeData.deviceFingerprint, metadata.deviceInfo?.userAgent, metadata.locationInfo?.ip);
                }
                else {
                    request.log.info({ userId: challengeData.sub, tenantId: challengeData.tenantId }, 'Device trust skipped: Tenant requires MFA');
                }
            }
            // Get user details
     ...
     */
    async verifyMFACode(userId, tenantId, method, code) {
        if (!this.mfaController) {
            return false;
        }
        try {
     ...
     * GET /auth/tenants
     */
    async listUserTenants(request, reply) {
        try {
            const authHeader = request.headers.authorization;
            if (!authHeader || !authHeader.startsWith('Bearer ')) {
                reply.status(401).send({
     ...
     */
    async updateDefaultTenant(request, reply) {
        try {
            const authHeader = request.headers.authorization;
            if (!authHeader || !authHeader.startsWith('Bearer ')) {
                reply.status(401).send({
    ...
    async switchTenant(request, reply) {
        try {
            ...
            const { tenantId: targetTenantId } = request.body;
            if (!targetTenantId) {
                reply.status(400).send({
                    error: 'Bad Request',
                    message: 'tenantId is required',
            ...
            const refreshTokenResult = await this.cacheManager.tokens.createRefreshToken(targetMembership.id, targetMembership.tenantId);
            ...
            request.log.info({ userId: targetMembership.id, email: targetMembership.email, tenantId: targetTenantId }, 'User switched tenant context');
            reply.send({
                accessToken,
                refreshToken: refreshTokenResult.token,
                expiresIn: this.accessTokenExpiry,
    ...
    async buildTenantMembershipResponse(email) {
        const memberships = await this.userService.findAllByEmail(email);
        if (!memberships.length) {
            return { tenants: [], defaultTenantId: null };
        }
        const tenantDetails = await Promise.all(memberships.map(async (membership) => {
            try {
    ...
    async notifyTenantAdmins(tenantId, requesterEmail) {
        try {
     ...
     */
    async checkRegistration(request, reply) {
        const { email } = request.body;
        try {
