/**
 * Configuration loader for web_search module
 */
...
export function loadConfig(): WebSearchConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as WebSearchConfig;
  let envConfig: Partial<WebSearchConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<WebSearchConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as WebSearchConfig;
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'web_search' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'web_search' });
  }
...
export function getConfig(): WebSearchConfig {
