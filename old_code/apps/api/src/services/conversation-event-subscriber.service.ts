...
import type { ConversationEventPayload } from './conversation-realtime.service.js';

/**
 * SSE connection for conversation events
 */
interface ConversationSSEConnection {
  id: string;
  tenantId: string;
  userId: string;
  response: ServerResponse;
  conversationIds: Set<string>; // Conversations this connection is subscribed to
  connectedAt: Date;
  lastHeartbeat: Date;
}
...
export class ConversationEventSubscriberService {
  ...
  private connections: Map<string, ConversationSSEConnection> = new Map();
  ...
  registerConnection(
    ...
    const connection: ConversationSSEConnection = {
      id: connectionId,
      tenantId,
      userId,
      response,
  ...
  private handleRedisMessage(channel: string, message: string): void {
    try {
      ...
      // Parse event payload
      const payload: ConversationEventPayload = JSON.parse(message);
  ...
  private broadcastToConnections(
    ...
    for (const [connectionId, connection] of this.connections) {
      ...
      try {
        ...
        this.monitoring.trackTrace(`Failed to send to connection ${connectionId}, removing`, 2 as any); // SeverityLevel.Warning
        this.connections.delete(connectionId);
      }
    }
