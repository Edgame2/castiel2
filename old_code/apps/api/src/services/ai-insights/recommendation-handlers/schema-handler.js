 ...
 */
export class SchemaRecommendationHandler extends BaseRecommendationHandler {
    ...
    async validate(recommendation) {
        ...
        for (const field of recommendation.fields || []) {
            ...
            if (field.name && !/^[a-zA-Z][a-zA-Z0-9_]*$/.test(field.name)) {
                errors.push(`Field name "${field.name}" must start with letter and contain only alphanumeric characters and underscores`);
            }
     ...
     */
    async suggestNextAction(appliedRecommendation, context) {
    ...
    normalizeOption(item, index) {
        ...
        const hasComplexValidations = recommendation.fields.some((f) => f.validation?.pattern || f.type === 'array' || f.type === 'object');
