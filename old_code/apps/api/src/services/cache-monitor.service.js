...
export class CacheMonitorService {
    ...
    startMonitoring() {
        ...
        this.metricsInterval = setInterval(() => {
            this.collectAndExportMetrics().catch((err) => {
                this.monitoring?.trackException(err, { operation: 'cache-monitor.collect-metrics' });
            });
    ...
    async getAggregatedStats() {
        ...
        const tokenValidationCacheStats = this.tokenValidationCache
            ? await this.getServiceStats('TokenValidationCache', this.tokenValidationCache)
            : undefined;
        ...
        const totalMemoryBytes = services.reduce((sum, s) => sum + (s.memoryUsageBytes ?? 0), 0);
        const totalInvalidations = services.reduce((sum, s) => sum + (s.invalidations ?? 0), 0);
     ...
     */
    async getServiceStats(serviceName, service) {
        try {
            ...
            this.monitoring?.trackException(error, { operation: 'cache-monitor.get-stats', serviceName });
            return {
                serviceName,
                enabled: false,
                healthy: false,
    ...
    async performHealthCheck() {
        ...
        if (!redisHealthy) {
            ...
            if (redisLatency > 50) {
                ...
                recommendations.push('Consider Redis server location or network optimization');
            }
            // Check memory usage
            try {
                ...
                if (usedMatch) {
                    memoryUsedBytes = parseInt(usedMatch[1], 10);
                }
                ...
                if (memoryUsedBytes && memoryMaxBytes) {
                    ...
                    if (usagePercent > this.config.alertThresholds.highMemoryUsage) {
                        ...
                        recommendations.push('Consider increasing Redis memory limit or reducing TTLs');
                    }
                }
            }
            catch (error) {
                this.monitoring?.trackException(error, { operation: 'cache-monitor.get-redis-memory' });
            }
        }
        ...
        for (const [serviceName, serviceStats] of Object.entries(stats.services)) {
            ...
            if (serviceStats.hitRate < this.config.alertThresholds.lowHitRate &&
                serviceStats.totalOperations > 100) {
                issues.push(`${serviceName} has low hit rate: ${serviceStats.hitRate.toFixed(1)}%`);
                recommendations.push(`Review ${serviceName} caching strategy and TTL configuration`);
            }
        }
        return {
            ...
            redis: {
                ...
                memoryUsedBytes,
                memoryMaxBytes,
                memoryUsagePercent: memoryUsedBytes && memoryMaxBytes
    ...
    async collectAndExportMetrics() {
        ...
        try {
            ...
            this.monitoring.trackMetric('cache.overall.hitRate', stats.aggregated.overallHitRate, {
                unit: 'percent',
            });
            this.monitoring.trackMetric('cache.overall.totalKeys', stats.aggregated.totalKeys, {
                unit: 'count',
            });
            if (stats.aggregated.totalMemoryBytes) {
                this.monitoring.trackMetric('cache.overall.memoryUsage', stats.aggregated.totalMemoryBytes, {
                    unit: 'bytes',
                });
            }
            this.monitoring.trackMetric('cache.overall.averageLatency', stats.performance.averageLatencyMs, {
                unit: 'milliseconds',
            });
            ...
            for (const [serviceName, serviceStats] of Object.entries(stats.services)) {
                ...
                this.monitoring.trackMetric(`cache.service.hitRate`, serviceStats.hitRate, {
                    service: serviceName,
                    unit: 'percent',
                });
                this.monitoring.trackMetric(`cache.service.operations`, serviceStats.totalOperations, {
                    service: serviceName,
                    unit: 'count',
                });
                if (serviceStats.memoryUsageBytes) {
                    this.monitoring.trackMetric(`cache.service.memoryUsage`, serviceStats.memoryUsageBytes, {
                        service: serviceName,
                        unit: 'bytes',
                    });
                }
            ...
            this.monitoring?.trackException(error, { operation: 'cache-monitor.collect-export-metrics' });
        }
    }
    /**
     * Check if any alert thresholds are breached
     */
    async checkAndTriggerAlerts(stats) {
        // Check overall hit rate
        if (stats.aggregated.overallHitRate <
        ...
        // Check latency
        if (stats.performance.averageLatencyMs >
    ...
    async getTopKeys() {
        ...
        for (const [key, hits] of sorted) {
            try {
                ...
                this.monitoring?.trackException(error, { operation: 'cache-monitor.get-key-info', key });
            }
        }
        return keyInfos;
    }
    ...
    trackKeyAccess(key) {
        ...
        if (this.keyAccessCount.size > 10000) {
            ...
            const sorted = Array.from(this.keyAccessCount.entries()).sort((a, b) => b[1] - a[1]);
    ...
    async generatePerformanceReport(startDate, endDate) {
        ...
        for (const [serviceName, serviceStats] of Object.entries(stats.services)) {
            ...
            if (serviceStats.hitRate < lowestHitRate &&
        ...
        if (leastEffectiveService) {
            recommendations.push(`Review ${leastEffectiveService} caching strategy (hit rate: ${lowestHitRate.toFixed(1)}%)`);
        }
        if (highestLatencyService && highestLatency > 50) {
            recommendations.push(`Optimize ${highestLatencyService} performance (latency: ${highestLatency.toFixed(1)}ms)`);
        }
        if (stats.aggregated.overallHitRate < 70) {
            recommendations.push('Consider increasing cache TTLs or implementing cache warming');
        }
        return {
            period: {
                start: startDate,
