 ...
 * 
 * Ingests Google Drive files and folders into the shard system.
...
interface GDriveIngestionWorkerConfig {
...
export class GDriveIngestionWorker {
  private worker: Worker;
  private cosmosClient: CosmosClient;
  private monitoring: IMonitoringProvider;
  private config: GDriveIngestionWorkerConfig;
  private queueProducer: QueueProducerService;

  constructor(
    config: GDriveIngestionWorkerConfig,
    ...
    this.worker = new Worker(
      ...
      async (job: Job<IngestionEventMessage>) => {
        // Only process Google Drive events
        if (job.data.source === 'gdrive') {
          return this.processGDriveIngestion(job);
        }
        // Skip non-GDrive events
        return;
      },
      {
        connection: redis,
        concurrency: parseInt(process.env.GDRIVE_INGESTION_CONCURRENCY || '3', 10),
        removeOnComplete: {
          age: 24 * 3600,
          count: 1000,
        },
    ...
    this.worker.on('completed', (job) => {
      if (job.data.source === 'gdrive') {
        this.monitoring.trackEvent('gdrive-ingestion-worker.completed', {
          jobId: job.id,
  ...
  private async processGDriveIngestion(
    ...
    try {
      this.monitoring.trackEvent('gdrive-ingestion-worker.started', {
        jobId: job.id,
        tenantId,
        sourceId,
        eventType,
      ...
      if (!normalizedShard) {
        this.monitoring.trackEvent('gdrive-ingestion-worker.normalization_skipped', {
          jobId: job.id,
          tenantId,
          sourceId,
      ...
      this.monitoring.trackException(error as Error, {
        context: 'GDriveIngestionWorker.processGDriveIngestion',
        jobId: job.id,
        tenantId,
        sourceId,
        duration,
