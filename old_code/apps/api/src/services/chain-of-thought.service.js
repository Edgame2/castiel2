...
export class ChainOfThoughtService {
    ...
    monitoring;
    config;
    constructor(llmService, monitoring, config) {
    ...
    async reason(request) {
        ...
        try {
            ...
            for (let i = 0; i < Math.min(subQuestions.length, maxSteps - 2); i++) {
                ...
                const analysisStep = await this.analyzeSubQuestion(subQ.question, subQ.type, request.context, steps.length + 1);
                steps.push(analysisStep);
                analyses.push(analysisStep.output);
            }
            // Synthesis step
            const synthesis = await this.synthesize(request.query, analyses, steps.length + 1);
            ...
            if (this.config.enableVerification && steps.length < maxSteps) {
                const verification = await this.verify(request.query, synthesis.output, request.context, steps.length + 1);
    ...
    async analyzeSubQuestion(question, type, context, stepNumber) {
        const startTime = Date.now();
    ...
    async synthesize(originalQuery, analyses, stepNumber) {
        const startTime = Date.now();
    ...
    async verify(query, answer, context, stepNumber) {
        const startTime = Date.now();
    ...
    buildReasoningExplanation(steps) {
        const lines = ['## Reasoning Process\n'];
    ...
    createSimpleResponse(request, steps, startTime) {
        return {
            originalQuery: request.query,
            steps,
    ...
    createErrorResponse(request, steps, startTime, error) {
        return {
            originalQuery: request.query,
            steps,
...
// Factory
// ============================================
export function createChainOfThoughtService(llmService, monitoring, config) {
    return new ChainOfThoughtService(llmService, monitoring, config);
}
//# sourceMappingURL=chain-of-thought.service.js.map