...
import { Shard, ACLEntry, PermissionLevel } from '../types/shard.types.js';
...
export class ACLService implements IACLService {
  ...
  constructor(
    ...
    private readonly aclCacheService: ACLCacheService | null,
  ...
  async checkPermission(context: PermissionCheckContext): Promise<ACLCheckResult> {
    ...
    try {
      ...
      if (this.aclCacheService) {
        const cached = await this.aclCacheService.getCachedPermissions(
          context.userId,
          context.shardId,
          context.tenantId
        );
      ...
      if (this.aclCacheService && result.source !== 'cache') {
        const cacheEntry: ACLCacheEntry = {
  ...
  async batchCheckPermissions(request: ACLBatchCheckRequest): Promise<ACLBatchCheckResult> {
    const startTime = Date.now();
    const results = new Map<string, ACLCheckResult>();
    let cacheHits = 0;
    let cacheMisses = 0;

    try {
      // Try to get cached results first
      let cachedResults = new Map<string, ACLCacheEntry>();
      if (this.aclCacheService) {
        cachedResults = await this.aclCacheService.batchGetCachedPermissions(
          request.userId,
          request.shardIds,
          request.tenantId
        );
  ...
  async revokePermission(input: RevokePermissionInput): Promise<void> {
    ...
    try {
      ...
      if (!input.permissions || input.permissions.length === 0) {
        ...
        const remainingPermissions = entry.permissions.filter(
   ...
   */
  async updateACL(input: UpdateACLInput): Promise<void> {
    const startTime = Date.now();
    ...
    try {
      ...
      if (input.addEntries) {
        for (const entry of input.addEntries) {
          const existingIndex = shard.acl.findIndex(
            (e) =>
        ...
        for (const entry of input.removeEntries) {
          ...
          if (existingIndex >= 0) {
            if (!entry.permissions || entry.permissions.length === 0) {
              ...
              const existing = shard.acl[existingIndex];
              const remainingPermissions = existing.permissions.filter(
                (p) => !entry.permissions!.includes(p)
              );
   ...
   */
  async invalidateCache(userId: string, shardId: string, tenantId: string): Promise<void> {
    if (!this.aclCacheService) {return;}
   ...
   */
  async invalidateShardCache(shardId: string, tenantId: string): Promise<void> {
    if (!this.aclCacheService) {return;}
  ...
  getStats(): ACLStats {
    const avgDuration =
      this.stats.totalChecks > 0 ? this.stats.totalDuration / this.stats.totalChecks : 0;
