...
export class AIModelSelectionService {
    ...
    monitoring;
    aiConfigService;
    constructor(aiConnectionService, modelRouter, monitoring, aiConfigService) {
    ...
    async selectModel(request) {
        ...
        try {
            ...
            const scored = await this.scoreConnections(connections.connections, complexity, request);
            if (scored.length === 0) {
                return await this.handleNoConnections(request);
            }
            // 6. Select best connection
            const best = scored[0];
            const connectionWithCreds = await this.aiConnectionService.getConnectionWithCredentials(best.connection.id);
     ...
     */
    async recordModelPerformance(modelId, metrics) {
        try {
     ...
     */
    async selectExplicitModel(request) {
     ...
     */
    async selectByContentType(request, modelType) {
     ...
     */
    async scoreConnections(connections, complexity, request) {
     ...
     */
    getCostScore(model, request, config) {
        ...
        if (preferQuality === 'economy') {
            ...
            // Balanced
            if (model.qualityTier === 'economy') {
                return 20;
            }
            else if (model.qualityTier === 'standard') {
                return 30;
            }
            else {
                return 20;
     ...
     */
    getCapabilityScore(model, request) {
        let score = 0;
        const maxScore = 30;
     ...
     */
    getComplexityFit(tier, complexity, thresholds) {
     ...
     */
    async getPerformanceScore(modelId, config) {
        if (!config.enabled) {
            return 0;
        }
        try {
     ...
     */
    mapContentTypeToModelType(contentType) {
        switch (contentType) {
     ...
     */
    async handleNoConnections(request) {
        return {
            success: false,
            error: 'NO_CONNECTIONS',
     ...
     */
    async handleContentTypeUnavailable(request, requestedType) {
        // Get available types
...
//# sourceMappingURL=ai-model-selection.service.js.map