/**
 * Configuration loader for performance-optimization module
...
let cachedConfig: PerformanceOptimizationConfig | null = null;
 ...
 */
export function loadConfig(): PerformanceOptimizationConfig {
  if (cachedConfig) {
    return cachedConfig;
  }
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as PerformanceOptimizationConfig;
  
  let envConfig: Partial<PerformanceOptimizationConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<PerformanceOptimizationConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as PerformanceOptimizationConfig;
