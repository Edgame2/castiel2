...
export class ManagerDashboardService {
    ...
    constructor(monitoring, teamService, opportunityService, quotaService, pipelineAnalyticsService, revenueAtRiskService, userManagementService) {
     ...
     */
    async getManagerDashboard(managerId, tenantId, options = {}) {
        ...
        try {
            ...
            const teamUserMap = new Map(); // teamId -> userIds
            ...
            const opportunityPromises = teams.map(team => this.opportunityService.listTeamOpportunities(team.id, tenantId, managerId, {}, { limit: 1000 }));
            ...
            const uniqueOpportunities = Array.from(new Map(allOpportunities
                .filter(opp => opp?.id) // Filter out any opportunities without IDs
                .map(opp => [opp.id, opp])).values());
            ...
            const riskMetrics = await this.calculateRiskMetrics(uniqueOpportunities, tenantId, managerId);
            // Get quotas
            const quotaMetrics = await this.getQuotaMetrics(teams, Array.from(allUserIds), tenantId, managerId);
            // Calculate close won/lost
            const closedWonLost = await this.calculateClosedWonLost(Array.from(allUserIds), tenantId, options.period);
            // Build team summaries
            const teamSummaries = await Promise.all(teams.map(team => this.buildTeamSummary(team, teamUserMap.get(team.id) || [], tenantId, managerId)));
            // Build team member summaries
            const teamMemberSummaries = await this.buildTeamMemberSummaries(Array.from(allUserIds), uniqueOpportunities, tenantId);
            const dashboard = {
                managerId,
                tenantId,
                teams: teamSummaries,
     ...
     * Calculate opportunity metrics
     */
    calculateOpportunityMetrics(opportunities) {
        let totalValue = 0;
     ...
     * Calculate stage summary
     */
    calculateStageSummary(opportunities) {
     ...
     * Calculate risk metrics
     */
    async calculateRiskMetrics(opportunities, tenantId, userId) {
        let totalRevenueAtRisk = 0;
        let highRisk = 0;
        let mediumRisk = 0;
        let lowRisk = 0;
     ...
     * Get quota metrics
     */
    async getQuotaMetrics(teams, userIds, tenantId, managerId) {
        ...
        const quotaPerformancePromises = relevantQuotas.map(async (quota) => {
            try {
                const performance = await this.quotaService.calculatePerformance(quota.id, tenantId, managerId);
     ...
     */
    async calculateClosedWonLost(userIds, tenantId, period) {
        ...
        const metricsPromises = validUserIds.map(async (userId) => {
            try {
                return await this.pipelineAnalyticsService.calculateClosedWonLost(userId, tenantId, { startDate, endDate });
            }
            catch (error) {
     ...
     * Build team summary
     */
    async buildTeamSummary(team, userIds, tenantId, managerId) {
        // Get team opportunities
        const teamOpps = await this.opportunityService.listTeamOpportunities(team.id, tenantId, managerId, {}, { limit: 1000 });
        ...
        const riskMetrics = await this.calculateRiskMetrics(opportunities, tenantId, managerId);
        ...
        try {
            ...
            if (quotas.length > 0) {
                const performance = await this.quotaService.calculatePerformance(quotas[0].id, tenantId, managerId);
     ...
     * Build team member summaries
     */
    async buildTeamMemberSummaries(userIds, allOpportunities, tenantId) {
     ...
     */
    async getUserQuotaAttainment(userId, tenantId) {
        try {
            const quotas = await this.quotaService.listQuotas(tenantId, {
                targetUserId: userId,
            ...
            if (quotas.length > 0) {
                const performance = await this.quotaService.calculatePerformance(quotas[0].id, tenantId, userId);
     ...
     */
    createEmptyDashboard(managerId, tenantId, options) {
        const now = new Date();
