  ...
  await fastify.register(swagger, {
    openapi: {
      ...
      info: {
        ...
        description: 'Specialized AI agent management service',
        version: '1.0.0',
      },
      servers: [{ url: '/api/v1', description: 'API Version 1' }],
      tags: [
        { name: 'Agents', description: 'Agent management' },
        { name: 'Agent Selection', description: 'Agent selection for tasks' },
        { name: 'Agent Execution', description: 'Agent execution and tracking' },
...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Agent Registry listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
