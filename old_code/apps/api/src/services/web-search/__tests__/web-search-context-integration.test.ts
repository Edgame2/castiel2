...
import { IntentAnalysisResult, AssembledContext } from '../../../types/ai-insights.types';
import { WebPageDocument, SemanticChunk } from '../types';

describe('WebSearchContextIntegrationService - Integration Tests', () => {
    let service: WebSearchContextIntegrationService;
    ...
    beforeEach(() => {
        ...
        service = new WebSearchContextIntegrationService(
            mockWebSearchService,
            mockEmbeddingService,
            mockContainer
        );
    ...
    describe('Auto-Trigger Detection', () => {
        it('should trigger web search for "search" intent type', async () => {
            const intent: IntentAnalysisResult = createMockIntent('search', 0.9);
            const baseContext = createMockContext();

            const result = await service.integrateWebSearchContext(
                testTenantId,
                testProjectId,
                intent,
                testQuery,
        ...
        it('should trigger web search for high-confidence intent with web keywords', async () => {
            const intent: IntentAnalysisResult = createMockIntent('analysis', 0.85);
        ...
        it('should trigger web search for current information queries', async () => {
            const intent: IntentAnalysisResult = createMockIntent('analysis', 0.8);
        ...
        it('should NOT trigger web search for low-confidence intent', async () => {
            const intent: IntentAnalysisResult = createMockIntent('summary', 0.5);
            const baseContext = createMockContext();
        ...
        it('should NOT trigger web search for internal-only queries', async () => {
            const intent: IntentAnalysisResult = createMockIntent('summary', 0.8);
    ...
    describe('Cached Page Retrieval', () => {
        ...
        it('should trigger new web search if no cached pages found', async () => {
            ...
            expect(mockWebSearchService.search).toHaveBeenCalledWith(
                testTenantId,
                expect.any(String),
                expect.objectContaining({ useCache: true, type: 'web' })
            );
        });
        ...
        it('should use custom max cache age when provided', async () => {
            ...
            await service.integrateWebSearchContext(
                testTenantId,
                testProjectId,
                intent,
                testQuery,
                baseContext,
                { maxCacheAge: 12 } // 12 hours
            );

            const queryCall = (mockContainer.items.query as any).mock.calls[0][0];
    ...
    describe('Semantic Retrieval', () => {
        it('should retrieve and rank chunks by relevance score', async () => {
            ...
            for (let i = 0; i < result.ragChunks.length - 1; i++) {
                expect(result.ragChunks[i].score).toBeGreaterThanOrEqual(
                    result.ragChunks[i + 1].score
                );
            }
        });
        ...
        it('should filter chunks below minimum relevance score', async () => {
            ...
            const result = await service.integrateWebSearchContext(
                ...
                testQuery,
                baseContext,
                { minRelevanceScore: 0.8 } // High threshold
            );
    ...
    describe('Vector Similarity Calculation', () => {
        ...
        it('should handle different embedding dimensions correctly', async () => {
            ...
            await expect(
                service.integrateWebSearchContext(
                    testTenantId,
                    testProjectId,
                    intent,
                    testQuery,
        ...
        it('should calculate average relevance score correctly', async () => {
            ...
            if (result.ragChunks.length > 0) {
                const manualAvg =
                    result.ragChunks.reduce((sum, chunk) => sum + chunk.score, 0) /
    ...
    describe('Deep Search Integration', () => {
        it('should enable deep search when requested', async () => {
            ...
            await service.integrateWebSearchContext(
                ...
                testQuery,
                baseContext,
                { enableDeepSearch: true, deepSearchPages: 5 }
            );
    ...
    describe('Error Handling', () => {
        it('should handle web search service failures gracefully', async () => {
            mockWebSearchService.search = vi.fn().mockRejectedValue(
        ...
        it('should handle embedding service failures gracefully', async () => {
            mockEmbeddingService.embed = vi.fn().mockRejectedValue(
        ...
        it('should handle pages with no chunks gracefully', async () => {
            const cachedPages: WebPageDocument[] = [
    ...
    describe('Metadata Tracking', () => {
        ...
        it('should format metadata for logging correctly', () => {
            ...
            const formatted = service.formatMetadataForLogging(result as any);
    ...
    function createMockChunk(id: string, content: string): SemanticChunk {
        return {
            id,
            content,
            startPosition: 0,
    ...
    function createMockWebPage(id: string, chunks: SemanticChunk[]): WebPageDocument {
        return {
            id,
    ...
    function createMockWebPages(count: number): WebPageDocument[] {
        return Array.from({ length: count }, (_, i) => {
            const chunks = Array.from({ length: 5 }, (_, j) =>
                createMockChunk(`chunk-${i}-${j}`, `This is test content for chunk ${j} about AI agents and trends.`)
            );
            return createMockWebPage(`page-${i}`, chunks);
        });
    }

    function createMockWebPagesWithDuplicates(): WebPageDocument[] {
