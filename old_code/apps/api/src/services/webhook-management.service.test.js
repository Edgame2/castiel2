...
describe('WebhookManagementService', () => {
    ...
    beforeEach(() => {
        ...
        service = new WebhookManagementService(mockConnectionService, mockSyncService, mockMonitoring, mockCache, 'https://api.example.com/webhooks');
    });
    afterEach(() => {
        vi.clearAllMocks();
    });
    describe('registerWebhook', () => {
        it('should register a webhook successfully', async () => {
            ...
            const registration = await service.registerWebhook(tenantId, integrationId, connectionId, events);
            ...
            expect(mockConnectionService.registerWebhookWithProvider).toHaveBeenCalledWith(connectionId, expect.stringContaining('https://api.example.com/webhooks/'));
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('webhook_registered', expect.any(Object));
        });
        it('should handle webhook registration failures', async () => {
            mockConnectionService.registerWebhookWithProvider.mockRejectedValue(new Error('Provider error'));
            const promise = service.registerWebhook('tenant-123', 'salesforce', 'conn-456', ['contact.created']);
            await expect(promise).rejects.toThrow('Provider error');
            expect(mockMonitoring.trackException).toHaveBeenCalledWith(expect.any(Error), expect.any(Object));
        });
        it('should support custom retry policy', async () => {
            ...
            const registration = await service.registerWebhook('tenant-123', 'salesforce', 'conn-456', ['contact.created'], { retryPolicy });
    ...
    describe('unregisterWebhook', () => {
        it('should unregister a webhook successfully', async () => {
            ...
            const registration = await service.registerWebhook('tenant-123', 'salesforce', 'conn-456', ['contact.created']);
            // Then unregister
            ...
            expect(mockConnectionService.unregisterWebhookWithProvider).toHaveBeenCalledWith(registration.connectionId, registration.providerWebhookId);
            expect(mockCache.delete).toHaveBeenCalled();
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('webhook_unregistered', expect.any(Object));
        });
    ...
    describe('processWebhookEvent', () => {
        it('should process valid webhook event with HMAC signature', async () => {
            ...
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('webhook_event_processed', expect.any(Object));
        });
        ...
        it('should handle Notion webhook signature verification', async () => {
            ...
            const registration = await service.registerWebhook('tenant-123', 'notion', 'conn-456', ['page.created']);
        ...
        it('should handle GitHub webhook signature verification', async () => {
            ...
            const registration = await service.registerWebhook('tenant-123', 'github', 'conn-456', ['push']);
        ...
        it('should skip processing if webhook is inactive', async () => {
            ...
            let registration = await service.registerWebhook('tenant-123', 'salesforce', 'conn-456', ['contact.created']);
            // Manually mark as inactive (in real scenario, would happen via API)
            registration.status = 'inactive';
            const bodyStr = JSON.stringify({ action: 'created' });
            const result = await service.processWebhookEvent(registration.id, {}, bodyStr);
            expect(result.success).toBe(false);
    ...
    describe('listRegistrations', () => {
        it('should list all registrations for tenant', async () => {
            ...
            const reg1 = await service.registerWebhook('tenant-123', 'salesforce', 'conn-456', ['contact.created']);
            const reg2 = await service.registerWebhook('tenant-123', 'notion', 'conn-789', ['page.created']);
        ...
        it('should filter registrations by integrationId', async () => {
            ...
            await service.registerWebhook('tenant-123', 'salesforce', 'conn-456', ['contact.created']);
            await service.registerWebhook('tenant-123', 'notion', 'conn-789', ['page.created']);
        ...
        it('should filter registrations by connectionId', async () => {
            ...
            const reg = await service.registerWebhook('tenant-123', 'salesforce', 'conn-456', ['contact.created']);
            const registrations = await service.listRegistrations('tenant-123', undefined, 'conn-456');
            expect(registrations).toHaveLength(1);
    ...
    describe('caching', () => {
        it('should cache webhook registrations', async () => {
            ...
            const registration = await service.registerWebhook('tenant-123', 'salesforce', 'conn-456', ['contact.created']);
            expect(mockCache.set).toHaveBeenCalledWith(expect.stringContaining(registration.id), registration, 5 * 60 * 1000 // 5 minutes
            );
        });
...
//# sourceMappingURL=webhook-management.service.test.js.map