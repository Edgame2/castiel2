...
export function requireTenantAdmin() {
    return async (request, _reply) => {
        ...
        const isAdmin = isGlobalAdmin(user) ||
            user.roles?.some(role => ['admin', 'tenant_admin', 'tenant-admin'].includes(role.toLowerCase()));
        if (!isAdmin) {
 ...
 */
export function createSessionIdleTimeoutMiddleware(sessionService, tenantService) {
    return async (request, reply) => {
        ...
        try {
            ...
            const idleCheck = await sessionService.checkSessionIdleTimeout(user.tenantId, user.id, sessionId, idleTimeoutMinutes);
            if (!idleCheck.valid) {
                ...
                if (requireReauth) {
                    ...
                    throw new UnauthorizedError(`Session expired due to inactivity (idle for ${idleCheck.idleMinutes} minutes). Please log in again.`);
                }
            }
 ...
 */
export function createIPAllowlistMiddleware(tenantService) {
    return async (request, _reply) => {
        if (!isAuthenticated(request)) {
            return; // Let requireAuth handle this
        }
        ...
        try {
            ...
            if (!isAllowed) {
                throw new ForbiddenError(`Access denied. Your IP address (${clientIP}) is not in the allowed list for this organization.`);
            }
        }
        catch (error) {
