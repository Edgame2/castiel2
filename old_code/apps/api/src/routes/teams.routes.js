 ...
 */
async function verifyTeamAccess(teamService, userId, teamId, tenantId, userRoles) {
 ...
 */
export async function registerTeamRoutes(server, options) {
    const { monitoring, shardRepository, shardTypeRepository, relationshipService, auditLogService, } = options;
    // Initialize service
    ...
    server.post('/api/v1/teams', {
        ...
        try {
            ...
            const team = await teamService.createTeam(input, authRequest.user.tenantId, authRequest.user.id);
            return reply.code(201).send(team);
        }
        catch (error) {
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.createTeam', statusCode });
    ...
    server.get('/api/v1/teams/:teamId', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const hasAccess = await verifyTeamAccess(teamService, authRequest.user.id, teamId.trim(), authRequest.user.tenantId, authRequest.user.roles);
            if (!hasAccess) {
            ...
            const statusCode = getErrorStatusCode(error);
            const errorMessage = error instanceof Error ? error.message : String(error);
            monitoring.trackException(error instanceof Error ? error : new Error(errorMessage), { operation: 'teams.getTeam', statusCode });
    ...
    server.get('/api/v1/teams', {
        ...
        try {
            ...
            if (query.externalSource && typeof query.externalSource === 'string') {
                filters.externalSource = query.externalSource;
            }
            ...
            const teams = await teamService.getTeams(authRequest.user.tenantId, filters);
            return reply.send(teams);
        }
        catch (error) {
            const statusCode = getErrorStatusCode(error);
            const errorMessage = error instanceof Error ? error.message : String(error);
            monitoring.trackException(error instanceof Error ? error : new Error(errorMessage), { operation: 'teams.getTeams', statusCode });
            return reply.code(statusCode).send({ error: errorMessage || 'Failed to list teams' });
        }
    });
    // PUT /api/v1/teams/:teamId - Update team
    server.put('/api/v1/teams/:teamId', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const isManager = await teamService.isUserManagerOfTeam(authRequest.user.id, teamId.trim(), authRequest.user.tenantId);
            ...
            const team = await teamService.updateTeam(teamId.trim(), input, authRequest.user.tenantId, authRequest.user.id);
            return reply.send(team);
        }
        catch (error) {
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.updateTeam', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to update team' });
        }
    });
    // DELETE /api/v1/teams/:teamId - Delete team
    server.delete('/api/v1/teams/:teamId', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.deleteTeam', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to delete team' });
        }
    });
    // ===============================================
    ...
    server.get('/api/v1/teams/:teamId/parent', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            // Verify user has access to the team
            const isMember = await teamService.isUserMemberOfTeam(authRequest.user.id, teamId.trim(), authRequest.user.tenantId);
            const isManager = await teamService.isUserManagerOfTeam(authRequest.user.id, teamId.trim(), authRequest.user.tenantId);
            const isAdmin = authRequest.user.roles?.includes('admin') ||
                authRequest.user.roles?.includes('super_admin');
            ...
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.getParentTeam', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to get parent team' });
        }
    });
    // GET /api/v1/teams/:teamId/children - Get child teams
    server.get('/api/v1/teams/:teamId/children', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.getChildTeams', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to get child teams' });
        }
    });
    // GET /api/v1/teams/:teamId/descendants - Get all descendant teams
    server.get('/api/v1/teams/:teamId/descendants', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.getDescendantTeams', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to get descendant teams' });
        }
    });
    // ===============================================
    // TEAM MEMBERS ROUTES
    ...
    server.get('/api/v1/teams/:teamId/members', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.getTeamMembers', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to get team members' });
        }
    });
    // GET /api/v1/teams/:teamId/manager - Get team manager
    server.get('/api/v1/teams/:teamId/manager', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.getTeamManager', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to get team manager' });
        }
    });
    // GET /api/v1/teams/:teamId/user-ids - Get all user IDs in team
    server.get('/api/v1/teams/:teamId/user-ids', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.getTeamUserIds', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to get team user IDs' });
        }
    });
    // GET /api/v1/teams/:teamId/user-ids/recursive - Get all user IDs including descendants
    server.get('/api/v1/teams/:teamId/user-ids/recursive', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.getTeamUserIdsRecursive', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to get team user IDs recursively' });
        }
    });
    // ===============================================
    ...
    server.get('/api/v1/users/:userId/teams', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.getUserTeams', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to get user teams' });
        }
    });
    // GET /api/v1/users/:userId/team - Get primary team for a user
    server.get('/api/v1/users/:userId/team', {
        onRequest: authGuards,
        schema: {
            tags: ['Teams'],
        ...
        try {
            ...
            const statusCode = getErrorStatusCode(error);
            monitoring.trackException(error instanceof Error ? error : new Error(String(error)), { operation: 'teams.getTeamForUser', statusCode });
            return reply.code(statusCode).send({ error: error.message || 'Failed to get team for user' });
        }
    });
}
