...
describe('Authorization Security Tests', () => {
    ...
    describe('Token Validation', () => {
        it('should reject tokens with invalid signature', async () => {
            ...
            const invalidToken = jwt.sign({ userId: 'user-123', tenantId: 'tenant-123' }, 'wrong-secret', { expiresIn: '1h' });
        ...
        it('should reject expired tokens', async () => {
            ...
            const expiredToken = jwt.sign({ userId: 'user-123', tenantId: 'tenant-123' }, secret, { expiresIn: '-1h' } // Expired
            );
            const mockRequest = createMockRequest({
                headers: {
        ...
        it('should reject tokens with missing required claims', async () => {
            ...
            const invalidToken = jwt.sign({ tenantId: 'tenant-123' }, secret, { expiresIn: '1h' });
            const mockRequest = createMockRequest({
                headers: {
                    authorization: `Bearer ${invalidToken}`,
                },
        ...
        it('should reject tampered tokens', async () => {
            // Create valid token
            const validToken = jwt.sign({ userId: 'user-123', tenantId: 'tenant-123' }, secret, { expiresIn: '1h' });
            // Tamper with token
    ...
    describe('Tenant Isolation', () => {
        it('should enforce tenant isolation in user context', async () => {
            ...
            const token1 = jwt.sign({ userId: 'user-123', tenantId: 'tenant-1' }, secret, { expiresIn: '1h' });
            const token2 = jwt.sign({ userId: 'user-456', tenantId: 'tenant-2' }, secret, { expiresIn: '1h' });
        ...
        it('should reject requests with mismatched tenant context', async () => {
            ...
            const token = jwt.sign({ userId: 'user-123', tenantId: 'tenant-1' }, secret, { expiresIn: '1h' });
            const mockRequest = {
                headers: {
...
//# sourceMappingURL=authorization.security.test.js.map