...
import type { Shard, VectorEmbedding } from '../types/shard.types.js';
import { AzureOpenAIService } from '@castiel/api-core';
import { VectorSearchCacheService } from './vector-search-cache.service.js';
import {
  extractTextFromShard,
  chunkText,
  combineChunks,
  prepareTextForEmbedding,
  calculateChunkingStats,
} from '../utils/text-processing.utils.js';
...
const DEFAULT_QUEUE_CONFIG: VectorizationQueueConfig = {
...
export class VectorizationService {
  private queueConfig: VectorizationQueueConfig;
  private processingJobs = new Map<string, VectorizationJob>();
  private retryTimeouts = new Map<string, NodeJS.Timeout>(); // Track retry timeouts for cleanup
  ...
  constructor(
    ...
    queueConfig?: Partial<VectorizationQueueConfig>,
    private readonly shardEmbeddingService?: ShardEmbeddingService, // Optional - if provided, use template-based embedding
    private readonly shardRepository?: ShardRepository, // Optional - needed for template-based embedding
    private readonly shardTypeRepository?: ShardTypeRepository // Optional - needed for template-based embedding
  ) {
  ...
  async vectorizeShard(request: VectorizeShardRequest): Promise<VectorizationStatusResponse> {
    ...
    try {
      ...
      if (validationErrors.length > 0) {
        throw new VectorizationError(
          `Invalid vectorization config: ${validationErrors.join(', ')}`,
      ...
      throw new VectorizationError(
        `Failed to create vectorization job: ${error.message}`,
        VectorizationErrorCode.UNKNOWN_ERROR,
        500,
        error
      );
    }
      ...
      try {
        ...
        if (shard) {
          ...
          job.result = {
            ...
            executionTimeMs: result.processingTimeMs || 0,
          };
          await this.saveJob(job);
      ...
      if (this.shardEmbeddingService && this.shardRepository && this.shardTypeRepository) {
        ...
        try {
          ...
          if (updatedShard && updatedShard.vectors) {
            ...
            throw new VectorizationError(
              'Failed to retrieve vectors after generation',
              VectorizationErrorCode.UNKNOWN_ERROR,
              500
            );
          }
          ...
          throw new VectorizationError(
            'Template-based embedding generation failed and no fallback available',
            VectorizationErrorCode.UNKNOWN_ERROR,
            500
          );
        }
        ...
        throw new VectorizationError(
          'ShardEmbeddingService not available',
          VectorizationErrorCode.UNKNOWN_ERROR,
          500
        );
      }
   ...
   */
  async getJobStatus(jobId: string): Promise<VectorizationStatusResponse | null> {
    const job = await this.getJob(jobId);
    if (!job) {
      return null;
    }
  ...
  async batchVectorize(request: BatchVectorizeRequest): Promise<BatchVectorizeResponse> {
    ...
    try {
      ...
      throw new VectorizationError(
        `Failed to create batch vectorization: ${error.message}`,
        VectorizationErrorCode.UNKNOWN_ERROR,
        500,
        error
      );
   ...
   */
  private calculateProgress(job: VectorizationJob): number {
    switch (job.status) {
  ...
  private isRetryableError(error: any): boolean {
    if (error instanceof VectorizationError) {
      return (
        error.code === VectorizationErrorCode.EMBEDDING_API_RATE_LIMIT ||
   ...
   */
  private async updateShard(shard: Shard): Promise<void> {
   ...
   */
  private async queryShardIds(
    tenantId: string,
    filter?: BatchVectorizeRequest['filter']
  ): Promise<string[]> {
    let query = 'SELECT c.id FROM c WHERE c.tenantId = @tenantId';
    const parameters: Array<{ name: string; value: any }> = [{ name: '@tenantId', value: tenantId }];
   ...
   * Save job to Redis
   */
  private async saveJob(job: VectorizationJob): Promise<void> {
   ...
   * Get job from Redis
   */
  private async getJob(jobId: string): Promise<VectorizationJob | null> {
   ...
   */
  private async invalidateCaches(tenantId: string, shardId: string): Promise<void> {
    try {
