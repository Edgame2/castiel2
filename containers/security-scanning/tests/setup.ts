...
cosmos_db:
  ...
  containers:
    ...
    pii_detections: security_pii_detections
    device_tracking: security_device_tracking
jwt:
...
vi.mock('yaml', () => ({
  parse: vi.fn((content: string) => {
    const config: any = {
      module: { name: 'security-scanning', version: '1.0.0' },
      server: { port: 3054, host: '0.0.0.0' },
      cosmos_db: {
        endpoint: process.env.COSMOS_DB_ENDPOINT,
        key: process.env.COSMOS_DB_KEY,
        database_id: process.env.COSMOS_DB_DATABASE_ID,
...
vi.mock('@coder/shared', () => {
  const clientInstance = {
    get: vi.fn().mockResolvedValue({ data: {}, structuredData: {} }),
