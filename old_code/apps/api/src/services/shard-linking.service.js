...
export class ShardLinkingService {
    ...
    constructor(shardRepository, monitoring, redis) {
        ...
        this.redis = redis;
        this.logger = {
            log: (message) => monitoring.trackEvent('shard-linking.log', { message }),
            error: (message) => monitoring.trackException(new Error(message), { operation: 'shard-linking' }),
            warn: (message) => monitoring.trackEvent('shard-linking.warn', { message }),
        };
    ...
    get cosmosDB() {
        return {
            queryDocuments: async (container, query, parameters, tenantId) => {
                return this.queryDocuments(query, parameters);
            },
            upsertDocument: async (container, document, tenantId) => {
     ...
     */
    async createLink(tenantId, projectId, input, createdByUserId) {
        try {
            // Validate input
            const validation = await this.validateLink(tenantId, projectId, input);
            if (!validation.isValid) {
                throw new Error(`Link validation failed: ${validation.errors.map((e) => e.message).join(', ')}`);
            }
            ...
            this.monitoring.trackException(error, { operation: 'shard-linking.createLink' });
            throw error;
        }
    }
    /**
     ...
     */
    async updateLink(tenantId, projectId, linkId, input, updatedByUserId) {
        try {
            ...
            this.monitoring.trackException(error, { operation: 'shard-linking.updateLink' });
            throw error;
        }
    }
    /**
     ...
     */
    async deleteLink(tenantId, projectId, linkId, deletedByUserId) {
        try {
            const link = await this.getLink(tenantId, projectId, linkId);
            if (!link) {
                throw new Error(`Link ${linkId} not found`);
            ...
            if (link.isBidirectional) {
                ...
                const { resources: reverseLinks } = await this.linksContainer.items.query({
                    query: reverseQuery,
                    parameters: [
            ...
            this.monitoring.trackException(error, { operation: 'shard-linking.deleteLink' });
            throw error;
        }
    }
    /**
     ...
     */
    async bulkCreateLinks(tenantId, input, createdByUserId) {
        try {
            const linkIds = [];
            const failures = [];
            for (let i = 0; i < input.links.length; i++) {
                try {
                    const link = await this.createLink(tenantId, input.projectId, input.links[i], createdByUserId);
     ...
     */
    async bulkCreateLinksMultiProject(tenantId, input, createdByUserId) {
        try {
            ...
            for (const [projectId, links] of linksByProject.entries()) {
                try {
                    const result = await this.bulkCreateLinks(tenantId, {
                        projectId,
                        links,
     ...
     */
    async getLink(tenantId, projectId, linkId) {
        try {
            ...
            const { resource: link } = await this.linksContainer.item(linkId, tenantId).read();
            ...
            this.monitoring.trackException(error, { operation: 'shard-linking.getLink' });
            return null;
        }
    }
    /**
     ...
     */
    async getLinks(tenantId, projectId, params = {}) {
        try {
            let query = `
      ...
      `;
            ...
            if (params.relationshipTypes && params.relationshipTypes.length > 0) {
                query += ` AND l.relationshipType IN (${params.relationshipTypes.map((_, i) => `@rel${i}`).join(',')})`;
                params.relationshipTypes.forEach((type, i) => {
            ...
            // Get total count
            const totalActivities = await this.cosmosDB.queryDocuments('project-shard-links', query + ` OFFSET 0 LIMIT 999999`, parameters, tenantId);
            // Apply pagination
            ...
            const links = await this.cosmosDB.queryDocuments('project-shard-links', paginatedQuery, parameters, tenantId);
     ...
     */
    async getShardWithLinks(tenantId, projectId, shardId) {
        try {
      ...
      `;
            const outgoingLinks = await this.cosmosDB.queryDocuments('project-shard-links', outgoingQuery, [
                { name: '@tenantId', value: tenantId },
                { name: '@projectId', value: projectId },
        ...
        ORDER BY l.priority ASC, l.createdAt DESC
      `;
            const incomingLinks = await this.cosmosDB.queryDocuments('project-shard-links', incomingQuery, [
                { name: '@tenantId', value: tenantId },
                { name: '@projectId', value: projectId },
                { name: '@shardId', value: shardId },
            ], tenantId);
     ...
     */
    async getLinkStatistics(tenantId, projectId) {
        try {
            ...
            const shardLinkCounts = new Map();
            Object.values(RelationshipType).forEach((type) => {
                byRelationshipType[type] = 0;
            });
            ...
            const mostLinkedShards = Array.from(shardLinkCounts.entries())
                ...
                direction: (stats.in > 0 && stats.out > 0 ? 'both' : stats.in > 0 ? 'in' : 'out'),
            }))
     ...
     */
    async validateLink(tenantId, projectId, input) {
        const errors = [];
        const warnings = [];
      ...
      `;
            try {
                const existing = await this.cosmosDB.queryDocuments('project-shard-links', existingQuery, [
                    { name: '@tenantId', value: tenantId },
                    { name: '@projectId', value: projectId },
     ...
     */
    async analyzeLinkImpact(tenantId, projectId, linkId) {
        try {
            const link = await this.getLink(tenantId, projectId, linkId);
            if (!link) {
                return {
      ...
      `;
            const relatedLinks = await this.cosmosDB.queryDocuments('project-shard-links', relatedQuery, [
                { name: '@tenantId', value: tenantId },
                { name: '@projectId', value: projectId },
                { name: '@fromShardId', value: link.fromShardId },
                { name: '@toShardId', value: link.toShardId },
            ...
            let riskLevel = 'low';
            if (link.relationshipType === RelationshipType.BLOCKING_TASK) {
                riskLevel = 'critical';
            }
            else if (link.relationshipType === RelationshipType.DEPENDS_ON) {
                riskLevel = 'high';
            }
            else if (link.relationshipType === RelationshipType.PARENT_OF) {
                riskLevel = 'high';
            }
            else if (link.relationshipType === RelationshipType.RELATED_CONTEXT) {
                riskLevel = 'low';
            }
            const analysis = {
     ...
     */
    async recordLinkAccess(tenantId, projectId, linkId) {
        try {
            const link = await this.getLink(tenantId, projectId, linkId);
            if (!link) {
                return;
     ...
     * Rate a link
     */
    async rateLink(tenantId, projectId, linkId, rating) {
        try {
     ...
     */
    async invalidateLinkCaches(tenantId, projectId) {
        const keysToDelete = [
            `links:${projectId}:*`,
            `shard-with-links:*`,
...
//# sourceMappingURL=shard-linking.service.js.map