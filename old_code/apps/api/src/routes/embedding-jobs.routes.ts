 ...
 */
export async function registerEmbeddingJobRoutes(
  server: FastifyInstance,
  monitoring: IMonitoringProvider
): Promise<void> {
  const repo = new EmbeddingJobRepository(monitoring)
  ...
  server.post<{
    ...
    '/api/v1/embedding-jobs/:id/retry',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const serviceBusService = (request.server as any).serviceBusService;
        if (serviceBusService) {
          const jobMessage: EmbeddingJobMessage = {
            shardId: job.shardId,
            tenantId: job.tenantId,
            shardTypeId: job.shardTypeId,
            dedupeKey: `retry-${job.id}-${Date.now()}`,
            revisionNumber: (job as any).revisionNumber || 0,
