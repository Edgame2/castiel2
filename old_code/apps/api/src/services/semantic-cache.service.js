...
export class SemanticCacheService {
    ...
    INDEX_PREFIX = 'ai:semantic:idx:';
    STATS_PREFIX = 'ai:semantic:stats:';
    constructor(redis, monitoring, embeddingService, config) {
        this.redis = redis;
     ...
     */
    async lookup(query, tenantId, context) {
        ...
        try {
            ...
            const result = await this.semanticSearch(tenantId, queryEmbedding, context);
    ...
    async store(query, response, metadata) {
        ...
        try {
            ...
            await this.redis.setex(key, this.config.ttlSeconds, JSON.stringify(entry));
     ...
     */
    async invalidate(tenantId, options) {
        try {
            if (options?.entryId) {
    ...
    async semanticSearch(tenantId, queryEmbedding, context) {
        const indexKey = this.getIndexKey(tenantId);
    ...
    async updateStats(tenantId, type, savings) {
        const statsKey = this.getStatsKey(tenantId);
        if (type === 'hit') {
    ...
    async enforceMaxEntries(tenantId) {
        const indexKey = this.getIndexKey(tenantId);
...
// Factory
// ============================================
export function createSemanticCacheService(redis, monitoring, embeddingService, config) {
