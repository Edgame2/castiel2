/**
 * ChainOfThoughtService (merged from llm-service – Plan W5 Layer 5)
...
export class ChainOfThoughtService {
  ...
  constructor() {
    ...
    this.containerName = this.config.cosmos_db.containers?.llm_outputs ?? 'llm_outputs';
  }
  ...
  async saveOutput(
    ...
    const llm = (this.config as { llm?: { provider?: string; model?: string; timeout_ms?: number } }).llm ?? { provider: 'mock', model: 'stub', timeout_ms: 10000 };
    const doc: LLMOutput = {
      id,
      tenantId,
      outputType,
      ...
      inputContext,
      output,
      llmProvider: llm.provider ?? 'mock',
      model: llm.model ?? 'stub',
      promptTemplate: '',
      latency,
      tokenCount: 0,
      generatedAt: now,
    ...
    try {
      ...
      await (container.items as { create: (doc: LLMOutput, opts?: { partitionKey: string }) => Promise<unknown> }).create(doc, { partitionKey: tenantId });
    } catch (error) {
      log.error('Failed to persist LLMOutput', error as Error, { service: 'ai-service', id, tenantId });
      throw error;
    }
  }
}
