/**
 * Configuration loader for risk-catalog module
 */
...
export interface RiskCatalogConfig {
  ...
  cosmos_db: { endpoint: string; key: string; database_id: string; containers: { main: string } };
  jwt: { secret: string };
...
let cachedConfig: RiskCatalogConfig | null = null;
...
export function loadConfig(): RiskCatalogConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as RiskCatalogConfig;
  let envConfig: Partial<RiskCatalogConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<RiskCatalogConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as RiskCatalogConfig;
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'risk-catalog' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'risk-catalog' });
  }
...
export function getConfig(): RiskCatalogConfig {
