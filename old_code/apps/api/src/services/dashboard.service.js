...
import { DashboardType, DashboardPermissionLevel, DEFAULT_FISCAL_YEAR_START, DEFAULT_FEATURE_FLAGS, } from '../types/dashboard.types.js';
/**
 * Dashboard Service
 */
export class DashboardService {
    ...
    monitoring;
    cacheService;
    globalConfig = DEFAULT_FEATURE_FLAGS;
    constructor(monitoring, cacheService) {
    ...
    async createDashboard(input) {
        ...
        try {
            ...
            const configTimeout = new Promise((resolve) => setTimeout(() => {
            ...
            this.monitoring.trackException(configError, { operation: 'dashboard.service.config-check' });
        ...
        const versionPromise = this.repository.saveDashboardVersion(dashboard.id, input.tenantId, 'widget_added', 'Dashboard created', input.userId).catch(err => {
            this.monitoring.trackException(err, { operation: 'dashboard.service.save-version' });
        });
        // Cache operations (non-blocking)
        if (this.cacheService) {
            Promise.all([
                this.cacheService.cacheDashboard(dashboard).catch(err => this.monitoring.trackException(err, { operation: 'dashboard.service.cache-dashboard' })),
                this.cacheService.invalidateMergedDashboard(input.tenantId, input.userId).catch(err => this.monitoring.trackException(err, { operation: 'dashboard.service.invalidate-cache' })),
            ]);
        }
     ...
     */
    async getDashboard(id, tenantId, userId) {
        // Try cache first
        if (this.cacheService) {
     ...
     */
    async getDashboardWithWidgets(id, tenantId, userId) {
     ...
     * Update dashboard
     */
    async updateDashboard(id, tenantId, userId, input) {
            ...
            const changeType = input.layout
                ...
                : input.settings
                    ...
                    : input.permissions
                        ? 'permissions_changed'
                        : 'widget_updated';
            await this.repository.saveDashboardVersion(id, tenantId, changeType, `Dashboard updated: ${Object.keys(input).join(', ')}`, userId);
            // Invalidate cache
            if (this.cacheService) {
     ...
     * Delete dashboard
     */
    async deleteDashboard(id, tenantId, userId) {
        const existing = await this.repository.getDashboard(id, tenantId);
        if (!existing) {
            return false;
        }
     ...
     */
    async getMergedDashboard(userId, tenantId, dashboardId, context, filters) {
        ...
        if (!primaryDashboard) {
            primaryDashboard = inheritedDashboards.find(d => d.structuredData.dashboardType === DashboardType.TENANT && d.structuredData.isDefault) || null;
        }
        ...
        if (primaryDashboard.structuredData.settings.showInheritedWidgets) {
            for (const dashboard of inheritedDashboards) {
                ...
                for (const widget of widgets) {
                    const override = overrides?.positionOverrides.find(o => o.widgetId === widget.id && o.sourceDashboardId === dashboard.id);
                    const isHidden = overrides?.hiddenWidgets.some(h => h.widgetId === widget.id && h.sourceDashboardId === dashboard.id) || false;
                    allWidgets.push({
                        widgetId: widget.id,
                        source: type,
        ...
        for (const widget of userWidgets) {
            const isHidden = overrides?.hiddenWidgets.some(h => h.widgetId === widget.id && h.sourceDashboardId === primaryDashboard.id) || false;
            allWidgets.push({
                widgetId: widget.id,
     ...
     */
    async setAsDefault(id, tenantId, userId) {
        const dashboard = await this.repository.getDashboard(id, tenantId);
        if (!dashboard) {
            return null;
        }
     ...
     * Duplicate dashboard
     */
    async duplicateDashboard(id, tenantId, userId, newName) {
    ...
    async createWidget(input) {
        ...
        // Save version
        await this.repository.saveDashboardVersion(input.dashboardId, input.tenantId, 'widget_added', `Widget added: ${input.name}`, input.userId);
     ...
     * Update widget
     */
    async updateWidget(id, tenantId, userId, input) {
        ...
        if (updated && (input.config || input.dataSource)) {
            await this.repository.saveDashboardVersion(widget.dashboardId, tenantId, 'widget_updated', `Widget updated: ${widget.structuredData.name}`, userId);
        }
     ...
     * Delete widget
     */
    async deleteWidget(id, tenantId, userId) {
        const widget = await this.repository.getWidget(id, tenantId);
        if (!widget) {
            return false;
        }
        ...
        if (result) {
            ...
            if (dashboard) {
                const newLayout = dashboard.structuredData.layout.desktop.filter((w) => w.widgetId !== id);
            ...
            // Save version
            await this.repository.saveDashboardVersion(widget.dashboardId, tenantId, 'widget_removed', `Widget removed: ${widget.structuredData.name}`, userId);
            // Invalidate widget and dashboard cache
            if (this.cacheService) {
                await this.cacheService.invalidateWidget(tenantId, id);
                await this.cacheService.invalidateDashboard(tenantId, widget.dashboardId);
     ...
     */
    async batchUpdatePositions(dashboardId, tenantId, userId, input) {
        ...
        if (dashboard) {
            const newLayout = dashboard.structuredData.layout.desktop.map((item) => {
        ...
        // Save version
        await this.repository.saveDashboardVersion(dashboardId, tenantId, 'layout_changed', `Layout updated: ${input.positions.length} widgets repositioned`, userId);
     ...
     */
    async hideWidget(userId, tenantId, dashboardId, widgetId, sourceDashboardId) {
     ...
     */
    async showWidget(userId, tenantId, dashboardId, widgetId, sourceDashboardId) {
        const overrides = await this.repository.getUserOverrides(userId, dashboardId, tenantId);
        if (!overrides) {
            return;
        }
        overrides.hiddenWidgets = overrides.hiddenWidgets.filter(h => !(h.widgetId === widgetId && h.sourceDashboardId === sourceDashboardId));
        await this.repository.saveUserOverrides({ ...overrides, tenantId });
    }
    /**
     * Override inherited widget position
     */
    async overrideWidgetPosition(userId, tenantId, dashboardId, widgetId, sourceDashboardId, position) {
        const overrides = await this.repository.getUserOverrides(userId, dashboardId, tenantId) || {
            userId,
            dashboardId,
            hiddenWidgets: [],
        ...
        const existingIndex = overrides.positionOverrides.findIndex(p => p.widgetId === widgetId && p.sourceDashboardId === sourceDashboardId);
        if (existingIndex >= 0) {
     ...
     */
    async saveFilterState(userId, tenantId, dashboardId, filterState) {
        const overrides = await this.repository.getUserOverrides(userId, dashboardId, tenantId) || {
            userId,
            dashboardId,
            hiddenWidgets: [],
     ...
     */
    async getDashboardVersions(dashboardId, tenantId) {
        return this.repository.getDashboardVersions(dashboardId, tenantId);
    }
    /**
     * Rollback to version
     */
    async rollbackToVersion(dashboardId, tenantId, userId, targetVersion) {
        ...
        if (updated) {
            await this.repository.saveDashboardVersion(dashboardId, tenantId, 'rollback', `Rolled back to version ${targetVersion}`, userId);
        }
        return updated;
    }
    // ============================================================================
        ...
        try {
            ...
            if (this.cacheService) {
                try {
                    ...
                    const timeoutPromise = new Promise((resolve) => setTimeout(() => resolve(null), 5000) // 5 second timeout for cache
                    );
                    ...
                    this.monitoring.trackException(cacheError, {
            ...
            if (this.cacheService) {
                this.cacheService.cacheTenantConfig(tenantConfig).catch((err) => {
                    this.monitoring.trackException(err, { operation: 'dashboard.service.cache-tenant-config' });
                });
            }
            ...
            this.monitoring.trackException(error, { operation: 'dashboard.service.get-effective-config', tenantId });
            return this.globalConfig;
        }
    }
    /**
     ...
     */
    async getFiscalYearConfig(tenantId) {
        // Try cache first
        if (this.cacheService) {
     ...
     */
    async saveFiscalYearConfig(tenantId, fiscalYearStart, userId) {
     ...
     */
    calculateDateRange(preset, fiscalYearStart) {
        const now = new Date();
     ...
     * Get recent shards
     */
    async getRecentShards(tenantId, limit = 5) {
     ...
     */
    resolvePermission(dashboard, userId, userRoles, userGroupIds) {
    ...
    getFiscalYearStart(fiscalYear, fiscalYearStart) {
    ...
    getFiscalYearEnd(fiscalYear, fiscalYearStart) {
        const start = this.getFiscalYearStart(fiscalYear + 1, fiscalYearStart);
        const end = new Date(start);
        end.setDate(end.getDate() - 1);
        return end;
    }
    getFiscalQuarterStart(fiscalYear, quarter, fiscalYearStart) {
    ...
    getFiscalQuarterEnd(fiscalYear, quarter, fiscalYearStart) {
        if (quarter === 4) {
