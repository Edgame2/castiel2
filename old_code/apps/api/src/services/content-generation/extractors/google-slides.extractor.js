...
export class GoogleSlidesExtractor extends BaseDocumentExtractor {
    ...
    async getDocumentMetadata(documentId, auth) {
        ...
        try {
            ...
            throw new Error(`Failed to get Google Slides metadata: ${error.message}`);
        }
    }
    /**
    ...
    async parseDocument(documentId, auth, request) {
        ...
        try {
                ...
                if (request.includeContext) {
                    ...
                    if (notesPageId) {
                        try {
                            ...
                            const notesText = this.extractTextFromPageElements(notesData.data.pageElements || []);
                            if (notesText.trim()) {
                                fullText += notesText + '\n';
                                elements.push({
                                    id: `notes-${slideId}`,
            ...
            throw new Error(`Failed to parse Google Slides document: ${error.message}`);
        }
    }
    /**
     ...
     */
    extractTextFromPageElements(pageElements) {
        let text = '';
        for (const element of pageElements) {
            if (element.shape?.text) {
...
//# sourceMappingURL=google-slides.extractor.js.map