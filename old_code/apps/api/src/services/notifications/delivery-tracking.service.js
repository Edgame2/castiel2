...
export class DeliveryTrackingService {
    ...
    monitoring;
    retryConfig;
    constructor(repository, monitoring, retryConfig) {
     ...
     */
    async recordAttempt(notification, attempt) {
        ...
        if (record) {
            ...
            if (attempt.status === 'sent') {
                ...
                if (record.attempts < this.retryConfig.maxAttempts) {
                    const delay = Math.min(this.retryConfig.initialDelayMs * Math.pow(this.retryConfig.backoffMultiplier, record.attempts - 1), this.retryConfig.maxDelayMs);
        ...
        // Save to repository
        const saved = await this.repository.update(notification.id, notification.tenantId, notification.userId, { delivery: updated.delivery });
        if (!saved) {
     ...
     */
    getDeliveryStatus(notification, channel) {
     ...
     */
    shouldRetry(notification, channel) {
     ...
     */
    getRetryDelay(notification, channel) {
        const records = this.getDeliveryStatus(notification, channel);
        if (records.length === 0) {
        ...
        const delay = Math.min(this.retryConfig.initialDelayMs * Math.pow(this.retryConfig.backoffMultiplier, attempts - 1), this.retryConfig.maxDelayMs);
        return delay;
    }
    /**
     * Mark delivery as bounced
     */
    async markBounced(notification, channel, bounceType, reason) {
     ...
     */
    async markUnsubscribed(notification, channel, reason) {
        return this.recordAttempt(notification, {
            channel,
            status: 'unsubscribed',
            error: reason,
