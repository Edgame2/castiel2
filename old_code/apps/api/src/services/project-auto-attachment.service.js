...
export class ProjectAutoAttachmentService {
    ...
    constructor(monitoring, shardRepository, config = {}) {
     ...
     */
    async findCandidateProjects(shard) {
        // Get all active projects for the tenant
        const projects = await this.shardRepository.findByShardType('c_project', shard.tenantId, {
     ...
     */
    async evaluateOverlapRules(shard, project) {
        const rules = [];
     ...
     */
    checkEntityOverlap(shard, project) {
     ...
     */
    checkActorOverlap(shard, project) {
        const shardData = shard.structuredData || {};
        const projectData = project.structuredData || {};
        ...
        if (Array.isArray(shardTeamMembers) && Array.isArray(projectTeamMembers)) {
            const overlap = shardTeamMembers.some(member => projectTeamMembers.some(pm => (typeof member === 'string' ? member : member.userId || member.id) ===
     ...
     */
    checkTimeWindowOverlap(shard, project) {
     ...
     */
    async checkExplicitReferences(shard, project) {
        const projectData = project.structuredData || {};
     ...
     */
    async attachShardToProject(shard, projectId, rules) {
        ...
        const relationship = {
            ...
            shardName: project.structuredData?.name || 'Project',
            createdAt: new Date(),
            metadata,
        };
     ...
     */
    async linkShardToProject(shardId, projectId, tenantId, userId) {
     ...
     */
    async unlinkShardFromProject(shardId, projectId, tenantId) {
        const shard = await this.shardRepository.findById(shardId, tenantId);
        if (!shard) {
            throw new Error('Shard not found');
        }
...
//# sourceMappingURL=project-auto-attachment.service.js.map