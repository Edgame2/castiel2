...
export class EntityLinkingService {
    shardManager;
    aiService; // Optional AI service for content analysis
    constructor(shardManager, aiService) {
     ...
     * Confidence: 100%
     */
    async findExplicitReferences(shard, tenantId) {
        const links = {
            opportunities: [],
            accounts: [],
            contacts: [],
        ...
        if (structuredData.opportunityId || structuredData.linkedOpportunityIds) {
            ...
            for (const oppId of opportunityIds) {
                try {
                    ...
                    const opportunity = await this.shardManager.get(`/api/v1/shards/${oppId}`, {
     ...
     * Confidence: 80-90%
     */
    async matchParticipants(shard, tenantId) {
        const links = {
            opportunities: [],
            accounts: [],
            contacts: [],
        ...
        for (const participant of participants) {
            try {
                ...
                const contacts = await this.shardManager.get(`/api/v1/shards?filter=shardTypeId eq 'contact' and tenantId eq '${tenantId}' and (structuredData.email eq '${participant}' or structuredData.emailAddress eq '${participant}')`, {
                    headers: { 'X-Tenant-ID': tenantId },
                });
                ...
                for (const contact of contactList) {
                    ...
                    const relatedOpportunities = await this.shardManager.get(`/api/v1/shards/${contact.id}/related?targetShardTypeId=opportunity&direction=both`, {
                        headers: { 'X-Tenant-ID': tenantId },
                    });
     ...
     * Confidence: 60-80%
     */
    async analyzeContent(shard, _tenantId) {
        const links = {
            opportunities: [],
            accounts: [],
            contacts: [],
     ...
     * Confidence: 40-60%
     */
    async findTemporalCorrelations(shard, tenantId) {
        const links = {
            opportunities: [],
            accounts: [],
            contacts: [],
        ...
        try {
            ...
            const opportunities = await this.shardManager.get(`/api/v1/shards?filter=shardTypeId eq 'opportunity' and tenantId eq '${tenantId}' and structuredData.closeDate ge '${dateRangeStart.toISOString()}' and structuredData.closeDate le '${dateRangeEnd.toISOString()}'`, {
                headers: { 'X-Tenant-ID': tenantId },
            });
     ...
     */
    extractTextContent(unstructuredData) {
    ...
    mergeLinks(...linkSets) {
        ...
        const seen = new Map(); // Map<entityId, bestLink>
        for (const linkSet of linkSets) {
            for (const category of ['opportunities', 'accounts', 'contacts']) {
