...
import {
  ...
  EmbeddingTemplateRecommendation,
  RiskLevel,
 ...
 */
export class EmbeddingTemplateRecommendationHandler extends BaseRecommendationHandler<EmbeddingTemplateRecommendation> {
  readonly type = 'embeddingTemplate' as const;

  /**
   * Validate embedding template recommendation
   */
  async validate(recommendation: EmbeddingTemplateRecommendation): Promise<{ valid: boolean; errors?: string[] }> {
    const errors: string[] = [];
   ...
   */
  async suggestNextAction(): Promise<null> {
    return null;
  }
   ...
   */
  private normalizeOption(item: any, index: number): RecommendationOption<EmbeddingTemplateRecommendation> {
    let recommendation: EmbeddingTemplateRecommendation;
  ...
  protected buildPromptContext(context: RecommendationContext): Record<string, any> {
    ...
    const schemaFields =
      context.shardType?.schema?.properties
        ? Object.entries(context.shardType.schema.properties).map(([name, def]: [string, any]) => ({
            name,
