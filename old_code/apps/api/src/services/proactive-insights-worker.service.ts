...
export interface ProactiveInsightsWorkerConfig {
  ...
  batchSize?: number; // Default: 10 tenants per batch
  maxConcurrentTenants?: number; // Default: 5
...
export class ProactiveInsightsWorker {
  ...
  private readonly maxConcurrentTenants: number;
  ...
  constructor(
    ...
    private tenantsContainer: any, // Container from @azure/cosmos
    config: ProactiveInsightsWorkerConfig = {}
  ) {
   ...
   */
  private async processTenants(): Promise<void> {
   ...
   * Get tenants to process
   */
  private async getTenantsToProcess(): Promise<string[]> {
    try {
      ...
      if (this.tenantService && typeof this.tenantService.listTenants === 'function') {
        try {
          ...
          return result.tenants.map((t: any) => t.id || t.tenantId).filter(Boolean);
        } catch (error) {
