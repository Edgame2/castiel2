...
export class OAuthController {
    ...
    cacheManager;
    accessTokenExpiry;
    constructor(oauthService, userService, cacheManager, accessTokenExpiry = '15m') {
     ...
     */
    async initiateGoogle(request, reply) {
    ...
    async handleProviderCallback(provider, request, reply) {
        try {
            ...
            const refreshResult = await this.cacheManager.tokens.createRefreshToken(user.id, user.tenantId);
    ...
    async handleOAuthUser(userInfo, tenantId) {
        ...
        if (existingUser) {
            ...
            const providerAlreadyLinked = providers.some((p) => p.provider === userInfo.provider && p.providerId === userInfo.id);
