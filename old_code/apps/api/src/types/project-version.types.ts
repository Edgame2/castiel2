...
export interface DetailedChange {
  entityId: string;
  entityType: string; // 'shard', 'project', 'link', etc.
  entityName?: string;
  changeType: ChangeType;
  deltas: ChangeDelta[];
  changedBy: string;
...
export interface ProjectVersion {
  ...
  content: {
    ...
    shards: any[]; // Full shard snapshot
    links: any[]; // Full link snapshot
    collaborators: any[];
    settings: Record<string, any>;
  };
  changes: DetailedChange[]; // What changed from previous version
  changeSummary: {
  ...
  childVersionIds?: string[]; // Next versions
  rollbackOf?: string; // If this is a rollback, which version
  rollbackTargetId?: string; // If this version was rolled back to
  tags?: string[]; // Release tags, milestones
...
export interface VersionHistoryEntry {
  ...
  tags?: string[];
  description?: string;
  iconUrl?: string; // For timeline visualization
}
...
export interface RollbackRequest {
  ...
  targetVersionId?: string; // If not provided, rollback to previous
