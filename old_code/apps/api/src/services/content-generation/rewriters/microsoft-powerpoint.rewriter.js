...
export class MicrosoftPowerPointRewriter extends BaseDocumentRewriter {
    ...
    async replacePlaceholders(documentId, _template, generatedValues, auth) {
        return this.executeWithRetry(async () => {
            try {
                ...
                await axios.put(`${this.graphApiBaseUrl}/me/drive/items/${documentId}/content`, modifiedPptxBuffer, {
                    headers: {
                        Authorization: `Bearer ${auth.accessToken}`,
                ...
                throw new Error(`Failed to replace placeholders in PowerPoint presentation: ${error.message}`);
            }
     ...
     */
    async replacePlaceholdersInSlideXml(zip, xmlPath, generatedValues) {
        ...
        return this.executeWithRetry(async () => {
            try {
                ...
                for (const slideFile of slideFiles) {
                    ...
                    await this.insertChartsInSlideXml(zip, slideFile, relsFile, chartPlaceholders, generatedCharts, rels, relationshipId);
                ...
                throw new Error(`Failed to insert charts in Microsoft PowerPoint presentation: ${error.message}`);
            }
        }, 'insert_charts');
    }
    /**
     ...
     */
    async insertChartsInSlideXml(zip, slideFile, _relsFile, chartPlaceholders, generatedCharts, rels, relationshipId) {
        ...
        for (const placeholder of chartPlaceholders) {
            ...
            if (xmlContent.includes(placeholderText)) {
                ...
                xmlContent = xmlContent.replace(new RegExp(placeholderText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), imagePictureXml);
            }
        }
        // Update the file in the ZIP
     ...
     */
    createImagePictureXml(relId, width, height) {
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
    ...
    async getDocumentUrl(documentId, auth) {
        return this.executeWithRetry(async () => {
            ...
            throw new Error(`Failed to get presentation URL: ${error.message}`);
        });
    }
    /**
     * Get folder path
    ...
    async deleteDocument(documentId, auth) {
        return this.executeWithRetry(async () => {
            try {
                ...
                throw new Error(`Failed to delete PowerPoint presentation: ${error.message}`);
            }
        }, 'delete');
    }
}
//# sourceMappingURL=microsoft-powerpoint.rewriter.js.map