export async function registerEmbeddingTemplateRoutes(server, monitoring) {
    ...
    server.get('/shard-types/:shardTypeId/embedding-template', {
        schema: {
        ...
        try {
            ...
            monitoring.trackException(error, { component: 'EmbeddingTemplateRoutes', operation: 'get' });
            return reply.code(500).send({ error: 'Failed to get embedding template' });
        }
    });
    // PUT /api/v1/shard-types/:shardTypeId/embedding-template
    server.put('/shard-types/:shardTypeId/embedding-template', {
        schema: {
        ...
        try {
            ...
            const updated = await shardTypeRepository.updateEmbeddingTemplate(req.params.shardTypeId, user.tenantId, req.body);
            ...
            monitoring.trackException(error, { component: 'EmbeddingTemplateRoutes', operation: 'update' });
    ...
    server.get('/embedding-templates', {
        ...
        try {
            ...
            monitoring.trackException(error, { component: 'EmbeddingTemplateRoutes', operation: 'list' });
            return reply.code(500).send({ error: 'Failed to list embedding templates' });
        }
    });
    server.log.info('✅ Embedding Template routes registered');
}
//# sourceMappingURL=embedding-template.routes.js.map