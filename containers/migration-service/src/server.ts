...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  await fastify.register(swagger, {
    openapi: {
      ...
      info: {
        ...
        description: 'Handle code migrations and refactoring',
        version: '1.0.0',
      },
      servers: [{ url: '/api/v1', description: 'API Version 1' }],
      tags: [
        { name: 'Migrations', description: 'Migration management' },
        { name: 'Migration Steps', description: 'Migration step management' },
        { name: 'Migration Execution', description: 'Migration execution and rollback' },
  ...
  initializeDatabase({
    ...
    containers: {
      migrations: 'migration_migrations',
...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Migration Service listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
