/**
 * Unit tests for RiskCatalogEventPublisher
 */

import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import * as config from '../../../src/config';
import * as publisherModule from '../../../src/events/publishers/RiskCatalogEventPublisher';
...
describe('RiskCatalogEventPublisher', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
  ...
  describe('initializeEventPublisher', () => {
    it('returns early and logs warn when RabbitMQ URL is not configured', async () => {
      ...
      expect(log.warn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ service: 'risk-catalog' })
      );
  ...
  describe('closeEventPublisher', () => {
    it('clears publisher so subsequent publish is skipped', async () => {
      ...
      await publisherModule.publishRiskCatalogEvent('risk_catalog.updated', 'tenant-1', {});

      expect(log.warn).toHaveBeenCalledWith(
        'Event publisher not initialized, skipping event',
        expect.objectContaining({ eventType: 'risk_catalog.updated', service: 'risk-catalog' })
      );
    });
  });

  describe('publishRiskCatalogEvent', () => {
    it('logs warn and returns when publisher is not initialized', async () => {
      await publisherModule.publishRiskCatalogEvent('risk_catalog.entry_created', 'tenant-1', { entryId: 'e1' });

      expect(log.warn).toHaveBeenCalledWith(
        'Event publisher not initialized, skipping event',
        expect.objectContaining({ eventType: 'risk_catalog.entry_created', service: 'risk-catalog' })
      );
    });
    ...
    it('calls publisher.publish when publisher is initialized', async () => {
      ...
      await publisherModule.publishRiskCatalogEvent(
        'risk_catalog.updated',
        'tenant-1',
        { entryId: 'e1', type: 'risk' },
    ...
    it('logs error when publisher.publish throws', async () => {
      ...
      await publisherModule.publishRiskCatalogEvent('risk_catalog.entry_created', 'tenant-1', {});
