...
export class IntentPatternService {
    ...
    aiConnectionService;
    repository;
    constructor(monitoring, unifiedAIClient, aiConnectionService) {
        ...
        this.repository = new IntentPatternRepository(client, config.cosmosDb.databaseId);
    }
    /**
    ...
    async initialize() {
        ...
        await IntentPatternRepository.ensureContainer(client, config.cosmosDb.databaseId);
    }
    /**
     ...
     */
    async testPattern(input) {
        const results = [];
        // Convert pattern strings to RegExp
        const regexPatterns = (input.pattern.patterns || []).map(p => new RegExp(p, 'i'));
        const excludePatterns = (input.pattern.excludePatterns || []).map(p => new RegExp(p, 'i'));
     ...
     */
    async suggestPatternFromSamples(input, tenantId = 'SYSTEM') {
        ...
        try {
            ...
            const connectionResult = await this.aiConnectionService.getDefaultConnection(tenantId);
            if (!connectionResult) {
...
Generate regex patterns, keywords, and phrases that would match these queries and similar ones.`;
            const response = await this.unifiedAIClient.chat(connection, apiKey, {
                messages: [
    ...
    async getPerformanceMetrics() {
        ...
        const totalClassifications = activePatterns.reduce((sum, p) => sum + p.metrics.totalMatches, 0);
        const avgAccuracy = activePatterns.length > 0
     ...
     */
    async recordMatch(patternId, confidence, wasCorrect) {
        ...
        const newAvgConfidence = (pattern.metrics.avgConfidence * pattern.metrics.totalMatches + confidence) /
            newTotalMatches;
...
//# sourceMappingURL=intent-pattern.service.js.map