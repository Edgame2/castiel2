...
describe('SalesforceAdapter', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        adapter = new SalesforceAdapter(mockMonitoring, mockConnectionService, tenantId, connectionId);
    });
    ...
    describe('push', () => {
        it('should create record successfully', async () => {
            ...
            if (definition.availableEntities.length > 0) {
                ...
                const result = await adapter.push({ Name: 'New Record' }, {
                    entity,
                    operation: 'create',
                });
                expect(result.success).toBe(true);
        ...
        it('should update record successfully', async () => {
            ...
            if (definition.availableEntities.length > 0) {
                ...
                const result = await adapter.push({ Id: 'existing-id', Name: 'Updated Record' }, {
                    entity,
                    operation: 'update',
                });
                expect(result.success).toBe(true);
        ...
        it('should handle push errors gracefully', async () => {
            ...
            if (definition.availableEntities.length > 0) {
                ...
                const result = await adapter.push({ Name: 'Test Record' }, {
                    entity,
                    operation: 'create',
                });
                expect(result.success).toBe(false);
...
//# sourceMappingURL=salesforce.adapter.test.js.map