...
import type { CacheOptimizationService } from '../services/cache-optimization.service.js';

export function registerCacheOptimizationRoutes(server: FastifyInstance): void {
  const cacheOptimization = (server as any).cacheOptimization as CacheOptimizationService | undefined;
   ...
   */
  server.get<{ Querystring: { pattern?: string } }>(
    '/api/admin/cache/optimization/access-patterns',
    {
      schema: {
