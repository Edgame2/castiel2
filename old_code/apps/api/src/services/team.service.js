...
export class TeamService {
    ...
    relationshipService;
    auditLogService;
    constructor(monitoring, shardRepository, shardTypeRepository, relationshipService, auditLogService) {
     ...
     */
    async getTeams(tenantId, filters) {
        const startTime = Date.now();
        try {
            const shardType = await this.shardTypeRepository.findByName(CORE_SHARD_TYPE_NAMES.USER_TEAMS, 'system');
            if (!shardType) {
            ...
            if (filters?.memberId) {
                teams = teams.filter(team => team.members && Array.isArray(team.members) && team.members.some(member => member?.userId === filters.memberId));
            }
     ...
     * Create a new team
     */
    async createTeam(input, tenantId, userId) {
        const startTime = Date.now();
        try {
            // Validate input
            ...
            if (input.parentTeamId) {
                const isValid = await this.validateTeamHierarchy('', // new team, no ID yet
                input.parentTeamId, tenantId);
                if (!isValid) {
     ...
     * Update a team
     */
    async updateTeam(teamId, input, tenantId, userId) {
        const startTime = Date.now();
        try {
            ...
            if (input.parentTeamId !== undefined && input.parentTeamId !== existingTeam.parentTeamId) {
                const isValid = await this.validateTeamHierarchy(teamId, input.parentTeamId || null, tenantId);
                if (!isValid) {
                    throw new Error('Invalid team hierarchy: circular reference detected');
                }
            }
            ...
            if (input.parentTeamId !== undefined) {
                ...
                if (existingTeam.parentTeamId) {
                    const relationships = await this.relationshipService.getRelationships(tenantId, existingTeam.parentTeamId, 'outgoing', { relationshipType: RelationshipType.PARENT_OF });
                    const relationship = relationships.find(r => r.targetShardId === teamId);
                    if (relationship) {
                        await this.relationshipService.deleteRelationship(relationship.id, tenantId);
                    }
                }
     ...
     * Delete a team
     */
    async deleteTeam(teamId, tenantId, userId) {
        const startTime = Date.now();
        try {
            ...
            if (team.parentTeamId) {
                const relationships = await this.relationshipService.getRelationships(tenantId, team.parentTeamId, 'outgoing', { relationshipType: RelationshipType.PARENT_OF });
                const relationship = relationships.find(r => r.targetShardId === teamId);
                if (relationship) {
                    await this.relationshipService.deleteRelationship(relationship.id, tenantId);
                }
     ...
     * Get parent team
     */
    async getParentTeam(teamId, tenantId) {
        const team = await this.getTeam(teamId, tenantId);
     ...
     */
    async getChildTeams(teamId, tenantId) {
     ...
     */
    async getDescendantTeams(teamId, tenantId, visited = new Set(), maxDepth = 50) {
     ...
     * Get team manager
     */
    async getTeamManager(teamId, tenantId) {
        const team = await this.getTeam(teamId, tenantId);
     ...
     */
    async getTeamUserIds(teamId, tenantId) {
        const team = await this.getTeam(teamId, tenantId);
        if (!team) {
            return [];
        }
     ...
     */
    async getTeamUserIdsRecursive(teamId, tenantId) {
        ...
        const descendantUserIdsPromises = descendants.map(descendant => this.getTeamUserIds(descendant.id, tenantId));
     ...
     */
    async getManagerTeams(userId, tenantId) {
     ...
     */
    async getTeamForUser(userId, tenantId) {
     ...
     */
    async isUserManagerOfTeam(userId, teamId, tenantId) {
        const team = await this.getTeam(teamId, tenantId);
        if (!team || !team.manager) {
            return false;
        }
     ...
     */
    async isUserMemberOfTeam(userId, teamId, tenantId) {
        const team = await this.getTeam(teamId, tenantId);
     ...
     */
    async validateTeamHierarchy(teamId, parentTeamId, tenantId) {
        if (!parentTeamId) {
