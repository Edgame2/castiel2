...
export class AzureADB2CController {
    ...
    ssoTeamSyncService;
    integrationService;
    constructor(azureService, userService, cacheManager, accessTokenExpiry, frontendUrl, ssoTeamSyncService, integrationService) {
    ...
    async initiateLogin(request, reply) {
        try {
            ...
            const { tenantId = 'default' } = request.params;
    ...
    async handleCallback(request, reply) {
        try {
            ...
            if (typeof this.accessTokenExpiry === 'string') {
                ...
                const result = ms(this.accessTokenExpiry);
            ...
            if (ssoTeamSyncService && integrationService && userInfo.groups && userInfo.groups.length > 0) {
                try {
                    ...
                    if (result.integrations.length > 0) {
                        ...
                        ssoTeamSyncService.syncTeamsOnLogin(user.id, stateData.tenantId, userInfo.groups, integrationId).catch((error) => {
...
//# sourceMappingURL=azure-ad-b2c.controller.js.map