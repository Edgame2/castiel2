/**
 * Configuration loader for integration-manager module
...
export interface IntegrationManagerConfig {
  ...
  rabbitmq: { url: string; exchange: string; queue: string; routingKeys?: string[] };
}

let cachedConfig: IntegrationManagerConfig | null = null;
 ...
 */
export function loadConfig(): IntegrationManagerConfig {
  if (cachedConfig) {
    return cachedConfig;
  }
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as IntegrationManagerConfig;
  
  let envConfig: Partial<IntegrationManagerConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<IntegrationManagerConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as IntegrationManagerConfig;
