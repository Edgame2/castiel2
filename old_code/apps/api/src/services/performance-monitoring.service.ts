...
export class PerformanceMonitoringService {
  ...
  async getTenantMetrics(
    ...
    try {
      ...
      this.monitoring.trackException(error as Error, { operation: 'performance-monitoring.getTenantMetrics', tenantId });
      return [];
    }
  }
  ...
  async getProjectMetrics(
    ...
    try {
      ...
      this.monitoring.trackException(error as Error, { operation: 'performance-monitoring.getProjectMetrics', projectId });
      return [];
    }
  }
   ...
   */
  async getTenantMetricsAggregated(
    tenantId: string,
    timeRangeMinutes: number = 60
  ): Promise<{
    ...
    return {
      ...
      avgAIResponseTime:
        aiChat.length > 0
          ...
          : 0,
      totalContextTokensUsed: contextAssembly.reduce(
        (sum, m) => sum + (m.contextTokensUsed || 0),
        0
      ),
      truncationFrequency:
        contextAssembly.length > 0
  ...
  async detectAnomalies(
    ...
    const latencies = metrics
      ...
      .map((m) => m.recommendationLatencyMs as number);
  ...
  private async flushMetricsBuffer(): Promise<void> {
    ...
    try {
      ...
      this.monitoring.trackException(error as Error, { operation: 'performance-monitoring.flushMetricsBuffer' });
  ...
  async cleanupOldMetrics(retentionDays: number = 30): Promise<void> {
    try {
      ...
      this.monitoring.trackException(error as Error, { operation: 'performance-monitoring.cleanupOldMetrics' });
    }
  }
