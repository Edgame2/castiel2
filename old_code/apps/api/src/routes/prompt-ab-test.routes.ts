...
export async function promptABTestRoutes(
  fastify: FastifyInstance,
  options: { abTestService: PromptABTestService }
) {
  ...
  if (!authDecorator) {
    fastify.log.warn('⚠️  Prompt A/B Test routes not registered - authentication decorator missing');
    return;
  }
   ...
   */
  fastify.post<{ Body: CreatePromptABTestInput }>(
    '/prompts/ab-tests',
    {
    ...
    async (request: FastifyRequest<{ Body: CreatePromptABTestInput }>, reply: FastifyReply) => {
      try {
   ...
   */
  fastify.patch<{ Params: { id: string }; Body: UpdatePromptABTestInput }>(
    '/prompts/ab-tests/:id',
    {
      preHandler: [authDecorator] as any,
      schema: {
    ...
    async (request, reply) => {
      try {
        ...
        const body = UpdateExperimentSchema.partial().parse(request.body) as Partial<UpdatePromptABTestInput>;
        const experiment = await abTestService.updateExperiment(
          user.tenantId,
          request.params.id,
          body as UpdatePromptABTestInput,
          user.id
        );
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        const experiment = await abTestService.startExperiment(
          user.tenantId,
          request.params.id,
          user.id
        );
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        const experiment = await abTestService.pauseExperiment(
          user.tenantId,
          request.params.id,
          user.id
        );
  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      try {
        ...
        const experiment = await abTestService.completeExperiment(
          user.tenantId,
          request.params.id,
          user.id
        );
  ...
  fastify.get<{ 
    ...
    async (request, reply) => {
      try {
        ...
        const format = (request.query.format || 'json') as 'csv' | 'json';
        const experiment = await abTestService.getExperiment(user.tenantId, request.params.id);
        if (!experiment) {
          return reply.status(404).send({ error: 'Experiment not found' });
        }
