 ...
 */
export async function registerOpportunityRoutes(server, options) {
     ...
     */
    server.get('/api/v1/opportunities', {
        ...
        try {
            ...
            if (query.status) {
                filters.status = Array.isArray(query.status)
                    ? query.status.filter((s) => typeof s === 'string')
            ...
            if (query.riskCategory) {
                filters.riskCategory = Array.isArray(query.riskCategory)
                    ? query.riskCategory.filter((r) => typeof r === 'string')
            ...
            const result = await opportunityService.listOwnedOpportunities(user.id, user.tenantId, filters, {
     ...
     */
    server.get('/api/v1/opportunities/:id', {
        onRequest: authGuards,
        schema: {
            tags: ['Opportunities'],
        ...
        try {
            ...
            const query = request.query;
            const result = await opportunityService.getOpportunity(params.id, user.tenantId, query.includeRelated !== false);
            // Check ownership/access
            const oppData = result.opportunity.structuredData;
            ...
            const isOwner = oppData?.ownerId === user.id ||
     ...
     */
    server.get('/api/v1/opportunities/account/:accountId', {
        onRequest: authGuards,
        schema: {
            tags: ['Opportunities'],
        ...
        try {
            ...
            const params = request.params;
            const query = request.query;
            const opportunities = await opportunityService.getOpportunitiesByAccount(params.accountId, user.tenantId, {
     ...
     */
    server.patch('/api/v1/opportunities/:id/stage', {
        onRequest: authGuards,
        schema: {
            tags: ['Opportunities'],
        ...
        try {
            ...
            const updated = await opportunityService.updateStage(params.id, body.stage, user.tenantId, user.id);
            return reply.code(200).send({
                success: true,
                data: updated,
            });
     ...
     */
    server.get('/api/v1/teams/:teamId/opportunities', {
        onRequest: authGuards,
        schema: {
        ...
        try {
            ...
            const isMember = await teamService.isUserMemberOfTeam(user.id, params.teamId.trim(), user.tenantId);
     ...
     */
    server.get('/api/v1/managers/:managerId/opportunities', {
        onRequest: authGuards,
        schema: {
        ...
        try {
            ...
            const result = await opportunityService.listManagerOpportunities(params.managerId.trim(), user.tenantId, user.id, filters, {
                limit: query.limit || 50,
                continuationToken: query.continuationToken,
                includeRisk: query.includeRisk === true,
