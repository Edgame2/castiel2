...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  initializeDatabase({
    ...
    containers: {
      ...
      contextTemplates: 'template_context_templates',
      emailTemplates: 'template_email_templates',
    },
  });
...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Template Service listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
