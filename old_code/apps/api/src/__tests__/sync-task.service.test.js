...
describe('SyncTaskService - Sync Execution', () => {
    ...
    describe('triggerSync', () => {
        it('should create execution and start async sync', async () => {
            ...
            expect(mockRepositories.syncExecutionRepository.create).toHaveBeenCalledWith(expect.objectContaining({
                syncTaskId: 'task-123',
                tenantId: 'tenant-123',
                triggeredBy: 'manual',
        ...
        it('should prevent duplicate execution of same task', async () => {
            ...
            await expect(service.triggerSync('task-123', 'tenant-123', 'user-123')).rejects.toThrow('already running');
        });
        it('should reject trigger on disabled task', async () => {
            ...
            await expect(service.triggerSync('task-123', 'tenant-123', 'user-123')).rejects.toThrow('disabled');
        });
    });
    ...
    describe('Batch Processing', () => {
        it('should process large dataset in batches', async () => {
            ...
            expect(mockMonitoring.trackMetric).toHaveBeenCalledWith('sync.records.fetched', 250, expect.any(Object));
        });
    });
    ...
    describe('Error Handling', () => {
        it('should handle adapter connection errors', async () => {
            ...
            expect(mockRepositories.syncExecutionRepository.update).toHaveBeenCalledWith(expect.any(String), 'tenant-123', expect.objectContaining({ status: 'failed' }));
        });
    ...
    describe('Progress Tracking', () => {
        it('should update execution progress during sync', async () => {
            ...
            expect(mockRepositories.syncExecutionRepository.update).toHaveBeenCalledWith(expect.any(String), 'tenant-123', expect.objectContaining({
    ...
    describe('Execution Completion', () => {
        it('should mark execution as successful when all records process', async () => {
            ...
            await service.triggerSync('task-123', 'tenant-123', 'user-123');
            await new Promise((resolve) => setTimeout(resolve, 200));
            expect(mockRepositories.syncExecutionRepository.complete).toHaveBeenCalledWith(expect.any(String), 'tenant-123', expect.stringMatching(/success|partial/), expect.any(Object));
        });
        it('should track sync completion metrics', async () => {
            ...
            expect(mockMonitoring.trackEvent).toHaveBeenCalledWith('syncTask.completed', expect.objectContaining({
                taskId: 'task-123',
                recordsProcessed: 2,
    ...
    describe('Retry Execution', () => {
        ...
        it('should prevent retry of successful execution', async () => {
            ...
            await expect(service.retryExecution('exec-123', 'tenant-123', 'user-123')).rejects.toThrow('Can only retry failed or partial executions');
        });
    });
...
//# sourceMappingURL=sync-task.service.test.js.map