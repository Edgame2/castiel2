...
export class IntegrationDeduplicationService {
     ...
     */
    async findDuplicates(tenantId, shardTypeId, data, rules) {
            ...
            for (const rule of rules) {
                ...
                switch (rule.matchType) {
                    case 'exact':
                        ...
                        break;
                    case 'fuzzy':
                        matches = await this.findFuzzyMatches(tenantId, shardTypeId, rule.field, fieldValue, rule.threshold || 0.8);
                        break;
                    case 'soundex':
                    case 'metaphone':
                        matches = await this.findPhoneticMatches(tenantId, shardTypeId, rule.field, fieldValue, rule.matchType);
                        break;
                }
     ...
     */
    async findExactMatches(tenantId, shardTypeId, field, value) {
     ...
     */
    async findFuzzyMatches(tenantId, shardTypeId, field, value, threshold) {
     ...
     */
    async findPhoneticMatches(tenantId, shardTypeId, field, value, algorithm) {
     ...
     */
    async mergeDuplicates(tenantId, shardIds, strategy) {
        ...
        try {
            // Fetch all shards
            const shards = await Promise.all(shardIds.map((id) => this.shardRepository.findById(id, tenantId)));
            ...
            switch (strategy) {
                case 'keep_first':
                    masterShard = validShards.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())[0];
                    break;
                case 'keep_last':
                    masterShard = validShards.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())[0];
                    break;
                case 'keep_most_complete':
                    masterShard = validShards.sort((a, b) => {
                        const aFields = Object.keys(a.structuredData).filter((k) => a.structuredData[k] != null).length;
                        const bFields = Object.keys(b.structuredData).filter((k) => b.structuredData[k] != null).length;
...
//# sourceMappingURL=integration-deduplication.service.js.map