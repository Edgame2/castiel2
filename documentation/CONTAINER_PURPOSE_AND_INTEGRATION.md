...
| **notification-manager** | Multi-channel notifications: in-app, email (SendGrid/SMTP/SES). Event-driven from RabbitMQ. | **Storage:** Cosmos DB for notification state. **Events:** Consumes events (e.g. auth emails, **anomaly.detected** for BI Risk alerts). **Config:** Email provider, app URL; gateway client path: `/api/v1/notifications`, `/api/v1/preferences`, `/api/v1/templates`. |
...
| **cache-service** | Cache administration: stats, clear, warm. Uses Redis. | **Downstream:** Redis; logging for audit. Used by other services that need cache control. |
| **cache-management** | Advanced cache: monitoring, optimization, semantic/vector search caching, warming. | **Storage:** Cosmos DB. **Events:** RabbitMQ. Complements cache-service with richer caching strategies. |
...
| **dashboard** | Dashboard CRUD, widget management, tenant-scoped dashboards. | **Storage:** Cosmos DB. **Gateway:** Client path `/api/v1/dashboards`. Used by UI for risk/pipeline/executive views. |
| **dashboard-analytics** | Executive/manager/board dashboards: revenue-at-risk, competitive intel, forecasting, risk heatmap, industry benchmark, prioritized opportunities. | **Storage:** Cosmos DB (dashboard_admin_data, dashboard_widget_cache). **Downstream:** risk-analytics (`GET /api/v1/risk-analysis/tenant/prioritized-opportunities`) when configured. **Gateway:** Proxies dashboard-analytics routes. |
...
| **collaboration-service** | Real-time collaboration: conversations, messages. | **Storage:** Cosmos DB (collaboration_*). **Downstream:** shard-manager, logging, user-management. |
| **collaboration-intelligence** | Collaborative insights, memory context, sharing. | **Storage:** Cosmos DB. **Events:** RabbitMQ. Builds on collaboration-service. |
...
| **performance-optimization** | Code and query optimization, bundle size, algorithm selection, baseline and optimized metrics. | **Storage:** Cosmos DB. **Downstream:** context-service, observability, execution, quality. |
...
## 3. Containers Without Full Runtime (Tests or Stub Only)

The following have tests or partial structure but may not be fully deployed or may be deprecated; they are listed for completeness:

- **compliance-service** – Tests only (ComplianceCheckService, CompliancePolicyService, etc.); no full server in listed structure.
- **migration-service** – Tests only (MigrationExecutorService, MigrationService, MigrationStepService); migration logic may live in configuration-service or elsewhere.
