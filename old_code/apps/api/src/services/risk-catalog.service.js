...
export class RiskCatalogService {
     ...
     */
    async ensureShardType() {
        const SYSTEM_TENANT_ID = 'system';
        const SYSTEM_USER_ID = 'system';
        try {
            // Check if shard type already exists
            const existing = await this.shardTypeRepository.findByName(CORE_SHARD_TYPE_NAMES.RISK_CATALOG, SYSTEM_TENANT_ID);
            if (existing) {
     ...
     */
    async getCatalog(tenantId, industryId) {
        const startTime = Date.now();
        try {
            // Ensure shard type exists
            await this.ensureShardType();
            // Get shard type ID for risk catalog
            const shardType = await this.shardTypeRepository.findByName(CORE_SHARD_TYPE_NAMES.RISK_CATALOG, 'system' // Global shard types are in system tenant
            );
            if (!shardType) {
            ...
            const disabledRiskIds = new Set(tenantOverridesResult.items
                .filter(shard => {
            ...
            const globalRisks = globalRisksResult.items.filter(shard => {
                const data = shard.structuredData;
            ...
            if (industryId) {
                ...
                industryRisks = industryRisksResult.items.filter(shard => {
                    const data = shard.structuredData;
                    return (data?.catalogType === 'industry' &&
            ...
            const tenantRisks = tenantRisksResult.items.filter(shard => {
                const data = shard.structuredData;
     ...
     */
    async createCustomRisk(tenantId, userId, input, userRoles) {
        const startTime = Date.now();
        try {
            // Check if user is super-admin
            const isSuperAdmin = userRoles?.some(role => ['super-admin', 'super_admin', 'superadmin', 'global_admin'].includes(role.toLowerCase())) || false;
            // Default catalogType to 'tenant' if not provided
            const catalogType = input.catalogType || 'tenant';
            ...
            await this.ensureShardType();
            // Get shard type
            const shardType = await this.shardTypeRepository.findByName(CORE_SHARD_TYPE_NAMES.RISK_CATALOG, 'system');
            if (!shardType) {
            ...
            const existing = existingResult.items.filter(shard => {
                const data = shard.structuredData;
     ...
     */
    async updateRisk(riskId, tenantId, userId, updates) {
        const startTime = Date.now();
        try {
            // Ensure shard type exists
            await this.ensureShardType();
            ...
            const risks = risksResult.items.filter(shard => {
                const data = shard.structuredData;
     ...
     */
    async duplicateRisk(sourceRiskId, sourceCatalogType, sourceIndustryId, targetTenantId, userId, newRiskId) {
        const startTime = Date.now();
        try {
            // Ensure shard type exists
            await this.ensureShardType();
            ...
            const sourceRisks = sourceRisksResult.items.filter(shard => {
                const data = shard.structuredData;
            ...
            const sourceData = sourceShard.structuredData;
     ...
     */
    async setRiskEnabledForTenant(riskId, catalogType, industryId, tenantId, userId, enabled) {
        const startTime = Date.now();
        try {
            // Ensure shard type exists
            await this.ensureShardType();
            ...
            const existingOverrides = tenantRisksResult.items.filter(shard => {
                const data = shard.structuredData;
     ...
     */
    async deleteRisk(riskId, tenantId, userId) {
        const startTime = Date.now();
        try {
            // Ensure shard type exists
            await this.ensureShardType();
     ...
     */
    async getPonderation(riskId, tenantId, industryId, opportunityType) {
        try {
            // Ensure shard type exists
            await this.ensureShardType();
            ...
            const systemRisks = systemRisksResult.items.filter(shard => {
                const data = shard.structuredData;
                return data?.riskId === riskId;
            });
            const tenantRisks = tenantRisksResult.items.filter(shard => {
            ...
            const data = riskShard.structuredData;
            const ponderations = data.ponderations || [];
            // Check for most specific override first
            if (opportunityType) {
                const oppTypeOverride = ponderations.find(p => p.scope === 'opportunity_type' &&
            ...
            if (industryId) {
                const industryOverride = ponderations.find(p => p.scope === 'industry' &&
            ...
            const tenantOverride = ponderations.find(p => p.scope === 'tenant' &&
                (!p.effectiveTo || p.effectiveTo > new Date()) &&
                p.effectiveFrom <= new Date());
            if (tenantOverride) {
...
//# sourceMappingURL=risk-catalog.service.js.map