...
import { AI_PROVIDERS, getModelById, DEFAULT_MODEL_SELECTION_CONFIG, } from '../types/ai-provider.types.js';
...
export class AIConfigService {
    ...
    userService;
    monitoring;
    constructor(cosmosClient, redis, keyVault, notificationService, userService, monitoring) {
     ...
     */
    setNotificationServices(notificationService, userService) {
        this.notificationService = notificationService;
        this.userService = userService;
    }
    // ============================================
    ...
    async getSystemConfig() {
        ...
        try {
            const { resource } = await this.systemConfigContainer.item(SYSTEM_CONFIG_ID, SYSTEM_CONFIG_ID).read();
            if (resource) {
     ...
     */
    async updateSystemConfig(input, updatedBy) {
     ...
     */
    async addSystemCredential(provider, apiKey, endpoint, deploymentMappings, updatedBy) {
        ...
        await this.updateSystemConfig({ ...config, systemCredentials: credentials }, updatedBy || 'system');
    }
    /**
     * Remove system-level AI credentials
     */
    async removeSystemCredential(provider, updatedBy) {
        const config = await this.getSystemConfig();
    ...
    async getTenantConfig(tenantId) {
        ...
        try {
            const { resource } = await this.tenantConfigContainer.item(tenantId, tenantId).read();
            if (resource) {
     ...
     * Now stores API keys in Azure Key Vault instead of encrypting in Cosmos DB
     */
    async addTenantCredential(tenantId, input, addedBy) {
        const systemConfig = await this.getSystemConfig();
     ...
     */
    async removeTenantCredential(tenantId, provider, removedBy) {
     ...
     */
    async toggleTenantCredential(tenantId, provider, isActive, updatedBy) {
        const config = await this.getTenantConfig(tenantId);
        if (!config?.tenantCredentials) {
            return;
        }
     ...
     *   3. System default model
     */
    async resolveAIConfig(tenantId, assistantModel) {
        const systemConfig = await this.getSystemConfig();
        ...
        let credentialSource = 'system';
        if (tenantConfig && !tenantConfig.useSystemCredentials) {
            const hasTenantCredential = tenantConfig.tenantCredentials?.some(c => c.provider === provider && c.isActive);
        ...
        const rateLimits = {
            requestsPerMinute: Math.min(systemConfig.globalRateLimits.requestsPerMinute, tenantConfig?.rateLimits?.requestsPerMinute || Infinity),
            tokensPerMinute: Math.min(systemConfig.globalRateLimits.tokensPerMinute, tenantConfig?.rateLimits?.tokensPerMinute || Infinity),
        };
    ...
    async getApiKey(tenantId, provider) {
        ...
        if (resolved.credentialSource === 'tenant') {
            const tenantConfig = await this.getTenantConfig(tenantId);
            const credential = tenantConfig?.tenantCredentials?.find(c => c.provider === provider && c.isActive);
            if (credential) {
    ...
    async getTenantAdminUserIds(tenantId) {
        ...
        try {
            ...
            this.monitoring?.trackException(error, { operation: 'ai-config.get-tenant-admin-ids' });
            return [];
        }
    }
    /**
    ...
    async notifyTenantAdmins(tenantId, notification) {
        ...
        try {
            ...
            this.monitoring?.trackException(error, { operation: 'ai-config.send-tenant-admin-notification' });
        }
    }
    /**
     * Check and send cost alerts if thresholds are exceeded
     */
    async checkAndSendCostAlerts(tenantId) {
        ...
        try {
            ...
            this.monitoring?.trackException(error, { operation: 'ai-config.check-cost-alerts' });
        }
    }
    /**
    ...
    async recordUsage(usage) {
        const systemConfig = await this.getSystemConfig();
        if (!systemConfig.features.enableUsageTracking) {
            return;
        }
        const record = {
            ...usage,
        ...
        this.checkAndSendCostAlerts(usage.tenantId).catch(error => {
            ...
            this.monitoring?.trackException(error, { operation: 'ai-config.check-cost-alerts-async' });
        });
    }
    /**
    ...
    async getUsageStats(tenantId, startDate, endDate) {
      ...
      `,
        ...
        const { resources } = await this.usageContainer.items.query(query).fetchAll();
        const stats = {
            totalTokens: 0,
            totalCost: 0,
     ...
     */
    async getBillingSummary(tenantId, startDate, endDate) {
    ...
    async getAvailableProviders(tenantId) {
        ...
        return Object.values(AI_PROVIDERS).map(providerDef => {
            const hasSystemCredential = systemConfig.systemCredentials?.some(c => c.provider === providerDef.provider);
            const hasTenantCredential = tenantConfig?.tenantCredentials?.some(c => c.provider === providerDef.provider && c.isActive);
     ...
     */
    getProviderSecretName(provider, tenantId) {
     ...
     */
    async storeApiKeyInKeyVault(provider, apiKey, tenantId) {
        ...
        try {
            ...
            this.monitoring?.trackException(error, { operation: 'ai-config.store-api-key' });
            // Provide helpful error message
            if (error instanceof Error) {
                if (error.message.includes('not configured')) {
                    throw new Error(`Key Vault is not configured. Please set AZURE_KEY_VAULT_URL and authentication credentials. ` +
                        `See documentation: docs/guides/ai-key-vault-migration.md`);
                }
                if (error.message.includes('Forbidden') || error.message.includes('Permission')) {
                    throw new Error(`Insufficient permissions to write to Key Vault. ` +
     ...
     */
    async retrieveApiKeyFromKeyVault(secretReference) {
        try {
            ...
            this.monitoring?.trackException(error, { operation: 'ai-config.retrieve-api-key', secretReference });
