...
export async function initializeEventPublisher(): Promise<void> {
  ...
  if (!config.rabbitmq.url) {
    log.warn('RabbitMQ URL not configured, event publishing disabled', { service: 'integration-sync' });
    return;
  }
...
export async function publishIntegrationSyncEvent(
  eventType: string,
  tenantId: string,
  data: any,
  ...
  if (!publisher) {
    log.warn('Event publisher not initialized, skipping event', { eventType, service: 'integration-sync' });
    return;
  }
