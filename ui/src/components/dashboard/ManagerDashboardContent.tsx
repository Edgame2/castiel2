/**
 * ManagerDashboardContent – fetches GET /api/v1/dashboards/manager and renders widgets (Plan §4.5, §891).
 * Widget types: revenue-at-risk, forecast-summary, early_warning_list, scenario_forecast, competitive_win_loss, top_at_risk_reasons.
 */
...
export type ManagerDashboardContentProps = {
  apiBaseUrl?: string;
  getHeaders?: () => HeadersInit | Promise<HeadersInit>;
};

function WidgetBlock({ w }: { w: ManagerWidget }) {
  const d = w.data || {};
  ...
  switch (w.type) {
    ...
    case 'early_warning_list': {
      const raw = (d.signals as unknown[]) ?? [];
      const signals = raw.map((s: unknown, i: number) => {
        const x = (s as Record<string, unknown>) || {};
        const sev = x.severity as string;
        return {
          id: String(x.id ?? x.warningId ?? i),
      ...
      return (
        <EarlyWarningCard signals={signals} title={t} />
      );
    }
    case 'scenario_forecast': {
      ...
      return (
        <div className="rounded-lg border p-4 bg-white dark:bg-gray-900">
          <ScenarioForecastChart period={period || '—'} p10={p10} p50={p50} p90={p90} title={t} />
        </div>
      );
    }
    ...
    case 'top_at_risk_reasons': {
      ...
      return (
        <TopAtRiskReasonsCard
          title={t}
          reasons={reasons.map((r) => ({
            reason: String(r?.reason ?? ''),
            count: Number(r?.count) || 0,
            ...(r?.suggestedMitigation && { suggestedMitigation: r.suggestedMitigation }),
          }))}
        />
      );
    }
...
export function ManagerDashboardContent({ apiBaseUrl = '', getHeaders }: ManagerDashboardContentProps) {
  const [widgets, setWidgets] = useState<ManagerWidget[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const base = apiBaseUrl.replace(/\/$/, '');
  const url = `${base}/api/v1/dashboards/manager`;
  ...
  if (loading) {
    return (
      <div className="rounded-lg border p-4 bg-white dark:bg-gray-900">
        <p className="text-sm text-gray-500">Loading manager dashboard…</p>
      </div>
    );
  }
  ...
  return <>{widgets.map((w) => <WidgetBlock key={w.id} w={w} />)}</>;
}
