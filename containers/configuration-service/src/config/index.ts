/**
 * Configuration loader for configuration-service module
...
let cachedConfig: ConfigurationServiceConfig | null = null;
 ...
 */
export function loadConfig(): ConfigurationServiceConfig {
  if (cachedConfig) {
    return cachedConfig;
  }
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as ConfigurationServiceConfig;
  
  let envConfig: Partial<ConfigurationServiceConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<ConfigurationServiceConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as ConfigurationServiceConfig;
