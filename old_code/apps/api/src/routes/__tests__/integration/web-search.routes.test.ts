...
import type { IWebSearchService } from '../../services/ai-insights/web-search.service'
import type { IWebScraperService } from '../../services/ai-insights/web-scraper.service'
...
interface SearchResponse {
    ...
    results: Array<{
        ...
        source: string
        content?: string
        chunks?: Array<{ text: string; tokens: number; embeddings?: number[] }>
    }>
    deepSearchStatus?: 'pending' | 'in-progress' | 'complete'
    metrics: {
        totalResults: number
 ...
 */
class MockFastifyApp implements Partial<FastifyInstance> {
    ...
    constructor(
        ...
        private webScraperService: IWebScraperService,
    ...
    private setupRoutes() {
        ...
        if (!this.routes.has('WS')) this.routes.set('WS', new Map())
    ...
    // POST /api/v1/insights/search
    private async postSearch(req: MockRequest, params: Record<string, string>): Promise<MockResponse> {
    ...
    private async getSearchHistory(
        req: MockRequest,
        params: Record<string, string>
    ): Promise<MockResponse> {
        const { id } = params
    ...
    private async postRecurringSearch(
        req: MockRequest,
        params: Record<string, string>
    ): Promise<MockResponse> {
    ...
    private async getRecurringSearches(
        req: MockRequest,
        params: Record<string, string>
    ): Promise<MockResponse> {
        const searches = [
    ...
    private async getRecurringSearch(
        req: MockRequest,
        params: Record<string, string>
    ): Promise<MockResponse> {
        const { id } = params
    ...
    private async executeRecurringSearch(
        req: MockRequest,
        params: Record<string, string>
    ): Promise<MockResponse> {
        const { id } = params
    ...
    private async getSearchStatistics(
        req: MockRequest,
        params: Record<string, string>
    ): Promise<MockResponse> {
        const stats = {
    ...
    private async wsDeepSearchProgress(
        req: MockRequest,
        params: Record<string, string>
    ): Promise<MockResponse> {
...
describe('Web Search API Routes', () => {
    let app: MockFastifyApp
    let mockSearchService: IWebSearchService
    let mockScraperService: IWebScraperService
    ...
    describe('GET /api/v1/insights/search/{id}/history', () => {
        ...
        it('should track cache vs fresh results', async () => {
            ...
            expect(executions.some((e: any) => e.provider === 'cache')).toBe(true)
            expect(executions.some((e: any) => e.provider === 'serpapi')).toBe(true)
        })
    })
    ...
    describe('GET /api/v1/admin/providers/health', () => {
        ...
        it('should include serpapi health', async () => {
            ...
            expect(response.statusCode).toBe(200)
            const serpapi = response.body.providers.find((p: any) => p.name === 'serpapi')
        ...
        it('should include bing health', async () => {
            ...
            expect(response.statusCode).toBe(200)
            const bing = response.body.providers.find((p: any) => p.name === 'bing')
