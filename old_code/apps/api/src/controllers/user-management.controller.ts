...
export class UserManagementController {
   ...
   */
  async createUser(
    request: FastifyRequest<{ Params: { tenantId: string }; Body: CreateUserRequest }>,
    reply: FastifyReply
  ) {
    try {
      const { tenantId } = request.params;
   ...
   */
  async updateUser(
    request: FastifyRequest<{ Params: { tenantId: string; userId: string }; Body: UpdateUserRequest }>,
    reply: FastifyReply
  ) {
    try {
      const { tenantId, userId } = request.params;
   ...
   */
  async importUsers(
    request: FastifyRequest<{ Params: { tenantId: string }; Body: { fileContent: string } }>,
    reply: FastifyReply
  ) {
    try {
      const { tenantId } = request.params;
      ...
      (request.server as any).monitoring?.trackEvent?.('user.import', {
        tenantId,
