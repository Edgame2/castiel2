...
import { IntegrationService, MAX_USER_CONNECTIONS_PER_INTEGRATION } from '@castiel/api-core';
import type { AuthUser } from '../types/auth.types.js';
import type { IntegrationConnection } from '../types/integration.types.js';
...
export class IntegrationController {
  ...
  async testConnection(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      ...
      const connectionService = (request.server as any).integrationConnectionService;
      ...
      const result = await connectionService.testConnection(
        params.id,
        user.tenantId,
        integration.userScoped ? user.id : undefined
      );
   ...
   * Update data access
   */
  async updateDataAccess(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const params = request.params as { id: string };
      const body = request.body as { allowedShardTypes: string[] };

      const integration = await this.service.updateDataAccess(
        params.id,
        user.tenantId,
        body.allowedShardTypes,
        user
      );
   ...
   */
  async updateSearchConfig(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const params = request.params as { id: string };
      const body = request.body as any;

      const integration = await this.service.updateSearchConfig(
        params.id,
        user.tenantId,
        body,
        user
   ...
   */
  async startOAuth(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const params = request.params as { id: string };
      ...
      // Start OAuth flow
      const result = await connectionService.startOAuthFlowForIntegration(
        integration,
        user.id,
        body.returnUrl || '/integrations'
      );
   ...
   */
  async handleOAuthCallback(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
   ...
   */
  async getUserConnections(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const params = request.params as { id: string };
      ...
      reply.status(200).send({
        connections: connections.map((conn: IntegrationConnection) => ({
          id: conn.id,
   ...
   */
  async createUserConnection(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    const params = request.params as { id: string };
    try {
      const user = getUser(request);
      const body = request.body as { credentials?: any; displayName?: string };
      ...
      if (body.displayName !== undefined) {
        ...
        const duplicateName = existingConnections.find(
          (conn: any) => conn.displayName?.toLowerCase() === trimmedDisplayName.toLowerCase()
        );
        if (duplicateName) {
      ...
      const connection = await connectionService.connectWithCustomCredentials(
        params.id,
        user.tenantId,
        user.id,
        body.credentials,
      ...
      // Audit log
      const auditLogService = (request.server as any).auditLogService;
      if (auditLogService) {
        auditLogService.log({
          tenantId: user.tenantId,
          category: 'system',
   ...
   */
  async updateUserConnection(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    const params = request.params as { id: string; connectionId: string };
    try {
      const user = getUser(request);
      const body = request.body as { displayName?: string; credentials?: any };
      ...
      const connection = await connectionService.getUserConnection(
        params.id,
        params.connectionId,
        user.tenantId,
        user.id
      ...
      if (body.displayName !== undefined) {
        ...
        const duplicateName = existingConnections.find(
          (conn: any) => conn.id !== params.connectionId && conn.displayName?.toLowerCase() === trimmedDisplayName.toLowerCase()
        );
        if (duplicateName) {
          reply.status(409).send({
            error: 'Conflict',
      ...
      if (body.credentials !== undefined) {
        ...
        await connectionService.connectWithCustomCredentials(
          params.id,
          user.tenantId,
          user.id,
          body.credentials,
          body.displayName || connection.displayName
        );
      }
      ...
      const updated = await connectionService.getUserConnection(
        params.id,
        params.connectionId,
        user.tenantId,
        user.id
   ...
   */
  async deleteUserConnection(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    const params = request.params as { id: string; connectionId: string };
    try {
      const user = getUser(request);
   ...
   */
  async testUserConnection(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    const params = request.params as { id: string; connectionId: string };
    try {
      const user = getUser(request);
      ...
      const result = await connectionService.testSpecificConnection(
        params.id,
        params.connectionId,
        user.tenantId,
        user.id
   ...
   */
  async getConnectionUsageStats(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const params = request.params as { id: string };
      ...
      const stats = await connectionService.getConnectionUsageStats(
        user.tenantId,
        user.id,
        integrationId
      );
   ...
   */
  async bulkDeleteUserConnections(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const params = request.params as { id: string };
      const body = request.body as { connectionIds: string[] };
      ...
      const result = await connectionService.bulkDeleteUserConnections(
        params.id,
        body.connectionIds,
        user.tenantId,
        user.id
   ...
   */
  async bulkTestUserConnections(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    try {
      const user = getUser(request);
      const params = request.params as { id: string };
      const body = request.body as { connectionIds: string[] };
      ...
      const result = await connectionService.bulkTestUserConnections(
        params.id,
        body.connectionIds,
        user.tenantId,
        user.id
