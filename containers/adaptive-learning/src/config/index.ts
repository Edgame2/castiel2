/**
 * Configuration loader for adaptive-learning module
 ...
 */
export function loadConfig(): AdaptiveLearningConfig {
  if (cachedConfig) {
    return cachedConfig;
  }
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as AdaptiveLearningConfig;
  
  let envConfig: Partial<AdaptiveLearningConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<AdaptiveLearningConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as AdaptiveLearningConfig;
