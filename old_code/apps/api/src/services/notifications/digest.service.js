...
export class DigestService {
    ...
    monitoring;
    config;
    constructor(notificationRepository, digestRepository, userService, monitoring, config, emailService, slackNotificationService, teamsNotificationService, preferenceRepository) {
     ...
     */
    async compileDigest(digest) {
        // Fetch all notifications in the digest
        const notifications = await this.notificationRepository.findByIds(digest.notificationIds, digest.tenantId, digest.userId);
        // Calculate summary
        const summary = {
        ...
        const compiled = await this.digestRepository.updateStatus(digest.id, digest.tenantId, digest.userId, 'compiled');
        if (!compiled) {
    ...
    async sendDigest(compilation) {
        ...
        if (notifications.length < this.config.minNotificationsForDigest) {
            ...
            await this.digestRepository.updateStatus(digest.id, digest.tenantId, digest.userId, 'sent', `Skipped: Only ${notifications.length} notification(s), minimum is ${this.config.minNotificationsForDigest}`);
            return { success: true };
        }
        try {
            ...
            if (result.success) {
                // Mark digest as sent
                await this.digestRepository.updateStatus(digest.id, digest.tenantId, digest.userId, 'sent');
                ...
                await this.digestRepository.updateStatus(digest.id, digest.tenantId, digest.userId, 'failed', result.error);
            ...
            await this.digestRepository.updateStatus(digest.id, digest.tenantId, digest.userId, 'failed', errorMessage);
     ...
     * Send email digest
     */
    async sendEmailDigest(digest, notifications, summary, userEmail) {
     ...
     */
    buildEmailDigestBody(digest, notifications, summary) {
        const periodLabel = digest.schedule === 'daily' ? 'Today' : 'This Week';
     ...
     */
    buildEmailDigestText(digest, notifications, summary) {
        const periodLabel = digest.schedule === 'daily' ? 'Today' : 'This Week';
        const periodStart = new Date(digest.periodStart);
        const periodEnd = new Date(digest.periodEnd);
        let text = `
...
Manage preferences: ${this.config.baseUrl || 'https://app.castiel.ai'}/notifications/preferences
     ...
     * Send Slack digest
     */
    async sendSlackDigest(digest, notifications, summary) {
     ...
     */
    buildSlackDigestContent(_digest, notifications, summary, _webhookUrl, channel) {
        const blocks = [];
     ...
     * Send Teams digest
     */
    async sendTeamsDigest(digest, notifications, summary) {
     ...
     */
    buildTeamsDigestContent(_digest, notifications, summary) {
        // Calculate unread count and types count
        const unreadCount = notifications.filter(n => n.status === 'unread').length;
        const typesCount = Object.keys(summary.byType).length;
        const card = {
        ...
        for (const [type, typeNotifications] of Object.entries(byType)) {
            card.body.push({
                ...
                size: 'Medium',
                weight: 'Bolder',
                text: `${this.getTypeEmoji(type)} ${type.toUpperCase()} (${typeNotifications.length})`,
                wrap: true,
                spacing: 'Medium',
            });
