/**
 * Unit tests for CacheManagementEventPublisher
 */

import { describe, it, expect, vi, beforeEach, afterEach } from 'vitest';
import * as config from '../../../src/config';
import * as publisherModule from '../../../src/events/publishers/CacheManagementEventPublisher';
...
describe('CacheManagementEventPublisher', () => {
  beforeEach(async () => {
    vi.clearAllMocks();
  ...
  describe('initializeEventPublisher', () => {
    it('returns early and logs warn when RabbitMQ URL is not configured', async () => {
      ...
      expect(log.warn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ service: 'cache_management' })
      );
  ...
  describe('closeEventPublisher', () => {
    it('clears publisher so subsequent publish is skipped', async () => {
      ...
      await publisherModule.publishCacheManagementEvent('cache_management.invalidated', 'tenant-1', {});

      expect(log.warn).toHaveBeenCalledWith(
        'Event publisher not initialized, skipping event',
        expect.objectContaining({ eventType: 'cache_management.invalidated', service: 'cache_management' })
      );
    });
  });

  describe('publishCacheManagementEvent', () => {
    it('logs warn and returns when publisher is not initialized', async () => {
      await publisherModule.publishCacheManagementEvent('cache_management.evicted', 'tenant-1', { key: 'k1' });

      expect(log.warn).toHaveBeenCalledWith(
        'Event publisher not initialized, skipping event',
        expect.objectContaining({ eventType: 'cache_management.evicted', service: 'cache_management' })
      );
    });
    ...
    it('calls publisher.publish when publisher is initialized', async () => {
      ...
      await publisherModule.publishCacheManagementEvent(
        'cache_management.invalidated',
        'tenant-1',
        { cacheKey: 'user:1', reason: 'ttl' },
    ...
    it('logs error when publisher.publish throws', async () => {
      ...
      await publisherModule.publishCacheManagementEvent('cache_management.evicted', 'tenant-1', {});
