...
// Get monitoring service for structured logging (if available)
const getMonitoring = () => {
  try {
...
const logError = (operation: string, error: unknown, context?: Record<string, unknown>) => {
  ...
  if (monitoring) {
    monitoring.trackException(errorObj, {
      operation,
      ...context,
    });
    monitoring.trackTrace(
      `[Documents Router] ${operation} error: ${errorObj.message}`,
      3, // Error severity
      {
        operation,
        ...context,
        errorMessage: errorObj.message,
        errorStack: errorObj.stack?.substring(0, 500),
      }
    );
  }
  // Note: If monitoring is not available, errors are silently ignored
  // This is acceptable for legacy/unused Express router
};
