...
export interface SCIMError {
  ...
  scimType?: string; // SCIM error type
}
...
export interface TenantSCIMConfig {
  id: string; // tenant-{tenantId}-scim
  tenantId: string;
  enabled: boolean;
  token: string; // Bearer token for SCIM authentication (hashed in DB)
  tokenHash: string; // SHA-256 hash of token
  endpointUrl: string; // Full SCIM endpoint URL
  createdAt: Date;
  updatedAt: Date;
  createdBy: string;
  lastRotatedAt?: Date;
  partitionKey: string; // Same as tenantId
}
