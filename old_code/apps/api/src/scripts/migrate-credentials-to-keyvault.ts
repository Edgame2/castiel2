...
interface MigrationOptions {
  ...
  legacyEncryptionKey: string; // Key used for old AES-256-GCM encryption
  dryRun?: boolean; // If true, don't actually migrate, just report
  deleteAfterMigration?: boolean; // If true, clear encrypted fields after migration
  batchSize?: number; // Number of connections to process at once
}
...
export class CredentialMigration {
  ...
  private integrationRepo: IntegrationDefinitionRepository;
    ...
    try {
      ...
      for (let i = 0; i < connections.length; i += this.batchSize) {
        ...
        const results = await Promise.allSettled(
          batch.map((conn) => this.migrateConnection(conn))
        );
        ...
        for (const result of results) {
          if (result.status === 'fulfilled') {
            ...
            if (migrationResult.success) {
              ...
              console.log(
                `  ✅ ${migrationResult.integrationId} (${migrationResult.connectionId}): ${migrationResult.credentialsMigrated} credentials`
              );
            } else {
              ...
              console.log(
                `  ❌ ${migrationResult.integrationId} (${migrationResult.connectionId}): ${migrationResult.error}`
              );
            }
          } else {
            summary.failedMigrations++;
   ...
   * Migrate OAuth credentials
   */
  private async migrateOAuthCredentials(
    connection: IntegrationConnection
  ): Promise<void> {
    ...
    if (!this.dryRun) {
      ...
      await this.secureCredentialService.storeOAuthCredentials(
        ...
        accessToken,
        refreshToken,
        Math.max(expiresIn, 60), // At least 60 seconds
        {
      ...
      if (this.deleteAfterMigration) {
        await this.connectionRepo.update(connection.id, connection.integrationId, {
          oauth: {
            ...connection.oauth,
            accessTokenEncrypted: undefined as any,
            refreshTokenEncrypted: undefined as any,
          },
        });
      }
    }
   ...
   * Migrate API key
   */
  private async migrateApiKey(connection: IntegrationConnection): Promise<void> {
    if (!connection.apiKey?.keyEncrypted) {return;}

    const apiKey = this.decrypt(connection.apiKey.keyEncrypted);

    if (!this.dryRun) {
      await this.secureCredentialService.storeCredential(
        connection.tenantId,
        connection.integrationId,
        connection.id,
   ...
   */
  private async migrateBasicAuth(
    connection: IntegrationConnection
  ): Promise<void> {
    if (!connection.basicAuth) {return;}
    ...
    if (this.deleteAfterMigration && !this.dryRun) {
      await this.connectionRepo.update(connection.id, connection.integrationId, {
        basicAuth: {
          ...
          usernameEncrypted: undefined as any,
          passwordEncrypted: undefined as any,
        },
      });
    }
  }
   ...
   */
  private hasEncryptedCredentials(connection: IntegrationConnection): boolean {
    return !!(
      connection.oauth?.accessTokenEncrypted ||
   ...
   */
  private async getAllConnections(options?: {
    tenantId?: string;
    integrationId?: string;
    ...
    console.warn(
      '⚠️  getAllConnections needs implementation in IntegrationConnectionRepository'
    );
