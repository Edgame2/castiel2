 ...
 */
export function registerAIConnectionsRoutes(fastify, connectionService, unifiedAIClient, monitoring) {
    // ============================================
     ...
     */
    fastify.get('/admin/ai/connections', {
        schema: {
     ...
     */
    fastify.get('/admin/ai/connections/:connectionId', {
        schema: {
     ...
     */
    fastify.post('/admin/ai/connections/:connectionId/test', {
        schema: {
     ...
     */
    fastify.post('/admin/ai/connections', {
        schema: {
        ...
        const roles = request.user?.roles || request.user?.roles;
     ...
     */
    fastify.patch('/admin/ai/connections/:connectionId', {
        schema: {
        ...
        const userId = request.user?.id || request.user?.id;
        try {
            const connection = await connectionService.updateConnection(request.params.connectionId, request.body, userId);
            return {
                success: true,
                connection: {
                    ...connection,
     ...
     */
    fastify.post('/admin/ai/connections/:connectionId/toggle-status', {
        schema: {
        ...
        try {
            ...
            const updated = await connectionService.updateConnection(request.params.connectionId, { status: newStatus }, userId);
            return {
                success: true,
                connection: {
                    ...updated,
     ...
     */
    fastify.delete('/admin/ai/connections/:connectionId', {
        schema: {
     ...
     */
    fastify.get('/tenant/ai/connections', {
        schema: {
            description: 'List available AI connections for tenant',
            tags: ['ai-tenant'],
        },
    }, async (request, reply) => {
        const tenantId = request.user?.tenantId || request.user?.tenantId;
        try {
     ...
     */
    fastify.post('/tenant/ai/connections', {
        schema: {
     ...
     */
    fastify.patch('/tenant/ai/connections/:connectionId', {
        schema: {
     ...
     */
    fastify.post('/tenant/ai/connections/:connectionId/toggle-status', {
        schema: {
     ...
     */
    fastify.delete('/tenant/ai/connections/:connectionId', {
        schema: {
     ...
     */
    fastify.get('/tenant/ai/connections/default/:type', {
        schema: {
        ...
        const tenantId = request.user?.tenantId || request.user?.tenantId;
        try {
            const credentials = await connectionService.getDefaultConnection(tenantId, request.params.type);
            if (!credentials) {
                return reply.status(404).send({
...
function detectProvider(endpoint, modelId) {
    const lowerEndpoint = endpoint.toLowerCase();
    const lowerModel = modelId.toLowerCase();
    // Azure patterns: openai.azure.com or cognitiveservices.azure.com
    if (lowerEndpoint.includes('openai.azure.com') || lowerEndpoint.includes('cognitiveservices.azure.com')) {
        return 'azure_openai';
    }
    ...
    if (lowerEndpoint.includes('googleapis.com') || lowerEndpoint.includes('vertex')) {
        return 'google_vertex';
    }
    if (lowerEndpoint.includes('api.cohere.ai')) {
        return 'cohere';
    }
...
async function testEmbedding(connection, apiKey, provider) {
...
async function testImageGeneration(connection, apiKey, provider) {
...
async function testModeration(connection, apiKey, provider) {
...
async function testVision(connection, apiKey, provider) {
    // 1x1 blue pixel PNG
...
async function testSpeechToText(connection, apiKey, provider) {
...
async function testTextToSpeech(connection, apiKey, provider) {
    const input = 'ping';
    if (provider === 'azure_openai') {
        const apiVersion = connection.version || '2024-02-15-preview';
        const deployment = connection.deploymentName || connection.modelId;
...
async function testVideoGeneration(connection, apiKey, provider) {
...
//# sourceMappingURL=ai-connections.routes.js.map