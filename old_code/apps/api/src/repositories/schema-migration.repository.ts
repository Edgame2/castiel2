 ...
 */
const SCHEMA_MIGRATION_CONTAINER_CONFIG: ContainerDefinition = {
...
export class SchemaMigrationRepository {
  ...
  async create(
    ...
    try {
      ...
      const requiresDataMigration = changes.some(c => 
        c.changeType !== 'added' || c.isBreaking
      );
      ...
      const { resource } = await this.container.items.create<SchemaMigration>(migration);

      this.monitoring.trackDependency(
        'cosmosdb.schemaMigration.create',
        'CosmosDB',
      ...
      return resource as SchemaMigration;
    } catch (error) {
      this.monitoring.trackException(error as Error, {
   ...
   */
  async findById(id: string, tenantId: string): Promise<SchemaMigration | undefined> {
    const startTime = Date.now();

    try {
      const { resource } = await this.container.item(id, tenantId).read<SchemaMigration>();

      this.monitoring.trackDependency(
        'cosmosdb.schemaMigration.read',
        'CosmosDB',
        config.cosmosDb.endpoint,
        Date.now() - startTime,
        true
   ...
   */
  async findLatestForShardType(
    tenantId: string,
    shardTypeId: string
  ): Promise<SchemaMigration | undefined> {
    const startTime = Date.now();
    ...
    try {
      ...
      this.monitoring.trackDependency(
        'cosmosdb.schemaMigration.findLatest',
        'CosmosDB',
        config.cosmosDb.endpoint,
        Date.now() - startTime,
        true
  ...
  async findByVersionRange(
    ...
    try {
      ...
      this.monitoring.trackDependency(
        'cosmosdb.schemaMigration.findByVersionRange',
        'CosmosDB',
        config.cosmosDb.endpoint,
        Date.now() - startTime,
        true
  ...
  async findMigrationPath(
    ...
    try {
      ...
      this.monitoring.trackDependency(
        'cosmosdb.schemaMigration.findMigrationPath',
        'CosmosDB',
        config.cosmosDb.endpoint,
        Date.now() - startTime,
        true
  ...
  async updateStatus(
    ...
    try {
      ...
      const updates: Partial<SchemaMigration> = {
        status,
        updatedAt: new Date(),
      };
      ...
      if (progress) {
        ...
        if (updates.progress.totalShards > 0) {
          updates.progress.percentComplete = Math.round(
            ((updates.progress.migratedShards + updates.progress.skippedShards) / 
      ...
      const { resource } = await this.container.item(id, tenantId).replace<SchemaMigration>(updated);

      this.monitoring.trackDependency(
        'cosmosdb.schemaMigration.updateStatus',
        'CosmosDB',
        config.cosmosDb.endpoint,
        Date.now() - startTime,
        true
   ...
   * List migrations
   */
  async list(options: MigrationQueryOptions): Promise<MigrationListResult> {
    const startTime = Date.now();
    ...
    try {
      const { resources, continuationToken: newContinuationToken } = await this.container.items
        .query<SchemaMigration>({ query, parameters }, { maxItemCount: limit, continuationToken })
        .fetchNext();

      this.monitoring.trackDependency(
        'cosmosdb.schemaMigration.list',
        'CosmosDB',
        config.cosmosDb.endpoint,
        Date.now() - startTime,
        true
   ...
   */
  async findPendingMigrations(tenantId?: string, limit: number = 10): Promise<SchemaMigration[]> {
    const startTime = Date.now();
    ...
    try {
      const { resources } = await this.container.items
        .query<SchemaMigration>({ query, parameters }, { maxItemCount: limit })
        .fetchAll();

      this.monitoring.trackDependency(
        'cosmosdb.schemaMigration.findPending',
        'CosmosDB',
        config.cosmosDb.endpoint,
        Date.now() - startTime,
        true
  ...
  async delete(id: string, tenantId: string): Promise<void> {
    ...
    try {
      ...
      this.monitoring.trackDependency(
        'cosmosdb.schemaMigration.delete',
        'CosmosDB',
        config.cosmosDb.endpoint,
        Date.now() - startTime,
        true
