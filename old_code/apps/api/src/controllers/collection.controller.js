...
export class CollectionController {
    ...
    constructor(monitoring, auditLogService) {
     ...
     */
    async createCollection(request, reply) {
        try {
            ...
            try {
                await this.documentAuditIntegration?.logUpdate(auth.tenantId, auth.id, collection.id, name, {
                    action: 'created',
                    collectionType,
                    visibility,
                    tags,
     ...
     */
    async getCollection(request, reply) {
        try {
            const auth = request.auth;
            if (!auth) {
                return reply.status(401).send({ error: 'Authentication required' });
     ...
     * List collections
     */
    async listCollections(request, reply) {
        try {
            const auth = request.auth;
            if (!auth) {
                return reply.status(401).send({ error: 'Authentication required' });
            ...
            if (collectionType) {
                collections = collections.filter((c) => c.structuredData.collectionType === collectionType);
            }
            reply.status(200).send({
                success: true,
                data: collections,
     ...
     */
    async updateCollection(request, reply) {
        try {
            const auth = request.auth;
            if (!auth) {
                return reply.status(401).send({ error: 'Authentication required' });
            ...
            try {
                const changes = {};
                if (updates.name) {
                    changes.name = { old: collection.structuredData?.name, new: updates.name };
                }
                if (updates.description !== undefined) {
                    changes.description = { old: collection.structuredData?.description, new: updates.description };
                }
                if (updates.visibility) {
                    changes.visibility = { old: collection.structuredData?.visibility, new: updates.visibility };
                }
                if (updates.tags) {
                    changes.tags = { old: collection.structuredData?.tags, new: updates.tags };
                }
                await this.documentAuditIntegration?.logUpdate(auth.tenantId, auth.id, id, updatedCollection?.structuredData?.name || id, changes, request.ip, request.headers['user-agent']);
            }
            catch (auditErr) {
     ...
     */
    async deleteCollection(request, reply) {
        try {
            const auth = request.auth;
            if (!auth) {
                return reply.status(401).send({ error: 'Authentication required' });
            ...
            try {
                await this.documentAuditIntegration?.logDelete(auth.tenantId, auth.id, id, collection.structuredData?.name || id, {
                    documentId: id,
     ...
     */
    async addDocuments(request, reply) {
        try {
            const auth = request.auth;
            if (!auth) {
                return reply.status(401).send({ error: 'Authentication required' });
            ...
            const collectionPermission = await this.shardRepository.checkPermission(id, auth.tenantId, auth.id);
            ...
            const accessResults = await Promise.allSettled(documentIds.map(async (docId) => {
                try {
                    const doc = await this.shardRepository.findById(docId, auth.tenantId);
                    if (!doc) {
                        return { id: docId, error: 'not_found' };
                    }
                    const hasAccess = await this.shardRepository.checkPermission(docId, auth.tenantId, auth.id);
            ...
            const resolvedResults = accessResults.map((result) => result.status === 'fulfilled' ? result.value : { id: 'unknown', error: 'check_failed' });
            // Check for errors
            ...
            const structuredData = collection.structuredData;
            ...
            try {
                for (const docId of newIds) {
                    await this.auditLogService?.log({
                        ...
                        eventType: 'document.added_to_collection',
                        outcome: 'success',
                        targetId: id,
     ...
     */
    async removeDocument(request, reply) {
        try {
            const auth = request.auth;
            if (!auth) {
                return reply.status(401).send({ error: 'Authentication required' });
            ...
            // Remove document from collection
            const structuredData = collection.structuredData;
            const updatedDocumentIds = (structuredData.documentIds || []).filter((id) => id !== documentId);
            ...
            try {
                await this.auditLogService?.log({
                    ...
                    actorEmail: auth.email,
                    category: 'data_management',
                    eventType: 'document.removed_from_collection',
                    outcome: 'success',
                    targetId: id,
                    targetType: 'collection',
     ...
     */
    async getCollectionDocuments(request, reply) {
        try {
            const auth = request.auth;
            if (!auth) {
                return reply.status(401).send({ error: 'Authentication required' });
            ...
            const documentResults = await Promise.allSettled(paginatedIds.map(async (docId) => {
                try {
                    const doc = await this.shardRepository.findById(docId, auth.tenantId);
                    if (!doc) {
                        return null;
            ...
            const accessibleDocuments = documentResults
            ...
            reply.status(200).send({
                ...
                hasMore: validatedOffset + validatedLimit < documentIds.length,
            });
        }
        catch (error) {
