/**
 * Configuration loader for forecasting module
 */
...
let cachedConfig: ForecastingConfig | null = null;
...
export function loadConfig(): ForecastingConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as ForecastingConfig;
  let envConfig: Partial<ForecastingConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<ForecastingConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as ForecastingConfig;
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'forecasting' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'forecasting' });
  }
...
export function getConfig(): ForecastingConfig {
