...
export class SCIMController {
   ...
   */
  async getServiceProviderConfig(request: FastifyRequest, reply: FastifyReply) {
   ...
   */
  async getResourceTypes(request: FastifyRequest, reply: FastifyReply) {
   ...
   */
  async getSchemas(request: FastifyRequest, reply: FastifyReply) {
   ...
   */
  async patchUser(request: FastifyRequest, reply: FastifyReply) {
    try {
      const tenantId = (request as any).scimTenantId;
      if (!tenantId) {
        return reply.code(400).send({
      ...
      const { id } = request.params as { id: string };
      const patchRequest = request.body as SCIMPatchRequest;
   ...
   */
  async patchGroup(request: FastifyRequest, reply: FastifyReply) {
    try {
      const tenantId = (request as any).scimTenantId;
      if (!tenantId) {
        return reply.code(400).send({
