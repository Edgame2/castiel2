...
import type { User, MFAMethod, MFARecoveryCode, TrustedDevice } from '@castiel/shared-types';
import type {
  MFAMethodType,
  EnrollTOTPResponse,
  EnrollSMSResponse,
...
export class MFAService {
  constructor(
    ...
    private readonly tenantsContainer: Container,
   ...
   */
  async enrollTOTP(userId: string, tenantId: string): Promise<EnrollTOTPResponse> {
    ...
    const enrollmentState: MFAEnrollmentState = {
      userId,
      tenantId,
      method: 'totp',
    ...
    await this.redis.setex(
      `mfa:enrollment:${enrollmentToken}`,
      10 * 60, // 10 minutes
  ...
  async verifyTOTP(
    ...
    userId: string,
    tenantId: string
  ): Promise<MFAEnrollmentVerificationResponse> {
    ...
    const state: MFAEnrollmentState = JSON.parse(stateStr);
   ...
   */
  async sendSMSOTP(user: User): Promise<void> {
    ...
    await this.redis.setex(
      `mfa:sms:${user.id}`,
      5 * 60,
      JSON.stringify({ otp: otpHash, attempts: 0 })
    );
   ...
   */
  async verifySMSOTP(user: User, code: string): Promise<boolean> {
   ...
   * Enroll Email OTP
   */
  async enrollEmail(userId: string, tenantId: string): Promise<EnrollEmailResponse> {
   ...
   */
  async verifyEmailOTP(user: User, code: string): Promise<boolean> {
   ...
   */
  private async generateRecoveryCodesInternal(user: User): Promise<string[]> {
    const codes: string[] = [];
    const hashedCodes: MFARecoveryCode[] = [];
   ...
   */
  async listMFAMethods(userId: string, tenantId: string): Promise<ListMFAMethodsResponse> {
    const user = await this.getUser(userId, tenantId);
  ...
  async disableMFAMethod(
    ...
    if (password) {
      ...
      const otherMethod = user.mfaMethods?.find(
        m => m.type !== method && m.status === 'active'
      );
      if (!otherMethod) {
   ...
   */
  async isDeviceTrusted(userId: string, tenantId: string, fingerprint: string): Promise<boolean> {
    const user = await this.getUser(userId, tenantId);
   ...
   */
  async updateMFAPolicy(request: UpdateMFAPolicyRequest): Promise<MFAPolicyResponse> {
   ...
   */
  private async getUser(userId: string, tenantId: string): Promise<User> {
    // Try cache first
   ...
   */
  private getMaskedInfo(method: MFAMethod): string | undefined {
