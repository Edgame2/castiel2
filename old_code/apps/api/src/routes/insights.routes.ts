...
interface ChatRequestBody {
  ...
  attachments?: { type: string; url: string; name: string; mimeType: string }[];
...
interface RegenerateMessageBody {
  conversationId?: string; // Required if message is in a conversation
  modelId?: string;
  temperature?: number;
...
export async function insightsRoutes(
  ...
  options: {
    ...
    multimodalAssetService?: any; // MultimodalAssetService - optional, for linking assets to messages
    ...
    prefix?: string; // Prefix passed from register options
  }
) {
  ...
  if (!authDecorator) {
    ...
    fastify.log.error({ decorators: Object.keys(fastify as any).filter(k => !k.startsWith('_') && k !== 'log').slice(0, 10) }, 'Available decorators');
    fastify.log.error({ decorators: (fastify as any).parent ? Object.keys((fastify as any).parent).filter(k => !k.startsWith('_') && k !== 'log').slice(0, 10) : 'No parent' }, 'Parent server decorators');
  ...
  const prefix = optionsPrefix || (fastify as any).prefix || '/api/v1'; // Default to expected prefix
  ...
  const rateLimiter = (fastify as any).rateLimiter || ((fastify as any).parent)?.rateLimiter;
  
  // Import rate limit middleware functions
  const {
    createAIInsightsChatRateLimitMiddleware,
    createAIInsightsChatTenantRateLimitMiddleware,
    createAIInsightsGenerateRateLimitMiddleware,
    createAIInsightsGenerateTenantRateLimitMiddleware,
    createAIInsightsQuickRateLimitMiddleware,
    createAIInsightsQuickTenantRateLimitMiddleware,
  ...
  if (chatUserRateLimit) {chatRateLimitHandlers.push(chatUserRateLimit);}
  if (chatTenantRateLimit) {chatRateLimitHandlers.push(chatTenantRateLimit);}

  const generateRateLimitHandlers = [authDecorator];
  if (generateUserRateLimit) {generateRateLimitHandlers.push(generateUserRateLimit);}
  if (generateTenantRateLimit) {generateRateLimitHandlers.push(generateTenantRateLimit);}

  const quickRateLimitHandlers = [authDecorator];
  if (quickUserRateLimit) {quickRateLimitHandlers.push(quickUserRateLimit);}
  if (quickTenantRateLimit) {quickRateLimitHandlers.push(quickTenantRateLimit);}
  ...
  fastify.post<{ Body: ChatRequestBody }>(
    ...
    async (request: FastifyRequest<{ Body: ChatRequestBody }>, reply: FastifyReply) => {
      ...
      const { tenantId, id: userId, roles } = (request as AuthenticatedRequest).user;
      ...
      if (contextAwareQueryParserService) {
        try {
          const parsedQuery = await contextAwareQueryParserService.parseQuery(
            body.content,
            tenantId,
            body.projectId || body.scope?.projectId
          );
              ...
              if (body.assetIds && body.assetIds.length > 0 && multimodalAssetService) {
                ...
                Promise.allSettled(
                  body.assetIds.map((assetId) =>
                    multimodalAssetService.updateAsset(assetId, tenantId, {
                      attachedTo: {
                        conversationId,
  ...
  const parentServer = (fastify as any).parent;
   ...
   */
  fastify.post<{ Params: { messageId: string }; Body: RegenerateMessageBody }>(
    '/insights/chat/messages/:messageId/regenerate',
    {
        ...
        if ('content' in response) {
          ...
          const updatedMessage = await conversationService.updateMessage(
            ...
            messageId,
            {
              ...
              // Note: metadata and warnings from InsightResponse are not part of ConversationMessage type
              // They are available in the response object instead
            }
          );
  ...
  fastify.post<{ Params: { messageId: string } }>(
    '/insights/chat/messages/:messageId/stop',
    {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        if (!activeStream) {
          ...
          const conversationId = (request.body as any)?.conversationId || (request.query as any)?.conversationId;
            ...
            if (conversation) {
              ...
              if (message) {
                ...
                const shardRepository = new ShardRepository(
                  (fastify as any).cosmosDbClient,
                  (fastify as any).monitoring
                );
                ...
                if ((conversationService as any).invalidateCache) {
                  await (conversationService as any).invalidateCache(activeStream.conversationId, tenantId);
                }
              }
            }
          } catch (error) {
   ...
   * POST /insights/quick
   */
  fastify.post<{ Body: QuickInsightBody }>(
    '/insights/quick',
    {
    ...
    async (request: FastifyRequest<{ Body: QuickInsightBody }>, reply: FastifyReply) => {
      const { tenantId, id: userId } = (request as AuthenticatedRequest).user;
      const body = request.body;

      const quickRequest: QuickInsightRequest = {
   ...
   */
  fastify.get<{ Params: { shardId: string }; Querystring: { limit?: number; context?: string } }>(
  ...
  fastify.get<{ Querystring: ConversationQueryOptions }>(
    routePath,
    {
      onRequest: [authDecorator],
      schema: {
      ...
      try {
        ...
        const response = {
          conversations: result.conversations.map(c => {
            ...
            if ((c as any).linkedShardsCount !== undefined) {
              conversationResponse.linkedShardsCount = (c as any).linkedShardsCount;
            }
  ...
  fastify.get<{
    Params: { id: string };
    Querystring: { includeMessages?: boolean; messageLimit?: number; messageOffset?: number; includeLinkedShards?: boolean };
  }>(
    ...
    async (request, reply) => {
      ...
      if (options.includeLinkedShards) {
        ...
        response.linkedShards = linkedShards.map(({ edge, shard }) => ({
          ...
          name: (shard.structuredData as any)?.name || (shard.structuredData as any)?.title || shard.shardTypeName || shard.id,
   ...
   */
  fastify.patch<{ Params: { id: string }; Body: UpdateConversationInput }>(
    '/insights/conversations/:id',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request: FastifyRequest<{ Params: { id: string }; Body: UpdateConversationInput }>, reply: FastifyReply) => {
      const { tenantId, id: userId } = (request as AuthenticatedRequest).user;
      const { id } = request.params;
      const input = request.body;
  ...
  fastify.post<{ Params: { id: string } }>(
    '/insights/conversations/:id/archive',
    {
      onRequest: [authDecorator],
      schema: {
   ...
   */
  fastify.post<{ Params: { id: string } }>(
    '/insights/conversations/:id/unarchive',
    {
      onRequest: [authDecorator],
      schema: {
  ...
  fastify.post<{ Params: { id: string }; Body: { userId: string; role?: string } }>(
    '/insights/conversations/:id/participants',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      const { tenantId, id: requestUserId } = (request as AuthenticatedRequest).user;
      const { id } = request.params;
      ...
      await conversationService.addParticipant(
        ...
        requestUserId,
        participantUserId,
        (role as 'participant' | 'viewer') || 'participant'
      );
  ...
  fastify.delete<{ Params: { id: string; participantId: string } }>(
    '/insights/conversations/:id/participants/:participantId',
    {
      onRequest: [authDecorator],
      schema: {
  ...
  fastify.post<{ Params: { messageId: string }; Body: FeedbackBody }>(
    ...
    async (request: FastifyRequest<{ Params: { messageId: string }; Body: FeedbackBody }>, reply: FastifyReply) => {
      const { tenantId, id: userId } = (request as AuthenticatedRequest).user;
      const { messageId } = request.params;
      const body = request.body;
      ...
      for (const conv of conversationsResult.conversations) {
        const data = conv.structuredData as { messages?: { id: string }[] };
        if (data.messages?.some((m: { id: string }) => m.id === messageId)) {
      ...
      const feedbackInput: AddFeedbackInput = {
        rating: body.rating,
        thumbs: body.thumbs,
  ...
  fastify.get<{
    ...
    Querystring: { 
      ...
      fields?: string; // Comma-separated list of fields to return (for performance)
    };
  }>(
    ...
    async (request, reply) => {
      ...
      return {
        conversations: result.conversations.map(c => {
          ...
          // Include linked shards count if available (from metadata)
          if ((c as any).linkedShardsCount !== undefined) {
            response.linkedShardsCount = (c as any).linkedShardsCount;
          }
  ...
  fastify.patch<{
    ...
    '/insights/conversations/:id/messages/:messageId',
    {
      onRequest: [authDecorator],
      schema: {
  ...
  fastify.post<{ Params: { id: string; messageId: string } }>(
    '/insights/conversations/:id/messages/:messageId/regenerate',
    {
      onRequest: chatRateLimitHandlers,
      schema: {
    ...
    async (request, reply) => {
      ...
      const result = await conversationService.regenerateResponseAfterEdit(
        id,
        tenantId,
        messageId,
        userId
  ...
  fastify.get<{ Params: { id: string; messageId: string } }>(
    '/insights/conversations/:id/messages/:messageId/history',
    {
      onRequest: [authDecorator],
      schema: {
  ...
  fastify.get<{ Params: { id: string } }>(
    '/insights/conversations/:id/archived-messages',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      return {
        archivedShards: archivedShards.map(shard => ({
          id: shard.id,
          messageCount: (shard.structuredData as any)?.messageCount || 0,
          archivedAt: (shard.structuredData as any)?.archivedAt,
          archivedBy: (shard.structuredData as any)?.archivedBy,
          firstMessageId: (shard.structuredData as any)?.firstMessageId,
          lastMessageId: (shard.structuredData as any)?.lastMessageId,
  ...
  fastify.post<{ 
    Params: { id: string };
    Body?: { forceAI?: boolean; maxMessages?: number };
  }>(
  ...
  fastify.get<{
    ...
    '/insights/conversations/:id/export',
    {
      onRequest: [authDecorator],
      schema: {
   ...
   */
  fastify.post<{
    Body: { entityName: string; projectId?: string; shardTypes?: string[]; limit?: number };
  }>(
    '/insights/entities/resolve',
    {
      onRequest: [authDecorator],
      schema: {
   ...
   */
  fastify.get<{ Querystring: { q: string; projectId?: string } }>(
    '/insights/entities/autocomplete',
    {
      onRequest: [authDecorator],
      schema: {
  ...
  fastify.get<{
    ...
    Querystring: { shardTypes?: string[]; limit?: number; offset?: number };
  }>(
    '/insights/projects/:projectId/entities',
    {
      onRequest: [authDecorator],
      schema: {
   ...
   */
  fastify.get<{ Querystring: { category?: string; scope?: string; search?: string } }>(
    '/insights/templates',
    {
      schema: {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const aiConfigService = (fastify as any).aiConfigService;
        ...
        if (aiConfigService) {
          ...
          const models = availableModels.chatModels.map((model: any) => {
   ...
   */
  fastify.patch<{
    Body: UpdateConversationConfigInput;
  }>(
    '/insights/conversations/config',
    {
      onRequest: [authDecorator],
  ...
  fastify.get<{
    ...
    '/insights/conversation-templates',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      const shardTypeRepo = (fastify as any).shardTypeRepository;
      if (!shardTypeRepo) {
      ...
      const templateType = shardTypes.shardTypes.find((st: any) => st.name === 'c_conversationTemplate');
      if (!templateType) {
        return { templates: [] };
      }
      ...
      const templates = await (fastify as any).shardRepository.list(tenantId, {
        ...filters,
        limit: 100,
      });

      // Filter templates
      const filtered = templates.shards.filter((shard: any) => {
      ...
      return {
        templates: filtered.map((shard: any) => ({
          id: shard.id,
  ...
  fastify.post<{
    ...
    '/insights/conversation-templates/:id/create',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        const conversation = await conversationService.createFromTemplate(
          tenantId,
          userId,
          templateId,
          variables,
        ...
        return {
          ...
          title: (conversation.structuredData as any).title,
          createdAt: conversation.createdAt,
        };
      } catch (error: unknown) {
        const errorMessage = error instanceof Error ? error.message : String(error);
   ...
   */
  fastify.post<{
    Body: CreateConversationInput & { threadTopic: string };
  }>(
    '/insights/conversations/threads',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        return {
          threadId: thread.id,
          title: (thread.structuredData as any).title,
          threadTopic: (thread.structuredData as any).threadTopic,
  ...
  fastify.post<{
    ...
    '/insights/conversations/:id/threads/:threadId',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        const conversation = await conversationService.addToThread(
          tenantId,
          userId,
          conversationId,
          threadId
        ...
        return {
          conversationId: conversation.id,
          threadId: (conversation.structuredData as any).threadId,
          threadOrder: (conversation.structuredData as any).threadOrder,
        };
      } catch (error: unknown) {
        const errorMessage = error instanceof Error ? error.message : String(error);
        if (errorMessage.includes('not found')) {
  ...
  fastify.delete<{
    ...
    '/insights/conversations/:id/threads',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        const conversation = await conversationService.removeFromThread(
          tenantId,
          userId,
          conversationId
        );
  ...
  fastify.get<{
    ...
    '/insights/conversations/threads/:threadId/members',
    {
      onRequest: [authDecorator],
      schema: {
      ...
      try {
        ...
        return {
          ...
          members: members.map(shard => ({
            id: shard.id,
            title: (shard.structuredData as any).title,
            threadOrder: (shard.structuredData as any).threadOrder,
            messageCount: (shard.structuredData as any).messageCount || 0,
            lastActivityAt: (shard.structuredData as any).lastActivityAt || shard.createdAt,
            createdAt: shard.createdAt,
          })),
  ...
  fastify.get<{
    ...
    '/insights/conversations/threads/:threadId/summary',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        return {
          threadId,
          title: (summary.thread.structuredData as any).title,
          threadTopic: (summary.thread.structuredData as any).threadTopic,
      ...
      try {
        ...
        return {
          threads: result.threads.map(shard => ({
            id: shard.id,
            title: (shard.structuredData as any).title,
            threadTopic: (shard.structuredData as any).threadTopic,
            messageCount: (shard.structuredData as any).messageCount || 0,
            lastActivityAt: (shard.structuredData as any).lastActivityAt || shard.createdAt,
            createdAt: shard.createdAt,
          })),
  ...
  fastify.post<{
    ...
    '/insights/conversations/:id/messages/:messageId/comments',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        const comment = await conversationService.addComment(
          conversationId,
          tenantId,
          messageId,
          userId,
   ...
   */
  fastify.patch<{
    Params: { id: string; messageId: string; commentId: string };
    Body: { content: string };
  }>(
    '/insights/conversations/:id/messages/:messageId/comments/:commentId',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        const comment = await conversationService.updateComment(
          conversationId,
          tenantId,
          messageId,
          commentId,
  ...
  fastify.delete<{
    ...
    async (request, reply) => {
      ...
      try {
        await conversationService.deleteComment(
          conversationId,
          tenantId,
          messageId,
          commentId,
  ...
  fastify.get<{
    ...
    async (request, reply) => {
      ...
      try {
        const comments = await conversationService.getComments(
          conversationId,
          tenantId,
          messageId
        );
  ...
  fastify.post<{
    Params: { id: string };
    Body: { userIds: string[]; role?: ParticipantRole; notify?: boolean };
  }>(
    '/insights/conversations/:id/invite',
    {
      onRequest: [authDecorator],
      schema: {
  ...
  fastify.get<{
    Params: { id: string };
    Querystring: { includeArchived?: boolean; forceRegenerate?: boolean };
  }>(
    '/insights/conversations/:id/analytics',
    {
      onRequest: [authDecorator],
      schema: {
  ...
  fastify.get<{
    ...
    '/insights/conversations/:id/analytics/summary',
    {
      onRequest: [authDecorator],
      schema: {
  ...
  fastify.post<{
    ...
    '/insights/conversations/:id/check-size',
    {
      onRequest: [authDecorator],
      schema: {
  ...
  fastify.post<{
    ...
    '/insights/conversations/:id/typing/start',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const userName = (request.user as any).name || (request.user as any).email || 'User';

        // Set typing indicator
        await conversationRealtimeService.setTypingIndicator(
          conversationId,
          tenantId,
          userId,
          userName,
        ...
        await conversationRealtimeService.broadcastTypingStart(
          conversationId,
          tenantId,
          userId,
          userName
  ...
  fastify.post<{
    ...
    '/insights/conversations/:id/typing/stop',
    {
      onRequest: [authDecorator],
      schema: {
  ...
  fastify.get<{
    ...
    '/insights/conversations/:id/typing',
    {
      onRequest: [authDecorator],
      schema: {
    ...
    async (request, reply) => {
      ...
      try {
        const indicators = await conversationRealtimeService.getTypingIndicators(
          conversationId,
          tenantId
        );
  ...
  fastify.get<{
    ...
    '/admin/insights/conversations/stats',
    {
      onRequest: [authDecorator],
      schema: {
