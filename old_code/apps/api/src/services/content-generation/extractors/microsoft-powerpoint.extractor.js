...
export class MicrosoftPowerPointExtractor extends BaseDocumentExtractor {
     ...
     */
    async downloadPresentation(documentId, auth) {
        try {
            ...
            throw new Error(`Failed to download presentation: ${error.message}`);
        }
    }
    /**
    ...
    async parseDocument(documentId, auth, request) {
        try {
            ...
            const slideFiles = Object.keys(zip.files).filter(name => name.startsWith('ppt/slides/slide') && name.endsWith('.xml')).sort(); // Sort to maintain slide order
            ...
            throw new Error(`Failed to parse Microsoft PowerPoint presentation: ${error.message}`);
        }
    }
    /**
     ...
     */
    extractTextFromSlideXml(parsed, slideIndex) {
        const elements = [];
        let fullText = '';
        ...
        if (spTree['p:sp']) {
            for (const shape of spTree['p:sp']) {
                ...
                if (shapeText.trim()) {
                    ...
                    const elementType = 'textBox'; // Default to textBox
                    elements.push({
                        id: shapeId,
                        type: elementType,
                        content: shapeText,
     ...
     */
    extractTextFromPowerPointShape(shape) {
        const textParts = [];
     ...
     */
    extractTextFromPowerPointTable(graphicFrame) {
        const rows = [];
     ...
     */
    extractTextFromPowerPointCell(cell) {
        const textParts = [];
     ...
     */
    extractTextFromNotesXml(parsed, slideIndex) {
        const elements = [];
        let fullText = '';
    ...
    async extractColors(documentId, auth, request) {
        try {
            ...
            const themeFiles = Object.keys(zip.files).filter(name => name.startsWith('ppt/theme/theme') && name.endsWith('.xml'));
...
//# sourceMappingURL=microsoft-powerpoint.extractor.js.map