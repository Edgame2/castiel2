...
} from '../services/secure-credential.service';
import {
...
describe('SecureCredentialService', () => {
  let service: SecureCredentialService;
  let mockKeyVault: Partial<KeyVaultService>;
  let mockMonitoring: Partial<IMonitoringProvider>;
  let mockConnectionRepo: Partial<IntegrationConnectionRepository>;
  let mockIntegrationRepo: Partial<IntegrationDefinitionRepository>;
  ...
  beforeEach(() => {
    ...
    service = new SecureCredentialService({
      keyVault: mockKeyVault as KeyVaultService,
      monitoring: mockMonitoring as IMonitoringProvider,
      connectionRepository: mockConnectionRepo as IntegrationConnectionRepository,
      integrationRepository: mockIntegrationRepo as IntegrationDefinitionRepository,
  ...
  describe('storeCredential', () => {
    it('should store a credential in Key Vault', async () => {
      ...
      expect(mockKeyVault.setSecret).toHaveBeenCalledWith(
        expect.stringContaining('integration-tenant-123-salesforce'),
        'sk_test_12345',
    ...
    it('should handle Key Vault errors gracefully', async () => {
      mockKeyVault.setSecret = vi.fn().mockRejectedValue(
        new Error('Key Vault unavailable')
      );
      ...
      ).rejects.toThrow('Failed to store credential in Key Vault');
  ...
  describe('storeOAuthCredentials', () => {
    it('should store both access and refresh tokens', async () => {
      const result = await service.storeOAuthCredentials(
        'tenant-123',
        'salesforce',
        'conn-456',
        'access_token_xyz',
    ...
    it('should set expiry for access token based on expiresIn', async () => {
      ...
      await service.storeOAuthCredentials(
        'tenant-123',
        'salesforce',
        'conn-456',
        'access_token_xyz',
      ...
      const setSecretCalls = (mockKeyVault.setSecret as any).mock.calls;
      const accessTokenCall = setSecretCalls[0];
      const expiryDate = accessTokenCall[2].expiresOn as Date;
  ...
  describe('storeCertificateAuth', () => {
    it('should store certificate and private key', async () => {
      const result = await service.storeCertificateAuth(
        'tenant-123',
        'custom-api',
        'conn-456',
        {
    ...
    it('should store private key with passphrase', async () => {
      await service.storeCertificateAuth(
        'tenant-123',
        'custom-api',
        'conn-456',
        {
  ...
  describe('getCredential', () => {
    it('should retrieve credential from Key Vault', async () => {
      ...
      const storeResult = await service.storeCredential(
        'tenant-123',
        'salesforce',
        'conn-456',
        CredentialType.API_KEY,
    ...
    it('should throw error if credential is expired', async () => {
      ...
      await expect(service.getCredential(storeResult.credentialId)).rejects.toThrow(
        'Credential expired'
      );
    });

    it('should throw error if credential metadata not found', async () => {
      await expect(service.getCredential('nonexistent-id')).rejects.toThrow(
    ...
    it('should bypass cache when requested', async () => {
      ...
      expect(mockKeyVault.getSecret).toHaveBeenCalledWith(
        expect.any(String),
  ...
  describe('getOAuthAccessToken', () => {
    ...
    it('should return valid access token', async () => {
      ...
      const token = await service.getOAuthAccessToken(
        'tenant-123',
        'salesforce',
        'conn-456'
      );
    ...
    it('should throw error if refresh fails and autoRefresh is enabled', async () => {
      ...
      await expect(
        service.getOAuthAccessToken('tenant-123', 'salesforce', 'conn-456', {
          autoRefresh: true,
        })
  ...
  describe('rotateCredential', () => {
    it('should rotate credential with new value', async () => {
      ...
      const rotateResult = await service.rotateCredential(
        storeResult.credentialId,
        'new_key_67890'
      );
  ...
  describe('rotateWebhookSecret', () => {
    ...
    it('should rotate existing webhook secret', async () => {
      ...
      const initial = await service.rotateWebhookSecret(
        'tenant-123',
        'salesforce',
        'conn-456'
      );
  ...
  describe('deleteCredential', () => {
    ...
    it('should throw error if credential not found', async () => {
      await expect(service.deleteCredential('nonexistent-id')).rejects.toThrow(
        'Credential metadata not found'
      );
    });
  });
  ...
  describe('deleteConnectionCredentials', () => {
    it('should delete all credentials for a connection', async () => {
      ...
      const deletedCount = await service.deleteConnectionCredentials(
        'tenant-123',
        'salesforce',
        'conn-456'
      );
