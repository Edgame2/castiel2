...
export class RevenueAtRiskService {
  ...
  async calculateForOpportunity(
    ...
    try {
      ...
      let riskEvaluation: RiskEvaluation | undefined = opportunityData.riskEvaluation;
      ...
      if (!riskEvaluation) {
        riskEvaluation = await this.riskEvaluationService.evaluateOpportunity(
          ...
          tenantId,
          userId,
          { includeHistorical: true, includeAI: false } // Fast evaluation
        );
      }
  ...
  async calculateForPortfolio(
    ...
    try {
      ...
      for (const opportunity of userOpportunities) {
        try {
          const risk = await this.calculateForOpportunity(
            opportunity.id,
            tenantId,
            userId
          );
  ...
  async calculateForTeam(
    ...
    try {
      ...
      const memberMap = new Map<string, Shard[]>();
  ...
  async calculateForTenant(
    ...
    try {
      ...
      // Group by owner
      const ownerMap = new Map<string, Shard[]>();
   ...
   */
  private categorizeRisk(riskScore: number): 'high' | 'medium' | 'low' {
