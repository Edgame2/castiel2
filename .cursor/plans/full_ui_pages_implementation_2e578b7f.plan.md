...
- **Route:** `/accounts` (page at [containers/ui/src/app/accounts/page.tsx](containers/ui/src/app/accounts/page.tsx)).
...
- **Routes:**  
  - `containers/ui/src/app/contacts/page.tsx` — list (data table: first column clickable → `/contacts/[id]`, Actions: Edit, Delete with AlertDialog + toast).  
  - `containers/ui/src/app/contacts/new/page.tsx` — create form (React Hook Form + Joi); submit `POST /api/v1/shards` with shard type for c_contact.  
  - `containers/ui/src/app/contacts/[id]/page.tsx` — view/edit (load `GET /api/v1/shards/:id`; form for edit; optional delete with confirm).  
...
- **Routes:**  
  - `containers/ui/src/app/products/page.tsx` — list (data table, first column → `/products/[id]`, Actions: View; call `GET /api/v1/products` or equivalent tenant-scoped endpoint).  
  - `containers/ui/src/app/products/[id]/page.tsx` — view (read-only or minimal edit if API supports it; `GET /api/v1/products/:id`).  
...
- **Account detail** [containers/ui/src/app/accounts/[id]/page.tsx](containers/ui/src/app/accounts/[id]/page.tsx): Remove `sampleHealth`. Fetch `GET /api/v1/accounts/:id/health` (risk_analytics) and optionally `GET /api/v1/shards/:id` for base fields. Loading and error states.
- **Opportunities** [containers/ui/src/app/opportunities/page.tsx](containers/ui/src/app/opportunities/page.tsx): Implement full list: data table with data from pipeline/risk APIs (per endpoints audit); first column → `/opportunities/[id]`; Actions (Edit, Delete if applicable). Loading/empty.
...
- **Dashboard** [containers/ui/src/app/dashboard/page.tsx](containers/ui/src/app/dashboard/page.tsx) and dashboard/board, executive, manager: Fetch from dashboard/analytics APIs where available; show real metrics or clear “no data” state; keep navigation links.
...
- **API usage:** Every backend call: `apiFetch` or fetch with `credentials: 'include'`; base URL from `getApiBaseUrl()` / `NEXT_PUBLIC_API_BASE_URL` only (no hardcoded URLs/ports). Protected routes: proxy must send `Authorization: Bearer <accessToken>` from cookie ([containers/ui/src/app/api/[...path]/route.ts](containers/ui/src/app/api/[...path]/route.ts)).
...
- **Loading/empty/errors:** Every list/detail page must implement: loading skeleton, explicit empty state when no data, and user-facing error via `GENERIC_ERROR_MESSAGE` ([containers/ui/src/lib/api.ts](containers/ui/src/lib/api.ts)); never expose raw `e.message` or stack.
...
| New pages | `containers/ui/src/app/accounts/page.tsx`, `contacts/page.tsx`, `contacts/new/page.tsx`, `contacts/[id]/page.tsx`, `products/page.tsx`, `products/[id]/page.tsx` |
| Wire existing | `containers/ui/src/app/accounts/[id]/page.tsx`, `opportunities/page.tsx`, opportunity subpages, dashboard pages |
| Shared | [containers/ui/src/lib/api.ts](containers/ui/src/lib/api.ts) (no change if already correct); proxy [containers/ui/src/app/api/[...path]/route.ts](containers/ui/src/app/api/[...path]/route.ts) (Bearer from cookie if not already); reusable data table / delete-dialog patterns if not present; optional `lib/schemas/` for form validation aligned to backend |
