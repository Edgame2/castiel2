...
export class MicrosoftGraphAdapter extends BaseIntegrationAdapter {
     ...
     */
    async makeGraphRequest(url, method = 'GET', body) {
    ...
    async getUserProfile() {
        try {
            const result = await this.makeGraphRequest('/me');
    ...
    async fetchTeams(config) {
        try {
            ...
            let hasMore = true;
            while (hasMore) {
                const result = await this.makeGraphRequest(url);
                if (result.error || !result.data) {
                    this.monitoring.trackException(new Error(result.error || 'Failed to fetch groups'), { operation: 'microsoft-graph.fetchTeams', tenantId: this.tenantId });
                    break;
                }
                ...
                for (const group of groups) {
                    ...
                    const membersResult = await this.makeGraphRequest(`/groups/${group.id}/members`);
                    ...
                    const teamName = group[nameMapping] || group.displayName;
                    teams.push({
                        externalId: group.id,
                        name: teamName,
                        managerExternalId,
    ...
    async push(data, options) {
        ...
        const result = await this.makeGraphRequest(url, method, data);
        if (result.error) {
            return {
                success: false,
                error: result.error,
    ...
    async search(options) {
        ...
        for (const entity of entitiesToSearch) {
            try {
                ...
                const result = await this.makeGraphRequest(searchUrl);
                if (result.data) {
     ...
     */
    async registerWebhook(events, callbackUrl, resource = '/me/drive/root') {
        ...
        const result = await this.makeGraphRequest('/subscriptions', 'POST', subscription);
        if (result.error || !result.data) {
...
// ============================================
export const MICROSOFT_GRAPH_DEFINITION = {
    id: 'microsoft-graph',
...
// ============================================
export const microsoftGraphAdapterFactory = {
...
//# sourceMappingURL=microsoft-graph.adapter.js.map