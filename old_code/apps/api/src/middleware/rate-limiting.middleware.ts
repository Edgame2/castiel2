...
import { IntegrationRateLimiter, type RateLimitConfig } from '@castiel/api-core';
...
export class RateLimitingMiddleware {
   ...
   */
  rateLimitEndpoint(operation: 'create' | 'update' | 'delete' | 'fetch') {
   ...
   */
  async getStatus(req: FastifyRequest) {
    const { integrationId, tenantId } = this.extractIntegrationInfo(req);
  ...
  private extractIntegrationInfo(req: FastifyRequest): {
    ...
    return { integrationId: integrationId as string, tenantId: tenantId as string };
  }

  private inferOperation(method: string, url: string): string | undefined {
 ...
 */
export function registerRateLimitingRoutes(
   ...
   */
  app.get(
    '/rate-limits/:integrationId/status',
   ...
   */
  app.get(
    '/rate-limits/:integrationId/queue',
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { integrationId } = req.params as { integrationId: string };
        const tenantId = (req as any).user?.tenantId;
   ...
   */
  app.post(
    '/rate-limits/:integrationId/process-queue',
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { integrationId } = req.params as { integrationId: string };
        const tenantId = (req as any).user?.tenantId;
   ...
   */
  app.post(
    '/rate-limits/:integrationId/reset',
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { integrationId } = req.params as { integrationId: string };
        const tenantId = (req as any).user?.tenantId;
   ...
   */
  app.put(
    '/rate-limits/:integrationId/limit',
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
        const { integrationId } = req.params as { integrationId: string };
   ...
   */
  app.put(
    '/rate-limits/tenant/:tenantId/limit',
    async (req: FastifyRequest, reply: FastifyReply) => {
      try {
