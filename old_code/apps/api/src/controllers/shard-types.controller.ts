...
export class ShardTypesController {
  private repository: ShardTypeRepository;
  ...
  listShardTypes = async (
    ...
    try {
      ...
      const result = await this.repository.list({
        ...
        continuationToken,
        orderBy: orderBy as 'name' | 'createdAt',
   ...
   */
  getShardType = async (
    req: FastifyRequest<{ Params: { id: string }; Querystring: any }>,
    reply: FastifyReply
  ): Promise<void> => {
    const startTime = Date.now();
    ...
    try {
      ...
      const { id } = req.params;
      const { includeInheritance = 'false' } = req.query as any;
  ...
  updateShardType = async (
    ...
    try {
      ...
      const resolvedIsGlobal =
        isGlobal === 'true' ? true : isGlobal === 'false' ? false : typeof isGlobal === 'boolean' ? isGlobal : undefined;

      const input: UpdateShardTypeInput = {
        name,
        displayName,
        description,
        category,
  ...
  validateSchemaEndpoint = async (
    ...
    try {
      ...
      const { schema, parentShardTypeId } = req.body as any;
  ...
  cloneShardType = async (
    ...
    try {
      ...
      const { id } = req.params;
      const { name, displayName, customizations } = req.body as any;
      ...
      // Clone the ShardType
      const clonedType = await this.repository.cloneShardType(
        id,
        tenantId,
        {
          name,
  ...
  triggerEnrichment = async (
    ...
    try {
      ...
      if (!this.enrichmentService) {
        const serverEnrichmentService = (req.server as any).enrichmentService;
      ...
      // Get enrichment configuration from shard type
      if (!shardType.enrichment) {
        reply.status(400).send({
          error: 'Enrichment configuration not found',
          message: 'ShardType has enrichment enabled but no configuration defined'
        });
        return;
      }

      // Ensure enrichment configuration exists in Cosmos DB
      // The enrichment service expects configs to be stored in Cosmos DB
      const enrichmentConfigId = shardType.enrichment.id || `enrichment-${id}`;
      
      // Create enrichment config from shard type's enrichment settings
      const enrichmentConfig = {
        ...shardType.enrichment,
        id: enrichmentConfigId,
        tenantId: tenantId || '',
        shardTypeId: id,
        name: shardType.enrichment.name || `${shardType.displayName} Enrichment`,
        description: shardType.enrichment.description || `Enrichment configuration for ${shardType.displayName}`,
      };

      // Ensure the config exists in Cosmos DB before triggering enrichment
      try {
        await this.enrichmentService.ensureEnrichmentConfig(enrichmentConfig);
      } catch (configError: any) {
        this.monitoring.trackException(
          configError instanceof Error ? configError : new Error(String(configError)),
          {
            operation: 'shardTypes.controller.triggerEnrichment.ensureConfig',
            shardTypeId: id,
            tenantId,
          }
        );
        reply.status(500).send({
          error: 'Failed to create enrichment configuration',
          message: configError.message || 'Unknown error during config creation',
        });
        return;
      }
