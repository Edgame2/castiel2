  ...
  await fastify.register(swagger, {
    openapi: {
      ...
      info: {
        ...
        description: 'Prompt management and optimization service',
        version: '1.0.0',
      },
      servers: [{ url: '/api/v1', description: 'API Version 1' }],
      tags: [
        { name: 'Prompts', description: 'Prompt management' },
        { name: 'A/B Tests', description: 'A/B testing' },
...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Prompt Service listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
    console.error('Failed to start Prompt Service:', error);
    process.exit(1);
  }
}
