...
import { OAuthController, type OAuthInitiateQuery, type OAuthCallbackQuery } from '../controllers/oauth.controller.js';
import {
  registerSchema,
  loginSchema,
...
import {
  ...
  createTokenRefreshRateLimitMiddleware,
  createLogoutRateLimitMiddleware,
  createTokenRevokeRateLimitMiddleware,
  createEmailVerificationRateLimitMiddleware,
  createResendVerificationRateLimitMiddleware,
...
export async function registerAuthRoutes(server: FastifyInstance): Promise<void> {
  ...
  server.post('/auth/impersonate', {
    ...
    onRequest: [(server as any).authenticate, requireAuth(), requireGlobalAdmin()],
  }, (request, reply) =>
    (authController as any).impersonate(request, reply)
  );
