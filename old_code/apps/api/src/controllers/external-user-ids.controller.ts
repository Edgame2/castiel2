...
import type { IntegrationExternalUserIdService } from '@castiel/api-core';
...
export class ExternalUserIdsController {
  constructor(
    private readonly externalUserIdService: IntegrationExternalUserIdService
  ) {}
   ...
   */
  async getUserExternalUserIds(
    request: FastifyRequest<{ Params: GetExternalUserIdsParams }>,
    reply: FastifyReply
  ): Promise<void> {
    try {
      ...
      const isAdmin = user.roles?.some((role) =>
        ['admin', 'owner', 'tenant_admin', 'super_admin', 'super-admin', 'superadmin', 'global_admin'].includes(
          role.toLowerCase()
        )
      );
      ...
      const externalUserIds = await this.externalUserIdService.getAllExternalUserIds(
        userId,
        tenantId
      );
   ...
   */
  async createOrUpdateExternalUserId(
    ...
    try {
      ...
      // Check for conflicts
      const conflict = await this.externalUserIdService.checkConflict(
        tenantId,
        body.integrationId,
        body.externalUserId,
        userId
      ...
      const externalUserId = await this.externalUserIdService.getExternalUserId(
        userId,
        tenantId,
        body.integrationId
      );
  ...
  async updateExternalUserId(
    ...
    try {
      ...
      await this.externalUserIdService.updateExternalUserId(
        userId,
        tenantId,
        integrationId,
        {
  ...
  async deleteExternalUserId(
    request: FastifyRequest<{ Params: DeleteExternalUserIdParams }>,
    reply: FastifyReply
  ): Promise<void> {
    try {
      const user = getUser(request);
   ...
   */
  async syncExternalUserId(
    request: FastifyRequest<{ Params: SyncExternalUserIdParams }>,
    reply: FastifyReply
  ): Promise<void> {
    try {
      const user = getUser(request);
      ...
      const existing = await this.externalUserIdService.getExternalUserId(
        userId,
        tenantId,
        integrationId
      );
      ...
      await this.externalUserIdService.syncExternalUserIdFromConnection(
        userId,
        tenantId,
        integrationId,
