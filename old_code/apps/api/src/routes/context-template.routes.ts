 ...
 */
export async function contextTemplateRoutes(
  server: FastifyInstance,
  options: ContextTemplateRoutesOptions
): Promise<void> {
  const { monitoring, shardRepository, shardTypeRepository, relationshipService, redis, aclService } = options;
  
  const controller = new ContextTemplateController(
    monitoring,
    shardRepository,
    shardTypeRepository,
    relationshipService,
