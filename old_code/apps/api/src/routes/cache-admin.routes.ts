 ...
 */
export async function registerCacheAdminRoutes(
  fastify: FastifyInstance,
  dependencies: CacheAdminRouteDependencies
): Promise<void> {
  try {
  ...
  const adminPreHandler = [requireAuth(), requireRole('admin', 'owner')];
    ...
    fastify.post(
      ...
      { preHandler: adminPreHandler },
      (request, reply) => controller.clearCache(request as any, reply)
    );
    ...
    fastify.post(
      ...
      { preHandler: adminPreHandler },
      (request, reply) => controller.warmCache(request as any, reply)
    );

    // GET /api/v1/admin/cache/warming/status - Get warming status
    fastify.get(
      '/api/v1/admin/cache/warming/status',
      { preHandler: adminPreHandler },
      (request, reply) => controller.getWarmingStatus(request, reply)
    );
    ...
    fastify.post(
      ...
      { preHandler: adminPreHandler },
      (request, reply) => controller.generateReport(request as any, reply)
    );
    ...
    dependencies.monitoring?.trackException(error as Error, {
