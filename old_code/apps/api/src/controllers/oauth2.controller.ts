...
export class OAuth2Controller {
  ...
  async authorize(
    request: FastifyRequest<{ Querystring: OAuth2AuthorizationRequest }>,
    reply: FastifyReply
  ): Promise<void> {
    try {
      const {
      ...
      if (response_type !== 'code') {
        const redirectUrl = this.buildErrorRedirect(
          redirect_uri,
      ...
      if (!client.allowedGrantTypes.includes(OAuth2GrantType.AUTHORIZATION_CODE)) {
        const redirectUrl = this.buildErrorRedirect(
          redirect_uri,
          'unauthorized_client',
          'Client is not authorized to use authorization code flow',
          state
        );
        return reply.redirect(redirectUrl);
      }
      ...
      const code = await this.oauth2AuthService.generateAuthorizationCode(
        client_id,
        user.id,
        user.tenantId,
        redirect_uri,
  ...
  async revoke(
    request: FastifyRequest<{ Body: OAuth2RevokeTokenRequest }>,
    reply: FastifyReply
  ): Promise<void> {
    try {
  ...
  private async handleAuthorizationCodeGrant(
    ...
    const body = request.body as OAuth2AuthorizationCodeTokenRequest;
    ...
    const authCode = await this.oauth2AuthService.validateAuthorizationCode(
      code,
      clientId,
      redirect_uri,
      code_verifier
    ...
    if (client.type === 'confidential') {
      ...
      const valid = await this.oauth2ClientService.verifyClientCredentials(
        clientId,
        clientSecret,
        authCode.tenantId
      );
    ...
    const tokens = await this.oauth2AuthService.generateTokens(
      client,
      authCode.scope,
      authCode.userId
    );
  ...
  private async handleClientCredentialsGrant(
    ...
    const body = request.body as OAuth2ClientCredentialsTokenRequest;
    const { scope } = body;
  ...
  private async handleRefreshTokenGrant(
    ...
    const body = request.body as OAuth2RefreshTokenRequest;
    ...
    const tokens = await this.oauth2AuthService.refreshAccessToken(
      refresh_token,
      clientId,
      requestedScopes
    );
