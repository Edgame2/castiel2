...
import {
  ...
  WidgetCatalogEntryWithTenantOverride,
...
export class WidgetCatalogService {
  private repository: WidgetCatalogRepository;
  ...
  async createCatalogEntry(
    input: CreateWidgetCatalogEntryInput,
    userId: string
  ...
  async updateCatalogEntry(
    entryId: string,
    input: UpdateWidgetCatalogEntryInput,
    userId: string
  ): Promise<WidgetCatalogEntry> {
    try {
      ...
      const updated: WidgetCatalogEntry = {
        ...existing,
   ...
   */
  async updateTenantWidgetAccess(
    tenantId: string,
    input: UpdateTenantWidgetAccessInput,
    userId: string
  ): Promise<TenantWidgetCatalogOverride> {
    try {
      const existing = await this.repository.getTenantWidgetOverride(
        tenantId,
        input.widgetCatalogEntryId
      );
      ...
      const override: TenantWidgetCatalogOverride = {
  ...
  async updateTenantWidgetConfig(
    tenantId: string,
    input: UpdateTenantWidgetConfigInput,
    userId: string
    ...
    try {
      ...
      const updated: TenantWidgetCatalogConfig = {
        ...existing,
        ...
        defaultWidgetCatalogEntryIds:
          input.defaultWidgetCatalogEntryIds ?? existing.defaultWidgetCatalogEntryIds,
        maxWidgetsPerDashboard:
          input.maxWidgetsPerDashboard ?? existing.maxWidgetsPerDashboard,
   ...
   */
  async getUserWidgetCatalog(
    tenantId: string,
    userRoles: string[],
    options?: Partial<WidgetCatalogListOptions>
  ): Promise<WidgetCatalogListResult> {
    try {
      ...
      const overrideMap = new Map(
        overrides.items.map((o) => [o.widgetCatalogEntryId, o])
      );
      ...
      const filteredEntries = allEntries.items
        .filter((entry) => {
          ...
          if (
            entry.visibilityLevel === WidgetVisibilityLevel.SPECIFIC_ROLES &&
            allowedRoles &&
      ...
      return {
        items: filteredEntries as WidgetCatalogEntryWithTenantOverride[],
