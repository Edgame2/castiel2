...
export class DocumentValidationService {
  ...
  async validateUpload(
    ...
    // Validate file size
    const fileSizeValidation = this.validateFileSize(
      fileSize,
      tenantSettings,
      globalSettings
    );
    ...
    // Validate MIME type
    const mimeTypeValidation = this.validateMimeType(
      mimeType,
      tenantSettings,
      globalSettings
    );
    ...
    return {
      ...
      tenantStorageAvailable:
        tenantSettings.maxStorageSizeBytes - tenantSettings.currentStorageUsed,
  ...
  private validateFileSize(
    ...
    if (fileSize > globalSettings.globalMaxFileSizeBytes) {
      errors.push(
        `File size (${this.formatBytes(fileSize)}) exceeds global maximum (${this.formatBytes(globalSettings.globalMaxFileSizeBytes)})`
      );
    }
    ...
    if (fileSize > tenantSettings.maxFileSizeBytes) {
      errors.push(
        `File size (${this.formatBytes(fileSize)}) exceeds tenant maximum (${this.formatBytes(tenantSettings.maxFileSizeBytes)})`
      );
    }
    ...
    if (fileSize > warningThreshold && fileSize <= tenantSettings.maxFileSizeBytes) {
      warnings.push(
        `File size (${this.formatBytes(fileSize)}) is approaching tenant limit (${this.formatBytes(tenantSettings.maxFileSizeBytes)})`
      );
    }
  ...
  private validateMimeType(
    ...
    if (
      tenantSettings.blockedMimeTypes &&
    ...
    if (
      tenantSettings.acceptedMimeTypes.length > 0 &&
      !this.isMimeTypeAllowed(mimeType, tenantSettings.acceptedMimeTypes)
    ) {
      errors.push(
        `File type '${mimeType}' is not in tenant's accepted types. Allowed: ${tenantSettings.acceptedMimeTypes.join(', ')}`
      );
    }

    // Check system allowlist (if configured)
    if (
      globalSettings.systemAcceptedMimeTypes.length > 0 &&
      !this.isMimeTypeAllowed(mimeType, globalSettings.systemAcceptedMimeTypes)
    ) {
      errors.push(
        `File type '${mimeType}' is not in system's accepted types`
      );
    }
  ...
  private validateQuota(
    ...
    if (fileSize > availableStorage) {
      errors.push(
        `Insufficient storage. File size (${this.formatBytes(fileSize)}) exceeds available storage (${this.formatBytes(availableStorage)}). Current usage: ${this.formatBytes(currentUsage)} / ${this.formatBytes(maxStorage)}`
      );
    }
    ...
    if (usagePercentage >= 90) {
      warnings.push(
        `Storage is ${usagePercentage.toFixed(1)}% full. Consider cleaning up old files.`
      );
    } else if (usagePercentage >= 80) {
      warnings.push(
        `Storage is ${usagePercentage.toFixed(1)}% full. Approaching limit.`
      );
    }
   ...
   * Validate rate limits
   */
  private validateRateLimits(tenantSettings: TenantDocumentSettings): {
    valid: boolean;
    errors: string[];
    warnings: string[];
  } {
    ...
    if (now > tenantSettings.dailyUploadCountResetAt) {
      ...
      if (tenantSettings.dailyUploadCount >= tenantSettings.dailyUploadLimit) {
        errors.push(
          `Daily upload limit reached (${tenantSettings.dailyUploadCount}/${tenantSettings.dailyUploadLimit}). Resets at ${tenantSettings.dailyUploadCountResetAt.toISOString()}`
        );
      } else if (
        tenantSettings.dailyUploadCount >=
        tenantSettings.dailyUploadLimit * 0.9
      ) {
        warnings.push(
          `Approaching daily upload limit (${tenantSettings.dailyUploadCount}/${tenantSettings.dailyUploadLimit})`
        );
      }
    }
    ...
    if (now > tenantSettings.monthlyUploadCountResetAt) {
      ...
      // Check monthly limit
      if (
        tenantSettings.monthlyUploadCount >= tenantSettings.monthlyUploadLimit
      ) {
        errors.push(
          `Monthly upload limit reached (${tenantSettings.monthlyUploadCount}/${tenantSettings.monthlyUploadLimit}). Resets at ${tenantSettings.monthlyUploadCountResetAt.toISOString()}`
        );
      } else if (
        tenantSettings.monthlyUploadCount >=
        tenantSettings.monthlyUploadLimit * 0.9
      ) {
        warnings.push(
          `Approaching monthly upload limit (${tenantSettings.monthlyUploadCount}/${tenantSettings.monthlyUploadLimit})`
        );
      }
    }
   ...
   */
  private isMimeTypeBlocked(mimeType: string, blockedTypes: string[]): boolean {
  ...
  validateCategory(
    ...
    const categoryExists = tenantSettings.categories.some(
      (cat) => cat.id === category && cat.isActive
    );
  ...
  validateTags(
    ...
    if (tenantSettings.controlledTags && tenantSettings.controlledTags.length > 0) {
      const invalidTags = tags.filter(
        (tag) => !tenantSettings.controlledTags!.includes(tag)
      );

      if (invalidTags.length > 0) {
        errors.push(
          `Invalid tags: ${invalidTags.join(', ')}. Allowed tags: ${tenantSettings.controlledTags.join(', ')}`
        );
      }
    }
    ...
    tags.forEach((tag) => {
      ...
      if (!/^[a-zA-Z0-9-_\s]+$/.test(tag)) {
        errors.push(
          `Tag '${tag}' contains invalid characters. Use only letters, numbers, hyphens, underscores, and spaces.`
        );
      }
    });
  ...
  resetRateLimitCounters(
    ...
    const now = new Date();
    const updates: Partial<TenantDocumentSettings> = {};
   ...
   */
  incrementUploadCounters(
    tenantSettings: TenantDocumentSettings,
    fileSize: number
  ): Partial<TenantDocumentSettings> {
    return {
