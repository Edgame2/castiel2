...
export interface AnalyzeImageRequest {
  imageUrl: string; // URL to the image (must be publicly accessible or Azure Blob Storage URL)
  includeOCR?: boolean; // Extract text from image
  includeObjects?: boolean; // Detect objects
  includeDescription?: boolean; // Generate description
  includeModeration?: boolean; // NSFW detection
}
...
export class ImageAnalysisService {
  private config: Required<ImageAnalysisConfig>;
   ...
   * Analyze an image
   */
  async analyzeImage(request: AnalyzeImageRequest): Promise<ImageAnalysisResponse> {
   ...
   */
  private buildAnalysisPrompt(request: AnalyzeImageRequest): string {
    const parts: string[] = [];
   ...
   * Call Azure OpenAI Vision API
   */
  private async callVisionAPI(imageUrl: string, prompt: string): Promise<any> {
   ...
   */
  private parseAnalysisResponse(response: string, request: AnalyzeImageRequest): ImageAnalysisResponse {
    try {
   ...
   */
  private countFaces(description: string): number {
    const facePatterns = [
      /(\d+)\s+face/i,
      /(\d+)\s+person/i,
      /(\d+)\s+people/i,
   ...
   */
  private parseLocation(location: string): { x: number; y: number; width: number; height: number } | undefined {
