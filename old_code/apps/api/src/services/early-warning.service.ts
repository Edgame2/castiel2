...
export class EarlyWarningService {
  ...
  private readonly STAGE_STAGNATION_DAYS = 30; // Alert if in same stage for 30+ days
  private readonly ACTIVITY_DROP_THRESHOLD = 0.5; // 50% drop in activity
  private readonly RISK_ACCELERATION_THRESHOLD = 0.2; // 20% increase in risk score
  ...
  async detectSignals(
    ...
    try {
      ...
      // Check for anomalies using comprehensive anomaly detection
      if (this.anomalyDetectionService) {
        try {
          const opportunityData = opportunity.structuredData as any;
          const anomalyResult = await this.anomalyDetectionService.detectOpportunityAnomalies(
            tenantId,
            opportunityId,
            {
              riskScore: opportunityData?.riskEvaluation?.overallRisk,
              forecast: opportunityData?.forecast,
              activityCount: opportunityData?.activityCount,
              stageDuration: opportunityData?.stageDuration,
              stakeholderCount: opportunityData?.stakeholderCount,
            }
          );

          // Convert high-severity anomalies to early warning signals
          for (const anomaly of anomalyResult.anomalies) {
            if (anomaly.severity === 'high' || anomaly.severity === 'critical') {
              const anomalySignal: EarlyWarningSignal = {
                id: uuidv4(),
                signalType: 'anomaly' as SignalType,
                severity: anomaly.severity === 'critical' ? 'critical' : 'high',
                description: `Anomaly Detected: ${anomaly.explanation.summary}. ${anomaly.explanation.impact}`,
                detectedAt: new Date(),
                evidence: [
                  {
                    type: 'anomaly',
                  ...
                  ...anomaly.explanation.factors.map(f => ({
          ...
          this.monitoring.trackException(error as Error, {
            operation: 'early-warning.detectSignals.anomaly-detection',
            tenantId,
            opportunityId,
          });
        }
  ...
  async checkStageStagnation(
    ...
    try {
      ...
      const sortedRevisions = [...revisions.revisions].sort(
        (a, b) => a.timestamp.getTime() - b.timestamp.getTime()
      );
      
      for (const revision of sortedRevisions) {
        let stageInRevision: string | undefined = undefined;
        ...
        if (revision.data.strategy === RevisionStorageStrategy.FULL_SNAPSHOT) {
          ...
          const delta = revision.data.delta as any;
          const stageChange = delta?.changes?.find((c: any) => 
            c.field === 'structuredData.stage' || c.field === 'stage'
          );
          if (stageChange) {
      ...
      const daysInStage = Math.floor(
        (Date.now() - lastStageChangeDate.getTime()) / (1000 * 60 * 60 * 24)
      );
  ...
  async checkActivityDrop(
    ...
    try {
      ...
      const recentRevisions = revisions.revisions.filter(
        r => r.timestamp.getTime() >= twoWeeksAgo
      );
      const olderRevisions = revisions.revisions.filter(
        r => r.timestamp.getTime() >= fourWeeksAgo && r.timestamp.getTime() < twoWeeksAgo
      );
   ...
   * Check for stakeholder churn
   */
  async checkStakeholderChurn(
    opportunity: Shard,
    tenantId: string
  ): Promise<EarlyWarningSignal | null> {
    try {
      // Get current stakeholder relationships
      const currentStakeholderEdges = await this.relationshipService.getRelationships(
        tenantId,
        opportunity.id,
        'both',
        {
      ...
      const currentChampionEdges = await this.relationshipService.getRelationships(
        tenantId,
        opportunity.id,
        'both',
        {
      ...
      const removedStakeholderIds = new Set<string>();
      ...
      for (const revision of revisions.revisions) {
        ...
        if (revision.data.strategy === RevisionStorageStrategy.DELTA) {
          const delta = revision.data.delta as any;
          const relationshipChanges = delta?.changes?.filter((c: any) => 
            c.field?.includes('internal_relationships') || 
  ...
  async checkRiskAcceleration(
    ...
    try {
      ...
      const currentEvaluation = await this.riskEvaluationService.evaluateOpportunity(
        opportunityId,
        tenantId,
        userId,
