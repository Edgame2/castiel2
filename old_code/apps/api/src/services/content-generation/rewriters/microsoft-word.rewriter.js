...
export class MicrosoftWordRewriter extends BaseDocumentRewriter {
            ...
            try {
                ...
                if (statusCode === 429) {
                    if (attempt >= maxRetries) {
                        ...
                        throw new Error(`Microsoft Graph API rate limit exceeded after ${maxRetries} attempts. Please try again later. ` +
                ...
                if (statusCode === 503 || statusCode === 502 || statusCode === 504) {
                    if (attempt >= maxRetries) {
                        ...
                        throw new Error(`Microsoft Graph API service unavailable after ${maxRetries} attempts. Please try again later. ` +
                            `Original error: ${error.message}`);
                    }
                    // Calculate exponential backoff: 2^attempt seconds (capped at 60 seconds)
                    const backoffSeconds = Math.min(Math.pow(2, attempt), 60);
                ...
                if (errorCode === 'ECONNABORTED' || errorCode === 'ETIMEDOUT' || error.message?.includes('timeout')) {
                    if (attempt >= maxRetries) {
                        ...
                        throw new Error(`Microsoft Graph API request timed out after ${maxRetries} attempts. ` +
    ...
    async duplicateDocument(sourceDocumentId, newName, destinationFolderId, auth) {
        return this.executeWithRetry(async () => {
            ...
            const _response = await axios.post(`${this.graphApiBaseUrl}/me/drive/items/${sourceDocumentId}/copy`, {
                name: newName,
                parentReference: {
            ...
            const folderResponse = await axios.get(`${this.graphApiBaseUrl}/me/drive/items/${destinationFolderId}/children?$filter=name eq '${encodeURIComponent(newName)}'`, {
                headers: {
                    Authorization: `Bearer ${auth.accessToken}`,
                },
                timeout: this.config.apiRequestTimeoutMs,
     ...
     */
    async replacePlaceholders(documentId, _template, generatedValues, auth) {
        return this.executeWithRetry(async () => {
            try {
                // Step 1: Download document content
                const downloadResponse = await axios.get(`${this.graphApiBaseUrl}/me/drive/items/${documentId}/content`, {
                    headers: {
                        Authorization: `Bearer ${auth.accessToken}`,
                    },
                ...
                await axios.put(`${this.graphApiBaseUrl}/me/drive/items/${documentId}/content`, modifiedDocxBuffer, {
                    headers: {
                        Authorization: `Bearer ${auth.accessToken}`,
                ...
                throw new Error(`Failed to replace placeholders in Word document: ${error.message}`);
            }
     ...
     */
    async replacePlaceholdersInXmlFile(zip, xmlPath, generatedValues) {
        ...
        for (const [placeholderName, value] of Object.entries(generatedValues)) {
            ...
            const placeholderPattern = new RegExp(`\\{\\{${placeholderName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\}\\}`, 'g');
    ...
    async insertCharts(documentId, template, generatedCharts, auth) {
        ...
        return this.executeWithRetry(async () => {
            try {
                ...
                // Process main document
                await this.insertChartsInXmlFile(zip, 'word/document.xml', chartPlaceholders, generatedCharts, rels, relationshipId);
                // Process headers
                const headerFiles = Object.keys(zip.files).filter(name => name.startsWith('word/header') && name.endsWith('.xml'));
                for (const headerFile of headerFiles) {
                    await this.insertChartsInXmlFile(zip, headerFile, chartPlaceholders, generatedCharts, rels, relationshipId);
                }
                // Process footers
                const footerFiles = Object.keys(zip.files).filter(name => name.startsWith('word/footer') && name.endsWith('.xml'));
                for (const footerFile of footerFiles) {
                    await this.insertChartsInXmlFile(zip, footerFile, chartPlaceholders, generatedCharts, rels, relationshipId);
                }
                ...
                throw new Error(`Failed to insert charts in Microsoft Word document: ${error.message}`);
            }
        }, 'insert_charts');
    }
    /**
     ...
     */
    async insertChartsInXmlFile(zip, xmlPath, chartPlaceholders, generatedCharts, rels, relationshipId) {
        const xmlFile = zip.file(xmlPath);
        if (!xmlFile) {
            return; // File doesn't exist, skip
        }
        ...
        for (const placeholder of chartPlaceholders) {
            ...
            if (xmlContent.includes(placeholderText)) {
                ...
                xmlContent = xmlContent.replace(new RegExp(placeholderText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), imageDrawingXml);
            }
        }
        // Update the file in the ZIP
        zip.file(xmlPath, xmlContent);
     ...
     */
    createImageDrawingXml(relId, width, height) {
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
    ...
    async getFolderPath(folderId, auth) {
        return this.executeWithRetry(async () => {
            const response = await axios.get(`${this.graphApiBaseUrl}/me/drive/items/${folderId}`, {
                headers: {
                    Authorization: `Bearer ${auth.accessToken}`,
                },
                timeout: this.config.apiRequestTimeoutMs,
    ...
    async deleteDocument(documentId, auth) {
        return this.executeWithRetry(async () => {
            try {
                await axios.delete(`${this.graphApiBaseUrl}/me/drive/items/${documentId}`, {
                    headers: {
                        Authorization: `Bearer ${auth.accessToken}`,
                    },
                    timeout: this.config.apiRequestTimeoutMs,
                ...
                throw new Error(`Failed to delete Word document: ${error.message}`);
            }
        }, 'delete');
    }
}
//# sourceMappingURL=microsoft-word.rewriter.js.map