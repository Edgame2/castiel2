...
export class PipelineAnalyticsService {
     ...
     */
    async calculatePipelineMetrics(userId, tenantId, options) {
        ...
        try {
            ...
            const result = await this.opportunityService.listOwnedOpportunities(userId, tenantId, filters, { limit: 1000 });
     ...
     */
    async calculateClosedWonLost(userId, tenantId, period) {
        const startTime = Date.now();
        try {
            // Get closed won opportunities
            const wonResult = await this.opportunityService.listOwnedOpportunities(userId, tenantId, {
                ownerId: userId,
                status: ['won'],
            ...
            const lostResult = await this.opportunityService.listOwnedOpportunities(userId, tenantId, {
                ownerId: userId,
                status: ['lost'],
                closeDateFrom: period.startDate,
                closeDateTo: period.endDate,
     ...
     */
    async organizeByRiskLevel(userId, tenantId) {
        const startTime = Date.now();
        try {
            ...
            const calculateTotals = (opportunities) => {
                let totalValue = 0;
                let totalRevenueAtRisk = 0;
     ...
     */
    async generateRecommendations(opportunityId, tenantId, userId) {
        const startTime = Date.now();
        try {
            const opportunity = await this.opportunityService.getOpportunity(opportunityId, tenantId, true);
            const data = opportunity.opportunity.structuredData;
