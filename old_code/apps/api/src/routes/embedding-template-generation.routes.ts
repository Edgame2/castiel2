...
export async function registerEmbeddingTemplateGenerationRoutes(
  server: FastifyInstance,
  monitoring: IMonitoringProvider
): Promise<void> {
  const shardTypeRepository = (server as any).shardTypeRepository
  const promptResolverService = (server as any).promptResolverService
  const azureOpenAI = (server as any).azureOpenAI
  ...
  server.post<{ Body: GenerateTemplateRequest }>(
    '/embedding-templates/generate',
    {
      schema: {
    ...
    async (req: FastifyRequest<{ Body: GenerateTemplateRequest }>, reply: FastifyReply) => {
      try {
        ...
        const shardType = await shardTypeRepository.findById(shardTypeId, user.tenantId).catch((error: unknown) => {
          monitoring.trackException(error as Error, { operation: 'embedding-template.findById', shardTypeId, tenantId: user.tenantId });
          return null;
        })

        // Find prompts by tag (embeddingTemplate)
        const prompts = promptResolverService
          ? await promptResolverService.listByTags(user.tenantId, [promptTag]).catch((error: unknown) => {
              monitoring.trackException(error as Error, { operation: 'embedding-template.listByTags', tenantId: user.tenantId, promptTag });
              return [];
            })
          : []
...
The JSON must match this structure:
{
  ...
  "enableVectorSearch": true
        ...
        let generatedTemplate: EmbeddingTemplate
        try {
