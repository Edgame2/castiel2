...
export class TenantService {
   ...
   * Find tenant by slug
   */
  private async findBySlug(slug: string): Promise<TenantDocument | null> {
    const { resources } = await this.tenantsContainer.items
      .query<TenantDocument>({
   ...
   */
  private async findByDomain(domain: string): Promise<TenantDocument | null> {
    const { resources } = await this.tenantsContainer.items
      .query<TenantDocument>({
   ...
   */
  private toResponse(tenant: TenantDocument): TenantResponse {
    return {
      id: tenant.id,
      name: tenant.name,
  ...
  async getTenantByDomain(domain: string): Promise<TenantResponse | null> {
    const tenant = await this.findByDomain(domain);
    return tenant ? this.toResponse(tenant) : null;
  }

  async appendAdminUser(tenantId: string, userId: string): Promise<void> {
   ...
   */
  static async ensureContainer(database: any): Promise<Container> {
