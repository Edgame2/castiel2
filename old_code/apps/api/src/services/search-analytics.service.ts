...
export class SearchAnalyticsService {
  ...
  private interactionsContainer: Container;
  private readonly RETENTION_DAYS = 90; // Keep analytics for 90 days
   ...
   */
  async recordSearchEvent(event: Omit<SearchQueryEvent, 'id' | 'queryHash'>): Promise<SearchQueryEvent> {
    const queryHash = this.hashQuery(event.query, event.filters);
    
    const searchEvent: SearchQueryEvent = {
      ...event,
      id: uuidv4(),
      queryHash,
    };
   ...
   */
  async recordInteraction(event: Omit<SearchInteractionEvent, 'queryHash'> & { query: string; filters?: Record<string, any> }): Promise<void> {
    const queryHash = this.hashQuery(event.query, event.filters);
    
    const interaction: SearchInteractionEvent = {
      ...event,
      queryHash,
    };
  ...
  async getQueryAnalytics(
    ...
    try {
      ...
      const { resources } = await this.analyticsContainer.items.query<SearchQueryEvent>(cosmosQuery).fetchAll();
  ...
  async getZeroResultQueries(
    ...
    try {
      ...
      const { resources } = await this.analyticsContainer.items.query<SearchQueryEvent>(query).fetchAll();

      // Group by query hash
      const grouped = new Map<string, SearchQueryEvent[]>();
      ...
      const zeroResultQueries: ZeroResultQuery[] = Array.from(grouped.entries()).map(([queryHash, events]) => {
  ...
  async getSatisfactionMetrics(
    ...
    try {
      ...
      const { resources } = await this.interactionsContainer.items.query<SearchInteractionEvent>(query).fetchAll();

      // Group by query hash
      const grouped = new Map<string, SearchInteractionEvent[]>();
      for (const event of resources) {
        const existing = grouped.get(event.queryHash) || [];
        existing.push(event);
        grouped.set(event.queryHash, existing);
      ...
      const metrics: SatisfactionMetric[] = Array.from(grouped.entries()).map(([hash, events]) => {
   ...
   */
  async getPopularTermsDashboard(
    request: SearchAnalyticsRequest
  ): Promise<PopularTermsDashboard> {
  ...
  private getStartDateForPeriod(endDate: Date, period: 'day' | 'week' | 'month' | 'all'): Date {
  ...
  private generateSuggestions(event: SearchQueryEvent): string[] {
  ...
  private async getTopQueries(
    ...
    _endDate: Date,
    _limit: number
  ): Promise<PopularTermsDashboard['topQueries']> {
    // Implementation would query Cosmos DB or Redis
    // For now, return empty array
    return [];
  }

  private async getTopZeroResultQueries(
    _tenantId: string,
    _startDate: Date,
    _endDate: Date,
    _limit: number
  ): Promise<PopularTermsDashboard['topZeroResultQueries']> {
    // Implementation would query Cosmos DB or Redis
    // For now, return empty array
    return [];
  }

  private async getSearchVolumeTrend(
    _tenantId: string,
    _startDate: Date,
    _endDate: Date
  ): Promise<PopularTermsDashboard['searchVolumeTrend']> {
    // Implementation would query Cosmos DB or Redis
    // For now, return empty array
    return [];
  }
  ...
  private async getAggregateMetrics(
    ...
    searchesByShardType?: Array<{ shardTypeId: string; count: number; averageResultCount: number }>;
  }> {
