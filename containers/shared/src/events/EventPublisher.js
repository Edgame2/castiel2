...
export class EventPublisher {
     ...
     * Publish event
     */
    async publish(eventType, tenantId, data, options) {
        if (!this.channel) {
            await this.connect();
        }
        if (!this.channel) {
        ...
        // Create event
        const event = createEvent(eventType, tenantId, data, {
        ...
        // Publish to exchange
        const published = this.channel.publish(this.exchange, routingKey, Buffer.from(JSON.stringify(event)), {
