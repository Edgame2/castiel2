...
} from '../types/project-analytics.types.js';
...
export class ProjectAnalyticsService {
   ...
   */
  async getProject(projectId: string, tenantId: string): Promise<Shard> {
    try {
   ...
   */
  async calculateHealthScore(project: Shard): Promise<ProjectHealthScore> {
    ...
    // Determine status
    let status: 'healthy' | 'at_risk' | 'critical' | 'unknown';
    if (overallScore >= 80) {
      status = 'healthy';
   ...
   */
  async calculatePredictiveCompletion(project: Shard): Promise<PredictiveCompletion> {
    const data = project.structuredData || {};
   ...
   */
  async calculateResourceOptimization(project: Shard): Promise<ResourceOptimization> {
    const data = project.structuredData || {};
  ...
  private calculateTimelineScore(data: any, factors: HealthFactor[]): number {
  ...
  private calculateBudgetScore(data: any, factors: HealthFactor[]): number {
  ...
  private calculateMilestoneScore(data: any, factors: HealthFactor[]): number {
  ...
  private async calculateActivityScore(project: Shard, factors: HealthFactor[]): Promise<number> {
  ...
  private calculateRiskScore(data: any, factors: HealthFactor[]): number {
  ...
  private calculateVelocity(data: any, factors: CompletionFactor[]): number {
    const progress = data.progress || 0;
    const startDate = data.startDate ? new Date(data.startDate) : null;
    const now = new Date();
  ...
  private calculateRemainingWork(data: any, factors: CompletionFactor[]): number {
    const progress = data.progress || 0;
  ...
  private calculateTeamCapacity(data: any, factors: CompletionFactor[]): number {
    const team = data.team || [];
  ...
  private calculateHistoricalFactor(project: Shard, factors: CompletionFactor[]): number {
  ...
  private calculateConfidence(factors: CompletionFactor[]): number {
    ...
    if (hasVelocity) {confidence += 0.2;}
    if (hasRemainingWork) {confidence += 0.2;}
    if (hasTeamCapacity) {confidence += 0.1;}
  ...
  private analyzeCurrentState(data: any): any {
    const team = data.team || [];
    const budget = data.budget || {};
    const startDate = data.startDate ? new Date(data.startDate) : null;
    const targetDate = data.targetDate ? new Date(data.targetDate) : null;
    ...
    return {
      teamAllocation: team.map((member: any, index: number) => {
  ...
  private generateTeamRecommendations(data: any, recommendations: OptimizationRecommendation[]): void {
    const team = data.team || [];
    if (team.length === 0) {
  ...
  private generateBudgetRecommendations(data: any, recommendations: OptimizationRecommendation[]): void {
    const budget = data.budget || {};
    const spent = budget.spent || 0;
    const allocated = budget.allocated || 0;
    const progress = data.progress || 0;
  ...
  private generateTimelineRecommendations(data: any, recommendations: OptimizationRecommendation[]): void {
    const startDate = data.startDate ? new Date(data.startDate) : null;
    const targetDate = data.targetDate ? new Date(data.targetDate) : null;
    const progress = data.progress || 0;
    const now = new Date();
  ...
  private generatePriorityRecommendations(data: any, recommendations: OptimizationRecommendation[]): void {
    const priority = data.priority;
    const status = data.status;
  ...
  private calculateOptimizedState(currentState: any, _recommendations: OptimizationRecommendation[]): any {
  ...
  private calculatePotentialImprovement(
    _healthScore: ProjectHealthScore,
