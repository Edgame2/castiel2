...
export class DashboardRepository {
  ...
  async createDashboard(input: CreateDashboardInput): Promise<Dashboard> {
    ...
    try {
      ...
      const structuredData: DashboardStructuredData = {
        name: input.name,
  ...
  async getDashboardWithWidgets(
    ...
    try {
      ...
      const dashboardDoc = resources.find((r: any) => r.shardTypeId === 'c_dashboard');
      if (!dashboardDoc) {
        return null;
      }

      const widgetDocs = resources.filter((r: any) => r.shardTypeId === 'c_dashboardWidget');

      const dashboard = this.shardToDashboard(dashboardDoc);
      const widgets = widgetDocs.map((r: any) => this.shardToWidget(r));

      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.dashboard.getWithWidgets',
        'CosmosDB',
        config.cosmosDb.endpoint,
        duration,
        true
  ...
  async updateDashboard(
    ...
    try {
      ...
      const now = new Date();
      const updatedStructuredData: DashboardStructuredData = {
  ...
  async deleteDashboard(id: string, tenantId: string): Promise<boolean> {
    ...
    try {
      ...
      const batchErrors: Array<{ chunkIndex: number; error: Error }> = [];
  ...
  async listDashboards(options: DashboardListOptions): Promise<DashboardListResult> {
    ...
    try {
      ...
      const dashboards = resources.slice(0, limit).map((r: any) => this.shardToDashboard(r));
      ...
      try {
        ...
        if (filter.userId) {finalCountQuery += ` AND c.structuredData.ownerId = @userId`;}
        if (filter.dashboardType) {finalCountQuery += ` AND c.structuredData.dashboardType = @dashboardType`;}
        if (filter.isDefault !== undefined) {finalCountQuery += ` AND c.structuredData.isDefault = @isDefault`;}
        if (filter.isTemplate !== undefined) {finalCountQuery += ` AND c.structuredData.isTemplate = @isTemplate`;}
        if (filter.search) {finalCountQuery += ` AND CONTAINS(LOWER(c.structuredData.name), LOWER(@search))`;}
        ...
        this.monitoring.trackException(countError as Error, { operation: 'dashboard.repository.count' });
      }

      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.dashboard.list',
        'CosmosDB',
        config.cosmosDb.endpoint,
        duration,
        true
   ...
   */
  async getInheritedDashboards(tenantId: string): Promise<Dashboard[]> {
    const startTime = Date.now();
    ...
    try {
      ...
      const dashboards = [
        ...systemResult.resources.map((r: any) => this.shardToDashboard(r)),
        ...tenantResult.resources.map((r: any) => this.shardToDashboard(r)),
      ];

      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.dashboard.getInherited',
        'CosmosDB',
        config.cosmosDb.endpoint,
        duration,
        true
  ...
  async createWidget(input: CreateWidgetInput): Promise<Widget> {
    ...
    try {
      ...
      const structuredData: WidgetStructuredData = {
        name: input.name,
        description: input.description,
        icon: input.icon,
  ...
  async getWidgetsByDashboard(dashboardId: string, tenantId: string): Promise<Widget[]> {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.widget.getByDashboard',
        'CosmosDB',
        config.cosmosDb.endpoint,
        duration,
        true
      ...
      return resources.map((r: any) => this.shardToWidget(r));
    } catch (error) {
      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.widget.getByDashboard',
  ...
  async batchUpdatePositions(
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.widget.batchUpdatePositions',
        'CosmosDB',
        config.cosmosDb.endpoint,
        duration,
        true
  ...
  async saveDashboardVersion(
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.dashboardVersion.save',
        'CosmosDB',
        config.cosmosDb.endpoint,
        duration,
        true
  ...
  async getDashboardVersions(
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.dashboardVersion.list',
        'CosmosDB',
        config.cosmosDb.endpoint,
        duration,
        true
  ...
  async getUserOverrides(
    ...
    try {
      ...
      return resources[0] as UserDashboardOverrides;
    } catch (error) {
      return null;
    }
  }
   ...
   */
  async saveUserOverrides(overrides: UserDashboardOverrides & { tenantId: string }): Promise<void> {
   ...
   */
  async getTenantDashboardConfig(tenantId: string): Promise<TenantDashboardConfig | null> {
    const startTime = Date.now();
   ...
   */
  async saveTenantDashboardConfig(config: TenantDashboardConfig): Promise<void> {
    await this.configContainer.items.upsert({
   ...
   */
  async getTenantFiscalYearConfig(tenantId: string): Promise<TenantFiscalYearConfig | null> {
    const startTime = Date.now();
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      this.monitoring.trackDependency(
        'cosmosdb.fiscalYearConfig.read',
        'CosmosDB',
        config.cosmosDb.endpoint,
        duration,
        true
   ...
   */
  async saveTenantFiscalYearConfig(config: TenantFiscalYearConfig): Promise<void> {
    await this.configContainer.items.upsert({
   ...
   */
  async queryRecentShards(
    tenantId: string,
    page: number,
    pageSize: number
  ): Promise<{ results: unknown[]; totalCount: number; hasMore: boolean }> {
    try {
   ...
   */
  async queryShardCountByType(tenantId: string): Promise<Array<{ type: string; count: number }>> {
    try {
      const query = `
   ...
   */
  async queryShardCountByStatus(tenantId: string): Promise<Array<{ status: string; count: number }>> {
    try {
      const query = `
   ...
   */
  async queryUserActivityTimeline(
    tenantId: string,
    userId: string,
    page: number,
    pageSize: number
   ...
   */
  async queryTeamActivitySummary(tenantId: string): Promise<{
    totalShards: number;
    activeUsers: number;
    shardsToday: number;
   ...
   * Query storage usage
   */
  async queryStorageUsage(tenantId: string): Promise<{
    totalShards: number;
    totalSize: number;
   ...
   * Query active users
   */
  async queryActiveUsers(tenantId: string): Promise<Array<{ userId: string; activityCount: number }>> {
    try {
  ...
  async executeCustomWidgetQuery(params: {
    ...
    aggregation?: { type: string; field?: string; groupBy?: string };
   ...
   */
  private shardToDashboard(doc: any): Dashboard {
    return {
      id: doc.id,
   ...
   */
  private shardToWidget(doc: any): Widget {
    return {
      id: doc.id,
      tenantId: doc.tenantId,
      userId: doc.userId,
