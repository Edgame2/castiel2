...
export class GoogleWorkspaceAdapter extends BaseIntegrationAdapter {
    ...
    peopleClient = null;
    tasksClient = null;
    adminClient = null; // Admin SDK client (requires @googleapis/admin or use googleapis)
    oauth2Client = null;
    userEmail = null;
    ...
    async initializeClients() {
        ...
        this.peopleClient = google.people({ version: 'v1', auth: this.oauth2Client });
            ...
            do {
                ...
                for (const group of groups) {
                    ...
                    if (membersResponse.data.members) {
                        memberExternalIds.push(...membersResponse.data.members.map((m) => m.id || '').filter(Boolean));
                    }
                    ...
                    const nameMapping = config.teamNameMapping?.['name'] || 'name';
                    const teamName = group[nameMapping] || group.name || group.email;
                    teams.push({
                        externalId: group.id,
                        name: teamName,
                        managerExternalId,
    ...
    async fetchGmailMessages(options) {
        ...
        try {
            ...
            const messagePromises = response.data.messages.slice(0, limit).map((msg) => this.gmailClient.users.messages.get({
                userId,
                id: msg.id,
                format: 'full',
            }));
            ...
            const records = messageResults
                ...
                if (result.status === 'fulfilled' && result.value.data) {
                    ...
                    this.monitoring.trackException(result.status === 'rejected' ? (result.reason instanceof Error ? result.reason : new Error(String(result.reason))) : new Error('Unknown error'), {
     ...
     * Fetch Gmail threads
     */
    async fetchGmailThreads(options) {
        if (!this.gmailClient) {
            throw new Error('Gmail client not initialized');
        }
        const { limit = 100, filters, modifiedSince, externalUserId } = options;
        ...
        try {
            ...
            const threadPromises = response.data.threads.slice(0, limit).map((thread) => this.gmailClient.users.threads.get({
                userId,
                id: thread.id,
            }));
    ...
    async fetchCalendarEvents(options) {
        ...
        try {
            ...
            const records = response.data.items.map((event) => this.normalizeCalendarEvent(event));
            return {
                records,
                hasMore: !!response.data.nextPageToken,
                cursor: response.data.nextPageToken || undefined,
    ...
    async fetchDriveFiles(options) {
        ...
        try {
            ...
            if (modifiedSince) {
                queryParts.push(`modifiedTime > '${modifiedSince.toISOString()}'`);
            }
            if (query) {
    ...
    async fetchContacts(options) {
        ...
        try {
            ...
            const records = response.data.connections.map((contact) => this.normalizeContact(contact));
            return {
                records,
                hasMore: !!response.data.nextPageToken,
                cursor: response.data.nextPageToken || undefined,
    ...
    async fetchTasks(options) {
        ...
        const { limit = 100, filters } = options;
        const tasklistId = filters?.tasklist || '@default';
        ...
        if (entitiesToSearch.includes('gmail_message') && this.gmailClient) {
            try {
                ...
                if (messages.data.messages) {
                    // Use Promise.allSettled to handle individual message fetch failures gracefully
                    const messageDetailResults = await Promise.allSettled(messages.data.messages.slice(0, limit).map(msg => this.gmailClient.users.messages.get({
                        userId,
                        id: msg.id,
                        format: 'metadata',
     ...
     */
    calculateRelevanceScore(query, record) {
     ...
     */
    extractHighlights(query, record) {
        const highlights = [];
        Object.values(record).forEach((value) => {
            if (typeof value === 'string' && value.toLowerCase().includes(query)) {
     ...
     */
    async pushGmailMessage(data, operation) {
        if (!this.gmailClient) {
            throw new Error('Gmail client not initialized');
        }
        try {
     ...
     * Push Calendar event
     */
    async pushCalendarEvent(data, operation) {
        if (!this.calendarClient) {
            throw new Error('Calendar client not initialized');
        }
        try {
     ...
     * Push Drive file
     */
    async pushDriveFile(data, operation) {
        if (!this.driveClient) {
            throw new Error('Drive client not initialized');
        }
        try {
     ...
     * Push Contact
     */
    async pushContact(data, operation) {
        if (!this.peopleClient) {
            throw new Error('People client not initialized');
        }
        try {
    ...
    async getEntitySchema(entityName) {
        ...
        return (definition.availableEntities.find((e) => e.name === entityName) || null);
    }
    /**
     ...
     */
    async registerWebhook(events, callbackUrl, resource) {
        await this.initializeClients();
        ...
        try {
            if (entity === 'gmail' || entity === 'gmail_message' || entity === 'gmail_thread') {
                ...
                const channel = await this.calendarClient.events.watch({
                    calendarId,
                    requestBody: {
     ...
     */
    async unregisterWebhook(webhookId, resourceId) {
        await this.initializeClients();
        try {
 ...
 */
export const GOOGLE_WORKSPACE_DEFINITION = {
    id: 'google-workspace',
    ...
    oauthConfig: {
        ...
        redirectUri: config.googleWorkspace?.redirectUri || '',
        pkce: true,
        additionalParams: {
            access_type: 'offline',
            prompt: 'consent',
 ...
 */
export const googleWorkspaceAdapterFactory = {
...
//# sourceMappingURL=google-workspace.adapter.js.map