...
export class InsightsService {
    ...
    modelRouter;
    conversationService;
    constructor(monitoring, cosmosService, intentAnalyzer, contextService, modelRouter, conversationService) {
    ...
    async generateInsight(request) {
        ...
        try {
            ...
            if (modelInfo.pricing) {
                perf.cost = {
                    amount: (perf.tokensUsed.prompt / 1000) * modelInfo.pricing.promptPer1k +
     ...
     */
    async generateInsightStream(request, onChunk) {
        const startTime = Date.now();
        const insightId = uuidv4();
        const conversationId = request.conversationId || uuidv4();
        try {
            ...
            const modelInfo = await this.selectModel(request, intent);
            let fullContent = '';
            const generation = await this.generateStream(request, intent, context, modelInfo, (token) => {
                fullContent += token;
                onChunk({
                    type: 'token',
                    data: { token },
     ...
     * Get insight by ID
     */
    async getInsight(tenantId, insightId, userId) {
        ...
        return await this.cosmosService.read(container, insightId, partitionKey);
    }
    /**
     * List insights with pagination
     */
    async listInsights(tenantId, userId, options = {}) {
        const container = this.cosmosService.getFeedbackContainer();
        const querySpec = {
            query: `
        SELECT * FROM c 
      ...
      `,
        ...
        return await this.cosmosService.query(container, querySpec, {
     ...
     * Delete insight
     */
    async deleteInsight(tenantId, insightId, userId) {
        const container = this.cosmosService.getFeedbackContainer();
        const partitionKey = this.cosmosService.buildPartitionKey(tenantId, insightId, userId);
     ...
     */
    async assembleContext(request, intent) {
        const startTime = Date.now();
        try {
     ...
     */
    async selectModel(request, intent) {
        try {
     ...
     */
    async generate(request, intent, context, modelInfo) {
        const startTime = Date.now();
        try {
            // Build system prompt
     ...
     */
    async generateStream(request, intent, context, modelInfo, onToken) {
        const startTime = Date.now();
        try {
            const systemPrompt = this.buildSystemPrompt(intent);
            const userPrompt = this.buildUserPrompt(request.query, context);
    ...
    async ground(content, context) {
        ...
        try {
            ...
            const verificationDetails = await Promise.all(statements.map((statement) => this.verifyStatement(statement, context)));
     ...
     */
    async verifyStatement(statement, context) {
     ...
     */
    async storeInsight(response) {
        const container = this.cosmosService.getFeedbackContainer();
        const document = {
            id: response.id,
            tenantId: response.tenantId,
            partitionKey: this.cosmosService.buildPartitionKey(response.tenantId, response.id, response.userId),
            type: 'insight',
            ...response,
        };
