...
export class IntegrationVisibilityService {
     ...
     */
    async getTenantCatalogView(tenantContext) {
        ...
        for (const entry of catalogEntries) {
            ...
            // Check whitelist
            if (entry.allowedTenants !== null && entry.allowedTenants?.length > 0) {
            ...
            if (entry.requiredPlan && tenantContext.pricingTier) {
                ...
                const requiredLevel = this.getTierLevel(entry.requiredPlan);
            ...
            const visibilityRule = await this.catalogRepository.getVisibilityRuleByTenantAndIntegration(tenantContext.tenantId, entry.integrationId);
     ...
     */
    async isIntegrationAvailable(tenantContext, integrationId) {
        ...
        const visibilityRule = await this.catalogRepository.getVisibilityRuleByTenantAndIntegration(tenantContext.tenantId, integrationId);
        if (visibilityRule) {
            if (!visibilityRule.isVisible) {
                return {
                    available: false,
     ...
     */
    async getIntegrationForTenant(tenantContext, integrationId) {
     ...
     */
    async getEffectiveRateLimit(tenantContext, integrationId) {
        const entry = await this.catalogRepository.getCatalogEntryByIntegrationId(integrationId);
        if (!entry) {
            return null;
        }
     ...
     */
    async getEffectiveCapabilities(tenantContext, integrationId) {
        const entry = await this.catalogRepository.getCatalogEntryByIntegrationId(integrationId);
        if (!entry) {
            return null;
        }
        ...
        if (visibilityRule?.customCapabilities) {
            ...
            capabilities = capabilities.filter((c) => visibilityRule.customCapabilities.includes(c));
        }
        return capabilities;
    }
    /**
     ...
     */
    async getEffectiveSyncDirections(tenantContext, integrationId) {
        const entry = await this.catalogRepository.getCatalogEntryByIntegrationId(integrationId);
        if (!entry) {
            return null;
        }
...
//# sourceMappingURL=integration-visibility.service.js.map