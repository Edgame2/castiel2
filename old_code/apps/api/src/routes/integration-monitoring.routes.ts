 ...
 */
export async function registerIntegrationMonitoringRoutes(
...
async function getSyncStatistics(
  ...
  monitoring: IMonitoringProvider | undefined
): Promise<any> {
  if (!cosmosClient) {
 ...
 * Get sync activity
 */
async function getSyncActivity(
  cosmosClient: CosmosClient | null,
  query: z.infer<typeof SyncActivityQuerySchema>,
  monitoring: IMonitoringProvider | undefined
): Promise<any> {
  if (!cosmosClient) {
    return {
...
async function getPerformanceMetrics(
  cosmosClient: CosmosClient | null,
  query: z.infer<typeof PerformanceQuerySchema>,
  monitoring: IMonitoringProvider | undefined
): Promise<any> {
  if (!cosmosClient) {
    return {
  ...
  try {
    ...
    const parameters: any[] = [{ name: '@startDate', value: startDate }];
