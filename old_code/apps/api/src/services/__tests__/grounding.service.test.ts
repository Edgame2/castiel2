...
import { GroundingService, ClaimType } from '../grounding.service.js';
import type {
    GroundedResponse,
    AssembledContext,
    Citation,
...
const createMockLLMService = (): Partial<LLMService> => ({
...
const createMockContext = (overrides?: Partial<AssembledContext>): AssembledContext => ({
    primary: {
        shardId: 'primary_1',
...
describe('GroundingService', () => {
    ...
    beforeEach(() => {
        ...
        service = new GroundingService(mockLLM as LLMService);
    });

    describe('Main Grounding Pipeline', () => {
        it('should ground response with citations', async () => {
            const response =
                'The deal is at risk because no contact in 14 days and budget concerns were raised. The close date is Q4 and the value is $500K.';
    ...
    describe('Claim Extraction', () => {
        ...
        it('should identify analytical claims', async () => {
            ...
            const analyticalClaims = result.claims.filter(
                (c) => c.category === 'analytical'
            );
    ...
    describe('Verification Status', () => {
        ...
        it('should link verified claims to citations', async () => {
            ...
            const verifiedClaimsWithCitations = result.claims.filter(
    ...
    describe('Hallucination Detection', () => {
        ...
        it('should handle multiple hallucinations', async () => {
            const response =
                'The deal is $10 billion with 100 stakeholders and 50 decision makers.';
            const context = createMockContext();
    ...
    describe('Citation Generation', () => {
        ...
        it('should avoid duplicate citations', async () => {
            const response =
                'The value is $500K. The value was set at $500K. The value is $500K.';
            const context = createMockContext();
    ...
    describe('Citation Injection', () => {
        ...
        it('should use sequential citation numbers', async () => {
            ...
            const citationNumbers = (result.groundedContent.match(/\[(\d+)\]/g) || []).map(
        ...
        it('should handle multiple citations for same claim', async () => {
            ...
            expect(result.groundedContent.length).toBeGreaterThanOrEqual(
                response.length
            );
        });
    });
    ...
    describe('Score Calculation', () => {
        ...
        it('should calculate lower score for unverified response', async () => {
            const unverifiedResponse =
                'The deal value is $10 billion with 1000 stakeholders.';
            const context = createMockContext();
        ...
        it('should reflect hallucinations in score', async () => {
            ...
            const resultWithError = await service.ground(
                responseWithHallucination,
                context
            );

            // Error case should have equal or lower score
            expect(resultWithError.groundingScore).toBeLessThanOrEqual(
                resultVerified.groundingScore + 0.1
            );
        });
    ...
    describe('Error Handling', () => {
        it('should handle LLM failures gracefully', async () => {
            const badLLM: Partial<LLMService> = {
                complete: vi.fn().mockRejectedValue(new Error('LLM error')),
            };
            const badService = new GroundingService(badLLM as LLMService);
    ...
    describe('Integration with Response Types', () => {
        ...
        it('should provide correct claim structure', async () => {
            ...
            for (const claim of result.claims) {
                ...
                expect(['factual', 'analytical', 'opinion', 'prediction']).toContain(
                    claim.category
                );
            }
        });
