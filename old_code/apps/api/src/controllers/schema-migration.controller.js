...
import { detectSchemaFormat } from '../types/shard-type.types.js';
...
export class SchemaMigrationController {
    ...
    constructor(monitoring, shardRepository, shardTypeRepository) {
        ...
        this.migrationService = new SchemaMigrationService(monitoring, shardRepository, shardTypeRepository);
    }
    /**
     ...
     * List migrations
     */
    listMigrations = async (req, reply) => {
     ...
     */
    previewSchemaChanges = async (req, reply) => {
        const startTime = Date.now();
        try {
            const auth = req.auth;
            if (!auth || !auth.tenantId) {
            ...
            // Check compatibility
            const compatibility = this.migrationService.checkCompatibility(shardType.schema, newSchema, schemaFormat);
     ...
     */
    createMigration = async (req, reply) => {
        const startTime = Date.now();
        try {
            const auth = req.auth;
            ...
            // Create migration
            const migration = await this.migrationService.createMigration(tenantId, shardType, newSchema, userId, { strategy, fieldMappings, defaultValues });
     ...
     */
    cancelMigration = async (req, reply) => {
        const startTime = Date.now();
        try {
            const auth = req.auth;
            if (!auth || !auth.tenantId) {
     ...
     */
    getMigrationsForShardType = async (req, reply) => {
        const startTime = Date.now();
        try {
            const auth = req.auth;
            if (!auth || !auth.tenantId) {
     ...
     */
    getMigrationPath = async (req, reply) => {
        const startTime = Date.now();
        try {
            const auth = req.auth;
            if (!auth || !auth.tenantId) {
            ...
            const migrations = await this.migrationService.getMigrationPath(tenantId, shardTypeId, fromVersion, toVersion);
