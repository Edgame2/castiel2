...
export class SyncTaskRepository {
     ...
     */
    async updateAfterExecution(id, tenantId, results) {
        const existing = await this.findById(id, tenantId);
        if (!existing) {
            return null;
        }
     ...
     */
    calculateNextRun(schedule, fromDate = new Date()) {
...
export class SyncExecutionRepository {
     ...
     */
    async findRunningByTask(syncTaskId, tenantId) {
        const query = {
     ...
     */
    async complete(id, tenantId, status, results) {
        const existing = await this.findById(id, tenantId);
        if (!existing) {
            return null;
        }
...
export class SyncConflictRepository {
     ...
     * Resolve conflict
     */
    async resolve(id, tenantId, input) {
        const existing = await this.findById(id, tenantId);
        if (!existing) {
            return null;
        }
...
//# sourceMappingURL=sync-task.repository.js.map