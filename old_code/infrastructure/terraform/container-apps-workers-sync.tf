# Container App for Workers Sync
# Excluded for hybrid-dev (containers run locally)

resource "azurerm_container_app" "workers_sync" {
  count = local.is_full_deployment ? 1 : 0

  name                         = "${var.resource_prefix}-workers-sync-${var.environment}"
  ...
  template {
    ...
    container {
      name   = "workers-sync"
      image  = "${azurerm_container_registry.main[0].login_server}/workers-sync:latest"
      cpu    = 0.5
      memory = "1Gi"
...
resource "azurerm_key_vault_access_policy" "container_app_workers_sync" {
  count = local.is_full_deployment ? 1 : 0

  key_vault_id = azurerm_key_vault.main.id
  tenant_id    = azurerm_container_app.workers_sync[0].identity[0].tenant_id
  object_id    = azurerm_container_app.workers_sync[0].identity[0].principal_id
...
resource "azurerm_role_assignment" "container_app_workers_sync_cosmos" {
  count = local.is_full_deployment ? 1 : 0
