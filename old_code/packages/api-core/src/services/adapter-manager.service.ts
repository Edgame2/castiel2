...
export class AdapterManagerService {
  ...
  async getAdapter(
    ...
    if (userId && integration.userScoped && this.connectionRepository) {
      try {
        ...
        const userConnections = await this.connectionRepository.findUserScoped(
          integration.id,
   ...
   */
  async testAdapterConnection(adapter: IntegrationAdapter): Promise<{ success: boolean; error?: string }> {
    try {
