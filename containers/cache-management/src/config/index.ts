/**
 * Configuration loader for cache-management module
 */
...
import { CacheManagementConfig } from '../types/config.types';

let cachedConfig: CacheManagementConfig | null = null;
...
export function loadConfig(): CacheManagementConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as CacheManagementConfig;
  let envConfig: Partial<CacheManagementConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<CacheManagementConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as CacheManagementConfig;
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'cache-management' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'cache-management' });
  }
...
export function getConfig(): CacheManagementConfig {
