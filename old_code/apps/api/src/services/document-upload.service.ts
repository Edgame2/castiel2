...
export class DocumentUploadService {
  ...
  async uploadDocument(
    ...
    this.monitoring.trackEvent('document-upload.received', { tenantId, fileName: fileData.fileName });

    try {
      // Step 1: Validate upload
      const validation = await this.validateUpload(
        fileData,
        tenantSettings,
        globalSettings
      );
      ...
      const blobUpload = await this.blobStorageService.uploadFile(
        tenantId,
        shardId,
        version,
        fileData.fileName,
      ...
      const versionEntry: DocumentVersionEntry = {
        version,
        uploadedAt: new Date(),
        uploadedBy: userId,
      ...
      if (this.serviceBusService) {
        if (verifyVirus) {
          ...
          try {
            ...
            this.monitoring.trackException(error as Error, { operation: 'document-upload.send-check-job' });
          ...
          try {
            const embeddingJobMessage: EmbeddingJobMessage = {
              shardId: shard.id,
              tenantId,
              shardTypeId: 'c_document',
              revisionNumber: 1,
          ...
          try {
            const projectId = metadata.projectId;
            const documentChunkJobMessage: DocumentChunkJobMessage = {
              shardId: shard.id,
              tenantId,
      ...
      return {
        success: true,
        shard: shard as unknown as DocumentShard,
        storagePath: blobUpload.path,
        fileSize: fileData.fileSize,
  ...
  async initializeChunkedUpload(
    ...
    try {
      ...
      const initResult = await this.blobStorageService.initChunkedUpload(
        tenantId,
        shardId,
        version,
        fileName,
  ...
  async completeChunkedUpload(
    ...
    try {
      ...
      const completeResult = await this.blobStorageService.completeChunkedUpload(
        sessionId,
        {
      ...
      const pathParts = this.blobStorageService.parseBlobPath(
        completeResult.blobPath
      );
      if (!pathParts) {
        throw new DocumentUploadError(
          'Failed to parse blob path',
          'INVALID_BLOB_PATH'
        );
      }
      ...
      const structuredData: DocumentStructuredData = {
        ...
        visibility:
          session.documentMetadata.visibility || tenantSettings.defaultVisibility,
        version,
        versionHistory: [versionEntry],
        uploadedBy: userId,
        uploadedByEmail: userEmail,
  ...
  private async validateUpload(
    ...
    const resetResult = this.validationService.resetRateLimitCounters(
      tenantSettings
    );
    ...
    return this.validationService.validateUpload(
      fileData.fileName,
      fileData.fileSize,
      fileData.mimeType,
      tenantSettings,
  ...
  private async validateMetadata(
    ...
    if (metadata.category) {
      const categoryValidation = this.validationService.validateCategory(
        metadata.category,
        tenantSettings
      );
    ...
    if (metadata.tags && metadata.tags.length > 0) {
      const tagsValidation = this.validationService.validateTags(
        metadata.tags,
        tenantSettings
      );
    ...
    if (metadata.visibility) {
      const visibilityValidation = this.validationService.validateVisibility(
        metadata.visibility,
        tenantSettings
      );
   ...
   */
  private async updateTenantCounters(
    tenantId: string,
    fileSize: number,
    tenantSettings: TenantDocumentSettings
  ): Promise<void> {
