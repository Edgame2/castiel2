...
export async function registerDocumentTemplateRoutes(
  ...
  server.register(async (api) => {
    ...
    api.post(
      ...
      controller.extractPlaceholders as any
    );
    ...
    api.get('/content/templates/:id', controller.get as any);
    ...
    api.delete('/content/templates/:id', controller.delete as any);

    // Activate template (Admin only)
    api.put('/content/templates/:id/activate', controller.activate as any);

    // Archive template (Admin only)
    api.put('/content/templates/:id/archive', controller.archive as any);

    // Update template colors (Admin only)
    api.put(
      '/content/templates/:id/colors',
      {
        schema: {
          body: {
            type: 'object',
      ...
      controller.updateColors as any
    );

    // Update placeholder configurations (Admin only)
    api.put(
      '/content/templates/:id/placeholders',
      {
        schema: {
          body: {
            type: 'object',
      ...
      controller.updatePlaceholderConfigs as any
    );
    ...
    api.get('/content/templates/:id/versions', controller.getVersions as any);

    api.post(
      '/content/templates/:id/rollback',
      {
        schema: {
          body: {
            type: 'object',
      ...
      controller.rollback as any
    );

    // Preview/Test placeholder generation (Admin only)
    api.post(
      '/content/templates/:id/placeholders/:placeholderName/test',
      {
        schema: {
          body: {
            type: 'object',
      ...
      controller.testPlaceholder as any
    );

    // Validate all placeholders (Admin only)
    api.get('/content/templates/:id/validate', controller.validate as any);

    // Generate document from template (User access)
    api.post(
      '/content/templates/:id/generate',
      {
    ...
    api.get(
      ...
      controller.findStuckJobs as any
    );

    // Mark stuck generation jobs as failed (Admin only)
    api.post(
      '/content/jobs/stuck/mark-failed',
      {
        schema: {
      ...
      controller.markStuckJobsAsFailed as any
    );

    // Cleanup old generation jobs (Admin only)
    api.post(
      '/content/jobs/cleanup',
      {
        schema: {
      ...
      controller.cleanupOldJobs as any
    );
    ...
    api.get(
      ...
      async (request: import('fastify').FastifyRequest, reply: import('fastify').FastifyReply) => {
        // Health check endpoint - check service dependencies
        const health = {
          ...
          checks: {
            serviceBus: {
              status: 'unknown',
              message: 'Service Bus check not implemented',
            },
            cosmosDb: {
              status: 'unknown',
              message: 'Cosmos DB check not implemented',
            },
    ...
    api.get(
      ...
      controller.listGoogleDriveFolders as any
    );

    // List OneDrive folders
    api.get(
      '/content/folders/microsoft',
      {
        schema: {
      ...
      controller.listOneDriveFolders as any
    );

    // Get folder details
    api.get(
      '/content/folders/:provider/:folderId',
      {
        schema: {
      ...
      controller.getFolderDetails as any
    );
  });
}
