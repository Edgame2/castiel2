/**
 * Configuration loader for ai-insights module
...
let cachedConfig: AIInsightsConfig | null = null;
...
export function loadConfig(): AIInsightsConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as AIInsightsConfig;
  
  let envConfig: Partial<AIInsightsConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<AIInsightsConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as AIInsightsConfig;
