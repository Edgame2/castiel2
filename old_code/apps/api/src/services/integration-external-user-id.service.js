...
export class IntegrationExternalUserIdService {
     ...
     */
    async storeExternalUserId(userId, tenantId, data) {
        try {
            // Get current user
            ...
            const existingIndex = externalUserIds.findIndex((ext) => ext.integrationId === data.integrationId);
            const now = new Date();
     ...
     */
    async getExternalUserId(userId, tenantId, integrationId) {
        try {
            const { resource: user } = await this.userContainer
                .item(userId, tenantId)
                .read();
            ...
            return (user.externalUserIds.find((ext) => ext.integrationId === integrationId) || null);
        }
        catch (error) {
            this.monitoring.trackException(error instanceof Error ? error : new Error(String(error)), {
     ...
     */
    async getAllExternalUserIds(userId, tenantId) {
        try {
            const { resource: user } = await this.userContainer
                .item(userId, tenantId)
                .read();
     ...
     */
    async getUserByExternalId(externalUserId, tenantId, integrationId) {
        try {
     ...
     */
    async updateExternalUserId(userId, tenantId, integrationId, updates) {
        try {
            const { resource: user } = await this.userContainer
                .item(userId, tenantId)
                .read();
            ...
            const externalUserIds = user.externalUserIds || [];
            const existingIndex = externalUserIds.findIndex((ext) => ext.integrationId === integrationId);
            if (existingIndex < 0) {
     ...
     */
    async removeExternalUserId(userId, tenantId, integrationId) {
        try {
            const { resource: user } = await this.userContainer
                .item(userId, tenantId)
                .read();
            ...
            const externalUserIds = user.externalUserIds || [];
            const filtered = externalUserIds.filter((ext) => ext.integrationId !== integrationId);
     ...
     */
    async syncExternalUserIdFromConnection(userId, tenantId, integrationId, connectionId) {
        try {
     ...
     */
    async checkConflict(tenantId, integrationId, externalUserId, excludeUserId) {
        try {
            const query = `
      ...
      `;
            ...
            for (const user of users) {
                ...
                const matching = user.externalUserIds?.find((ext) => ext.integrationId === integrationId &&
...
//# sourceMappingURL=integration-external-user-id.service.js.map