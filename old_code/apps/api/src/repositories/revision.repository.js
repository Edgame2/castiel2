...
import { DEFAULT_RETENTION_POLICY, calculateRevisionTTL, RevisionStorageStrategy, } from '../types/revision.types.js';
 ...
 */
const REVISION_CONTAINER_CONFIG = {
...
export class RevisionRepository {
    ...
    constructor(monitoring, retentionPolicy = DEFAULT_RETENTION_POLICY, compressionThreshold = 10 * 1024 // 10KB
    ) {
    ...
    async create(input) {
        ...
        try {
            ...
            const { data: processedData, compressed, originalSize, compressedSize } = await this.compressData(input.data.snapshot || input.data.delta);
            ...
            this.monitoring.trackDependency('cosmosdb.revision.create', 'CosmosDB', config.cosmosDb.endpoint, duration, true);
            ...
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.create', 'CosmosDB', config.cosmosDb.endpoint, duration, false);
            this.monitoring.trackException(error instanceof Error ? error : new Error(String(error)), {
    ...
    async findById(id, tenantId) {
        ...
        try {
            ...
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.findById', 'CosmosDB', config.cosmosDb.endpoint, duration, true);
            if (!resource) {
                return null;
            }
            ...
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.findById', 'CosmosDB', config.cosmosDb.endpoint, duration, false);
     ...
     */
    async findByRevisionNumber(shardId, tenantId, revisionNumber) {
        const startTime = Date.now();
        try {
            const querySpec = {
            ...
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.findByRevisionNumber', 'CosmosDB', config.cosmosDb.endpoint, duration, true);
            ...
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.findByRevisionNumber', 'CosmosDB', config.cosmosDb.endpoint, duration, false);
            this.monitoring.trackException(error instanceof Error ? error : new Error(String(error)), {
     ...
     */
    async getNextRevisionNumber(shardId, tenantId) {
        const startTime = Date.now();
        try {
            const querySpec = {
            ...
            const { resources } = await this.container.items.query(querySpec).fetchAll();
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.getNextRevisionNumber', 'CosmosDB', config.cosmosDb.endpoint, duration, true);
            const maxRevision = resources[0] || 0;
            return maxRevision + 1;
        }
        catch (error) {
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.getNextRevisionNumber', 'CosmosDB', config.cosmosDb.endpoint, duration, false);
     ...
     */
    async getLatestRevision(shardId, tenantId) {
        const startTime = Date.now();
        try {
            const querySpec = {
            ...
            const { resources } = await this.container.items.query(querySpec).fetchAll();
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.getLatestRevision', 'CosmosDB', config.cosmosDb.endpoint, duration, true);
            const revision = resources.length > 0 ? resources[0] : null;
            ...
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.getLatestRevision', 'CosmosDB', config.cosmosDb.endpoint, duration, false);
            this.monitoring.trackException(error instanceof Error ? error : new Error(String(error)), {
    ...
    async list(options) {
        ...
        try {
            ...
            const revisions = await Promise.all(resources.map(async (revision) => {
                if (revision.data.compressed) {
                    if (revision.data.snapshot) {
                        revision.data.snapshot = await this.decompressData(revision.data.snapshot, true);
                    }
            ...
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.list', 'CosmosDB', config.cosmosDb.endpoint, duration, true);
            return {
                revisions,
            ...
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.list', 'CosmosDB', config.cosmosDb.endpoint, duration, false);
            this.monitoring.trackException(error instanceof Error ? error : new Error(String(error)), {
     ...
     */
    async compareRevisions(fromRevisionId, toRevisionId, tenantId) {
        try {
            ...
            const changes = this.computeDiff(fromData, toData);
            return {
                fromRevision,
                toRevision,
                changes,
     ...
     */
    async getStats(shardId, tenantId) {
        const startTime = Date.now();
        try {
            const querySpec = {
                query: `
        ...
        `,
            ...
            for (const item of changeTypeResources) {
                changeTypeBreakdown[item.changeType] = item.count;
            }
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.getStats', 'CosmosDB', config.cosmosDb.endpoint, duration, true);
            return {
            ...
            const duration = Date.now() - startTime;
            this.monitoring.trackDependency('cosmosdb.revision.getStats', 'CosmosDB', config.cosmosDb.endpoint, duration, false);
            this.monitoring.trackException(error, {
     ...
     */
    async cleanupOldRevisions(tenantId) {
        try {
