...
export class SerpAPIProvider extends BaseSearchProvider {
    ...
    async search(query: string, options?: SearchOptions): Promise<ProviderSearchResponse> {
        return this.retryWithBackoff(async () => {
            ...
            const results = (response.data.organic_results || []).map(
...
export class BingSearchProvider extends BaseSearchProvider {
    ...
    async search(query: string, options?: SearchOptions): Promise<ProviderSearchResponse> {
        return this.retryWithBackoff(async () => {
            ...
            const startTime = Date.now();
            const results = (response.data.webPages?.value || []).map(
                (result: any, index: number) => ({
                    position: index + 1,
                    title: result.name,
                    url: result.url,
...
export class GoogleSearchProvider extends BaseSearchProvider {
    ...
    constructor(config: ProviderConfig & { searchEngineId: string }) {
        super(config);
