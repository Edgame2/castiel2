...
import { EventTriggerSource } from '../types/shard-event.types.js';
...
interface AuthContext {
  ...
  email: string;
  roles: string[];
  userId?: string; // Deprecated alias for id
}
...
export class ShardsController {
  private repository: ShardRepository;
  private revisionRepository: RevisionRepository;
  private monitoring: IMonitoringProvider;
  private eventService?: ShardEventService;
  private lazyMigrationService?: LazyMigrationService;
  private shardTypeRepository: ShardTypeRepository;
  private riskEvaluationService?: RiskEvaluationService;
  private serverInstance?: any; // Store server instance for lazy service retrieval

  constructor(
    monitoring: IMonitoringProvider,
    cacheService: ShardCacheService,
    eventService?: ShardEventService,
    enableLazyMigration: boolean = true,
    serviceBusService?: QueueService,
    redactionService?: any, // Phase 2: RedactionService
    ...
    serverInstance?: any, // Optional - for lazy service retrieval
  ) {
    this.monitoring = monitoring;
    this.repository = new ShardRepository(
      monitoring,
      cacheService,
      serviceBusService,
      redactionService,
    ...
    if (enableLazyMigration) {
      ...
      this.lazyMigrationService = new LazyMigrationService(
        monitoring,
        this.repository,
        shardTypeRepository
      );
   ...
   */
  private getRiskEvaluationService(): RiskEvaluationService | undefined {
    ...
    if (this.serverInstance) {
      const service = (this.serverInstance as any).riskEvaluationService;
      if (service) {
   ...
   */
  private async queueRiskEvaluationIfOpportunity(
    shard: Shard,
    tenantId: string,
    userId: string,
    trigger: 'shard_created' | 'opportunity_updated'
  ): Promise<void> {
   ...
   */
  private sendError(reply: FastifyReply, error: ShardError): void {
  ...
  private handleError(
    ...
    const shardError = new ShardError(
      ShardErrorCode.SHARD_VALIDATION_FAILED,
      `Failed to ${operation.replace('.', ' ')} shard`,
    ...
    try {
      ...
      if (!tenantId || !userId) {
        throw new ShardError(
          ShardErrorCode.SHARD_ACCESS_DENIED,
          'Unauthorized: Missing tenant or user context'
        );
      }
      ...
      const revisionNumber = await this.revisionRepository.getNextRevisionNumber(
        shard.id,
        tenantId
      );
      ...
      this.emitEvent(
        () => this.eventService!.emitCreated(shard, {
          triggeredBy: userId,
          triggerSource: 'api' as EventTriggerSource,
        }),
        'create'
      );
  ...
  listShards = async (
    ...
    try {
      ...
      for (const shard of result.shards) {
        const hasReadAccess = await this.hasPermission(
          shard.id,
          tenantId,
          userId,
          PermissionLevel.READ
    ...
    try {
      ...
      if (this.lazyMigrationService) {
        try {
          ...
          this.monitoring.trackException(migrationError as Error, {
  ...
  updateShard = async (
    ...
    try {
      ...
      // Check permission
      const hasWriteAccess = await this.hasPermission(
        id,
        tenantId,
        userId,
        PermissionLevel.WRITE
      ...
      this.emitEvent(
        () => this.eventService!.emitUpdated(updated, existing, {
          triggeredBy: userId,
          triggerSource: 'api' as EventTriggerSource,
        }),
        'update'
  ...
  deleteShard = async (
    ...
    const startTime = Date.now();
    const { id } = req.params;
    const { hard = 'false' } = req.query as any;
    ...
    try {
      ...
      // Check permission
      const hasDeleteAccess = await this.hasPermission(
        id,
        tenantId,
        userId,
        PermissionLevel.DELETE
      ...
      this.emitEvent(
        () => this.eventService!.emitDeleted(existing, {
          triggeredBy: userId,
          triggerSource: 'api' as EventTriggerSource,
          previousState: {
