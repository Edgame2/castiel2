 ...
 */
const OPTION_LIST_CONTAINER_CONFIG = {
    id: 'optionLists',
    partitionKey: {
        paths: ['/tenantId'],
    },
...
export class OptionListRepository {
    ...
    async initialize() {
        ...
        try {
            ...
            const { container } = await database.containers.createIfNotExists(OPTION_LIST_CONTAINER_CONFIG);
    ...
    async list(options) {
      ...
      `,
        ...
        try {
            const queryIterator = this.container.items.query(query, {
                maxItemCount: limit,
     ...
     */
    async isNameUnique(name, tenantId, excludeId) {
        await this.ensureInitialized();
        const query = {
            query: `
        SELECT c.id FROM c 
...
//# sourceMappingURL=option-list.repository.js.map