...
vi.mock('yaml', () => ({
  parse: vi.fn((content: string) => {
    const config: any = {
      module: { name: 'ai-conversation', version: '1.0.0' },
      ...
      services: {
        ...
        context_service: { url: 'http://localhost:3015' },
        shard_manager: { url: 'http://localhost:3001' },
        embeddings: { url: 'http://localhost:3016' },
      },
    };
    return config;
  }),
