/**
 * Configuration loader for context-service module
...
let cachedConfig: ContextServiceConfig | null = null;
 ...
 */
export function loadConfig(): ContextServiceConfig {
  if (cachedConfig) {
    return cachedConfig;
  }
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as ContextServiceConfig;
  
  let envConfig: Partial<ContextServiceConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<ContextServiceConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as ContextServiceConfig;
