...
export class WebhookDeliveryService {
    ...
    retryInterval = null;
    isProcessing = false;
    constructor(redis, monitoring, eventService) {
        this.redis = redis;
    ...
    async processEvent(payload) {
        ...
        for (const webhook of webhooks) {
            ...
            const delivery = await this.deliveryRepository.create(webhook.id, tenantId, payload, webhook.retryCount + 1 // maxAttempts = retryCount + initial attempt
            );
     ...
     */
    async processAuditEvent(payload) {
        ...
        for (const webhook of result.webhooks) {
            ...
            const hasAuditSubscription = webhook.events.some((evt) => typeof evt === 'string' && evt.startsWith('document.'));
            if (!hasAuditSubscription) {
                continue;
            }
    ...
    matchesFilters(webhook, payload) {
        ...
        if (filters.status && filters.status.length > 0 && payload.shardSnapshot) {
            const shardStatus = payload.shardSnapshot.status;
     ...
     */
    async handleDeliveryFailure(webhook, delivery, result) {
     ...
     */
    async triggerManual(webhookId, tenantId, payload) {
        const webhook = await this.webhookRepository.findById(webhookId, tenantId);
        if (!webhook) {
            throw new Error(`Webhook not found: ${webhookId}`);
        }
        const delivery = await this.deliveryRepository.create(webhookId, tenantId, payload, 1 // Single attempt for manual triggers
        );
        await this.attemptDelivery(webhook, delivery);
