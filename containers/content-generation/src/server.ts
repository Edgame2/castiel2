...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Content Generation service listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
    console.error('Failed to start Content Generation service:', error);
    process.exit(1);
  }
}
