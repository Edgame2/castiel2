...
export class ContentGenerationService {
    ...
    shardRepository;
    conversionService;
    constructor(monitoring, templateService, insightService, shardRepository, conversionService) {
     ...
     */
    async generateDocument(tenantId, userId, templateId, context) {
        ...
        try {
            ...
            const resolvedVariables = { ...context.variables };
            ...
            const insightPromises = variables.map(async (varName) => {
                ...
                if (config?.type === 'insight' && config.insightTemplateId) {
                    try {
                        ...
                        const response = await this.insightService.generate(tenantId, userId, request);
            ...
            this.monitoring.trackException(error, { operation: 'document.generate' });
            throw error;
        }
    }
    /**
     ...
     */
    async generateContent(prompt, _connection, _apiKey, options) {
        const startTime = Date.now();
        try {
            ...
            const response = await this.insightService.generate(options.tenantId, options.userId, request);
            ...
            if (!this.conversionService) {
                throw new Error(`Format conversion to ${format} requires ConversionService to be injected. ` +
