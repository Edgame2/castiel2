/**
 * Configuration loader for ai_analytics module
 */
...
let cachedConfig: AiAnalyticsConfig | null = null;
...
export function loadConfig(): AiAnalyticsConfig {
  ...
  const defaultConfig = parseYaml(readFileSync(defaultPath, 'utf8')) as AiAnalyticsConfig;
  let envConfig: Partial<AiAnalyticsConfig> = {};
  const envPath = join(configDir, `${env}.yaml`);
  if (existsSync(envPath)) {
    envConfig = parseYaml(readFileSync(envPath, 'utf8')) as Partial<AiAnalyticsConfig>;
  }
  ...
  const resolved = resolveEnvVars(config) as AiAnalyticsConfig;
  ...
  if (!resolved.jwt.secret || resolved.jwt.secret === 'change-me-in-production') {
    ...
    log.warn('JWT_SECRET is using default value', { service: 'ai_analytics' });
  }
  
  if (!resolved.rabbitmq.url) {
    log.warn('RABBITMQ_URL not configured. Event publishing will be disabled.', { service: 'ai_analytics' });
  }
...
export function getConfig(): AiAnalyticsConfig {
