...
export class CleanupJobService {
    ...
    intervalId = null;
    isRunning = false;
    constructor(sessionService, tokenService, blacklistService, monitoring) {
    ...
    start(intervalMs = 60 * 60 * 1000) {
        ...
        this.runCleanup().catch((error) => {
            this.monitoring?.trackException(error, { operation: 'cleanup-job.initial-cleanup' });
        });
        // Schedule periodic runs
        this.intervalId = setInterval(() => {
            this.runCleanup().catch((error) => {
                this.monitoring?.trackException(error, { operation: 'cleanup-job.scheduled-cleanup' });
            });
        }, intervalMs);
    }
    /**
    ...
    async runCleanup() {
        ...
        try {
            ...
            this.monitoring?.trackException(error, { operation: 'cleanup-job.run-cleanup' });
            throw error;
        }
    }
    /**
     ...
     */
    async cleanupTenantSessions(tenantId) {
...
//# sourceMappingURL=cleanup-job.service.js.map