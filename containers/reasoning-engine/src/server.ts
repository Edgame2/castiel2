...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  const fastify = Fastify({
    ...
    genReqId: () => randomUUID(),
    bodyLimit: 10485760,
    requestTimeout: 300000, // 5 minutes for complex reasoning
  });
  ...
  await fastify.register(swagger, {
    openapi: {
      ...
      tags: [
        { name: 'Reasoning', description: 'Reasoning task management' },
...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Reasoning Engine listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
    console.error('Failed to start Reasoning Engine:', error);
    process.exit(1);
  }
}
