...
export class RiskEvaluationService {
  ...
  constructor(
    ...
    // Optional: ML services for enhanced risk scoring
   ...
   * Allows ML services to be injected after RiskEvaluationService is created
   */
  setMLServices(
  ...
  async evaluateOpportunity(
    ...
    try {
      ...
      // Get ML risk score prediction if ML services are available
      let mlRiskScore: number | undefined;
      let mlPrediction: import('../types/ml.types.js').RiskScorePrediction | undefined;
      ...
      if (this.featureStoreService && this.modelService) {
        try {
          ...
          // Get ML prediction
          mlPrediction = await this.modelService.predictRiskScore(
            featureSet.features,
            opportunity.structuredData?.industry as string | undefined,
            `risk-eval-${opportunityId}`
          );

          mlRiskScore = mlPrediction.riskScore;

          // Track ML prediction
          this.monitoring.trackMetric('risk-evaluation.ml_prediction', mlRiskScore, {
            opportunityId,
            tenantId,
            modelVersion: mlPrediction.modelVersion,
          });
        } catch (error) {
          // Log but don't fail - ML is optional
          this.monitoring.trackException(error as Error, {
            operation: 'risk-evaluation.ml_prediction',
            opportunityId,
            tenantId,
          });
        }
      ...
      // Combine ML prediction with rule-based detection if available
      const { globalScore, categoryScores } = await this.calculateRiskScore(
        detectedRisks,
        tenantId,
        opportunity,
        mlRiskScore // Pass ML score for weighted combination
      );
...
- Recommended mitigation actions`,
  ...
  async calculateRiskScore(
    ...
    let globalScore = normalizedRisks.reduce((sum, risk) => {
      return sum + (risk.ponderation * risk.confidence * risk.contribution);
    }, 0);

    // Combine with ML prediction if available
    if (mlRiskScore !== undefined) {
      const weights = await this.getWeightsForOpportunity(tenantId, {
        industry: (opportunity.structuredData as any)?.industry,
        dealSize: (opportunity.structuredData as any)?.amount ? ((opportunity.structuredData as any).amount < 50000 ? 'small' : (opportunity.structuredData as any).amount < 500000 ? 'medium' : 'large') : undefined,
        stage: (opportunity.structuredData as any)?.stage,
        dealValue: (opportunity.structuredData as any)?.amount,
      });
      
      // Weighted combination: rule-based score and ML score
      const mlWeight = weights.ml || this.DEFAULT_WEIGHTS.ml;
      const ruleWeight = 1.0; // Rule-based weight
      const totalWeight = mlWeight + ruleWeight;
      
      globalScore = (globalScore * ruleWeight + mlRiskScore * mlWeight) / totalWeight;
    }
