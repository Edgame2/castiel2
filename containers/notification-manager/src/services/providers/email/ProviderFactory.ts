...
export async function createEmailProvider(
  ...
  const defaultFrom = config.from_address || appConfig.notification.providers.email?.from_address || 'noreply@coder.ide';
  const defaultFromName = config.from_name || appConfig.notification.providers.email?.from_name || 'Coder IDE';
