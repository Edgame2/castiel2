...
export class DeepSearchService {
    ...
    constructor(
        cosmosService: WebSearchCosmosService,
    ...
    async deepSearch(
        ...
        projectId: string,
        sourceQuery: string,
        options?: DeepSearchOptions & { onProgress?: ProgressCallback }
    ): Promise<DeepSearchResult> {
        const startTime = Date.now();
        const urls = Array.isArray(searchResults[0])
            ? (searchResults as string[])
            : (searchResults as SearchResult[]).map((r) => r.url);
        ...
        for (let i = 0; i < pagesToScrape.length; i++) {
            ...
            try {
                ...
                // 1. Scrape the page
                const doc = await this.scraperService.scrapeAndCreateDocument(
                    url,
                    tenantId,
                    projectId,
                    sourceQuery,
                ...
                this.monitoring?.trackException(error as Error, { operation: 'deep-search.scrape-page', url });
                result.statistics.failedPages++;
            }
        }
     ...
     */
    private getSearchResultId(searchResults: SearchResult[] | string[], url: string): string | undefined {
        if (Array.isArray(searchResults[0]) && typeof searchResults[0] === 'string') {
            return undefined;
        }

        const searchResult = (searchResults as SearchResult[]).find((r) => r.url === url);
     ...
     */
    private emitProgress(callback: ProgressCallback | undefined, progress: ScrapingProgress): void {
        if (callback) {
            callback(progress);
        }
    }
    ...
    ): Promise<{ page: WebPageDocument; content: string; similarity: number }[]> {
