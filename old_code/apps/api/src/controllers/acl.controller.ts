...
export class ACLController {
  ...
  async revokePermission(
    ...
    try {
      ...
      const input: RevokePermissionInput = {
        ...
        userId,
        roleId,
        permissions: permissions as PermissionLevel[] | undefined,
  ...
  async updateACL(
    ...
    try {
      ...
      const input: UpdateACLInput = {
        shardId,
        addEntries: addEntries as any,
        removeEntries: removeEntries as any,
  ...
  async getUserPermissions(
    ...
    try {
      ...
      // Get permissions
      const permissions = await this.aclService.getUserPermissions(
        targetUserId,
        shardId,
  ...
  async checkPermission(
    ...
    try {
      ...
      // Validate permission
      const validPermissions = Object.values(PermissionLevel);
      if (!validPermissions.includes(requiredPermission as PermissionLevel)) {
      ...
      const context: PermissionCheckContext = {
        ...
        tenantId: authRequest.user.tenantId,
        shardId,
        requiredPermission: requiredPermission as PermissionLevel,
      };
  ...
  async batchCheckPermissions(
    ...
    try {
      ...
      const batchRequest: ACLBatchCheckRequest = {
        userId: targetUserId,
        tenantId: authRequest.user.tenantId,
        shardIds,
        requiredPermission: requiredPermission as PermissionLevel,
