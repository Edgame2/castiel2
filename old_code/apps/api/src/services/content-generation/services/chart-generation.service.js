...
export class ChartGenerationService {
    ...
    shardRepository;
    insightService;
    constructor(monitoring, shardRepository, insightService) {
     ...
     */
    async generateChart(placeholder, config, template, context) {
        ...
        try {
            ...
            const chartData = await this.getChartData(config, template, context);
            ...
            const colors = this.prepareChartColors(config.chartConfig, template.dominantColors || []);
            // Step 3: Render chart to image
            const chartImage = await this.renderChart(config.chartConfig.chartType, chartData, colors, config.chartConfig);
     ...
     */
    async getChartData(config, template, context) {
        ...
        if (config.contextTemplateId || context?.projectId) {
            const shardData = await this.extractShardChartData(config, template, context);
        ...
        if (this.insightService && config.description) {
            const aiData = await this.generateAiChartData(config, template, context);
     ...
     */
    extractManualChartData(variables, placeholderName) {
     ...
     */
    async extractShardChartData(config, template, context) {
                ...
                for (const [, value] of Object.entries(structuredData)) {
                    if (Array.isArray(value) && value.length > 0) {
                        ...
                        if (typeof firstItem === 'object' && firstItem !== null) {
                            ...
                            const labelField = Object.keys(firstItem).find(k => ['label', 'name', 'title', 'category'].includes(k.toLowerCase()));
                            const valueField = Object.keys(firstItem).find(k => ['value', 'amount', 'count', 'quantity'].includes(k.toLowerCase()));
     ...
     */
    async generateAiChartData(config, template, context) {
        if (!this.insightService || !config.description) {
            return null;
        }
        try {
            ...
            const response = await this.insightService.generate(template.tenantId, template.userId, insightRequest);
     ...
     */
    prepareChartColors(chartConfig, templateColors) {
     ...
     */
    async renderChart(chartType, data, colors, config) {
     ...
     */
    generateChartHTML(chartType, data, colors, config) {
...
<body>
  ...
  <script>
     ...
     */
    mapChartTypeToChartJS(chartType) {
        const mapping = {
            'bar': 'bar',
            'line': 'line',
            'pie': 'pie',
     ...
     */
    generateChartJSConfig(chartType, labels, values, colors, config) {
        const baseConfig = {
            type: chartType,
            data: {
                labels,
...
//# sourceMappingURL=chart-generation.service.js.map