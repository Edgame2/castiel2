...
export class HubSpotAdapter extends BaseIntegrationAdapter {
     ...
     */
    async makeHubSpotRequest(url, method = 'GET', body) {
    ...
    async getUserProfile() {
        try {
            ...
            const result = await this.makeHubSpotRequest('/integrations/v1/me');
    ...
    async fetch(options) {
        ...
        const result = await this.makeHubSpotRequest(url);
        if (result.error) {
    ...
    async push(data, options) {
        ...
        const result = await this.makeHubSpotRequest(url, method, body);
        if (result.error) {
            return {
                success: false,
                error: result.error,
    ...
    async search(options) {
        ...
        for (const entity of entitiesToSearch) {
            try {
                ...
                const result = await this.makeHubSpotRequest(searchUrl, 'POST', searchBody);
                if (result.data) {
                    const items = result.data.results.slice(0, limit).map((item) => {
    ...
    async registerWebhook(events, callbackUrl) {
        ...
        const result = await this.makeHubSpotRequest('/webhooks/v3/subscriptions', 'POST', subscription);
        if (result.error || !result.data) {
            throw new Error(`Failed to register webhook: ${result.error || 'Unknown error'}`);
        }
        // Update webhook URL (separate call)
        await this.makeHubSpotRequest(`/webhooks/v3/subscriptions/${result.data.subscriptionId}`, 'PATCH', { eventUrl: callbackUrl });
        return {
    ...
    // =====================
    normalizeRecord(record, _entity) {
        return {
            id: record.id,
            ...record.properties,
    ...
    getSearchPropertiesForEntity(entity) {
        const propertyMap = {
...
const ENTITIES = [CONTACT_ENTITY, COMPANY_ENTITY, DEAL_ENTITY, TICKET_ENTITY];
// ============================================
// Integration Definition
// ============================================
export const HUBSPOT_DEFINITION = {
    id: 'hubspot',
    name: 'hubspot',
    displayName: 'HubSpot',
...
// ============================================
export const hubspotAdapterFactory = {
...
//# sourceMappingURL=hubspot.adapter.js.map