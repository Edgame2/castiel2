...
export class ProactiveInsightsDigestWorker {
    ...
    pollIntervalMs;
    enabled;
    constructor(redis, emailService, deliveryPreferencesRepository, userService, monitoring, analyticsService, config = {}) {
        this.redis = redis;
        this.emailService = emailService;
        this.deliveryPreferencesRepository = deliveryPreferencesRepository;
     ...
     */
    async processDigestQueue(queueKey) {
     ...
     */
    async shouldSendDigest(preferences) {
     ...
     */
    isTimeToSend(frequency, digestTime) {
     ...
     * Send digest email
     */
    async sendDigestEmail(tenantId, userId, insights, preferences) {
     ...
     */
    async recordDigestAnalytics(tenantId, insights, status, latencyMs) {
        if (!this.analyticsService) {
            return;
        }
        // Record analytics event for each insight in the digest
        // Use the same latency for all insights since they're sent together
        const analyticsPromises = insights.map((insight) => this.analyticsService.recordDeliveryEvent({
            tenantId,
     ...
     */
    buildDigestEmailBody(insights, periodLabel) {
  ...
  `;
     ...
     */
    buildDigestEmailText(insights, periodLabel) {
...
//# sourceMappingURL=proactive-insights-digest-worker.service.js.map