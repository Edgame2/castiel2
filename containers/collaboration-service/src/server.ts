...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  await fastify.register(swagger, {
    openapi: {
      ...
      tags: [
        { name: 'Collaboration', description: 'Collaboration features' },
  ...
  initializeDatabase({
    ...
    containers: {
      conversations: 'collaboration_conversations',
...
export async function start(): Promise<void> {
  try {
    ...
    console.log(`Collaboration Service listening on http://${config.server.host}:${config.server.port}`);
  } catch (error) {
