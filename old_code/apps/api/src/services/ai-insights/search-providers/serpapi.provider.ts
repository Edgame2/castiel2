...
export class SerpApiProvider {
    ...
    async search(query: string, options?: { searchType?: string; maxResults?: number }): Promise<SearchResultItem[]> {
        ...
        const response = await this.http.get<SerpApiResponse>(this.baseUrl, {
            params: {
                ...params,
                api_key: this.apiKey,
            },
    ...
    private buildParams(query: string, searchType: string, maxResults = 10): Record<string, any> {
    ...
    private parseResults(data: SerpApiResponse, searchType: string, maxResults = 10): SearchResultItem[] {
        ...
        try {
            ...
            this.monitoring?.trackException(error as Error, { operation: 'serpapi.provider.parse-response' });
            throw error;
        }
