# Container App for Workers Ingestion
# Excluded for hybrid-dev (containers run locally)

resource "azurerm_container_app" "workers_ingestion" {
  count = local.is_full_deployment ? 1 : 0

  name                         = "${var.resource_prefix}-workers-ingestion-${var.environment}"
  ...
  template {
    ...
    container {
      name   = "workers-ingestion"
      image  = "${azurerm_container_registry.main[0].login_server}/workers-ingestion:latest"
      cpu    = 0.5
      memory = "1Gi"
...
resource "azurerm_key_vault_access_policy" "container_app_workers_ingestion" {
  count = local.is_full_deployment ? 1 : 0

  key_vault_id = azurerm_key_vault.main.id
  tenant_id    = azurerm_container_app.workers_ingestion[0].identity[0].tenant_id
  object_id    = azurerm_container_app.workers_ingestion[0].identity[0].principal_id
...
resource "azurerm_role_assignment" "container_app_workers_ingestion_cosmos" {
  count = local.is_full_deployment ? 1 : 0
