...
export class MicrosoftWordExtractor extends BaseDocumentExtractor {
  ...
  async parseDocument(
    ...
    try {
      ...
      throw new Error(`Failed to parse Microsoft Word document: ${(error as Error).message}`);
    }
  }
  ...
  private extractTextFromWordXml(
    parsed: any,
    elementType: 'paragraph' | 'header' | 'footer',
    pageIndex: number
   ...
   */
  private extractTextFromWordParagraph(paragraph: any): string {
   ...
   */
  private extractTextFromWordTable(table: any): string {
    const rows: string[] = [];
   ...
   */
  private extractTextFromWordCell(cell: any): string {
    const textParts: string[] = [];
  ...
  protected async extractColors(
    ...
    try {
      ...
      const themeFiles = Object.keys(zip.files).filter(name => 
        name.startsWith('word/theme/theme') && name.endsWith('.xml')
      );
