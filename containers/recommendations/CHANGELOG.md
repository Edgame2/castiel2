...
### Added
- **W11 Super Admin – Tenant templates (Plan §7.2):** `TenantTemplate` and `TenantTemplateFeedbackSnapshot` types; templates stored in `recommendation_config` with partitionKey `_global`, id prefix `tenant_template_`. `TenantTemplateService`: `listTemplates()`, `getTemplate(templateId)`, `createTemplate(input, createdBy)`, `applyTemplate(templateId, tenantIds, appliedBy)` (writes feedback config per tenant). `GET /api/v1/admin/tenant-templates` returns `{ items: TenantTemplate[] }`. `POST /api/v1/admin/tenant-templates` (body: name, description?, feedbackConfig, methodology?, defaultLimits?) creates a template; returns 201. `POST /api/v1/admin/tenant-templates/:templateId/apply` (body: tenantIds) applies template to tenants; returns `{ applied: string[], failed: { tenantId, error }[] }`; 404 if template not found.
- **W11 Super Admin – List tenants:** `GET /api/v1/admin/tenants` returns `{ items: TenantSummary[] }`. Stub returns empty array; tenant list to be populated from auth/tenant store when available. Response shape: id, name, industry, status, createdAt, activeUsers, activeOpportunities.
