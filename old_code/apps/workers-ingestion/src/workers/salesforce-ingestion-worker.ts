...
interface SalesforceIngestionWorkerConfig {
...
export class SalesforceIngestionWorker {
  private worker: Worker;
  private cosmosClient: CosmosClient;
  private monitoring: IMonitoringProvider;
  private config: SalesforceIngestionWorkerConfig;
  private queueProducer: QueueProducerService;

  constructor(
    config: SalesforceIngestionWorkerConfig,
    ...
    this.worker = new Worker(
      ...
      async (job: Job<IngestionEventMessage>) => {
        // Only process Salesforce events
        if (job.data.source === 'salesforce') {
          return this.processSalesforceIngestion(job);
        }
        // Skip non-Salesforce events
        return;
      },
      {
        connection: redis,
        concurrency: parseInt(process.env.SALESFORCE_INGESTION_CONCURRENCY || '3', 10),
        removeOnComplete: {
          age: 24 * 3600,
          count: 1000,
        },
    ...
    this.worker.on('completed', (job) => {
      if (job.data.source === 'salesforce') {
        this.monitoring.trackEvent('salesforce-ingestion-worker.completed', {
          jobId: job.id,
    ...
    this.worker.on('failed', (job, err) => {
      if (job?.data?.source === 'salesforce') {
        this.monitoring.trackException(err, {
          context: 'SalesforceIngestionWorker',
          jobId: job.id,
          tenantId: job.data.tenantId,
        });
      }
  ...
  private async processSalesforceIngestion(
    ...
    try {
      this.monitoring.trackEvent('salesforce-ingestion-worker.started', {
        jobId: job.id,
        tenantId,
        sourceId,
        eventType,
      ...
      if (!normalizedShard) {
        this.monitoring.trackEvent('salesforce-ingestion-worker.normalization_skipped', {
          jobId: job.id,
          tenantId,
          sourceId,
      ...
      this.monitoring.trackException(error as Error, {
        context: 'SalesforceIngestionWorker.processSalesforceIngestion',
        jobId: job.id,
        tenantId,
        sourceId,
        duration,
